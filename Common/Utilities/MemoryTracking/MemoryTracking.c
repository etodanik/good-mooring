...
static void            tracyAllocationUnlock(int* guard)
{
    (void)guard;
    pthread_mutex_unlock(&tracyAllocationMutex);
}
#define TRACY_ALLOCATION_LOCK()                                         \
    int tracyGuard __attribute__((cleanup(tracyAllocationUnlock))) = 0; \
    pthread_mutex_lock(&tracyAllocationMutex)
#else
