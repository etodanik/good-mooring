...
static bool rtree_box_contains_point(const float* b, const float* p) { return !(b[0] > p[0] || b[1] > p[1] || b[2] < p[0] || b[3] < p[1]); }
...
void rtree_split(RTree* pRTree, const uint32_t treeIndex, const float* leftrtree_box, const float* rightrtree_box, const uint32_t leftCount,
