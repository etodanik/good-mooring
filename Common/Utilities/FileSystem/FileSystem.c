...
// MARK: - Error Handling
/************************************************************************/
static THREAD_LOCAL TFFSErrorContext sFsErrCtx;

TFFSErrorContext* __fs_err_ctx(void) { return &sFsErrCtx; }
...
TFFSErrorCode translateErrno(uint32_t err)
{
    ...
    {
    case EPERM:
        return TF_FS_NOT_PERMITTED_ERR;
    case ENOENT:
        return TF_FS_NOT_FOUND_ERR;
    case EACCES:
        return TF_FS_ACCESS_DENIED_ERR;
    case ENOTDIR:
        return TF_FS_NOT_DIR_ERR;
    case EISDIR:
        return TF_FS_IS_DIR_ERR;
    case EINVAL:
        return TF_FS_INVALID_PARAM_ERR;
    case ENOSPC:
        return TF_FS_INSUFFICIENT_SPACE;
    case EROFS:
        return TF_FS_READ_ONLY_ERR;
    default:
        return TF_FS_INTERNAL_ERR;
    }
}
...
TFResourceDirectoryInfo gResourceDirectories[TF_RD_COUNT] = { { 0 } };
...
void parse_path_statement(char* PathStatment, size_t size)
{
    ...
    for (uint32_t i = 0; i < TF_RD_COUNT; i++)
    {
...
static bool ioMemoryStreamClose(TFFileStream* fs)
{
    MEMSD(stream, fs);
...
static size_t ioMemoryStreamRead(TFFileStream* fs, void* dst, size_t size)
{
    __FS_NO_ERR;
    if (!(fs->mMode & TF_FM_READ))
    {
        // Attempting to read from stream that doesn't have TF_FM_READ flag
...
static size_t ioMemoryStreamWrite(TFFileStream* fs, const void* src, size_t size)
{
    __FS_NO_ERR;
    if (!(fs->mMode & TF_FM_WRITE))
    {
        // Attempting to write to stream that doesn't have TF_FM_WRITE flag
        __FS_SET_ERR(TF_FS_NOT_PERMITTED_ERR);
        return 0;
    }
    ...
    if (size > availableCapacity)
    {
        ...
        if (!newBuffer)
        {
            __FS_SET_ERR(TF_FS_INSUFFICIENT_SPACE);
            return 0;
        }
...
static bool ioMemoryStreamSeek(TFFileStream* fs, TFSeekBaseOffset baseOffset, ssize_t seekOffset)
{
    __FS_NO_ERR;
    MEMSD(stream, fs);
    ...
    case TF_SBO_START_OF_FILE:
    {
        ...
        {
            __FS_SET_ERR(TF_FS_INVALID_PARAM_ERR);
            return false;
        }
...
static ssize_t ioMemoryStreamGetPosition(TFFileStream* fs)
{
    MEMSD(stream, fs);
    return (ssize_t)stream->mCursor;
}

static ssize_t ioMemoryStreamGetSize(TFFileStream* fs)
{
    MEMSD(stream, fs);
    return stream->mSize;
}

static bool ioMemoryStreamFlush(TFFileStream* fs)
{
    (void)fs;
    // No-op.
    return true;
...
static bool ioMemoryStreamIsAtEnd(TFFileStream* fs)
{
    MEMSD(stream, fs);
    return (ssize_t)stream->mCursor == stream->mSize;
}

static bool ioMemoryStreamMemoryMap(TFFileStream* fs, size_t* outSize, void const** outData)
{
    __FS_NO_ERR;
    if (fs->mMode & TF_FM_WRITE)
    {
...
static TFIFileSystem gMemoryFileIO = {
    NULL,
    ioMemoryStreamClose,
    ioMemoryStreamRead,
    ioMemoryStreamWrite,
...
bool fsIsMemoryStream(TFFileStream* pStream) { return pStream->pIO == &gMemoryFileIO; }

bool fsIsSystemFileStream(TFFileStream* pStream) { return pStream->pIO == pSystemFileIO; }

bool fsOpenStreamFromMemory(const void* buffer, size_t bufferSize, TFFileMode mode, bool owner, TFFileStream* fs)
{
...
bool fsOpenStreamFromPath(TFResourceDirectory resourceDir, const char* fileName, TFFileMode mode, TFFileStream* pOut)
{
    MTRACY_ZONE("fsOpenStreamFromPath");
    TFIFileSystem* io = gResourceDirectories[resourceDir].pIO;
    return io->Open(io, resourceDir, fileName, mode, pOut);
}

size_t fsReadBstringFromStream(TFFileStream* stream, bstring* pStr, size_t symbolsCount)
{
...
bool fsFindStream(TFFileStream* pStream, const void* pFind, size_t findSize, ssize_t maxSeek, ssize_t* pPosition)
{
    ...
    for (; maxSeek != 0; --maxSeek)
    {
        ...
        while (true)
        {
            if (byte == pattern[patternPos])
            {
                ...
                {
                    bool result = fsSeekStream(pStream, TF_SBO_CURRENT_POSITION, -(ssize_t)findSize);
                    UNREF_PARAM(result);
                    ASSERT(result);
...
bool fsFindReverseStream(TFFileStream* pStream, const void* pFind, size_t findSize, ssize_t maxSeek, ssize_t* pPosition)
{
    ASSERT(pStream && pFind && pPosition);
    ASSERT(findSize < STREAM_FIND_BUFFER_SIZE);
    ASSERT(maxSeek >= 0);
    ...
    {
        uint8_t byte;
        if (!fsSeekStream(pStream, TF_SBO_CURRENT_POSITION, -1))
            return false;

        size_t readBytes = fsReadFromStream(pStream, &byte, sizeof(byte));
        ASSERT(readBytes == 1);
        UNREF_PARAM(readBytes);
        fsSeekStream(pStream, TF_SBO_CURRENT_POSITION, -1);
...
FORGE_API bool fsStreamWrapMemoryMap(TFFileStream* fs)
{
    ...
    TFFileStream wrapFs;
    if (!fsOpenStreamFromMemory(mem, size, TF_FM_READ, false, &wrapFs))
    {
    ...
    TFFileStream* wrappedFs = tf_malloc(sizeof *wrappedFs);
    if (!wrappedFs)
    {
...
void fsSetPathForResourceDir(TFIFileSystem* pIO, TFResourceDirectory resourceDir, const char* bundledFolder)
{
    ASSERT(pIO);
    TFResourceDirectoryInfo* dir = &gResourceDirectories[resourceDir];
...
static uint64_t archiveHashMurmur2_64(void const* key, size_t len, uint64_t seed)
{
    ...
    {
        uint64_t k;
        memcpy(&k, (void const*)data, sizeof(uint64_t));
        data++;
...
struct BunyArMetadata
{
    ...
    TFFileStream  ownedStream;
    TFFileStream* archiveStream;
...
static void initBunyArFsInterface(TFIFileSystem*, struct BunyArMetadata*);

static inline struct BunyArMetadata* getFsArchive(TFIFileSystem* fs) { return (struct BunyArMetadata*)fs->pUser; }

static inline struct BunyArFileStream* getFsBunyArStream(TFFileStream* fs) { return (struct BunyArFileStream*)fs->mUser.data[0]; }
...
static inline size_t bunyArStreamRead(struct BunyArMetadata* a, uint64_t position, uint64_t size, void* dst)
{
    ...
    if (fsGetStreamSeekPosition(a->archiveStream) != (ssize_t)position &&
        !fsSeekStream(a->archiveStream, TF_SBO_START_OF_FILE, (ssize_t)position))
    {
        readed = 0;
    }
    else
...
static const struct TFArchiveOpenDesc BUNYAR_OPEN_DESC_DEFAULT = { 0 };

static bool bunyArchiveOpen(TFFileStream* stream, uint64_t memorySize, const void* memory, const struct TFArchiveOpenDesc* desc,
                            TFIFileSystem* out)
{
    __FS_NO_ERR;
    ...
    if (streamMode)
    {
        headerReaded = fsSeekStream(stream, TF_SBO_START_OF_FILE, 0) && fsReadFromStream(stream, &header, sizeof header) == sizeof header;
    }
...
bool fsArchiveOpen(TFResourceDirectory rd, const char* path, const struct TFArchiveOpenDesc* desc, TFIFileSystem* out)
{
    TFFileStream stream;
    if (!fsOpenStreamFromPath(rd, path, TF_FM_READ, &stream))
        return false;
...
bool fsArchiveOpenFromStream(TFFileStream* stream, const struct TFArchiveOpenDesc* desc, TFIFileSystem* out)
{
...
FORGE_API bool fsArchiveClose(TFIFileSystem* fs)
{
    if (!fs || !fs->pUser)
        return true;
...
bool fsArchiveGetNodeId(TFIFileSystem* fs, const char* fileName, uint64_t* outUid)
{
    struct BunyArMetadata* archive = getFsArchive(fs);
...
static bool ioArchiveGetFileUid(TFIFileSystem* fs, TFResourceDirectory rd, const char* fileName, uint64_t* outUid)
{
...
static bool ioArchiveOpenByUid(TFIFileSystem* inFs, uint64_t index, TFFileMode mode, TFFileStream* pOutStream)
{
    __FS_NO_ERR;
    ...
    {
        __FS_SET_ERR(TF_FS_NOT_FOUND_ERR);
        return false;
    }
    ...
    case BUNYAR_FILE_FORMAT_RAW:
    {
        ...
        __FS_SET_ERR(TF_FS_INTERNAL_ERR);
        return false;
    }
    case BUNYAR_FILE_FORMAT_LZ4_BLOCKS:
    case BUNYAR_FILE_FORMAT_ZSTD_BLOCKS:
...
static bool ioArchiveFsOpen(TFIFileSystem* fs, const TFResourceDirectory rd, const char* fileName, TFFileMode mode,
                            TFFileStream* pOutStream)
{
    uint64_t index;
...
static bool ioArchiveFsClose(TFFileStream* fs)
{
    if (!fs->pIO)
        return true;
...
static size_t ioArchiveFsRead(TFFileStream* pFile, void* outputBuffer, size_t outputSize)
{
...
static bool ioArchiveFsSeek(TFFileStream* pFile, TFSeekBaseOffset baseOffset, ssize_t seekOffset)
{
...
static ssize_t ioArchiveFsGetSeekPosition(TFFileStream* pFile)
{
    return (ssize_t)getFsBunyArStream((TFFileStream*)(uintptr_t)pFile)->position;
}

static ssize_t ioArchiveFsGetFileSize(TFFileStream* fs)
{
    struct BunyArFileStream* stream = getFsBunyArStream(fs);
    return (ssize_t)stream->node->originalFileSize;
}

static bool ioArchiveFsIsAtEnd(TFFileStream* pFile)
{
    struct BunyArFileStream* stream = getFsBunyArStream((TFFileStream*)(uintptr_t)pFile);

    return stream->position >= stream->node->originalFileSize;
}

static bool ioArchiveMemoryMap(TFFileStream* fs, size_t* outSize, void const** outData)
{
    *outSize = 0;
    *outData = NULL;

    struct BunyArFileStream* stream = getFsBunyArStream((TFFileStream*)(uintptr_t)fs);
...
static void initBunyArFsInterface(TFIFileSystem* fs, struct BunyArMetadata* archive)
{
    memset(fs, 0, sizeof *fs);
...
void fsArchiveGetDescription(TFIFileSystem* fs, struct BunyArDescription* outInfo)
{
...
bool fsArchiveGetNodeDescription(TFIFileSystem* fs, uint64_t nodeId, struct BunyArNodeDescription* outInfo)
{
    __FS_NO_ERR;
    memset(outInfo, 0, sizeof *outInfo);
...
bool fsArchiveGetFileBlockMetadata(TFFileStream* pFile, struct BunyArBlockFormatHeader* outHeader, const BunyArBlockPointer** outBlockPtrs)
{
    __FS_NO_ERR;
