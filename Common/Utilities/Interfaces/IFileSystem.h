...
#define TF_FS_MAX_PATH 320
#else
#define TF_FS_MAX_PATH 512
#endif
...
typedef enum TFResourceDirectory
{
    ...
    ____rd_lib_counter_begin = TF_RD_OTHER_FILES + 1,

    // Add libraries here
    TF_RD_MIDDLEWARE_0 = ____rd_lib_counter_begin,
    TF_RD_MIDDLEWARE_1,
    TF_RD_MIDDLEWARE_2,
    TF_RD_MIDDLEWARE_3,
    TF_RD_MIDDLEWARE_4,
...
typedef enum TFSeekBaseOffset
{
    TF_SBO_START_OF_FILE = 0,
    TF_SBO_CURRENT_POSITION,
    TF_SBO_END_OF_FILE,
} TFSeekBaseOffset;

typedef enum TFFileMode
{
    ...
    TF_FM_ALLOW_READ = 1 << 4,

    // RW mode
    TF_FM_READ_WRITE = TF_FM_READ | TF_FM_WRITE,

    // W mode and set position to the end
    TF_FM_WRITE_APPEND = TF_FM_WRITE | TF_FM_APPEND,

    // R mode and set position to the end
    TF_FM_READ_APPEND = TF_FM_READ | TF_FM_APPEND,

    // RW mode and set position to the end
    TF_FM_READ_WRITE_APPEND = TF_FM_READ_WRITE | TF_FM_APPEND,

    // -- mode and -- and also read access for other processes.
    TF_FM_WRITE_ALLOW_READ = TF_FM_WRITE | TF_FM_ALLOW_READ,
    TF_FM_READ_WRITE_ALLOW_READ = TF_FM_READ_WRITE | TF_FM_ALLOW_READ,
    TF_FM_WRITE_APPEND_ALLOW_READ = TF_FM_WRITE_APPEND | TF_FM_ALLOW_READ,
    TF_FM_READ_WRITE_APPEND_ALLOW_READ = TF_FM_READ_WRITE_APPEND | TF_FM_ALLOW_READ,
} TFFileMode;

typedef struct TFIFileSystem TFIFileSystem;

struct TFFileStreamUserData
{
    uintptr_t data[6];
};

typedef struct TFResourceDirectoryInfo
{
    TFIFileSystem* pIO;
    char           mPath[TF_FS_MAX_PATH];
    bool           mBundled;
} TFResourceDirectoryInfo;

/// After stream is opened, only TFFileStream::pIO must be used for this stream.
/// Example:
...
/// The reason for this is that TFIFileSystem::Open can open stream using another
/// TFIFileSystem handle.
///
/// It is best to use TFIFileSystem IO shortcuts "fsReadFromStream(&stream,...)"
typedef struct TFFileStream
{
    TFIFileSystem*              pIO;
    TFFileMode                  mMode;
    struct TFFileStreamUserData mUser; // access to this field is IO exclusive
} TFFileStream;

typedef struct TFFileSystemInitDesc
{
    const char* pAppName;
    void*       pPlatformData;
    ...
    bool        mIsTool;
} TFFileSystemInitDesc;

struct TFIFileSystem
{
    bool (*Open)(TFIFileSystem* pIO, const TFResourceDirectory resourceDir, const char* fileName, TFFileMode mode, TFFileStream* pOut);

    /// Closes and invalidates the file stream.
    bool (*Close)(TFFileStream* pFile);

    /// Returns the number of bytes read.
    size_t (*Read)(TFFileStream* pFile, void* outputBuffer, size_t bufferSizeInBytes);

    /// Reads at most `bufferSizeInBytes` bytes from sourceBuffer and writes them into the file.
    /// Returns the number of bytes written.
    size_t (*Write)(TFFileStream* pFile, const void* sourceBuffer, size_t byteCount);

    /// Seeks to the specified position in the file, using `baseOffset` as the reference offset.
    bool (*Seek)(TFFileStream* pFile, TFSeekBaseOffset baseOffset, ssize_t seekOffset);

    /// Gets the current seek position in the file.
    ssize_t (*GetSeekPosition)(TFFileStream* pFile);

    /// Gets the current size of the file. Returns -1 if the size is unknown or unavailable.
    ssize_t (*GetFileSize)(TFFileStream* pFile);

    /// Flushes all writes to the file stream to the underlying subsystem.
    bool (*Flush)(TFFileStream* pFile);

    /// Returns whether the current seek position is at the end of the file stream.
    bool (*IsAtEnd)(TFFileStream* pFile);

    // Acquire unique file identifier.
    // Only Archive FS supports it currently.
    bool (*GetFileUid)(TFIFileSystem* pIO, TFResourceDirectory rd, const char* name, uint64_t* outUid);

    // Open file using unique identifier. Use GetFileUid to get uid.
    bool (*OpenByUid)(TFIFileSystem* pIO, uint64_t uid, TFFileMode fm, TFFileStream* pOut);
    ...
    bool (*MemoryMap)(TFFileStream* fs, size_t* outSize, void const** outData);

    // getSystemHandle
    void* (*GetSystemHandle)(TFFileStream* fs);

    void* pUser;
};

/// Default file system using C File IO or Bundled File IO (Android) based on the TFResourceDirectory
FORGE_API extern TFIFileSystem* pSystemFileIO;
...
typedef struct TFFSErrorContext
{
    TFFSErrorCode code;
    const char*   func;
...
/// unless TFFileSystemInitDesc::toolsFilesystem = true
FORGE_API bool initFileSystem(TFFileSystemInitDesc* pDesc);
...
/// 'desc' can be NULL
FORGE_API bool fsArchiveOpen(TFResourceDirectory rd, const char* fileName, const struct TFArchiveOpenDesc* desc, TFIFileSystem* out);
...
FORGE_API bool fsArchiveOpenFromStream(TFFileStream* stream, const struct TFArchiveOpenDesc* desc, TFIFileSystem* out);

/// Archive can be opened on "read from memory" mode.
/// Benifits:
/// - no need for TFArchiveOpenDesc::protectStreamCriticalSection
...
FORGE_API bool fsArchiveOpenFromMemory(uint64_t msize, const void* m, const struct TFArchiveOpenDesc* desc, TFIFileSystem* out);

FORGE_API bool fsArchiveClose(TFIFileSystem* pArchive);

/************************************************************************/
// MARK: - File IO
/************************************************************************/

/// Opens the file at `filePath` using the mode `mode`, returning a new TFFileStream that can be used
/// to read from or modify the file. May return NULL if the file could not be opened.
FORGE_API bool fsOpenStreamFromPath(TFResourceDirectory resourceDir, const char* fileName, TFFileMode mode, TFFileStream* pOut);

/// Opens a memory buffer as a TFFileStream, returning a stream that must be closed with `fsCloseStream`.
/// Use 'fsStreamMemoryMap' to do the opposite.
FORGE_API bool fsOpenStreamFromMemory(const void* buffer, size_t bufferSize, TFFileMode mode, bool owner, TFFileStream* pOut);

FORGE_API bool fsFindStream(TFFileStream* fs, const void* pFind, size_t findSize, ssize_t maxSeek, ssize_t* pPosition);
FORGE_API bool fsFindReverseStream(TFFileStream* fs, const void* pFind, size_t findSize, ssize_t maxSeek, ssize_t* pPosition);

/// Checks if stream is a standard system stream
FORGE_API bool fsIsSystemFileStream(TFFileStream* fs);
/// Checks if stream is a memory stream
FORGE_API bool fsIsMemoryStream(TFFileStream* fs);

/// symbolsCount can be SIZE_MAX, then reads until the end of file
/// appends '\0' to the end of string
FORGE_API size_t fsReadBstringFromStream(TFFileStream* stream, struct bstring* pStr, size_t symbolsCount);
...
/// fsCloseStream(TFFileStream*) takes care of cleaning wrapped stream.
/// So checking return value is optional.
FORGE_API bool fsStreamWrapMemoryMap(TFFileStream* fs);

FORGE_API void* fsGetSystemHandle(TFFileStream* fs);
/************************************************************************/
// MARK: - TFIFileSystem IO shortcuts
/************************************************************************/

/// Closes and invalidates the file stream.
static inline bool fsCloseStream(TFFileStream* fs)
{
    if (!fs->pIO)
        return true;
...
/// Returns the number of bytes read.
static inline size_t fsReadFromStream(TFFileStream* fs, void* pOutputBuffer, size_t bufferSizeInBytes)
{
...
/// Reads at most `bufferSizeInBytes` bytes from sourceBuffer and writes them into the file.
/// Returns the number of bytes written.
static inline size_t fsWriteToStream(TFFileStream* fs, const void* pSourceBuffer, size_t byteCount)
{
    if (!fs->pIO->Write)
        return 0;
...
/// Seeks to the specified position in the file, using `baseOffset` as the reference offset.
static inline bool fsSeekStream(TFFileStream* fs, TFSeekBaseOffset baseOffset, ssize_t seekOffset)
{
    return fs->pIO->Seek(fs, baseOffset, seekOffset);
}

/// Gets the current seek position in the file.
static inline ssize_t fsGetStreamSeekPosition(TFFileStream* fs) { return fs->pIO->GetSeekPosition(fs); }

/// Gets the current size of the file. Returns -1 if the size is unknown or unavailable.
static inline ssize_t fsGetStreamFileSize(TFFileStream* fs) { return fs->pIO->GetFileSize(fs); }

/// Flushes all writes to the file stream to the underlying subsystem.
static inline bool fsFlushStream(TFFileStream* fs)
{
    if (!fs->pIO->Flush)
        return false;
...
/// Returns whether the current seek position is at the end of the file stream.
static inline bool fsStreamAtEnd(TFFileStream* fs) { return fs->pIO->IsAtEnd(fs); }

static inline bool fsIoGetFileUid(TFIFileSystem* pIO, TFResourceDirectory rd, const char* fileName, uint64_t* outUid)
{
    if (!pIO->GetFileUid)
        return false;
...
static inline bool fsIoOpenByUid(TFIFileSystem* pIO, uint64_t index, TFFileMode mode, TFFileStream* pOutStream)
{
    if (!pIO->OpenByUid)
        return false;
...
static inline bool fsStreamMemoryMap(TFFileStream* fs, size_t* outSize, void const** outData)
{
    if (!fs->pIO->MemoryMap)
        return false;
...
/// The `resourceDir` will making use of the given TFIFileSystem `pIO` file functions.
...
FORGE_API void fsSetPathForResourceDir(TFIFileSystem* pIO, TFResourceDirectory resourceDir, const char* bundledFolder);
...
// Artificial limit, to follow TF_FS_MAX_PATH.
// Avoid using TF_FS_MAX_PATH here because it varies, usually it is equal to 512
...
FORGE_API void fsArchiveGetDescription(TFIFileSystem* pArchive, struct BunyArDescription* outInfo);

FORGE_API bool fsArchiveGetNodeDescription(TFIFileSystem* pArchive, uint64_t nodeId, struct BunyArNodeDescription* outInfo);

// Same as GetFileUid(), but without fileName postprocessing.
// Uses fileName directly without resolving through TFResourceDirectory
// to search for file node.
FORGE_API bool fsArchiveGetNodeId(TFIFileSystem* fs, const char* fileName, uint64_t* outUid);

FORGE_API bool fsArchiveGetFileBlockMetadata(TFFileStream* pFile, struct BunyArBlockFormatHeader* outHeader,
