...
extern DXGI_COLOR_SPACE_TYPE util_to_dx12_colorspace(TFColorSpace colorspace);

HMODULE hook_get_d3d12_module_handle() { return GetModuleHandle(TEXT("d3d12.dll")); }

void hook_post_init_renderer(TFRenderer* pRenderer) { UNREF_PARAM(pRenderer); }

void hook_post_remove_renderer(TFRenderer* pRenderer) { UNREF_PARAM(pRenderer); }

HRESULT WINAPI d3d12dll_DXGIGetDebugInterface1(UINT Flags, REFIID riid, void** ppFactory);

void hook_enable_debug_layer(const TFRendererContextDesc* pDesc, TFRendererContext* pContext)
{
    UNREF_PARAM(pDesc);
...
#ifdef HOLOLENS2
#define PSS_ALIGN __declspec(align(8))

typedef PSS_ALIGN struct
{
...
{
    D3D12_PIPELINE_STATE_SUBOBJECT_TYPE type;
    D3D12_RASTERIZER_DESC               data;
} PSS_RasterizerDesc;

typedef PSS_ALIGN struct
{
    D3D12_PIPELINE_STATE_SUBOBJECT_TYPE type;
    D3D12_DEPTH_STENCIL_DESC            data;
} PSS_DepthStencilDesc;

typedef PSS_ALIGN struct
{
    D3D12_PIPELINE_STATE_SUBOBJECT_TYPE type;
    D3D12_INPUT_LAYOUT_DESC             data;
} PSS_InputLayoutDesc;

typedef PSS_ALIGN struct
{
    D3D12_PIPELINE_STATE_SUBOBJECT_TYPE type;
    D3D12_INDEX_BUFFER_STRIP_CUT_VALUE  data;
} PSS_IBStripCutValue;

typedef PSS_ALIGN struct
{
    D3D12_PIPELINE_STATE_SUBOBJECT_TYPE type;
    D3D12_PRIMITIVE_TOPOLOGY_TYPE       data;
} PSS_PrimitiveTopologyType;

typedef struct D3D12_RT_FORMAT_ARRAY D3D12_RT_FORMAT_ARRAY;
typedef PSS_ALIGN struct
{
    D3D12_PIPELINE_STATE_SUBOBJECT_TYPE type;
    D3D12_RT_FORMAT_ARRAY               data;
} PSS_RTFormatArray;
...
{
    D3D12_PIPELINE_STATE_SUBOBJECT_TYPE type;
    DXGI_SAMPLE_DESC                    data;
} PSS_SampleDesc;

typedef PSS_ALIGN struct
{
    D3D12_PIPELINE_STATE_SUBOBJECT_TYPE type;
    D3D12_VIEW_INSTANCING_DESC          data;
} PSS_ViewInstancingDesc;

typedef struct
{
    PSS_RootSignature         RootSignature;
    PSS_ShaderBytecode        VS;
    PSS_ShaderBytecode        PS;
    PSS_ShaderBytecode        GS;
    PSS_ShaderBytecode        HS;
    PSS_ShaderBytecode        DS;
    PSS_BlendDesc             BlendState;
    PSS_UINT                  SampleMask;
    PSS_RasterizerDesc        RasterizerState;
    PSS_DepthStencilDesc      DepthStencilState;
    PSS_InputLayoutDesc       InputLayout;
    PSS_IBStripCutValue       IBStripCutValue;
    PSS_PrimitiveTopologyType PrimitiveTopologyType;
    PSS_RTFormatArray         RTVFormats;
    PSS_DSVFormat             DSVFormat;
    PSS_SampleDesc            SampleDesc;
    PSS_UINT                  NodeMask;
    PSS_ViewInstancingDesc    ViewInstancing;
} PipelineStream;

typedef struct
{
    PSS_RootSignature  RootSignature;
    PSS_ShaderBytecode CS;
    PSS_UINT           NodeMask;
} ComputePipelineStream;

HRESULT hook_create_graphics_pipeline_state_vr(ID3D12Device* pDevice, const D3D12_GRAPHICS_PIPELINE_STATE_DESC* pDesc, bool vrMultiview,
                                               ID3D12PipelineState** ppPipeline)
{
    PipelineStream stream = { 0 };

    stream.RootSignature.type = D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_ROOT_SIGNATURE;
    stream.RootSignature.data = pDesc->pRootSignature;
    stream.VS.type = D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_VS;
    stream.VS.data = pDesc->VS;
    stream.PS.type = D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_PS;
    stream.PS.data = pDesc->PS;
    stream.GS.type = D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_GS;
    stream.GS.data = pDesc->GS;
    stream.HS.type = D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_HS;
    stream.HS.data = pDesc->HS;
    stream.DS.type = D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_DS;
    stream.DS.data = pDesc->DS;
    stream.BlendState.type = D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_BLEND;
    stream.BlendState.data = pDesc->BlendState;
    stream.SampleMask.type = D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_SAMPLE_MASK;
    stream.SampleMask.data = pDesc->SampleMask;
    stream.RasterizerState.type = D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_RASTERIZER;
    stream.RasterizerState.data = pDesc->RasterizerState;
    stream.DepthStencilState.type = D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_DEPTH_STENCIL;
    stream.DepthStencilState.data = pDesc->DepthStencilState;
    stream.InputLayout.type = D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_INPUT_LAYOUT;
    stream.InputLayout.data = pDesc->InputLayout;
    stream.IBStripCutValue.type = D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_IB_STRIP_CUT_VALUE;
    stream.IBStripCutValue.data = pDesc->IBStripCutValue;
    stream.PrimitiveTopologyType.type = D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_PRIMITIVE_TOPOLOGY;
    stream.PrimitiveTopologyType.data = pDesc->PrimitiveTopologyType;
    stream.RTVFormats.type = D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_RENDER_TARGET_FORMATS;
    stream.RTVFormats.data.NumRenderTargets = pDesc->NumRenderTargets;
    memcpy(stream.RTVFormats.data.RTFormats, pDesc->RTVFormats, sizeof(pDesc->RTVFormats));
    stream.DSVFormat.type = D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_DEPTH_STENCIL_FORMAT;
    stream.DSVFormat.data = pDesc->DSVFormat;
    stream.SampleDesc.type = D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_SAMPLE_DESC;
    stream.SampleDesc.data = pDesc->SampleDesc;
    stream.NodeMask.type = D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_NODE_MASK;
    stream.NodeMask.data = pDesc->NodeMask;

    D3D12_VIEW_INSTANCE_LOCATION viewInstanceLocations[2] = { { 0, 0 }, { 0, 1 } };
    stream.ViewInstancing.type = D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_VIEW_INSTANCING;
    stream.ViewInstancing.data.ViewInstanceCount = vrMultiview ? 2 : 0;
    stream.ViewInstancing.data.pViewInstanceLocations = viewInstanceLocations;
    stream.ViewInstancing.data.Flags = D3D12_VIEW_INSTANCING_FLAG_NONE;
    ...
    HRESULT        hres = COM_CALL(QueryInterface, pDevice, IID_ARGS(ID3D12Device2, &device2));
    if (!SUCCEEDED(hres))
        return E_NOINTERFACE;

    hres = COM_CALL(CreatePipelineState, device2, &streamDesc, IID_ARGS(ID3D12PipelineState, ppPipeline));
    COM_CALL(Release, device2);
...
#ifdef HOLOLENS2
    return hook_create_graphics_pipeline_state_vr(pDevice, pDesc, true, ppPipeline);
#else
...
void hook_remove_pipeline(TFPipeline* pPipeline)
{
#if defined(TF_ENABLE_WORKGRAPH)
    if (TF_PIPELINE_TYPE_WORKGRAPH == pPipeline->mDx.mType)
    {
...
HRESULT hook_add_special_buffer(TFRenderer* pRenderer, const D3D12_RESOURCE_DESC* pDesc, const D3D12_CLEAR_VALUE* pClearValue,
                                D3D12_RESOURCE_STATES state, uint32_t flags, TFBuffer* pBuffer)
{
    UNREF_PARAM(pClearValue);
    if (!(flags & TF_BUFFER_CREATION_FLAG_MARKER))
    {
        return E_NOINTERFACE;
    }
...
HRESULT hook_add_special_texture(TFRenderer* pRenderer, const D3D12_RESOURCE_DESC* pDesc, const D3D12_CLEAR_VALUE* pClearValue,
                                 D3D12_RESOURCE_STATES startState, uint32_t flags, TFTexture* pOutTexture)
{
...
HRESULT hook_add_placed_resource(TFRenderer* pRenderer, const TFResourcePlacement* pPlacement, const D3D12_RESOURCE_DESC* pDesc,
...
{
    LONG x_intersection = (LONG)maxi(0L, mini(rect1.right, rect2.right) - maxi(rect1.left, rect2.left));
    LONG y_intersection = (LONG)maxi(0L, mini(rect1.bottom, rect2.bottom) - maxi(rect1.top, rect2.top));
    LONG intersection = x_intersection * y_intersection;
    return intersection;
}

TinyImageFormat hook_get_recommended_swapchain_format(TFRenderer* pRenderer, const TFSwapChainDesc* pDesc, TFColorSpace colorSpace)
{
    ...
    CHECK_HRESULT(COM_CALL(QueryInterface, bestOutput, IID_REF(IDXGIOutput6), &output6));
    ...
    if (outputSupportsHDR)
    {
        ...
        TFQueue*         pQueue = pDesc->ppPresentQueues[0];
...
uint32_t hook_get_swapchain_image_index(TFSwapChain* pSwapChain) { return COM_CALL(GetCurrentBackBufferIndex, pSwapChain->mDx.pSwapChain); }

HRESULT hook_acquire_next_image(ID3D12Device* pDevice, TFSwapChain* pSwapChain)
{
    UNREF_PARAM(pDevice);
...
HRESULT hook_queue_present(TFQueue* pQueue, TFSwapChain* pSwapChain, uint32_t imageIndex)
{
    UNREF_PARAM(pQueue);
...
void hook_dispatch(TFCmd* pCmd, uint32_t groupCountX, uint32_t groupCountY, uint32_t groupCountZ)
{
    COM_CALL(Dispatch, pCmd->mDx.pCmdList, groupCountX, groupCountY, groupCountZ);
}

HRESULT hook_signal(TFQueue* pQueue, ID3D12Fence* pFence, uint64_t fenceValue)
{
    return COM_CALL(Signal, pQueue->mDx.pQueue, pFence, fenceValue);
}

HRESULT hook_wait(TFQueue* pQueue, ID3D12Fence* pFence, uint64_t fenceValue)
{
    return COM_CALL(Wait, pQueue->mDx.pQueue, pFence, fenceValue);
}

HRESULT hook_signal_flush(TFQueue* pQueue, ID3D12Fence* pFence, uint64_t fenceValue) { return hook_signal(pQueue, pFence, fenceValue); }
...
extern void hook_fill_gpu_desc(ID3D12Device* pDevice, D3D_FEATURE_LEVEL featureLevel, DXGPUInfo* pInOutDesc)
{
    ...
    wcstombs_s(&numConverted, pGpuDesc->mName, TF_MAX_GPU_VENDOR_STRING_LENGTH, desc.Description, TF_MAX_GPU_VENDOR_STRING_LENGTH);
}
...
void hook_modify_heap_flags(TFDescriptorType a, D3D12_HEAP_FLAGS* b)
{
    UNREF_PARAM(a);
    UNREF_PARAM(b);
}

void hook_modify_buffer_resource_desc(const TFBufferDesc* a, D3D12_RESOURCE_DESC* b)
{
    UNREF_PARAM(a);
    UNREF_PARAM(b);
}

void hook_modify_texture_resource_flags(TFTextureCreationFlags a, D3D12_RESOURCE_FLAGS* b)
{
    UNREF_PARAM(a);
    UNREF_PARAM(b);
}
...
void hook_pre_resolve_query(TFCmd* pCmd) { UNREF_PARAM(pCmd); }

void hook_fill_occlusion_query(TFCmd* pCmd, uint32_t queryIndex)
{
    UNREF_PARAM(pCmd);
...
extern const D3D12_COMMAND_LIST_TYPE gDx12CmdTypeTranslator[TF_MAX_QUEUE_TYPE];
HRESULT hook_create_cmd(ID3D12Device* pDevice, TFQueueType queueType, uint32_t nodeMask, ID3D12CommandAllocator* pAlloc, TFCmd* pCmd)
{
...
void hook_remove_cmd(TFCmd* pCmd) { COM_RELEASE(pCmd->mDx.pCmdList); }

void hook_resource_barrier(TFCmd* pCmd, UINT NumBarriers, const D3D12_RESOURCE_BARRIER* pBarriers)
{
    COM_CALL(ResourceBarrier, pCmd->mDx.pCmdList, NumBarriers, pBarriers);
}

void hook_copy_buffer_region(TFCmd* pCmd, ID3D12Resource* pDstBuffer, UINT64 DstOffset, ID3D12Resource* pSrcBuffer, UINT64 SrcOffset,
                             UINT64 NumBytes)
{
    COM_CALL(CopyBufferRegion, pCmd->mDx.pCmdList, pDstBuffer, DstOffset, pSrcBuffer, SrcOffset, NumBytes);
}

void hook_copy_texture_region(TFCmd* pCmd, const D3D12_TEXTURE_COPY_LOCATION* pDst, UINT DstX, UINT DstY, UINT DstZ,
