...
typedef struct DXGPUInfo
{
    ...
    char                                mName[TF_MAX_GPU_VENDOR_STRING_LENGTH];
...
extern void hook_post_init_renderer(TFRenderer* pRenderer);
extern void hook_post_remove_renderer(TFRenderer* pRenderer);
extern void hook_enable_debug_layer(const TFRendererContextDesc* pDesc, TFRendererContext* pRenderer);
...
extern HRESULT hook_create_cmd(ID3D12Device* pDevice, TFQueueType queueType, uint32_t nodeMask, ID3D12CommandAllocator* pAlloc,
                               TFCmd* pCmd);
extern void    hook_remove_cmd(TFCmd* pCmd);
...
extern HRESULT hook_create_graphics_pipeline_state_vr(ID3D12Device* pDevice, const D3D12_GRAPHICS_PIPELINE_STATE_DESC* pDesc,
                                                      bool vrMultiview, ID3D12PipelineState** ppPipeline);
#endif
...
extern HRESULT hook_add_special_buffer(TFRenderer* pRenderer, const D3D12_RESOURCE_DESC* pDesc, const D3D12_CLEAR_VALUE* pClearValue,
                                       D3D12_RESOURCE_STATES startState, uint32_t flags, TFBuffer* pOutBuffer);
extern HRESULT hook_add_special_texture(TFRenderer* pRenderer, const D3D12_RESOURCE_DESC* pDesc, const D3D12_CLEAR_VALUE* pClearValue,
                                        D3D12_RESOURCE_STATES startState, uint32_t flags, TFTexture* pOutTexture);
extern HRESULT hook_add_placed_resource(TFRenderer* pRenderer, const TFResourcePlacement* pPlacement, const D3D12_RESOURCE_DESC* pDesc,
                                        const D3D12_CLEAR_VALUE* pClearValue, D3D12_RESOURCE_STATES startState,
                                        ID3D12Resource** ppOutResource);
// HRESULT create_swap_chain(struct TFRenderer* pRenderer, struct TFSwapChain* pSwapChain, DXGI_SWAP_CHAIN_DESC1* desc,
// IDXGISwapChain1** swapchain);
extern void    hook_remove_pipeline(TFPipeline* pPipeline);

extern TinyImageFormat hook_get_recommended_swapchain_format(TFRenderer* pRenderer, const TFSwapChainDesc* pDesc, TFColorSpace colorSpace);
extern uint32_t        hook_get_swapchain_image_index(TFSwapChain* pSwapChain);
extern HRESULT         hook_acquire_next_image(ID3D12Device* pDevice, TFSwapChain* pSwapChain);
extern HRESULT         hook_queue_present(TFQueue* pQueue, TFSwapChain* pSwapChain, uint32_t swapChainImageIndex);
extern void            hook_dispatch(TFCmd* pCmd, uint32_t groupCountX, uint32_t groupCountY, uint32_t groupCountZ);
extern HRESULT         hook_signal(TFQueue* pQueue, ID3D12Fence* pFence, uint64_t fenceValue);
extern HRESULT         hook_wait(TFQueue* pQueue, ID3D12Fence* pFence, uint64_t fenceValue);
extern HRESULT         hook_signal_flush(TFQueue* pQueue, ID3D12Fence* pFence, uint64_t fenceValue);
...
extern void        hook_modify_heap_flags(TFDescriptorType type, D3D12_HEAP_FLAGS* pInOutFlags);
extern void        hook_modify_buffer_resource_desc(const TFBufferDesc* pBuffer, D3D12_RESOURCE_DESC* pInOutDesc);
extern void        hook_modify_texture_resource_flags(TFTextureCreationFlags creationFlags, D3D12_RESOURCE_FLAGS* pInOut);
...
extern void        hook_pre_resolve_query(TFCmd* pCmd);
extern void        hook_fill_occlusion_query(TFCmd* pCmd, uint32_t sampleCount);
...
extern void hook_cmd_write_marker(TFCmd*, const TFMarkerDesc*);
#endif

// Smooth over differences between ID3D12GraphicsCommandList1/ID3D12XboxDmaCommandList
extern void hook_resource_barrier(TFCmd* pCmd, UINT NumBarriers, const D3D12_RESOURCE_BARRIER* pBarriers);
extern void hook_copy_buffer_region(TFCmd* pCmd, ID3D12Resource* pDstBuffer, UINT64 DstOffset, ID3D12Resource* pSrcBuffer, UINT64 SrcOffset,
                                    UINT64 NumBytes);
extern void hook_copy_texture_region(TFCmd* pCmd, const D3D12_TEXTURE_COPY_LOCATION* pDst, UINT DstX, UINT DstY, UINT DstZ,
...
extern void hook_ClearDepthStencilView(_Inout_ ID3D12GraphicsCommandList1* pCommandList, _In_ D3D12_CPU_DESCRIPTOR_HANDLE DepthStencilView,
...
extern void hook_ClearRenderTargetView(_Inout_ ID3D12GraphicsCommandList1* pCommandList, _In_ D3D12_CPU_DESCRIPTOR_HANDLE RenderTargetView,
...
extern void hook_IASetPrimitiveTopology(_Inout_ ID3D12GraphicsCommandList1* pCommandList, _In_ D3D12_PRIMITIVE_TOPOLOGY PrimitiveTopology);
...
extern void hook_BeginQuery(_Inout_ ID3D12GraphicsCommandList1* pCommandList, _In_ ID3D12QueryHeap* pQueryHeap, _In_ D3D12_QUERY_TYPE Type,
                            _In_ UINT Index);

extern void hook_EndQuery(_Inout_ ID3D12GraphicsCommandList1* pCommandList, _In_ ID3D12QueryHeap* pQueryHeap, _In_ D3D12_QUERY_TYPE Type,
                          _In_ UINT Index);
...
extern HRESULT hook_CreateStateObject(ID3D12Device5* This, const D3D12_STATE_OBJECT_DESC* pDesc, REFIID riid, _Out_ void** ppStateObject);
#endif
...
extern void hook_BuildRaytracingAccelerationStructure(
    ID3D12GraphicsCommandList4* This, _In_ const D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC* pDesc, _In_ UINT NumPostbuildInfoDescs,
