...
NSData*           util_resolve_counter_sample_buffer(uint32_t startSample, uint32_t sampleCount, id<MTLCounterSampleBuffer> pSampleBuffer);
...
void cmdBindDescriptorSet(TFCmd* pCmd, uint32_t index, TFDescriptorSet* pDescriptorSet)
{
    ...
    if (!pCmd->pBoundPipeline)
        return;
    uint8_t stages =
        (pCmd->pBoundPipeline->mType == TF_PIPELINE_TYPE_COMPUTE) ? TF_SHADER_STAGE_COMP : TF_SHADER_STAGE_VERT | TF_SHADER_STAGE_FRAG;
        ...
        if (pDesc)
        {
            ...
            case TF_DESCRIPTOR_TYPE_RW_TEXTURE:
            {
                const MTLResourceUsage usage =
                    type == TF_DESCRIPTOR_TYPE_RW_TEXTURE ? MTLResourceUsageRead | MTLResourceUsageWrite : MTLResourceUsageRead;
                ...
                for (uint32_t j = 0; j < arrayCount; ++j)
                {
                    id<MTLTexture> texture =
                        pParam->mUseTextureDescriptors ? pParam->ppTextureDescriptors[j]->pSrvUav : pParam->ppTextures[j]->mDesc.pSrvUav;
                    ASSERT(texture);
                    if (useArgumentBuffer)
                    {
                        ...
                        root->pArr[arrayStart + j] = texture;
                }
                break;
            }
            case TF_DESCRIPTOR_TYPE_INDIRECT_COMMAND_BUFFER:
...
void util_to_blend_desc(const TFBlendStateDesc* pDesc, MTLRenderPipelineColorAttachmentDescriptorArray* attachments,
...
static void QueryGpuDesc(TFGpuDesc* pGpuDesc)
{
    ...
    pGpuDesc->mMaxVertexInputBindings =
        TF_MAX_VERTEX_BINDINGS;                // there are no special vertex buffers for input in Metal, only regular buffers
...
#ifdef ENABLE_GPU_FAMILY_8
    if ([(__bridge id<MTLDevice>)pGpuDesc->pGPU supportsFamily:MTLGPUFamilyApple8] &&
        [(__bridge id<MTLDevice>)pGpuDesc->pGPU supportsFamily:MTLGPUFamilyMac2])
    {
...
void exitRendererContext(TFRendererContext* pContext)
{
    ...
    {
        if (pContext->mGpus[i].pCounterSetTimestamp)
            CFRelease(pContext->mGpus[i].pCounterSetTimestamp);
        if (pContext->mGpus[i].pGPU)
            CFRelease(pContext->mGpus[i].pGPU);
    }
    SAFE_FREE(pContext);
}
...
void addSwapChain(TFRenderer* pRenderer, const TFSwapChainDesc* pDesc, TFSwapChain** ppSwapChain)
{
    ...
    {
        LOGF(eWARNING, "Metal: CAMetalLayer device and TFRenderer device are not matching. Fixing by assigning TFRenderer device to "
                       "CAMetalLayer device");
...
void addBuffer(TFRenderer* pRenderer, const TFBufferDesc* pDesc, TFBuffer** ppBuffer)
{
    ...
    if (pBuffer->pBuffer && !(resourceOptions & MTLResourceStorageModePrivate) &&
        (pDesc->mFlags & TF_BUFFER_CREATION_FLAG_PERSISTENT_MAP_BIT))
    {
...
void addRenderTarget(TFRenderer* pRenderer, const TFRenderTargetDesc* pDesc, TFRenderTarget** ppRenderTarget)
{
    ...
    MTRACY_TEXT(pDesc->pName);
    MTRACY_VALUE(uint64_t(pDesc->mWidth) * pDesc->mHeight);
    ASSERT(pRenderer);
    ASSERT(pDesc);
...
void cmdBindRenderTargets(TFCmd* pCmd, const TFBindRenderTargetsDesc* pDesc)
{
    ...
    auto decode = [](const TFBindRenderTargetDesc& input)
    {
        ...
        out.mLoadAction = input.mLoadAction;
        out.mStoreAction = input.mStoreAction;
        out.mLoadActionStencil = input.mLoadActionStencil;
        out.mStoreActionStencil = input.mStoreActionStencil;
        out.mClearValue = input.mClearValue;
        out.mOverrideClearValue = input.mOverrideClearValue;
        if (view)
        {
            out.mUseMipSlice = view->mUseMipSlice;
            out.mUseArraySlice = view->mUseArraySlice;
            out.mMipSlice = view->mMipSlice;
            out.mArraySlice = view->mArraySlice;
        }
        return out;
    };
    ...
    @autoreleasepool
    {
        ...
        TFSampleCount sampleCount =
            pDesc->mRenderTargetCount > 0 ? pDesc->mRenderTargets[0].pRenderTarget->mSampleCount : TF_SAMPLE_COUNT_1;
...
void cmdUpdateSubresource(TFCmd* pCmd, TFTexture* pTexture, TFBuffer* pIntermediate, const SubresourceDataDesc* pSubresourceDesc)
{
    ...
    MTLSize sourceSize =
        MTLSizeMake(pSubresourceDesc->mWidth, pSubresourceDesc->mHeight, max(1, pTexture->mDepth >> pSubresourceDesc->mMipLevel));
...
void add_texture(TFRenderer* pRenderer, const TFTextureDesc* pDesc, TFTexture** ppTexture, const bool isRT)
{
    ...
    if (pDesc->mFormat == TinyImageFormat_D24_UNORM_S8_UINT &&
        !(pRenderer->pGpu->mFormatCaps[pDesc->mFormat] & TF_FORMAT_CAP_RENDER_TARGET))
    {
    ...
    else
    {
        ...
        if (!pTexture->pTexture)
        {
            if (pRenderer->pGpu->mHeaps)
            {
                ...
                if (pRenderer->pGpu->mPlacementHeaps)
                {
                    VmaAllocationInfo allocInfo = util_render_alloc(pRenderer, TF_RESOURCE_MEMORY_USAGE_GPU_ONLY, memoryType,
                                                                    sizeAlign.size, sizeAlign.align, &pTexture->pAllocation);
