...
typedef struct TFFileWatcher
{
    char                   mPath[TF_FS_MAX_PATH];
    TFFileWatcherEventMask mEventMask;
    DWORD                  mNotifyFilter;
    TFFileWatcherCallback  mCallback;
...
{
    setCurrentThreadName("TFFileWatcher");
    TFFileWatcher* fs = (TFFileWatcher*)data;
    ...
    while (fs->mRun)
    {
        ...
        for (;;)
        {
            ...
            case FILE_ACTION_RENAMED_NEW_NAME:
                action = TF_FWE_CREATED;
                ignoreAction = !(fs->mEventMask & TF_FWE_CREATED);
                break;
            case FILE_ACTION_MODIFIED:
                if (fs->mNotifyFilter & FILE_NOTIFY_CHANGE_LAST_WRITE)
                    action = TF_FWE_MODIFIED;
                if (fs->mNotifyFilter & FILE_NOTIFY_CHANGE_LAST_ACCESS)
                    action = TF_FWE_ACCESSED;
                break;
            case FILE_ACTION_REMOVED:
            case FILE_ACTION_RENAMED_OLD_NAME:
                action = TF_FWE_DELETED;
                ignoreAction = !(fs->mEventMask & TF_FWE_DELETED);
                break;
            default:
                break;
            }
            ...
            if (!ignoreAction)
            {
                ...
                char fullPathToFile[TF_FS_MAX_PATH] = { 0 };
...
TFFileWatcher* fsCreateFileWatcher(const char* path, TFFileWatcherEventMask eventMask, TFFileWatcherCallback callback,
                                   void* callbackUserData)
{
    TFFileWatcher* watcher = (TFFileWatcher*)tf_calloc(1, sizeof(TFFileWatcher));
    // watcher->mWatchDir = fsCopyPath(path);
    strcpy(watcher->mPath, path);

    uint32_t notifyFilter = 0;
    if (eventMask & TF_FWE_MODIFIED)
    {
        notifyFilter |= FILE_NOTIFY_CHANGE_LAST_WRITE;
    }
    if (eventMask & TF_FWE_ACCESSED)
    {
        notifyFilter |= FILE_NOTIFY_CHANGE_LAST_ACCESS;
    }
    if (eventMask & (TF_FWE_DELETED | TF_FWE_CREATED))
    {
    ...
    strncpy(threadDesc.mThreadName, "TFFileWatcher", sizeof(threadDesc.mThreadName));
    initThread(&threadDesc, &watcher->mThread);

    return watcher;
}

void fsFreeFileWatcher(TFFileWatcher* fileWatcher)
{
...
bool fsRemoveFile(const TFResourceDirectory resourceDir, const char* fileName)
{
    const char* resourcePath = fsGetResourceDirectory(resourceDir);
    char        filePath[TF_FS_MAX_PATH] = {};
...
bool fsRenameFile(const TFResourceDirectory resourceDir, const char* fileName, const char* newFileName)
{
    const char* resourcePath = fsGetResourceDirectory(resourceDir);
    ...
    {
        char newfilePath[TF_FS_MAX_PATH] = {};
...
bool fsCopyFile(const TFResourceDirectory sourceResourceDir, const char* sourceFileName, const TFResourceDirectory destResourceDir,
                const char* destFileName)
{
    ...
    char sourceFilePath[TF_FS_MAX_PATH] = {};
    ...
    char destFilePath[TF_FS_MAX_PATH] = {};
...
bool fsFileExist(const TFResourceDirectory resourceDir, const char* fileName)
{
    const char* resourcePath = fsGetResourceDirectory(resourceDir);
    char        filePath[TF_FS_MAX_PATH] = {};
    fsAppendPathComponent(resourcePath, fileName, filePath);
...
bool create_directories(const char* path, bool recursive)
{
    ...
    char directoryPath[TF_FS_MAX_PATH] = {};
    strncpy(directoryPath, path, TF_FS_MAX_PATH - 1);
    directoryPath[TF_FS_MAX_PATH - 1] = 0;
...
bool fsCreateDirectory(const TFResourceDirectory resourceDir, const char* path, bool recursive)
{
    const char* resourcePath = fsGetResourceDirectory(resourceDir);
    char        directoryPath[TF_FS_MAX_PATH] = {};
...
bool fsCheckPath(TFResourceDirectory rd, const char* path, bool* exist, bool* isDir, bool* isFile)
{
    char fullPath[TF_FS_MAX_PATH] = { 0 };
...
time_t fsGetLastModifiedTime(TFResourceDirectory rd, const char* fileName)
{
...
struct WindowsFsDirectoryIterator
{
    ...
    char             openedPath[TF_FS_MAX_PATH];
};

static bool windowsFsDirectoryIteratorReopen(struct WindowsFsDirectoryIterator* iterator)
{
    wchar_t buffer[TF_FS_MAX_PATH] = {};
    size_t  utf16Len;
    if (mbstowcs_s(&utf16Len, buffer, iterator->openedPath, TF_FS_MAX_PATH - 2))
        return false;
...
bool fsDirectoryIteratorOpen(TFResourceDirectory rd, const char* dir, TFFsDirectoryIterator* out)
{
    *out = 0;
...
void fsDirectoryIteratorClose(TFFsDirectoryIterator data)
{
    if (!data)
        return;
...
bool fsDirectoryIteratorNext(TFFsDirectoryIterator data, struct TFFsDirectoryIteratorEntry* outEntry)
{
    struct WindowsFsDirectoryIterator* iterator = (struct WindowsFsDirectoryIterator*)data;
