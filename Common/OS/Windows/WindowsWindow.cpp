...
RECT         getCenteredWindowRect(TFWindowDesc* winDesc);
static DWORD PrepareStyleMask(TFWindowDesc* winDesc);
...
static void WindowRectFromClient(TFRectDesc* rectDescOut, const TFRectDesc* clientRect, DWORD windowStyle, bool bMenu)
{
    RECT windowedRect = {};
...
// Fetches the fullscreen rectangle from the system and assigns it to TFWindowDesc
static void ResetWindowDescFullScreenRect(TFWindowDesc* winDesc)
{
...
// Fetches the client rectangle from the system and assigns it to TFWindowDesc
static void ResetWindowDescClientRect(TFWindowDesc* winDesc)
{
    RECT clientRect;
...
// Fetches the window rectangle from the system and assigns it to TFWindowDesc
static void ResetWindowDescWindowedRect(TFWindowDesc* winDesc)
{
    RECT windowedRect;
    HWND hwnd = (HWND)winDesc->handle.window;
...
static void UpdateWindowDescClientSize(TFWindowDesc* winDesc, int32_t clientWidth, int32_t clientHeight)
{
...
static void UpdateWindowDescClientPos(TFWindowDesc* winDesc, int32_t xPos, int32_t yPos)
{
...
static void UpdateWindowDescWindowedPos(TFWindowDesc* winDesc, int32_t xPos, int32_t yPos)
{
    int32_t dx = xPos - winDesc->clientRect.left;
    int32_t dy = yPos - winDesc->clientRect.top;
...
static void OffsetRectToDisplay(TFWindowDesc* winDesc, LPRECT rect)
{
...
static void onResize(TFWindowDesc* wnd, int32_t newSizeX, int32_t newSizeY)
{
...
static void updateMonitorResolution(TFMonitorDesc* pMonitor, uint32_t newWidth, uint32_t newHeight)
{
...
void adjustWindow(TFWindowDesc* winDesc, bool isDisplayChange)
{
    HWND hwnd = (HWND)winDesc->handle.window;
...
void collectMonitorInfo()
{
    ...
    if (monitorCount)
    {
        ...
        gMonitors = (TFMonitorDesc*)tf_calloc(monitorCount, sizeof(TFMonitorDesc));
        for (int adapterIndex = 0;; ++adapterIndex)
        {
            if (!EnumDisplayDevicesW(NULL, adapterIndex, &adapter, 0))
                break;
            ...
            for (int displayIndex = 0;; displayIndex++)
            {
                ...
                {
                    TFMonitorDesc mDesc = gMonitors[0];
    ...
    {
        TFMonitorDesc* pMonitor = &gMonitors[monitor];
        DEVMODEW       devMode = {};
...
void openWindow(const char* app_name, TFWindowDesc* winDesc)
{
    ...
    TFRectDesc rect = winDesc->clientRect;
    DWORD      windowStyle = PrepareStyleMask(winDesc);
    ...
    WCHAR  app[TF_FS_MAX_PATH] = {};
    size_t charConverted = 0;
    mbstowcs_s(&charConverted, app, app_name, TF_FS_MAX_PATH);
    ...
    if (hwnd != NULL)
    {
        winDesc->handle.type = TF_WINDOW_HANDLE_TYPE_WIN32;
...
void setWindowRect(TFWindowDesc* winDesc, const TFRectDesc* rect)
{
    ASSERT(winDesc);
    ASSERT(rect);
    if (winDesc->fullScreen)
...
void toggleFullscreen(TFWindowDesc* pWindow)
{
...
{
    TFMonitorDesc* monitor = getMonitor(monitorIndex);
...
bool getResolutionSupport(const TFMonitorDesc* pMonitor, const TFResolution* pRes)
{
