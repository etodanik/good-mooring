...
static const TFInputEnum gScanCodeToInput[256][2] = {
...
struct XGamepad
{
    ...
    TFInputPortIndex    mPortIndex;
    bool                mActive;
};

static XGamepad gXGamepads[TF_MAX_GAMEPADS] = {};
static int32_t  gCursorPos[2] = {};
static int32_t  gRawMouseDelta[2] = {};

static TFWindowDesc* pWindow = {};
/************************************************************************/
// Gamepad
/************************************************************************/

TFInputPortIndex GamepadFindEmptySlot()
{
...
TFInputPortIndex GamepadAddHIDController(const HIDDeviceInfo* pDeviceInfo)
{
    UNREF_PARAM(pDeviceInfo);
    TFInputPortIndex portIndex = GamepadFindEmptySlot();
...
void GamepadRemoveHIDController(TFInputPortIndex portIndex)
{
    if (portIndex < 0 || portIndex >= TF_MAX_GAMEPADS)
    {
        return;
    }
...
{
    static const char* xinputControllers[TF_MAX_GAMEPADS] = {
    ...
    else
    {
        ...
        if (xgpad.mActive)
        {
            ...
            if (gGamepadRemovedCb)
            {
                gGamepadRemovedCb(xgpad.mPortIndex);
            }
        }
    }
}
...
static void GamepadUpdateState(uint32_t index)
{
    ...
    TFInputPortIndex portIndex = xgpad.mPortIndex;
...
static void ProcessKeyEvent(const MSG* msg)
{
    ...
    TFInputEnum key = gScanCodeToInput[scanCode][isExtendedKey];
    gInputValues[key] = keyDown ? 1.0f : 0.0f;
    gInputRepeat[key] = keyDown;
}
...
/************************************************************************/
void platformInitInput(TFWindowDesc* window)
{
    pWindow = window;
    gRelativeMouseLock = {};
...
{
    GamepadDefault();
    for (uint32_t index = 0; index < TF_MAX_GAMEPADS; ++index)
    {
...
#endif
        captureCursor(pWindow, capture && inputGetValue(0, MOUSE_2));
    }

    inputUpdateDoubleStates();
}
...
void platformInputEvent(const MSG* msg)
{
    ...
    case WM_MOUSEMOVE:
    {
        gCursorPos[0] = GET_X_LPARAM(msg->lParam);
        gCursorPos[1] = GET_Y_LPARAM(msg->lParam);
        break;
    }
    case WM_MOUSEWHEEL:
    {
