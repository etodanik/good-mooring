...
bool initFileSystem(TFFileSystemInitDesc* pDesc)
{
    if (gInitialized)
    {
...
static inline FORGE_CONSTEXPR const char* fsFileModeToString(TFFileMode mode)
{
    mode = (TFFileMode)(mode & ~TF_FM_ALLOW_READ);
    switch (mode)
    {
    case TF_FM_READ:
        return "rb";
    case TF_FM_WRITE:
        return "wb";
    case TF_FM_WRITE_APPEND:
        return "ab";
    case TF_FM_READ_WRITE:
        return "rb+";
    case TF_FM_READ_APPEND:
        return "ab+";
    default:
        return "r";
    }
...
static inline FORGE_CONSTEXPR const char* fsOverwriteFileModeToString(TFFileMode mode)
{
    switch (mode)
    {
    case TF_FM_READ_WRITE:
...
bool PlatformOpenFile(TFResourceDirectory resourceDir, const char* fileName, TFFileMode mode, TFFileStream* pOut)
{
    __FS_NO_ERR;
    memset(pOut, 0, sizeof *pOut);

    char filePath[TF_FS_MAX_PATH] = { 0 };
    strcat_s(filePath, TF_FS_MAX_PATH, gResourceDirectories[resourceDir].mPath);
    strcat_s(filePath, TF_FS_MAX_PATH, fileName);

    // Path utf-16 conversion
    size_t  filePathLen = strlen(filePath) + 1;
    wchar_t pathStr[TF_FS_MAX_PATH];
    ASSERT(filePathLen < TF_FS_MAX_PATH);
    ...
    if (!fp)
    {
        ...
        if ((mode & TF_FM_READ_WRITE) == TF_FM_READ_WRITE)
        {
...
static bool ioWindowsFsOpen(TFIFileSystem*, TFResourceDirectory rd, const char* fileName, TFFileMode mode, TFFileStream* pOut)
{
    return PlatformOpenFile(rd, fileName, mode, pOut);
}

static ssize_t ioWindowsFsGetSize(TFFileStream* fs)
{
    WSD(stream, fs);
...
static bool ioWindowsFsMemoryMap(TFFileStream* fs, size_t* outSize, void const** outData)
{
    __FS_NO_ERR;
    *outSize = 0;
    *outData = NULL;
...
static bool ioWindowsFsClose(TFFileStream* fs)
{
    __FS_NO_ERR;
    WSD(stream, fs);
...
static size_t ioWindowsFsRead(TFFileStream* fs, void* dst, size_t size)
{
    WSD(stream, fs);
...
static size_t ioWindowsFsWrite(TFFileStream* fs, const void* src, size_t size)
{
    __FS_NO_ERR;
    if ((fs->mMode & (TF_FM_WRITE | TF_FM_APPEND)) == 0)
    {
        __FS_SET_ERR(TF_FS_NOT_PERMITTED_ERR);
        return 0;
    }
...
static bool ioWindowsFsSeek(TFFileStream* fs, TFSeekBaseOffset base, ssize_t offset)
{
    __FS_NO_ERR;
    int origin = SEEK_SET;
    switch (base)
...
static ssize_t ioWindowsFsGetPosition(TFFileStream* fs)
{
    __FS_NO_ERR;
    WSD(stream, fs);
...
bool ioWindowsFsFlush(TFFileStream* fs)
{
    __FS_NO_ERR;
    WSD(stream, fs);
...
bool ioWindowsFsIsAtEnd(TFFileStream* fs)
{
    WSD(stream, fs);
    return feof(stream->file) != 0;
}

void* ioWindowsGetSystemHandle(TFFileStream* fs)
{
    WSD(stream, fs);
...
static TFIFileSystem gWindowsFileIO = {
    ioWindowsFsOpen,
    ioWindowsFsClose,
    ioWindowsFsRead,
    ioWindowsFsWrite,
...
TFIFileSystem* pSystemFileIO = &gWindowsFileIO;
