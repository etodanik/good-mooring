...
fsl_orbis_root    = os.path.join(forge_root, 'PS4', 'Common', 'Tools', 'ForgeShadingLanguage')
fsl_prospero_root = os.path.join(forge_root, 'Prospero', 'Common', 'Tools', 'ForgeShadingLanguage')
fsl_xbox_root     = os.path.join(forge_root, 'Xbox', 'Common', 'Tools', 'ForgeShadingLanguage')
fsl_switch_root     = os.path.join(forge_root, 'Switch', 'Common', 'Tools', 'ForgeShadingLanguage')
sys.path.extend( [fsl_root, fsl_orbis_root, fsl_prospero_root, fsl_xbox_root, fsl_switch_root])
...
if not 'FSL_COMPILER_DXC' in os.environ:
    os.environ['FSL_COMPILER_DXC'] = os.path.normpath(forge_root+'/Common/Graphics/ThirdParty/OpenSource/DirectXShaderCompiler/bin/x64')
if not 'FSL_COMPILER_VK' in os.environ:
    os.environ['FSL_COMPILER_VK'] = os.path.normpath(forge_root+'/Common/Graphics/ThirdParty/OpenSource/VulkanSDK/bin/Win32/')
    os.environ['FSL_COMPILER_LINUX_VK'] = os.path.normpath(forge_root+'/Common/Graphics/ThirdParty/OpenSource/VulkanSDK/bin/Linux/')
...
def get_args():
    ...
    parser.add_argument('-D', '--defines', help='optional additional defines', nargs='+')
    ...
    parser.add_argument('--analyzeOffline', default=False, action='store_true', 
                        help='Perform additional lowering for supported target architectures in order to perform static analysis')
    parser.add_argument('-t', '--ispc_targetOs', help='ISPC: The target OS to compile for')
    parser.add_argument('-a', '--ispc_architecture', help='ISPC: The target architecture to compile for (x86_64/arm64)')
    parser.add_argument('--ispc_headerDestinationCPU', help='ISPC: The destination directory for CPU headers')
    parser.add_argument('--ispc_headerDestinationShader', help='ISPC: The destination directory for FSL shader headers')
...
def process_binary_declaration(job):
    ...
    if platform == Platforms.ISPC and not out_filepath.endswith(".ispc"):
        out_filepath += ".ispc"
    ...
    if args.compile:
        ...
        if args.verbose:
            if platform.name.lower() == 'ispc':
                print(f'Compiling {os.getpid()}: ISPC x86:{binary.ispc_target_x86} arm:{binary.ispc_target_arm} {binary.stage.name} \n\t{out_filepath}\n\t{bin_filepath}')
            else:
                print(f'Compiling {os.getpid()}: {platform.name} {binary.stage.name} \n\t{out_filepath}\n\t{bin_filepath}')
        try:
            compile_binary(platform, args.debug, binary, out_filepath, bin_filepath, args.intermediateDestination, args.defines, args.analyzeOffline)
        except SystemExit:
            return 1
...
def main():
    ...
    binary_declarations, fsl_dependencies = collect_shader_decl(args, fsl_input, args.ispc_targetOs, args.ispc_architecture, args.intermediateDestination, platforms, regen, dependencies, [])
