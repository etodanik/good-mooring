...
// Types
#define MatrixRef(coopDT, dimM, dimK, layout)            dx::linalg::MatrixRef<coopDT, dimM, dimK, layout>

#define RWMatrixRef(coopDT, dimM, dimK, layout)          dx::linalg::RWMatrixRef<coopDT, dimM, dimK, layout>

#define MatrixRefTranspose(coopDT, dimM, dimK, layout)   dx::linalg::MatrixRef<coopDT, dimM, dimK, layout, true>

#define RWMatrixRefTranspose(coopDT, dimM, dimK, layout) dx::linalg::RWMatrixRef<coopDT, dimM, dimK, layout, true>

#define VectorRef(coopDT)                                dx::linalg::VectorRef<coopDT>

#define RWVectorRef(coopDT)                              dx::linalg::RWVectorRef<coopDT>

#define CoopInterpretedVector(elemType, count, coopDT)   dx::linalg::InterpretedVector<elemType, count, coopDT>

// Construct interpreted vector
#define CoopVecMake(vec, elemType, count, coopDT)        dx::linalg::MakeInterpretedVector<coopDT, elemType, count>(vec)
...
#define CoopVecMulSimple(outType, matrixRef, interpVec) dx::linalg::Mul<outType>(matrixRef, interpVec)

// Matrix * Vector + Bias
#define CoopVecMulAdd(outType, inType, inCount, matBufType, coopDT, matDT, dimM, dimK, layout, transpose, biasBufType, biasDT, matrix, \
                      vector, bias)                                                                                                    \
    dx::linalg::MulAdd<outType, inType, inCount, matBufType, coopDT, matDT, dimM, dimK, layout, transpose, biasBufType, biasDT>(       \
        matrix, vector, bias)

#define CoopVecMulAddSimple(outType, matrixRef, interpVec, biasRef) dx::linalg::MulAdd<outType>(matrixRef, interpVec, biasRef)
...
#define CoopVecOuterProductAccumulateSimple(vec1, vec2, matrixRef) dx::linalg::OuterProductAccumulate(vec1, vec2, matrixRef)

// Vector accumulate
#define CoopVecAccumulate(vec, buffer, offset)                     dx::linalg::VectorAccumulate(vec, buffer, offset)
#endif