...
inline intersection_params GetMetalIntersectionParams()
{
    ...
    // Force opacity
    static_assert(!((rayFlags & RAY_FLAG_FORCE_OPAQUE) && (rayFlags & RAY_FLAG_FORCE_NON_OPAQUE)),
                  "Forcing both Opaque and Non-Opaque not allowed");
    ...
    // Cull opacity
    static_assert(!((rayFlags & RAY_FLAG_CULL_OPAQUE) && (rayFlags & RAY_FLAG_CULL_NON_OPAQUE)),
                  "Culling both Opaque and Non-Opaque not allowed");
    ...
    static_assert(!((rayFlags & RAY_FLAG_SKIP_TRIANGLES) && (rayFlags & RAY_FLAG_SKIP_AABB)),
                  "Skipping both Triangles and AABB not allowed");
...
// Desktop/mobile PoC renders a single view.
#ifndef VR_VIEW_ID
#define VR_VIEW_ID 0
#endif
