...
#define make_f3x3_row_elems(E00, E01, E02, E10, E11, E12, E20, E21, E22) f3x3(E00, E10, E20, E01, E11, E21, E02, E12, E22)
...
inline void AtomicStore(device uint& dst, uint val) { atomic_store_explicit((device atomic_uint*)&dst, val, memory_order_relaxed); }
...
inline uint AtomicLoad(device uint& src) { return atomic_load_explicit((device atomic_uint*)&src, memory_order_relaxed); }
inline uint AtomicLoad(threadgroup uint& src) { return atomic_load_explicit((threadgroup atomic_uint*)&src, memory_order_relaxed); }
...
void AtomicMax(device uint& dst, uint val) { atomic_fetch_max_explicit((device atomic_uint*)&(dst), val, memory_order_relaxed); }
...
void AtomicMin(device uint& dst, uint val) { atomic_fetch_min_explicit((device atomic_uint*)&(dst), val, memory_order_relaxed); }
...
void AtomicMinU64(device uint64_t& DEST, ulong VALUE) { atomic_min_explicit((device atomic_ulong*)&(DEST), VALUE, memory_order_relaxed); }
void AtomicMaxU64(device uint64_t& DEST, ulong VALUE) { atomic_max_explicit((device atomic_ulong*)&(DEST), VALUE, memory_order_relaxed); }
#endif
...
#define BeginNonUniformResourceIndex(X, Y) \
    {                                      \
        uint __resourceIndex = X;          \
        uint X = min(__resourceIndex, Y - 1U);
...
#define LoadByte3(BYTE_BUFFER, ADDRESS) \
    uint3(((BYTE_BUFFER)[((ADDRESS) >> 2) + 0]), ((BYTE_BUFFER)[((ADDRESS) >> 2) + 1]), ((BYTE_BUFFER)[((ADDRESS) >> 2) + 2]))
...
#define StoreByte2(BYTE_BUFFER, ADDRESS, VALUE)     \
    (BYTE_BUFFER)[((ADDRESS) >> 2) + 0] = VALUE[0]; \
    (BYTE_BUFFER)[((ADDRESS) >> 2) + 1] = VALUE[1];
#define StoreByte3(BYTE_BUFFER, ADDRESS, VALUE)     \
    (BYTE_BUFFER)[((ADDRESS) >> 2) + 0] = VALUE[0]; \
    (BYTE_BUFFER)[((ADDRESS) >> 2) + 1] = VALUE[1]; \
    (BYTE_BUFFER)[((ADDRESS) >> 2) + 2] = VALUE[2];
#define StoreByte4(BYTE_BUFFER, ADDRESS, VALUE)     \
    (BYTE_BUFFER)[((ADDRESS) >> 2) + 0] = VALUE[0]; \
    (BYTE_BUFFER)[((ADDRESS) >> 2) + 1] = VALUE[1]; \
    (BYTE_BUFFER)[((ADDRESS) >> 2) + 2] = VALUE[2]; \
    (BYTE_BUFFER)[((ADDRESS) >> 2) + 3] = VALUE[3];
...
// TFSampler types
#define TF_FILTER_NEAREST               nearest
#define TF_FILTER_LINEAR                linear

#define TF_MIPMAP_MODE_NEAREST          nearest
#define TF_MIPMAP_MODE_LINEAR           linear

#define TF_ADDRESS_MODE_REPEAT          repeat
#define TF_ADDRESS_MODE_MIRROR          mirrored_repeat
#define TF_ADDRESS_MODE_CLAMP_TO_EDGE   clamp_to_edge
#define TF_ADDRESS_MODE_CLAMP_TO_BORDER clamp_to_border

#define TF_CMP_NEVER                    never
#define TF_CMP_LESS                     less
#define TF_CMP_EQUAL                    equal
#define TF_CMP_LEQUAL                   less_equal
#define TF_CMP_GREATER                  greater
#define TF_CMP_NOTEQUAL                 not_equal
#define TF_CMP_GEQUAL                   greater_equal
#define TF_CMP_ALWAYS                   always
...
// #TODO - Define this
// #define TF_INDIRECT_COMMAND_BUFFER 1
...
void cmdDrawInstanced(command_buffer cmdbuf, uint slot, PRIMITIVE_TYPE prim, uint vertexCount, uint firstVertex, uint instanceCount,
                      uint firstInstance)
{
...
void cmdDraw(command_buffer cmdbuf, uint slot, PRIMITIVE_TYPE prim, uint vertexCount, uint firstVertex, uint instanceCount,
             uint firstInstance)
{
...
template<typename T>
void cmdDrawIndexedInstanced(command_buffer cmdbuf, uint slot, PRIMITIVE_TYPE prim, T indexBuffer, uint indexCount, uint firstIndex,
                             uint firstVertex, uint instanceCount, uint firstInstance)
{
    render_command cmd(cmdbuf, slot);
...
template<typename T>
void cmdDrawIndexed(command_buffer cmdbuf, uint slot, PRIMITIVE_TYPE prim, T indexBuffer, uint indexCount, uint firstIndex,
                    uint firstVertex)
{
