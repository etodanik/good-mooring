...
# Fast hash without random seed
def fnv1a(data: bytearray, v: int = 2166136261):
    for p in data:
        v = ((v ^ p) * 16777619) & 0xFFFFFFFF
    return v
...
platform_langs = {
    ...
    Platforms.ISPC :              'ISPC',
    Platforms.HOLOLENS2 :         'DIRECT3D12'
}

class OSTargets(Enum):
    NONE = 0
    WINDOWS = 1
    MACOS = 2
    LINUX = 3# Steam
    ...
    PS5 = 8
    SWITCH = 9
class ArchTargets(Enum):
    NONE = 0
    ARM64 = 1
    X86_64 = 2

class ISPCX86Targets(Enum):
    NONE    = 0
    SSE2   = 1
    SSE4 = 2
    AVX1    = 3
    AVX2   = 4
    AVX512KNL = 5 # Xeon Phi chips codename Knights Landing
    AVX512SKX = 6 # Skylake Xeon CPUs
    AVX512SPR = 7 # Sapphire Rapids Xeon CPUs, 4th generation Xeon Scalable
    ARM_NEON   = 8
# TODO: Intel GPU instruction sets?
class ISPCArmTargets(Enum):
    NONE = 0
    NEON = 1 # ISPC currently only outputs NEON for ARM targets
...
feature_mask[Features.MULTIVIEW] = [Platforms.QUEST, Platforms.HOLOLENS2]
...
class ShaderBinary:
    def __init__(self):
        ...
        self.ispc_target_x86 = ISPCX86Targets.NONE
        self.ispc_target_arm = ISPCArmTargets.NEON # Neon is the only on available at the moment so implicitly default to that
        self.ispc_cpu_header_dir = None
        self.ispc_shader_header_dir = None
        self.ispc_os_target = OSTargets.NONE
        self.ispc_arch_target = ArchTargets.NONE
        self.int_dir = ""

    def __hash__(self):
        result = 0
        for i, (_, value) in enumerate(vars(self).items()):           
            def bytearrayof(val):
                bytesvalue = bytearray()
                if val is None:
                    pass
                elif issubclass(type(val), Enum):
                    bytesvalue = bytearray(val.value)
                elif isinstance(val, str):
                    bytesvalue = bytearray(val, 'utf-8')
                elif isinstance(val, list) or isinstance(val, tuple):
                    for subval in val:
                        bytesvalue += bytearrayof(subval)
                elif isinstance(val, dict):
                    for subval in val.items():
                        bytesvalue += bytearrayof(subval)
                else:
                    bytesvalue = bytearray(val)
                return bytesvalue

            if i == 0:
                result = fnv1a(bytearrayof(value))
            else:
                result = fnv1a(bytearrayof(value), result)
        return abs(result)
...
def internal_dependencies():
    return [
        ...
        os.path.join(os.path.dirname(__file__), 'includes', 'ispc.h'),
        ...
        os.path.join(os.path.dirname(__file__), 'generators', 'ispc.py'),
    ]

def internal_timestamp():
    existing_deps = [d for d in internal_dependencies() if os.path.exists(d)]
    return max( [ os.path.getmtime(d) for d in existing_deps ] )
...
def getShader(platform: Platforms, binary: ShaderBinary, fsl: list, dst=None, line_directives=True) -> Shader:
    ...
    if binary.stage is Stages.GRAPH:
        ...
        entry_args = []
        waveops_flags = []
    elif binary.stage is Stages.ISPC:
        stage = Stages.ISPC
        entry_ret = 'void'
        entry_args = []
        waveops_flags = []
    ...
    for bdep in deps:
        ...
        for platform in platforms:
            ...
            if platform == Platforms.ISPC:
                ispc_files = [ \
                    os.path.join(args.ispc_headerDestinationCPU, f'{platform_filename}.h'), \
                    os.path.join(args.ispc_headerDestinationShader, f'{platform_filename}.h'), \
                    os.path.join(args.destination, platform.name, f'{platform_filename}.ispc'), \
                    os.path.join(args.destination, platform.name, f'{platform_filename}.ispc.i'), \
                    os.path.join(args.binaryDestination, platform.name, f'{platform_filename}.o'), \
                ]

                file_missing = False
                for ispc_file in ispc_files:
                    if not os.path.exists(ispc_file):
                        file_missing = True
                        break
                    timestamp = getmtime(ispc_file)
                    dst_timestamp = min(dst_timestamp, timestamp) if dst_timestamp > 0 else timestamp
                if file_missing:
                    regen.add(platform_filename)
                    continue
                bin_timestamp = dst_timestamp
            else:
...
def _collect_shader_decl_process(args):
    binary, args, platforms, pp, include_dirs, source, filepath = args
    ...
    for platform in platforms:
        ...
        if args.incremental:
            ...
            deps = []
            dep_index = 0
            for line in open(deps_filepath).readlines():
                line = line.replace('\\\n', '').strip()
                for dep in line.split(' '):
                    if dep == '-:':
                        continue
                    if dep_index > 0 and deps[dep_index - 1].endswith('\\'):
                        last = deps[dep_index - 1].replace('\\', '')
                        deps[dep_index - 1] = f"{last} {dep}"
                    else:
                        deps.append(dep)
                    dep_index += 1
            fsl_dependencies = [binary.filename] + [ f'\"{d.strip()}\"' for d in deps ] + ['']
        ...
        if binary.stage is Stages.ROOTSIG:
            if platform_langs[platform] == 'DIRECT3D12':
                doProcess = True
        if binary.stage is Stages.ISPC:
            doProcess = True

        if doProcess:
            binary.preprocessed_srcs[platform] = shaderSource.decode().splitlines(keepends=True)

    return binary, fsl_dependencies


def collect_shader_decl(args, filepath: str, os_target: str, arch_target: str, int_dir: str, platforms, regen, dependencies, binary_declarations):
    pp = []
    if os.name == 'nt':
        pp += [join(dirname(dirname(dirname(dirname(__file__)))), 'Common', 'Utilities', 'ThirdParty', 'OpenSource', 'mcpp', 'bin', 'mcpp.exe')]
    else:
        pp += ['cc', '-E', '-']
    ...
    filedir = os.path.abspath(dirname(filepath))
    ...
    for i, line in enumerate(source):
        ...
        if stage == Stages.ISPC and Platforms.ISPC not in platforms:
            print(f'ERROR: #ispc specified in a shaders list file which was not set to language "ISPC" (it is set to {platforms})');
            sys.exit(1)
        ...
        if stage is not Stages.NONE:
            ...
            if Platforms.ISPC in platforms:
                if os_target and os_target.lower() == 'windows':
                    binary.ispc_os_target = OSTargets.WINDOWS
                elif os_target and os_target.lower() == 'macos':
                    binary.ispc_os_target = OSTargets.MACOS
                elif os_target and os_target.lower() == 'linux':
                    binary.ispc_os_target = OSTargets.LINUX
                elif os_target and os_target.lower() == 'ios':
                    binary.ispc_os_target = OSTargets.IOS
                elif os_target and os_target.lower() == 'android':
                    binary.ispc_os_target = OSTargets.ANDROID
                elif os_target and os_target.lower() == 'xbox':
                    binary.ispc_os_target = OSTargets.XBOX
                elif os_target and os_target.lower() == 'ps4':
                    binary.ispc_os_target = OSTargets.PS4
                elif os_target and os_target.lower() == 'ps5':
                    binary.ispc_os_target = OSTargets.PS5
                elif os_target and os_target.lower() == 'switch':
                    binary.ispc_os_target = OSTargets.SWITCH
                else:
                    print(f'ERROR: invalid ISPC targetOs argument (-t) passed "{os_target}, Make sure you specify -t when invoking fsl.py with ISPC."');
                    sys.exit(1)

                if arch_target and arch_target.lower() == 'arm64':
                    binary.ispc_arch_target = ArchTargets.ARM64
                elif arch_target and arch_target.lower() == 'x86_64':
                    binary.ispc_arch_target = ArchTargets.X86_64
                else:
                    print(f'ERROR: invalid ISPC architecture argument (-a) passed "{arch_target}, Make sure you specify -a when invoking fsl.py with ISPC."');
                    sys.exit(1)

            binary.int_dir = int_dir
            ...
            binary.ispc_cpu_header_dir = args.ispc_headerDestinationCPU
            binary.ispc_shader_header_dir = args.ispc_headerDestinationShader
            ...
            for flag in [ f[3:] for f in decl if f.startswith('FT_')]:
                if flag in Features._member_names_:
                    ...
                    if Features.MULTIVIEW == Features[flag]:
                        embed_ext = True
            if embed_ext and not ext_embedded:
                meta_source = [ b'#include "fsl_ext.h"\n' ] + meta_source
                ext_embedded = True

            if stage == Stages.ISPC:
                x86_flags = [ f.strip() for f in decl if f.startswith('x86:')]
                if len(x86_flags) > 1:
                    print("WARNING: More than one x86 instruction set specified. Only the last one will be used.\n\t", line);
                if len(x86_flags) == 0:
                    print("WARNING: No explicit x86 instruction set was specified. Defaulting to SSE2 for compatibility. If you want to, for example, target AVX, then specify it with: 'x86:avx'.\n\t", line);
                    binary.ispc_target_x86 = ISPCX86Targets.SSE2
                for flag in x86_flags:
                    if len(flag) <= len('x86:'):
                        print('x86 instruction set argument without specified value: ', line)
                        sys.exit(1)
                    instr_set = flag.split(":")[1];
                    if instr_set.lower() == "sse2":
                        binary.ispc_target_x86 = ISPCX86Targets.SSE2
                    elif instr_set.lower() == "sse4" or instr_set.lower() == "sse4.2":
                        binary.ispc_target_x86 = ISPCX86Targets.SSE4
                    elif instr_set.lower() == "avx" or instr_set.lower() == "avx1":
                        binary.ispc_target_x86 = ISPCX86Targets.AVX1
                    elif instr_set.lower() == "avx2":
                        binary.ispc_target_x86 = ISPCX86Targets.AVX2
                    elif instr_set.lower() == "avx512knl":
                        binary.ispc_target_x86 = ISPCX86Targets.AVX512KNL
                    elif instr_set.lower() == "avx512skx":
                        binary.ispc_target_x86 = ISPCX86Targets.AVX512SKX
                    elif instr_set.lower() == "avx512spr":
                        binary.ispc_target_x86 = ISPCX86Targets.AVX512SPR
                    elif instr_set.lower() == "avx512":
                        print('ERROR: For AVX512 you must choose on of 3 specific targets: avx512knl, avx512skx, avx512spr. See ISPC User Guide for more information: https://ispc.github.io/ispc.html\n\t', line)
                        sys.exit(1)
                    else:
                        print(f'ERROR: Invalid/Unsupported simd x86 instruction set "{instr_set}". Please use one of the following: sse2, sse4, avx, avx2, avx512knl, avx512skx, avx512spr \n\t', line)
                        sys.exit(1)
                for flag in [ f.strip() for f in decl if f.startswith('arm:')]:
                    if len(flag) <= len('arm:'):
                        print('ERROR: arm instruction set argument without specified value: ', line)
                        sys.exit(1)
                    instr_set = flag.split(":")[1];
                    if instr_set.lower() == "neon":
                        binary.ispc_target_arm = ISPCArmTargets.NEON
                    else:
                        print(f'ERROR: Invalid/Unsupported simd ARM instruction set "{instr_set}". Please use one of the following: neon \n\t', line)
                        sys.exit(1)

            binary_macro = hash(binary)
    ...
    for binary in binary_declarations:
        ...
        if not done:
            tasks.append((binary, args, platforms, pp, include_dirs, source, filepath))

    if len(tasks) > 0:
        print(f'Preprocessing {filepath} ({len(tasks)}/{len(binary_declarations)})', flush=True)
        start_time = time.time()

        nprocess = min(cpu_count() if args.mp == -1 else args.mp, len(tasks))
        with Pool(processes=nprocess) as pool:
            results = pool.map(_collect_shader_decl_process, tasks)
            for i, (binary, fsl_deps) in enumerate(results):
                if fsl_deps is None:
                    sys.exit(1)
                if binary is not None:
                    binary_declarations[i] = binary
                fsl_dependencies[binary_declarations[i].filename] = fsl_deps
...
def get_clang_format_path():
    ...
    if sys.platform.startswith('darwin'):
        clang_format_path = script_dir+"/../../../Tools/clang-tools/Darwin/clang-format"
    elif sys.platform.startswith('linux'):
        if platform.machine() == 'x86_64':
            clang_format_path = script_dir+"/../../../Tools/clang-tools/Linux/x86_64/clang-format"
        elif platform.machine() == 'arm64' or platform.machine() == 'aarch64':
            clang_format_path = script_dir+"/../../../Tools/clang-tools/Linux/arm64/clang-format"
        else:
            raise ValueError("Unsupported Linux Machine Architecture")
    elif sys.platform.startswith('win'):
        clang_format_path = script_dir+"/../../../Tools/clang-tools/Windows/clang-format.exe"
    else:
...
def clang_format_file(path):
    ...
    if not os.access(clang_format_path, os.X_OK):
        from shutil import which
        clang_format_path = which('clang-format')
        if not clang_format_path:
            return # Formatting does not change the generated program.
    subprocess.run([clang_format_path, "-i", path], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
