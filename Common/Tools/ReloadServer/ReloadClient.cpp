...
How ReloadServer works:
    ...
    7. On next call to `App.update`, `platformUpdateReloadClient` calls `requestReload(TFReloadDesc{ TF_RELOAD_TYPE_SHADER })` (and
...
#define DAEMON_CMD "Common/Tools/ReloadServer/ReloadServer.sh"
#else
#define DAEMON_CMD "Common\\Tools\\ReloadServer\\ReloadServer.bat"
#endif
...
typedef struct ReloadClient
{
    ...
    // Each line can have a max length of TF_FS_MAX_PATH + 2, where 2 is the max number of line terminators '\r\n'.
    // Therefore max size of the entire file is 3 * (TF_FS_MAX_PATH + 2).
    static constexpr int ExpectedLineCount = 3;
    char                 mReloadServerFileBuf[ExpectedLineCount * (TF_FS_MAX_PATH + 2)];

    // UI
    TFUIWindowDesc mUiDesc;
    const char*    pReloadShaderButtonLabel;
...
static void addUpdatedShaders(uint8_t* ptr)
{
    ...
    for (uint32_t i = 0; i < nShaders; ++i)
    {
        ...
        {
            if (strncmp(fileName, cur->mPath, TF_FS_MAX_PATH) == 0)
            {
                pExistingShader = cur;
                break;
            }
...
{
    TFFileStream stream;
    if (!fsOpenStreamFromPath(TF_RD_SHADER_BINARIES, RELOAD_SERVER_FILE, TF_FM_READ, &stream))
    {
        LOGF(eERROR,
             "Failed to read `%s` from disk in mount point TF_RD_SHADER_BINARIES. "
...
{
    uint8_t buffer[TF_FS_MAX_PATH + 64] = {};
    buffer[0] = MSG_RECOMPILE;
    uint8_t*    ptr = buffer + 1;
    const char* toSend[] = { getShaderPlatformName(), bdata(&gClient.mIntermediateDir) };
    uint32_t    toSendLen[] = { (uint32_t)strnlen(toSend[0], TF_FS_MAX_PATH), (uint32_t)blength(&gClient.mIntermediateDir) };
    for (int i = 0; i < 2; ++i)
    {
...
static void requestRecompileThreadFunc(void* userdata)
{
    ...
    TFMutexLock lock(gClient.mLock);
    ...
    while (!success && tryCount < 5)
    {
        ...
        success = socketCreateClient(&sock, &gClient.mAddr, NETWORK_SOCKET_CREATE_NONE);
        if (!success)
        {
            threadSleep(1000);
        }
...
bool platformInitReloadClient(void)
{
    ...
    gClient.pReloadShaderButtonLabel = "Reload shaders (Ctrl-S)";
...
bool platformGetReloadBinary(const char* path, void** pByteCode, uint32_t* pByteCodeSize)
{
    ...
    for (UpdatedShader* cur = gClient.pUpdatedShaders; cur != nullptr; cur = cur->pNext)
    {
        if (strncmp(cur->mPath, path, TF_FS_MAX_PATH) == 0)
        {
...
static void platformRequestReloadUICallback(void* pUserData)
{
    UNREF_PARAM(pUserData);
    platformRequestReload();
}

void platformUpdateReloadClientUI(void)
{
#ifdef ENABLE_FORGE_UI
    if (!gClient.mUiDesc.pWindowTitle)
    {
        // Likely initResourceLoaderInterface was called with several renderers,
        // which does not call platformInitReloadClient, so the reload control window description was not set up.
        return;
    }

    if (UI_WINDOW_IS_VISIBLE(uiBeginWidgetWindow(&gClient.mUiDesc)))
    {
        uiLayoutAutoTextRows(1);
        if (UI_WIDGET_IS_PRESSED(uiButton(gClient.pReloadShaderButtonLabel)))
        {
            platformRequestReloadUICallback(NULL);
        }

        if (gClient.mIsButtonActiveState)
        {
            uiDynamicText(&gClient.mNotification, { 1.0f, 1.0f, 1.0f, 1.0f }, TF_TEXT_MODE_WRAPPED, TF_ALIGN_WRAPPED);
        }
...
void platformUpdateReloadClient(void)
{
    ...
    {
        TFReloadDesc desc{ TF_RELOAD_TYPE_SHADER };
        requestReload(&desc);
    }
...
void platformSetupReloadClientUI(const TFUIWindowDesc* pDesc)
{
    if (!gClient.mDidInit)
    {
        return;
    ...
    uiFuncDesc.pFunc = platformRequestReloadUICallback;
    uiFuncDesc.pFuncData = NULL;
    luaRegisterWidgetFunction(&uiFuncDesc);

    TFLuaWidgetVariableDesc uiNotificationDesc;
    uiNotificationDesc.pLabel = "Reload Shaders Notification";
