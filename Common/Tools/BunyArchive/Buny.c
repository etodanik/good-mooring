...
struct ThreadsSharedMemory
{
    ...
    TFConditionVariable conditionBlocks;
    ...
    bool                activity;
    TFMutex             managerMutex;
    TFConditionVariable managerCondition;
...
static bool tf_seek(TFFileStream* fs, size_t pos) { return fsSeekStream(fs, TF_SBO_START_OF_FILE, (ssize_t)pos); }

static bool tf_write(TFFileStream* fs, size_t size, void* data)
{
...
static bool bunyArLibArchiveWrite(TFResourceDirectory rd, const char* dstPath, struct bunyArLibPacketIo packetIo,
                                  const struct BunyArLibCreateDesc* desc, struct BunyArLibCreateMetadata* md)
{
    ...
    TFFileStream archiveFs = { 0 };
    if (!fsOpenStreamFromPath(rd, dstPath, TF_FM_WRITE, &archiveFs))
    {
...
static void fileAssemblyReadTask(void* data, uint64_t thid)
{
    ...
    {
        if (!fsOpenStreamFromPath(file->entry->inputRd, file->entry->inputPath, TF_FM_READ | TF_FM_ALLOW_READ, &file->fileStream))
        {
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            threadSystemAddTasks(tsm->threadSystem, fileAssemblyCompressTask, 1, 0, block);
        }
    ...
    while (!tsm->error && !tsm->archiveError && entryId < desc->entryCount)
    {
        ...
        for (uint64_t i = 0; i < tsm->maxAssemblyLines; ++i)
        {
            ...
            if (readStatus == BLOCK_TASK_STATUS_IDLE)
            {
                ...
                threadSystemAddTasks(tsm->threadSystem, fileAssemblyReadTask, 1, 0, file);
...
static bool initThreadSharedMemory(struct ThreadsSharedMemory* tsm, TFResourceDirectory rd, const char* dstPath,
                                   const struct BunyArLibCreateDesc* desc, struct BunyArLibCreateMetadata* md)
{
    memset(tsm, 0, sizeof(*tsm));
    ...
    struct TFThreadDesc threadInfo = { 0 };
...
static bool bunyArLibCreateArchive(TFResourceDirectory rd, const char* dstPath, const struct BunyArLibCreateDesc* desc,
                                   struct BunyArLibCreateMetadata* md)
{
...
bool bunyArLibCreate(TFResourceDirectory rd, const char* dstPath, const struct BunyArLibCreateDesc* inDesc, bool skipPreProcess)
{
    char** strings = NULL;
    ...
    if (skipPreProcess)
    {
        memcpy(&desc, inDesc, sizeof(struct BunyArLibCreateDesc));
    }
    else
    {
...
bool bunyArLibExtract(struct TFIFileSystem* archive, TFResourceDirectory rd, const char* dstDir, const struct BunyArLibExtractDesc* desc)
{
    struct BunyArDescription archiveInfo;
    fsArchiveGetDescription(archive, &archiveInfo);

    TFFileStream fsOut = { 0 };
    TFFileStream fsIn = { 0 };
    ...
    for (uint64_t i = 0; i < loopLimit; ++i)
    {
        ...
        char     buffer[64 * 1024];
        ASSERT(sizeof buffer >= TF_FS_MAX_PATH);
        ...
        if (!fsIoOpenByUid(archive, uid, TF_FM_READ, &fsIn))
        {
        ...
        if (!fsOpenStreamFromPath(rd, buffer, TF_FM_WRITE, &fsOut))
        {
