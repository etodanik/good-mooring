...
bool socketCreateServer(Socket* sock, const SocketAddr* addr, int backlog, SocketCreateFlags flags)
{
    ASSERT(sock);
    *sock = SOCKET_INVALID;
    ...
    if (flags & NETWORK_SOCKET_CREATE_TCP_NODELAY)
    {
        setsockopt(s, IPPROTO_TCP, TCP_NODELAY, (const void*)&yes, sizeof(yes));
    }
...
bool socketCreateClient(Socket* sock, const SocketAddr* addr, SocketCreateFlags flags)
{
    ASSERT(sock);
    *sock = SOCKET_INVALID;
