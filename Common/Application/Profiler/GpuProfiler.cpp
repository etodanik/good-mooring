...
#ifndef TF_ENABLE_GPU_PROFILER
ProfileToken initGpuProfiler(TFRenderer* /*pRenderer*/, TFQueue* /*pQueue*/, const char* /*pName*/) { return PROFILE_INVALID_TOKEN; }
void         cmdBeginGpuFrameProfile(TFCmd* /*pCmd*/, ProfileToken /*nProfileToken*/, bool /*bUseMarker*/) {}
void         cmdEndGpuFrameProfile(TFCmd* /*pCmd*/, ProfileToken /*nProfileToken*/) {}
ProfileToken cmdBeginGpuTimestampQuery(TFCmd* /*pCmd*/, ProfileToken /*nProfileToken*/, const char* /*pName*/, bool /*bUseMarker*/)
{
    return PROFILE_INVALID_TOKEN;
}
void         cmdEndGpuTimestampQuery(TFCmd* /*pCmd*/, ProfileToken /*nProfileToken*/) {}
...
extern "C" void mapBuffer(TFRenderer* pRenderer, TFBuffer* pBuffer, TFReadRange* pRange);
extern "C" void unmapBuffer(TFRenderer* pRenderer, TFBuffer* pBuffer);
...
static void calculateTimes(TFCmd* pCmd, GpuProfiler* pGpuProfiler, uint32_t index)
{
    ...
    {
        TFMutexLock lock(ProfileGetMutex());
...
void initGpuProfiler(TFRenderer* pRenderer, TFQueue* pQueue, GpuProfiler** ppGpuProfiler, const char* pName)
{
...
ProfileToken cmdBeginGpuTimestampQuery(TFCmd* pCmd, struct GpuProfiler* pGpuProfiler, const char* pName, bool addMarker = true,
                                       const float3& color = { 1, 1, 0 }, bool isRoot = false)
{
    ...
    if (!node)
    {
        ...
        uint32_t scope_color =
            static_cast<uint32_t>(color.x * 255) << 16 | static_cast<uint32_t>(color.y * 255) << 8 | static_cast<uint32_t>(color.z * 255);
    ...
    TFQueryDesc desc = { node->mIndex };
    cmdBeginQuery(pCmd, pGpuProfiler->pQueryPool[pGpuProfiler->mBufferIndex], &desc);

    if (addMarker)
    {
        cmdBeginDebugMarker(pCmd, color.x, color.y, color.z, pName);
    }
...
void cmdEndGpuTimestampQuery(TFCmd* pCmd, struct GpuProfiler* pGpuProfiler, bool isRoot = false)
{
    UNREF_PARAM(isRoot);
    // Record gpu time
    TFQueryDesc desc = { pGpuProfiler->pCurrentNode->mIndex };
...
ProfileToken initGpuProfiler(TFRenderer* pRenderer, TFQueue* pQueue, const char* pName)
{
