...
{
    ownerAABB->min.x = (fminf(point.x, ownerAABB->min.x));
    ownerAABB->min.y = (fminf(point.y, ownerAABB->min.y));
    ownerAABB->min.z = (fminf(point.z, ownerAABB->min.z));

    ownerAABB->max.x = (fmaxf(point.x, ownerAABB->max.x));
    ownerAABB->max.y = (fmaxf(point.y, ownerAABB->max.y));
    ownerAABB->max.z = (fmaxf(point.z, ownerAABB->max.z));
}
void adjustAABB(TFAABB* ownerAABB, const TFAABB& otherAABB)
{
    ownerAABB->min.x = (fminf(otherAABB.min.x, ownerAABB->min.x));
    ownerAABB->min.y = (fminf(otherAABB.min.y, ownerAABB->min.y));
    ownerAABB->min.z = (fminf(otherAABB.min.z, ownerAABB->min.z));

    ownerAABB->max.x = (fmaxf(otherAABB.max.x, ownerAABB->max.x));
    ownerAABB->max.y = (fmaxf(otherAABB.max.y, ownerAABB->max.y));
    ownerAABB->max.z = (fmaxf(otherAABB.max.z, ownerAABB->max.z));
}
