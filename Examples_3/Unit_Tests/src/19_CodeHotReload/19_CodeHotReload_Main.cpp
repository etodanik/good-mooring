...
{
#endif
    FORGE_API int systemRun(const char* command, const char** arguments, size_t argumentCount, TFFileStream* pFs);
#ifdef __cplusplus
}
#endif
#endif
...
// we need the forward declaration to call the game code and later the linker will link everything.
#if !FORGE_CODE_HOT_RELOAD
...
static const char* findMsBuildPath()
{
    ...
    static char cachedPath[TF_FS_MAX_PATH] = { 0 };
    ...
    TFFileStream fs{};
    fsOpenStreamFromPath(TF_RD_DEBUG, tempFilename, TF_FM_WRITE, &fs);
    const int result = systemRun(VS_WHERE_PATH, params, TF_ARRAY_COUNT(params), &fs);
    fsCloseStream(&fs);

    if (fsOpenStreamFromPath(TF_RD_DEBUG, tempFilename, TF_FM_READ, &fs))
    {
        char         buffer[TF_FS_MAX_PATH] = { 0 };
        const size_t readSize = fsReadFromStream(&fs, buffer, TF_FS_MAX_PATH);
        if (result == 0)
        {
    ...
    {
        LOGF(eERROR, "vswhere didn't write to file TF_RD_DEBUG/%s or can't open the file", tempFilename);
    }
...
const TFResourceDirectory RD_PLUGIN = TF_RD_MIDDLEWARE_0;
...
TFShader*   pSpriteShader = NULL;
TFBuffer*   pSpriteVertexBuffers[gDataBufferCount] = { NULL };
TFBuffer*   pSpriteIndexBuffer = NULL;
TFBuffer*   pSpriteVertexBuffer = NULL;
TFPipeline* pSpritePipeline = NULL;

TFDescriptorSet* pDescriptorSetPersistent = NULL;
TFDescriptorSet* pDescriptorSetPerFrame = NULL;

TFTexture* pSpriteTexture = NULL;
...
static void MoveSystem(ecs_iter_t* it)
{
    ...
    for (int i = 0; i < it->count; i++)
    {
        ...
        if (pos.pos.x < bounds->xMin)
        {
            move.vel.x = (-move.vel.x);
            pos.pos.x = (bounds->xMin);
        }
        if (pos.pos.x > bounds->xMax)
        {
            move.vel.x = (-move.vel.x);
            pos.pos.x = (bounds->xMax);
        }
        if (pos.pos.y < bounds->yMin)
        {
            move.vel.y = (-move.vel.y);
            pos.pos.y = (bounds->yMin);
        }
        if (pos.pos.y > bounds->yMax)
        {
            move.vel.y = (-move.vel.y);
            pos.pos.y = (bounds->yMax);
        }
    }
}
...
static void FillRenderData(ecs_iter_t* it, uint32_t baseIndex)
{
    ...
    const AppDataComponent* appData = ecs_singleton_get(it->world, AppDataComponent);
    const Vector3           camPos(appData->cameraPos.x, appData->cameraPos.y, 0.f);
    ...
    for (int i = 0; i < it->count; i++)
    {
        ...
        spriteData.modelMtx = mat4::translation((Vector3(position.pos.x, position.pos.y, 0.f) - camPos) * globalScale) *
...
public:
    bool Init()
    {
        ...
        // Instance buffer
        TFBufferLoadDesc spriteVbDesc = {};
        spriteVbDesc.mDesc.mDescriptors = TF_DESCRIPTOR_TYPE_BUFFER;
        spriteVbDesc.mDesc.mMemoryUsage = TF_RESOURCE_MEMORY_USAGE_CPU_TO_GPU;
        spriteVbDesc.mDesc.mFlags = TF_BUFFER_CREATION_FLAG_NONE;
        spriteVbDesc.mDesc.mStartState = TF_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE;
        ...
        TFBufferLoadDesc spriteIBDesc = {};
        spriteIBDesc.mDesc.mDescriptors = TF_DESCRIPTOR_TYPE_INDEX_BUFFER;
        spriteIBDesc.mDesc.mMemoryUsage = TF_RESOURCE_MEMORY_USAGE_GPU_ONLY;
        ...
        TFBufferLoadDesc spriteVBDesc = {};
        spriteVBDesc.mDesc.mDescriptors = TF_DESCRIPTOR_TYPE_VERTEX_BUFFER;
        spriteVBDesc.mDesc.mMemoryUsage = TF_RESOURCE_MEMORY_USAGE_GPU_ONLY;
        ...
        char gamePluginPath[TF_FS_MAX_PATH] = "./" GAME_PLUGIN_ADD_EXTENSION(GAME_PLUGIN_NAME);
    ...
    bool Load(TFReloadDesc* pReloadDesc)
    {
        ...
        gGuiWindowDesc.pWindowTitle = "CodeHotReload";
        ...
        // Register UI functionality for Lua
        {
            ...
            luaFuncDesc.pLabel = "Rebuild Game";
            luaFuncDesc.pFuncData = pWindow;
            luaFuncDesc.pFunc = RebuildGame;
            luaRegisterWidgetFunction(&luaFuncDesc);
#endif
#endif
            luaVarDesc.mWidgetType = TF_WIDGET_TYPE_CHECKBOX;
            luaVarDesc.pLabel = "Threading";
            luaVarDesc.pBool = &gMultiThread;
            luaRegisterWidgetVariable(&luaVarDesc);
        }
    ...
    void Draw()
    {
        ...
        TFBufferUpdateDesc vboUpdateDesc = { pSpriteVertexBuffers[mSettings.mFrameIdx] };
        vboUpdateDesc.mCurrentState = TF_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE;
        ...
        // Draw Sprites pass
        {
            ...
            {
                cmdBeginGpuTimestampQuery(cmd, gGpuProfileToken, "Draw Sprites");
                ...
                cmdBindIndexBuffer(cmd, pSpriteIndexBuffer, TF_INDEX_TYPE_UINT16, 0);
...
private:
    ...
    void addPipelines()
    {
        ...
        // TFVertexLayout for sprite drawing.
        TFPipelineDesc desc = {};
    ...
    void updateGui()
    {
        if (UI_WINDOW_IS_VISIBLE(uiBeginWidgetWindow(&gGuiWindowDesc)))
        {
#if FORGE_CODE_HOT_RELOAD
#if !defined(__APPLE__)
            const int testScriptCount = sizeof(gTestScripts) / sizeof(gTestScripts[0]);
            ...
            uiCheckbox("Threading", &gMultiThread);

#if FORGE_CODE_HOT_RELOAD
#if defined(__APPLE__)
            uiLabel("Use Command+B in Xcode to rebuild CodeHotReload_Main.", TF_ALIGN_LEFT);
#else
            if (UI_WIDGET_IS_PRESSED(uiButton("Rebuild Game")))
            {
                RebuildGame(pWindow);
            }
#endif // defined(__APPLE__)
...
#if FORGE_CODE_HOT_RELOAD
#if !defined(__APPLE__)
    static void RebuildGame(void* pUserData)
    {
        TFWindowDesc* pWindow = (TFWindowDesc*)pUserData;
        ...
        // The build setup for The Forge for PC contains all project files in the same directory,
        ...
        char         buildLogFile[TF_FS_MAX_PATH] = "CodeReloadBuild.log";
        TFFileStream fs{};
        fsOpenStreamFromPath(TF_RD_DEBUG, buildLogFile, TF_FM_WRITE, &fs);
        const int buildResult = systemRun(buildCommand, args, buildArgUsed, &fs);
        fsCloseStream(&fs);

        if (buildResult != 0)
        {
            LOGF(eWARNING, "Couldn't rebuild the project. Build log file is TF_RD_DEBUG/'%s'", buildLogFile);

            char message[1024] = {};
            snprintf(message, sizeof(message),
                     "Compilation of the Hot Reloadable module failed.\nPlease check the log file in: TF_RD_DEBUG/%s", buildLogFile);

            errorMessagePopup("Build Failed", message, &pWindow->handle, NULL);
        }
    }
#endif
#endif
