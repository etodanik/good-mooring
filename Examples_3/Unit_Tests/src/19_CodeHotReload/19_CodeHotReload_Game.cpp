...
int onCodeLoad(struct cr_plugin* ctx)
{
    ...
    // Currently we cannot call ASSERT or any other function from this project.
    // In order to support asserts we would need to compile OS into a dll and link it with this module,
    // as all our unit tests are already set up to use OS as static library doint that would be a mayor
    ...
    tfMainCodeReload(struct cr_plugin* ctx, enum cr_op operation)
{
    ...
    case CR_UNLOAD:
        return onCodeUnload(ctx); // Preparing for a new reload
    case CR_CLOSE:
