...
struct SpectrumMode { float real, imaginary, omega, waveNumber; };
...
struct HullWake {
    Vec3 position; float amplitude;
    Vec3 forward; float length;
    Vec3 velocity; float width;
    float spacing; uint32_t sourceBody; float padding[2];
};
constexpr unsigned MaxHullWakes=16, LocalWaterSize=256;
struct PacketSample { float height,dx,dz,verticalVelocity,velocityX,velocityZ,accelerationX,accelerationZ; };
struct WaterSample { float height; Vec3 displacement, normal, velocity, acceleration; };
struct OceanMetrics {
...
bool emitWavePacket(Ocean* ocean,Vec3 position,Vec3 direction,float wavelength,float energyJoules,bool backgroundPair=false,uint32_t sourceBody=UINT32_MAX);
