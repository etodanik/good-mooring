...
struct WaterLook {
    ...
    void sunlight(float4& out) const {
        ...
        out[0]=std::cos(azimuth)*std::cos(elevation); out[1]=std::sin(elevation);
        out[2]=std::sin(azimuth)*std::cos(elevation); out[3]=float(cloudSteps);
    }
    void preset(unsigned level) {
        fftSize=level==2?512:256; gridSize=level==0?128:level==1?256:512;
        ...
        microDetail=level==0?.65f:1.3f; normalFilter=1; antialias=true;
        foamRelief=level==0?0:.025f;
        bloom=level==0?0:.08f;
    }
    void parameters(float4& weather,float4& effects) const {
        weather[0]=overcast; weather[1]=rain; weather[2]=foam; weather[3]=scattering;
        effects[0]=shafts; effects[1]=exposure; effects[2]=shaftSteps>0?1.0f:0.0f; effects[3]=float(debugView);
    }
    void fidelity(float4& detail,float4& quality,float4& optics,float4& foamSettings) const {
        detail[0]=normalFilter; detail[1]=microDetail; detail[2]=roughness; detail[3]=foamScale;
        quality[0]=float(particles); quality[1]=float(shaftSteps); quality[2]=float(shadowSize); quality[3]=antialias?1.0f:0.0f;
        optics[0]=sunStrength; optics[1]=reflection; optics[2]=refraction; optics[3]=contactFoam;
        foamSettings[0]=foamDecay; foamSettings[1]=foamSpread; foamSettings[2]=breakingThreshold; foamSettings[3]=sprayRate;
    }
};
}
