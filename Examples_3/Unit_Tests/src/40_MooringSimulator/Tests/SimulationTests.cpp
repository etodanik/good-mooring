...
int main()
{
    ...
    World* w = createWorld();
    CHECK(canOperateHelm(w,0)); CHECK(!canOperateHelm(w,1));
    ...
    CHECK(snapshot(w,false).travelling); CHECK(!canOperateHelm(w,0));
    ...
    for (unsigned i=0;i<120;++i) { updateCamera(camera,FixedStep); panCamera(camera,1,-1); zoomCamera(camera,.1f); }
    ...
    for (unsigned i=0;i<600;++i) step(w);
    CHECK(!snapshot(w,false).travelling); CHECK(!canOperateHelm(w,0));
    enqueue(w,{CommandType::Attend,1,Station::Helm}); step(w);
    CHECK(!canOperateHelm(w,1));
    setHelmSkill(w,1,2); enqueue(w,{CommandType::Attend,1,Station::Helm}); step(w);
    CHECK(!canOperateHelm(w,1));
    for (unsigned i=0;i<100;++i) step(w);
    CHECK(canOperateHelm(w,1));
    ...
    for (unsigned i=0;i<3600;++i) step(w);
    ...
    CHECK(std::fabs(snapshot(w,false).position.y) < 2);
    for (unsigned i=0;i<20;++i) { resetWorld(w); step(w); }
    CHECK(statistics(w).allocationCount == warm);
    for (unsigned i=0;i<64;++i) CHECK(enqueue(w,{CommandType::Brake,0,Station::Helm,0,1}));
    CHECK(!enqueue(w,{CommandType::Brake,0,Station::Helm,0,1}));
    CHECK(statistics(w).commandOverflow==1);
    step(w); destroyWorld(w);

    auto boat = monohullLayout(true); PropellerState flow = {};
    ...
    for (unsigned i=1;i<240;++i) updatePropeller(boat.propellers[0],flow,1,i*FixedStep,FixedStep);
    ...
    auto larger = boat.windAreas[0]; larger.lateralArea*=2;
    ...
    auto single=monohullLayout(); PropellerState moving={};
    for(unsigned i=0;i<240;++i) updatePropeller(single.propellers[0],moving,1,i*FixedStep,FixedStep);
    ...
    auto aft=single.propellers[0]; aft.position.z=-6;
    CHECK(rudderWash(aft,moving,single.rudders[0].position,4).z==0);
    for(unsigned i=241;i<800;++i) updatePropeller(single.propellers[0],moving,-1,i*FixedStep,FixedStep);
    ...
    auto sideways=single.propellers[0]; sideways.axis={1,0,0};
    PropellerState sidewaysFlow={};
    for(unsigned i=0;i<240;++i) updatePropeller(sideways,sidewaysFlow,1,i*FixedStep,FixedStep);
    CHECK(rudderWash(sideways,sidewaysFlow,{-1.5f,-.65f,-3.3f},239*FixedStep).x<0);
    auto* cat = createWorld(true); for (unsigned i=0;i<300;++i) step(cat);
    CHECK(std::isfinite(snapshot(cat,false).position.y));
    enqueue(cat,{CommandType::Throttle,0,Station::Helm,0,.7f});
    for(unsigned i=0;i<180;++i) step(cat);
    CHECK(snapshot(cat,false).rotation.y>0);
    resetWorld(cat); enqueue(cat,{CommandType::Throttle,0,Station::Helm,1,.7f});
    for(unsigned i=0;i<180;++i) step(cat);
    CHECK(snapshot(cat,false).rotation.y<0); destroyWorld(cat);
    auto* drift=createWorld(); SeaState environment; environment.windWaveHeight=environment.swellHeight=0;
    environment.windSpeed=0; environment.current={.5f,0,0}; setEnvironment(drift,environment);
    for(unsigned i=0;i<480;++i) step(drift);
    CHECK(snapshot(drift,false).velocity.x>0);
    environment.current={}; environment.windSpeed=7; environment.windDirection=0; setEnvironment(drift,environment);
    for(unsigned i=0;i<480;++i) step(drift);
    CHECK(snapshot(drift,false).velocity.x>0); CHECK(snapshot(drift,false).rotation.y>0);
    CHECK(statistics(drift).steadyStepAllocations==0); destroyWorld(drift);
