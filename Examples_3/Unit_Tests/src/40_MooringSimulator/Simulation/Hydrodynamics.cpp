...
VesselLayout monohullLayout(bool twinRudders)
{
    ...
    v.lightMass=7985; v.payload=815; v.mass=v.lightMass+v.payload;
    v.length=11.99f; v.waterlineLength=11.70f; v.beam=4.18f; v.draft=2.17f;
    v.centerOfMass={0,-.35f,0};
    v.inertia={v.mass*3.0f*3.0f,v.mass*3.15f*3.15f,v.mass*1.25f*1.25f};
    v.resistanceLinear={1120,0,96}; v.resistanceQuadratic={1680,0,152}; v.heaveDampingPerArea=750;
    v.propellerCount = 1; v.rudderCount = twinRudders ? 2 : 1;
...
VesselLayout oceanis401Layout()
{
    ...
    auto v=monohullLayout(true); v.name="Oceanis 40.1";
...
VesselLayout catamaranLayout()
{
    ...
    v.name="Lagoon 42"; v.lightMass=12100; v.payload=1900; v.mass=v.lightMass+v.payload;
    v.length=12.80f; v.waterlineLength=12.5f; v.beam=7.70f; v.draft=1.25f; v.hullSpacing=6.0f;
    v.centerOfMass={0,.45f,0};
    v.inertia={v.mass*3.3f*3.3f,v.mass*4.1f*4.1f,v.mass*3.0f*3.0f};
    v.resistanceLinear={1450,0,130}; v.resistanceQuadratic={2100,0,210};
    v.propellerCount=2;
    for (unsigned i=0; i<2; ++i) {
        ...
        v.propellers[i].walkAhead=.008f; v.propellers[i].walkAstern=.04f;
        v.rudders[i].position={v.propellers[i].position.x,-.70f,-3.35f};
        v.rudders[i].area=.60f; v.rudders[i].span=1.0f;
    }
    v.windAreas[0].lateralArea=7.2f; v.windAreas[1].lateralArea=6.5f;
...
float propellerImmersion(float centerDepth,float diameter)
{
    ...
    if (diameter <= 0 || centerDepth <= -.5f*diameter) return 0;
    if (centerDepth >= .5f*diameter) return 1;
    float u=std::clamp(2*centerDepth/diameter,-1.0f,1.0f);
    return (std::acos(-u)+u*std::sqrt(std::max(0.0f,1-u*u)))/3.14159265f;
}
void updatePropeller(const Propeller& p, PropellerState& state, float throttle, float time, float dt,float immersion,float advanceVelocity)
{
    ...
    float advance=advanceVelocity*std::copysign(1.0f,state.rpm)/std::max(.25f,std::fabs(state.rpm)*p.maximumRevolutionsPerSecond*p.diameter);
...
Vec3 rudderWash(const Propeller& p, const PropellerState& state, Vec3 position, float time)
{
    ...
    for (unsigned i = 0; i < count; ++i) {
        ...
        if (sample.velocity * axial <= 0 || std::fabs(sample.velocity) < 0.05f) continue;
        ...
        if (error < bestError && time - sample.time >= travel - FixedStep) {
            bestError = error; flow = sample.velocity;
        }
    }
    if (bestError > 2.5f * FixedStep) return {};
...
Vec3 windForce(const WindArea& area, Vec3 wind)
{
    ...
    return {0.5f * 1.225f * area.coefficient * area.lateralArea * wind.x * std::fabs(wind.x),
            0, 0.5f * 1.225f * area.coefficient * area.frontalArea * wind.z * std::fabs(wind.z)};
}
}
