...
enum class Station : uint8_t { Helm = 0, Cockpit = 1, Port = 2, Starboard = 3, Bow = 6 };
enum class Difficulty : uint8_t { Beginner, Intermediate, Advanced };
enum class CommandType : uint8_t { Attend, Wheel, Throttle, Brake };
struct Command {
    CommandType type;
    uint8_t actor;
    Station station;
...
struct PhysicsBody { uint32_t handle; uint8_t layout; };
struct Snapshot {
