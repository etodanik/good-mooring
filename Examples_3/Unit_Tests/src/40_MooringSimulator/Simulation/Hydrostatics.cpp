...
SubmergedVolume submergedTetrahedron(const BuoyancyCell& cell,Vec3 normal,float distance) {
    ...
    JPH::Vec3 vertices[4];
    for(unsigned i=0;i<4;++i) vertices[i]=toJolt(cell.vertices[i]);
    ...
    JPH::PolyhedronSubmergedVolumeCalculator calculator(JPH::Mat44::sIdentity(),vertices,sizeof(JPH::Vec3),4,
        JPH::Plane(toJolt(normal),-distance),scratch JPH_IF_DEBUG_RENDERER(,JPH::RVec3::sZero()));
    if(calculator.AreAllAbove()) return {};
    if(calculator.AreAllBelow()) return {cell.volume,cell.center};
    ...
    for(unsigned i=0;i<4;++i) if(i!=reference) face[count++]=i;
    auto a=vertices[face[0]],b=vertices[face[1]],c=vertices[face[2]];
    if((b-a).Cross(c-a).Dot(vertices[reference]-a)>0) std::swap(face[1],face[2]);
    calculator.AddFace(int(face[0]),int(face[1]),int(face[2]));
    float volume; JPH::Vec3 center; calculator.GetResult(volume,center);
...
static void makeCells(VesselLayout& v) {
    ...
    for(unsigned hull=0;hull<(v.hullSpacing>0?2u:1u);++hull) {
        ...
        for(unsigned i=0;i<HullTriangles;++i) {
            Vec3 points[3]; hullTriangle(v,hull,i,points);
            auto volume=tetra(toJolt(origin),toJolt(points[0]),toJolt(points[1]),toJolt(points[2]));
            if(volume.volume<1e-8f) continue;
            auto& cell=v.cells[v.cellCount++]; cell={{origin,points[0],points[1],points[2]},volume.center,volume.volume};
        ...
        JPH::Vec3 corners[8];
        for(unsigned i=0;i<8;++i) corners[i]=center+JPH::Vec3((i&1?1:-1)*halfWidth,(i&2?1:-1)*h*.5f,i&4?1:-1);
        constexpr unsigned faces[6][4]={{0,4,6,2},{1,3,7,5},{0,1,5,4},{2,6,7,3},{0,2,3,1},{4,5,7,6}};
        for(const auto& face:faces) for(unsigned i=1;i<3;++i) {
...
void buildHullVolumes(VesselLayout& v) {
    ...
    for(unsigned iteration=0;iteration<24;++iteration) {
        v.hullDraft=(low+high)*.5f; makeCells(v);
        float volume=0;
        for(unsigned i=0;i<v.cellCount;++i) volume+=submergedTetrahedron(v.cells[i],{0,1,0},0).volume;
        if(volume*1025<v.mass) low=v.hullDraft; else high=v.hullDraft;
    }
    // Balance fore-and-aft moments at the authored waterline as well as mass.
    SubmergedVolume submerged={};
    for(unsigned i=0;i<v.cellCount;++i) accumulate(submerged,submergedTetrahedron(v.cells[i],{0,1,0},0));
...
HydrostaticState hydrostaticState(const VesselLayout& v,float heel,float height) {
    ...
    auto normal=q.Conjugated()*JPH::Vec3::sAxisY(); SubmergedVolume wet={};
    for(unsigned i=0;i<v.cellCount;++i) accumulate(wet,submergedTetrahedron(v.cells[i],fromJolt(normal),-height));
...
HydrostaticState equilibriumAtHeel(const VesselLayout& v,float heel) {
    ...
    for(unsigned i=0;i<30;++i) {
        float height=(low+high)*.5f; auto state=hydrostaticState(v,heel,height);
        if(state.volume*1025>v.mass) low=height; else high=height;
    }
