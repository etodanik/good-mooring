...
struct Rudder {
    Vec3 position; float area, span, maximumAngle;
    Vec3 chord = {0,0,1}, spanDirection = {0,1,0};
    float stockOffset = .05f, reverseEfficiency = .65f;
};
struct WindArea { Vec3 position; float lateralArea, frontalArea, coefficient; };
struct SteeringMechanism { float frictionMoment=6, damping=40, inertia=8; bool backdrivable=true; };
...
struct BuoyancyCell { Vec3 vertices[4], center; float volume; };
struct SubmergedVolume { float volume; Vec3 center; };
struct HydrostaticState { float volume; Vec3 center; float rightingArm,height; };
struct VesselLayout {
...
struct FlowSample { float time, velocity; };
...
void updatePropeller(const Propeller& definition, PropellerState& state, float throttle, float time, float dt,
                     float immersion = 1, float advanceVelocity = 0);
