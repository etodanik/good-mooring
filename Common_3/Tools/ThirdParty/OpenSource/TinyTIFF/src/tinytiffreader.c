...
#else
    #define TinyTIFFReader_POSTYPE ssize_t
...
int TIFFReader_get_byteorder(void) {
    union {
        long l;
        char c[4];
    } test;
...
static TinyTIFFReaderFrame TinyTIFFReader_getEmptyFrame(void) {
...
static void TinyTIFFReader_freeEmptyFrame(TinyTIFFReaderFrame f) {
    if (f.stripoffsets) tf_free(f.stripoffsets);
    f.stripoffsets=NULL;
    if (f.stripbytecounts) tf_free(f.stripbytecounts);
    f.stripbytecounts=NULL;
    if (f.description) tf_free(f.description);
    f.description=NULL;
}
...
bool TinyTIFFReader_fopen(TinyTIFFReaderFile* tiff, TFResourceDirectory resourceDir, const char* filename)
{
#ifdef TINYTIFF_USE_WINAPI_FOR_FILEIO
    ...
    return tiff->hFile != INVALID_HANDLE_VALUE;
#else
#  ifdef HAVE_FOPEN_S
    return fopen_s(&(tiff->file), filename, "rb") == 0;
#  else
    return fsOpenStreamFromPath(resourceDir, filename, TF_FM_READ, &tiff->file);
#  endif
#endif // TINYTIFF_USE_WINAPI_FOR_FILEIO
}
...
#  else
    return fsSeekStream(&tiff->file, TF_SBO_START_OF_FILE, (long)offset);
#  endif
...
#  else
    return fsSeekStream(&tiff->file, TF_SBO_CURRENT_POSITION, (long)offset);
#  endif
#endif // TINYTIFF_USE_WINAPI_FOR_FILEIO
}
...
#  else
    return (unsigned long)fsReadFromStream(&tiff->file, ptr, size * count);
#  endif
#endif // TINYTIFF_USE_WINAPI_FOR_FILEIO
}
...
int TinyTIFFReader_fgetpos(TinyTIFFReaderFile* tiff, TinyTIFFReader_POSTYPE* pos) {
    ...
    *pos= SetFilePointer( tiff->hFile,
    ...
    return 0;
#else
    *pos = fsGetStreamSeekPosition(&tiff->file);
    return 0;
#endif // TINYTIFF_USE_WINAPI_FOR_FILEIO
}
...
int TinyTIFFReader_fsetpos(TinyTIFFReaderFile* tiff, const TinyTIFFReader_POSTYPE* pos) {
    ...
    return 0;
#else
    return fsSeekStream(&tiff->file, TF_SBO_START_OF_FILE, *pos) ? 0 : -1;
#endif // TINYTIFF_USE_WINAPI_FOR_FILEIO
}
...
const char* TinyTIFFReader_getLastError(TinyTIFFReaderFile* tiff) {
    if (tiff)
    {
        LOGF(eERROR, "%s", tiff->lastError);
        return tiff->lastError;
    }
    return NULL;
}
...
static void TinyTIFFReader_freeIFD(TinyTIFFReader_IFD d) {
    if (d.pvalue /*&& d.count>1*/) { tf_free(d.pvalue);  d.pvalue=NULL; }
    if (d.pvalue2 /*&& d.count>1*/) { tf_free(d.pvalue2);  d.pvalue2=NULL; }
}
...
static TinyTIFFReader_IFD TinyTIFFReader_readIFD(TinyTIFFReaderFile* tiff) {
    ...
    switch(d.type) {
        ...
        case TIFF_TYPE_ASCII:
            if (d.count>0) {
              d.pvalue=(uint32_t*)tf_calloc(d.count, sizeof(uint32_t));
                if (d.pvalue) {
                  if (d.count<=4) {
                    unsigned int i;
                    for (i=0; i<4; i++) {
        ...
        case TIFF_TYPE_RATIONAL: {
            d.pvalue = (uint32_t*)tf_calloc(d.count, sizeof(uint32_t));
            if (d.pvalue) {
                d.pvalue2 = (uint32_t*)tf_calloc(d.count, sizeof(uint32_t));
                if (d.pvalue2) {
            ...
            switch(ifd.tag) {
                ...
                case TIFF_FIELD_BITSPERSAMPLE: {
                     ...
                     case TIFF_FIELD_COMPRESSION:
                         tiff->currentFrame.compression = (uint16_t)ifd.value;
                         break;
                case TIFF_FIELD_STRIPOFFSETS:
                    if (ifd.count>0 && ifd.pvalue) { // max U32
                        tiff->currentFrame.stripcount=ifd.count;
                        tiff->currentFrame.stripoffsets = (uint32_t*)tf_calloc(ifd.count, sizeof(uint32_t));
                ...
                case TIFF_FIELD_SAMPLESPERPIXEL:
                    tiff->currentFrame.samplesperpixel = (uint16_t)ifd.value;
                    break;
                case TIFF_FIELD_ROWSPERSTRIP:
                    tiff->currentFrame.rowsperstrip = (uint16_t)ifd.value;
                    break;
                case TIFF_FIELD_SAMPLEFORMAT:
                    tiff->currentFrame.sampleformat = (uint16_t)ifd.value;
                    break;
                case TIFF_FIELD_IMAGEDESCRIPTION: {
                    //printf("TIFF_FIELD_IMAGEDESCRIPTION: (tag: %u, type: %u, count: %u)\n", ifd.tag, ifd.type, ifd.count);
                    if (tiff->currentFrame.description) tf_free(tiff->currentFrame.description);
                    tiff->currentFrame.description = (char*)tf_calloc(ifd.count + 1, sizeof(char));
                ...
                case TIFF_FIELD_STRIPBYTECOUNTS:
                    if (ifd.count>0 && ifd.pvalue) {
                        tiff->currentFrame.stripcount=ifd.count;
                        tiff->currentFrame.stripbytecounts = (uint32_t*)tf_calloc(ifd.count, sizeof(uint32_t));
                ...
                case TIFF_FIELD_PLANARCONFIG:
                    tiff->currentFrame.planarconfiguration = (uint16_t)ifd.value;
                    break;
                case TIFF_FIELD_ORIENTATION:
                    tiff->currentFrame.orientation = (uint8_t)ifd.value;
                    break;
                case TIFF_FIELD_PHOTOMETRICINTERPRETATION:
                    tiff->currentFrame.photometric_interpretation = (uint8_t)ifd.value;
                    break;
                case TIFF_FIELD_FILLORDER:
                    tiff->currentFrame.fillorder = (uint8_t)ifd.value;
                    break;
                ...
                case TIFF_FIELD_RESOLUTIONUNIT:
                    tiff->currentFrame.resolutionunit = (uint16_t)ifd.value;
                    break;
                default:
#ifdef TINYTIFF_ADDITIONAL_DEBUG_MESSAGES
        ...
        if (tiff->currentFrame.stripcount>0 && tiff->currentFrame.stripbytecounts && tiff->currentFrame.stripoffsets) {
            ...
            if (tiff->currentFrame.samplesperpixel==1 || tiff->currentFrame.planarconfiguration==TIFF_PLANARCONFIG_PLANAR) {
                ...
                for (strip=0; strip<tiff->currentFrame.stripcount; strip++) {
                    ...
                    const int hasToReadFromStrip=(int)TinyTIFFReader_doRangesOverlap(sample_start_bytes, sample_end_bytes, fileimageidx_bytes, fileimageidx_bytes+stripsize_bytes, &bytes_to_read_start, &bytes_to_read_end);
#ifdef TINYTIFF_ADDITIONAL_DEBUG_MESSAGES
                ...
                for (strip=0; strip<tiff->currentFrame.stripcount; strip++) {
                    ...
                    if (stripsize_bytes>last_stripsize_bytes) {
                        if (stripdata) tf_free(stripdata);
                        stripdata=(uint8_t*)tf_malloc(stripsize_bytes);
                        if (!stripdata) {
                            tiff->wasError=TINYTIFF_TRUE;
                            TINYTIFF_SET_LAST_ERROR(tiff, "unable to allocate memory\0");
                            return TINYTIFF_FALSE;
...
TinyTIFFReaderFile* TinyTIFFReader_open(TFResourceDirectory resourceDir, const char* filename)
{
    TinyTIFFReaderFile* tiff=(TinyTIFFReaderFile*)tf_malloc(sizeof(TinyTIFFReaderFile));
    if (tiff) {
        tiff->filesize=0;
        tiff->currentFrame=TinyTIFFReader_getEmptyFrame();

        bool result = TinyTIFFReader_fopen(tiff, resourceDir, filename);
        if (!result)
        {
            tf_free(tiff);
            return NULL;
        }
        tiff->filesize = fsGetStreamFileSize(&tiff->file);
        tiff->systembyteorder = (uint8_t)TIFFReader_get_byteorder();
...
uint16_t TinyTIFFReader_getBitsPerSample(TinyTIFFReaderFile* tiff, int sample) {
    ((void)(sample));
    if (tiff) {
        return (uint16_t)tiff->currentFrame.bitspersample;
    }
    return 0;
}
...
const char *TinyTIFFReader_getVersion(void)
{
