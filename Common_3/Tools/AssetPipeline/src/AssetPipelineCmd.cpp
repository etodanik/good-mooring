...
const AssetPipelineProcessCommand gAssetPipelineCommands[] = { { "-pa", PROCESS_ANIMATIONS }, { "-ptfx", PROCESS_TFX },
                                                               { "-pgltf", PROCESS_GLTF },    { "-pt", PROCESS_TEXTURES },
                                                               { "-pies", PROCESS_IES },      { "-pf", PROCESS_FONTS },
                                                               { "-pwz", PROCESS_WRITE_ZIP }, { "-pwza", PROCESS_WRITE_ZIP_ALL },
                                                               { "-pp", PROCESS_PACKAGING } };
...
void PrintHelp()
{
    ...
    printf("\n\t\t--normalmap\t\t Process texture as normal map\n");
    printf("\n\t\t--cubemap\t\t Process texture as cube map, input is an image with height = 6 x width with cube images in order "
           "X+,X-,Y+,Y-,Z+,Z-\n");
    ...
    printf("\n\t%s\t(IES LM 63 to DDS)\tProcess IES Profiles\n", gAssetPipelineCommands[PROCESS_IES].mCommandString);
    printf("\n\t%s\t(MSDF)\tProcess MSDF generation from font ttf and otf.\n", gAssetPipelineCommands[PROCESS_FONTS].mCommandString);
    printf("\n\t\t--glyph_resolution\t: Define resolution of glyph (int), default = 32. \n");
    printf("\n\t\t--pixel_range\t: Define the width of the distance field gradient around glyph contours, measured in texture pixels "
           "(float), default = 1.0. \n");
    printf("\n\t\t--only_ascii\t: Define generation only for ascii glyph [0, 127], default = false. \n");
    ...
    printf("\n\t%s\t(Assets to package)\tProcessPackaging\n", gAssetPipelineCommands[PROCESS_PACKAGING].mCommandString);
    printf("\nCommon Options:\n");
...
int AssetPipelineCmd(int argc, char** argv)
{
    ...
    params.mRDInput = TF_RD_MIDDLEWARE_1;
    params.mRDOutput = TF_RD_MIDDLEWARE_2;
    params.mRDZipWrite = TF_RD_MIDDLEWARE_3;

    char filePath[TF_FS_MAX_PATH] = { 0 };
    char fileNameWithoutExt[TF_FS_MAX_PATH] = { 0 };
    char fileName[TF_FS_MAX_PATH] = { 0 };
    char iext[TF_FS_MAX_PATH] = { 0 };
    ...
    char   inputPath[TF_FS_MAX_PATH] = { 0 };
    size_t size = fsNormalizePath(input, '/', inputPath);
    // make sure it is terminated by / and zero
    if (inputPath[size - 1] != '/')
    {
        inputPath[size] = '/';
        inputPath[size + 1] = 0;
    ...
    char outputPath[TF_FS_MAX_PATH] = { 0 };
    ...
    fsSetPathForResourceDir(pSystemFileIO, TF_RD_LOG, "");
