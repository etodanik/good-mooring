...
bool CreateRuntimeSkeleton(TFResourceDirectory resourceDirInput, const char* skeletonInputfile, TFResourceDirectory resourceDirGltfOutput,
                           TFResourceDirectory resourceDirOzzOutput, const char* skeletonOutputFile, ozz::animation::Skeleton* pOutSkeleton,
                           ProcessAssetsSettings* settings);

bool CreateRuntimeAnimations(TFResourceDirectory resourceDirInput, const char* animationInputFile, const char* animationOutputPath,
...
cgltf_result cgltf_parse_and_load(TFResourceDirectory resourceDir, const char* skeletonAsset, cgltf_data** ppData, void** ppFileData)
{
    // Import the glTF with the animation
    TFFileStream file = {};
    if (!fsOpenStreamFromPath(resourceDir, skeletonAsset, TF_FM_READ, &file))
    {
    ...
    for (uint32_t i = 0; i < data->buffers_count; ++i)
    {
        ...
        {
            char parent[TF_FS_MAX_PATH] = { 0 };
            fsGetParentPath(skeletonAsset, parent);
            char path[TF_FS_MAX_PATH] = { 0 };
            fsAppendPathComponent(parent, uri, path);
            TFFileStream fs = {};
            if (fsOpenStreamFromPath(resourceDir, path, TF_FM_READ, &fs))
            {
...
cgltf_result cgltf_write(TFResourceDirectory resourceDir, const char* skeletonAsset, cgltf_data* data)
{
    cgltf_options options = {};
    options.memory_alloc = [](void* user, cgltf_size size)
    {
    ...
    TFFileStream file = {};
    if (!fsOpenStreamFromPath(resourceDir, skeletonAsset, TF_FM_WRITE, &file))
    {
        tf_free(writeBuffer);
...
static void DiscoverExtraAnimationsForMesh(TFResourceDirectory resourceDirInput, const char* searchRootDir,
...
bool ProcessAnimations(AssetPipelineParams* assetParams, ProcessAnimationsParams* pProcessAnimationsParams)
{
    ...
    for (uint32_t i = 0, end = (uint32_t)arrlen(pSkeletonAndAnims); i < end; ++i)
    {
        ...
        else
        {
            ...
            TFFileStream file = {};
            if (!fsOpenStreamFromPath(assetParams->mRDOutput, skeletonOutput, TF_FM_READ, &file))
            {
...
#define RETURN_IF_TFX_ERROR(expression)     \
    ...
    for (int i = 0; i < tfxFileCount; ++i)
    {
        ...
        char        outputTemp[TF_FS_MAX_PATH] = {};
        fsGetPathFileName(input, outputTemp);
        char output[TF_FS_MAX_PATH] = {};
        fsAppendPathExtension(outputTemp, "gltf", output);

        char binFilePath[TF_FS_MAX_PATH] = {};
        fsAppendPathExtension(outputTemp, "bin", binFilePath);

        TFFileStream tfxFile = {};
        fsOpenStreamFromPath(assetParams->mRDInput, input, TF_FM_READ, &tfxFile);
        ...
        cgltf_size        offset = 0;
        TFFileStream      binFile = {};
        fsOpenStreamFromPath(assetParams->mRDOutput, binFilePath, TF_FM_WRITE, &binFile);
        size_t fileSize = 0;
...
{
    if (matrix[0].w != 0.0 || matrix[1].w != 0.0 || matrix[2].w != 0.0 || matrix[3].w != 1.0)
    {
        return false;
    }
                           ...
                           ProcessAssetsSettings* settings)
{
    ...
    while (arrlen(boneData) != 0)
    {
        ...
        else
        {
            ...
            Matrix4 mat;
            mat[0] = (vec4(node->matrix[0], node->matrix[1], node->matrix[2], node->matrix[3]));
            mat[1] = (vec4(node->matrix[4], node->matrix[5], node->matrix[6], node->matrix[7]));
            mat[2] = (vec4(node->matrix[8], node->matrix[9], node->matrix[10], node->matrix[11]));
            mat[3] = (vec4(node->matrix[12], node->matrix[13], node->matrix[14], node->matrix[15]));

            if (isTrsDecomposable(mat))
            {
                // extract translation
                joint.transform.translation = mat[3].getXYZ();

                // extract the scaling factors from columns of the matrix
                Matrix3 upperMat = f4x4GetUpperf3x3(mat);
                vec3    pScaling = vec3(length(upperMat[0]), length(upperMat[1]), length(upperMat[2]));
                ...
                if (pScaling.x)
                    upperMat[0] = (upperMat[0] / pScaling.x);
                if (pScaling.y)
                    upperMat[1] = (upperMat[1] / pScaling.y);
                if (pScaling.z)
                    upperMat[2] = (upperMat[2] / pScaling.z);

                // and generate the rotation quaternion from it
                joint.transform.rotation = quatFromf3x3(upperMat);
            }
        }
    ...
    TFFileStream file = {};
    if (!fsOpenStreamFromPath(resourceDirOzzOutput, skeletonOutputFile, TF_FM_WRITE, &file))
        return true;
    ...
    // Generate joint remaps in the gltf
    {
        ...
        for (uint32_t i = 0; i < data->buffers_count; ++i)
        {
            ...
            {
                char parentPath[TF_FS_MAX_PATH] = {};
                fsGetParentPath(skeletonInputfile, parentPath);
                char bufferOut[TF_FS_MAX_PATH] = {};
                ...
                TFFileStream fs = {};
                fsOpenStreamFromPath(resourceDirGltfOutput, bufferOut, TF_FM_WRITE, &fs);
                                   ...
                                   ozz::animation::Skeleton* skeleton, const char* animationSourceFile)
{
    ...
    TFFileStream file = {};
    if (!fsOpenStreamFromPath(animationSettings->mSkeletonAndAnimOutRd, animOutFile, TF_FM_WRITE, &file))
    {
...
static inline constexpr TFShaderSemantic util_cgltf_attrib_type_to_semantic(cgltf_attribute_type type, uint32_t index)
{
    switch (type)
    {
    case cgltf_attribute_type_position:
        return TF_SEMANTIC_POSITION;
    case cgltf_attribute_type_normal:
        return TF_SEMANTIC_NORMAL;
    case cgltf_attribute_type_tangent:
        return TF_SEMANTIC_TANGENT;
    case cgltf_attribute_type_color:
        return TF_SEMANTIC_COLOR;
    case cgltf_attribute_type_joints:
        return TF_SEMANTIC_JOINTS;
    case cgltf_attribute_type_weights:
        return TF_SEMANTIC_WEIGHTS;
    case cgltf_attribute_type_texcoord:
        return (TFShaderSemantic)(TF_SEMANTIC_TEXCOORD0 + index);
    default:
        return TF_SEMANTIC_TEXCOORD0;
    }
}
...
void OnGLTFFind(TFResourceDirectory resourceDir, const char* filename, void* pUserData)
{
...
void CheckIfGltfHasAnimations(TFResourceDirectory resourceDirInput, const char* gltfFileName, uint32_t& numAnimationsOut)
{
    ...
    TFFileStream file = {};
    if (!fsOpenStreamFromPath(resourceDirInput, gltfFileName, TF_FM_READ, &file))
    {
...
void buildMeshlets(const uint* indices, size_t indexCount, const float3* vertexPositions, size_t vertexCount, size_t vertexPositionsStride,
                   ...
                   TFMeshlet** meshlets, TFMeshletData** meshletsData)
{
    ...
    TFMeshletData* meshletsDataTmp = NULL;
    ...
    if (meshletsTmp && meshletsDataTmp && meshletVerticesTmp && meshletTrianglesTmp)
    {
        ...
        for (uint m = 0; m < arrlenu(meshletsTmp); m++)
        {
            ...
            TFMeshletData* meshletData = &meshletsDataTmp[m];
        ...
        *meshlets = (TFMeshlet*)meshletsTmp;
...
static void geomOptimize(TFGeometryData* geomData, MeshOptimizerFlags optimizationFlags, TFIndexType indexType, uint32_t indexOffset,
                         uint32_t indexCount, uint32_t vertexOffset, uint32_t* vertexCount)
{
    ...
    meshopt_Stream streams[TF_MAX_SEMANTICS];
    uint32_t       validStreamCount = 0;
    int32_t        posAttributeIdx = -1;
    for (size_t i = 0; i < TF_MAX_SEMANTICS; i++)
    {
        if (!geomData->pCpuData->mVertexStrides[i])
            continue;

        if (i == TF_SEMANTIC_POSITION)
        {
            posAttributeIdx = (int32_t)i;
        }

        streams[validStreamCount].data =
            (uint8_t*)geomData->pCpuData->pAttributes[i] + vertexOffset * geomData->pCpuData->mVertexStrides[i];
        streams[validStreamCount].size = geomData->pCpuData->mVertexStrides[i];
        streams[validStreamCount].stride = geomData->pCpuData->mVertexStrides[i];
    ...
    if (indexType == TF_INDEX_TYPE_UINT16)
    {
        newVertCount = (uint32_t)meshopt_generateVertexRemapMulti(remap, (uint16_t*)geomData->pCpuData->pIndices + indexOffset, indexCount,
                                                                  *vertexCount, streams, validStreamCount);
        meshopt_remapIndexBuffer((uint16_t*)geomData->pCpuData->pIndices + indexOffset,
                                 (uint16_t*)geomData->pCpuData->pIndices + indexOffset, indexCount, remap);
    }
    else
    {
        newVertCount = (uint32_t)meshopt_generateVertexRemapMulti(remap, (uint32_t*)geomData->pCpuData->pIndices + indexOffset, indexCount,
                                                                  *vertexCount, streams, validStreamCount);
        meshopt_remapIndexBuffer((uint32_t*)geomData->pCpuData->pIndices + indexOffset,
                                 (uint32_t*)geomData->pCpuData->pIndices + indexOffset, indexCount, remap);
    }
    ...
    for (size_t i = 0; i < TF_MAX_SEMANTICS; i++)
    {
        ...
        uint8_t* buffer = (uint8_t*)geomData->pCpuData->pAttributes[i] + vertexOffset * geomData->pCpuData->mVertexStrides[i];
        meshopt_remapVertexBuffer(buffer, buffer, *vertexCount, geomData->pCpuData->mVertexStrides[i], remap);
    }
    ...
    *vertexCount = newVertCount;
    ...
    if (indexType == TF_INDEX_TYPE_UINT16)
    {
        ...
        {
            uint16_t* src = (uint16_t*)geomData->pCpuData->pIndices + indexOffset;
        ...
        {
            uint16_t* src = (uint16_t*)geomData->pCpuData->pIndices + indexOffset;
            float*    pos = (float*)((uint8_t*)geomData->pCpuData->pAttributes[posAttributeIdx] +
                                  vertexOffset * geomData->pCpuData->mVertexStrides[posAttributeIdx]);

            const float kThreshold = 1.01f;
            meshopt_optimizeOverdraw(src, src, indexCount, pos, *vertexCount, geomData->pCpuData->mVertexStrides[posAttributeIdx],
                                     kThreshold);
        }

        if (optimizationFlags & MESH_OPTIMIZATION_FLAG_VERTEXFETCH)
        {
            uint16_t* indices = (uint16_t*)geomData->pCpuData->pIndices + indexOffset;
    ...
    else
    {
        if (optimizationFlags & MESH_OPTIMIZATION_FLAG_VERTEXCACHE)
        {
            uint32_t* src = (uint32_t*)geomData->pCpuData->pIndices + indexOffset;
            meshopt_optimizeVertexCache(src, src, indexCount, *vertexCount);
        }
        ...
        if (optimizationFlags & MESH_OPTIMIZATION_FLAG_VERTEXFETCH)
        {
            uint32_t* indices = (uint32_t*)geomData->pCpuData->pIndices + indexOffset;
            newVertCount = (uint32_t)meshopt_optimizeVertexFetchRemap(remap, indices, indexCount, *vertexCount);
            meshopt_remapIndexBuffer(indices, indices, indexCount, remap);
...
{
    TFVertexLayout* pVertexLayout = glTFParams->pVertexLayout;
    ...
    if (glTFParams->mProcessForPackaging)
    {
        glTFParams->outPackagingData.pPackageMetadata = (TFPackageDesc*)tf_malloc(sizeof(TFPackageDesc));
        InitPackageMetadataForPackage(gltfFileCount, glTFParams->outPackagingData.pPackageMetadata);

        glTFParams->outPackagingData.pTextureMetadata = (TFTexturePackagedDesc*)tf_calloc(gltfFileCount, sizeof(TFTexturePackagedDesc));
    }

    for (uint32_t gltfFileIndex = 0; gltfFileIndex < gltfFileCount; ++gltfFileIndex)
    {
        const char* fileName = (char*)gltfFiles[gltfFileIndex].data;
        ...
        if (glTFParams->mProcessForPackaging)
        {
            CollectGeoNameForPackage(newFileName, glTFParams->outPackagingData.pPackageMetadata);
        }
        ...
        for (uint32_t j = 0; j < data->buffers_count; ++j)
        {
            ...
            if (strncmp(uri, "data:", 5) != 0 && !strstr(uri, "://"))
            {
                ...
                fsAppendPathComponent(parent, uri, path);
                TFFileStream fs = {};
                if (fsOpenStreamFromPath(assetParams->mRDInput, path, TF_FM_READ, &fs))
                {
        ...
        if (glTFParams->mProcessForPackaging)
        {
            InitTextureMetadataForPackage(data, &glTFParams->outPackagingData.pTextureMetadata[gltfFileIndex]);
        }

        cgltf_attribute* vertexAttribs[TF_MAX_SEMANTICS] = {};

        uint32_t indexCount = 0;
        uint32_t vertexCount = 0;
        uint32_t drawCount = 0;
        uint32_t jointCount = 0;
        uint32_t vertexAttribCount[TF_MAX_SEMANTICS] = {};

        // Find number of traditional draw calls required to draw this piece of geometry
        // Find total index count, total vertex count
        for (uint32_t meshIndex = 0; meshIndex < data->meshes_count; ++meshIndex)
        {
            ...
            {
                glTFParams->pReadExtrasCallback(meshIndex, (uint32_t)data->meshes_count, data->json + mesh->extras.start_offset,
                                                meshExtrasLength, glTFParams->pCallbackUserData);
            }
            ...
            for (uint32_t primitiveIndex = 0; primitiveIndex < mesh->primitives_count; ++primitiveIndex)
            {
                ...
                for (uint32_t k = 0; k < prim->attributes_count; ++k)
                {
                    ...
                    ASSERT(semanticIdx < TF_MAX_SEMANTICS);
                ...
                if (glTFParams->mProcessForPackaging)
                {
                    CollectTextureIndicesForPackage(data, meshIndex, primitiveIndex,
                                                    &glTFParams->outPackagingData.pTextureMetadata[gltfFileIndex]);
                    GenerateStreamZonesForPackage(data, meshIndex, primitiveIndex,
                                                  &glTFParams->outPackagingData.pTextureMetadata[gltfFileIndex]);
                }
            }
        }
        ...
        PackingFunction vertexPacking[TF_MAX_SEMANTICS] = {};
        uint32_t        vertexAttrStrides[TF_MAX_SEMANTICS] = {};
        ...
        totalGeomSize += round_up(sizeof(TFGeometry), 16);
        totalGeomSize += round_up(drawCount * sizeof(TFIndirectDrawIndexArguments), 16);

        uint32_t totalGeomDataSize = 0;
        totalGeomDataSize += round_up(sizeof(TFGeometryData), 16);
        ...
        TFGeometry* geom = (TFGeometry*)tf_calloc(1, totalGeomSize);
        ASSERT(geom);

        TFGeometryData* geomData = (TFGeometryData*)tf_calloc(1, totalGeomDataSize);
        ASSERT(geomData);
        ...
        {
            const TFVertexAttrib*  attr = &pVertexLayout->mAttribs[attrIdx];
        ...
        cpuDataSize += sizeof(TFGeometryData::CpuData);
        cpuDataSize += indexCount * indexStride;

        for (uint32_t s = 0; s < TF_MAX_SEMANTICS; ++s)
        {
            // Only copy attribute count if we care about this attribute
            if (vertexAttrStrides[s] == 0)
                vertexAttribCount[s] = 0;

            cpuDataSize += vertexAttrStrides[s] * vertexAttribCount[s];
        }

        geomData->pCpuData = (TFGeometryData::CpuData*)tf_calloc(1, cpuDataSize);
        geomData->pCpuData->pIndices = geomData->pCpuData + 1;

        // Same strides as the ones used in the GPU
        for (uint32_t s = 0; s < TF_MAX_SEMANTICS; ++s)
        {
            geomData->pCpuData->mVertexStrides[s] = vertexAttrStrides[s];
            geomData->pCpuData->mAttributeCount[s] = vertexAttribCount[s];
        }

        geomData->pCpuData->pAttributes[TF_SEMANTIC_POSITION] = (uint8_t*)geomData->pCpuData->pIndices + (indexCount * indexStride);
        ...
        ASSERT(((const char*)geomData->pCpuData) + cpuDataSize ==
               ((char*)geomData->pCpuData->pAttributes[TF_MAX_SEMANTICS - 1] +
                geomData->pCpuData->mVertexStrides[TF_MAX_SEMANTICS - 1] * geomData->pCpuData->mAttributeCount[TF_MAX_SEMANTICS - 1]));

        COMPILE_ASSERT(TF_ARRAY_COUNT(geomData->pCpuData->mVertexStrides) == TF_ARRAY_COUNT(geomData->pCpuData->pAttributes));
        for (uint32_t j = 1; j < TF_ARRAY_COUNT(geomData->pCpuData->mVertexStrides); ++j)
        {
            // If the attribute is not present in the gltf file we just don't save anything
            if (geomData->pCpuData->mVertexStrides[j] == 0)
                geomData->pCpuData->pAttributes[j] = nullptr;
        }

        geom->mDrawArgCount = drawCount;
        geom->mIndexType = (sizeof(uint16_t) == indexStride) ? TF_INDEX_TYPE_UINT16 : TF_INDEX_TYPE_UINT32;
        ...
        if (glTFParams->mOptimizationFlags != MESH_OPTIMIZATION_FLAG_OFF)
        {
            for (uint32_t s = 0; s < TF_MAX_SEMANTICS; ++s)
            {
                geomData->pCpuData->mAttributeCount[s] = 0;
            }
        }
        ...
        for (uint32_t j = 0; j < data->meshes_count; ++j)
        {
            for (uint32_t p = 0; p < data->meshes[j].primitives_count; ++p)
            {
                ...
                for (uint32_t a = 0; a < prim->attributes_count; ++a)
                {
                    ...
                    ASSERT(semanticIdx < TF_ARRAY_COUNT(geomData->pCpuData->pAttributes));

                    if (semanticIdx == TF_SEMANTIC_POSITION)
                        pos_attr = attr;

                    // TODO: this should probably be an ASSERT, we don't want to loose data when using pCpuData
                    if (geomData->pCpuData->mVertexStrides[semanticIdx] != 0)
                    {
                        const uint32_t stride = geomData->pCpuData->mVertexStrides[semanticIdx];

                        const uint8_t* src =
                            (uint8_t*)attr->data->buffer_view->buffer->data + attr->data->offset + attr->data->buffer_view->offset;
                        uint8_t* dst = (uint8_t*)geomData->pCpuData->pAttributes[semanticIdx] + (uint64_t)vertexCount * stride;
                        ...
                        //       The inconvenience in that case would be that in order to get the pCpuData data we would have to unpack the
                        //       interleaved attributes, another option would be to store interleaved buffers AND pCpuData buffers
                        //       (duplicate some data). If we duplicate data would be better to only store pCpuData data for the meshes that
                        //       require it rather than all of them, that would require more configuration parameters per mesh while running
                        //       the AssetPipeline.
                ...
                if (glTFParams->mOptimizationFlags != MESH_OPTIMIZATION_FLAG_OFF)
                {
                    geomOptimize(geomData, MESH_OPTIMIZATION_FLAG_ALL, (TFIndexType)geom->mIndexType, indexCount,
                                 (uint32_t)(prim->indices->count), vertexCount, &optimizedVertexCount);

                    for (uint32_t s = 0; s < TF_MAX_SEMANTICS; ++s)
                    {
                        if (geomData->pCpuData->mVertexStrides[s] > 0)
                        {
                            geomData->pCpuData->mAttributeCount[s] += optimizedVertexCount;
                        }
                    }
                }
                ...
                if (glTFParams->mProcessMeshlets)
                {
                    ...
                    float3* positions =
                        (float3*)((uint8_t*)geomData->pCpuData->pAttributes[TF_SEMANTIC_POSITION] + vertexCount * pos_attr->data->stride);
                    ...
                    TFMeshlet*     subMeshlets = NULL;
                    TFMeshletData* meshletsData = NULL;
                    // indices for positions
                    uint*          meshletVertices = NULL;
                    uint8_t*       meshletTriangles = NULL;

                    buildMeshlets((uint32_t*)geomData->pCpuData->pIndices + indexCount, prim->indices->count, positions,
                ...
                {
                    for (uint32_t idx = 0; idx < prim->indices->count; ++idx)
                        ((uint16_t*)geomData->pCpuData->pIndices)[indexCount + idx] += (uint16_t)vertexCount;
                }
                else
                {
                    for (uint32_t idx = 0; idx < prim->indices->count; ++idx)
                        ((uint32_t*)geomData->pCpuData->pIndices)[indexCount + idx] += vertexCount;
                }
        ...
        if (glTFParams->mOptimizationFlags != MESH_OPTIMIZATION_FLAG_OFF)
        {
            ...
            for (uint32_t s = TF_SEMANTIC_POSITION + 1; s < TF_MAX_SEMANTICS; ++s)
            {
                largestSizedAttribute =
                    max(largestSizedAttribute, (size_t)geomData->pCpuData->mVertexStrides[s] * geomData->pCpuData->mAttributeCount[s]);
            }
            void* tempStagingBuffer = tf_malloc(largestSizedAttribute);

            for (uint32_t s = TF_SEMANTIC_POSITION + 1; s < TF_MAX_SEMANTICS; ++s)
            {
                size_t size = (size_t)geomData->pCpuData->mVertexStrides[s] * geomData->pCpuData->mAttributeCount[s];
                if (size > 0)
                {
                    memcpy(tempStagingBuffer, geomData->pCpuData->pAttributes[s], size);
                }
                ...
                if (size > 0)
                {
                    memcpy(geomData->pCpuData->pAttributes[s], tempStagingBuffer, size);
                }
            }
        ...
        else
        {
            ...
            geom->pGeometryBuffer = NULL;
            geom->pDrawArgs = NULL;
            fsWriteToStream(&fStream, geom, totalGeomSize);

            // Write null values to file since the pointers are set afterwars
            TFGeometryData::CpuData* pTempCpuData = geomData->pCpuData;
            void*                    pTempUserData = geomData->pUserData;
            mat4*                    pTempInverseBindPoses = geomData->pInverseBindPoses;
            uint32_t*                pTempJointRemaps = geomData->pJointRemaps;

            geomData->pCpuData = NULL;
            geomData->pUserData = NULL;
            geomData->pInverseBindPoses = NULL;
            geomData->pJointRemaps = NULL;
            fsWriteToStream(&fStream, &totalGeomDataSize, sizeof(uint32_t));
            fsWriteToStream(&fStream, geomData, totalGeomDataSize);
            geomData->pCpuData = pTempCpuData;
            geomData->pUserData = pTempUserData;
            geomData->pInverseBindPoses = pTempInverseBindPoses;
            geomData->pJointRemaps = pTempJointRemaps;
            fsWriteToStream(&fStream, &cpuDataSize, sizeof(uint32_t));
            geomData->pCpuData->pIndices = NULL;

            for (uint32_t s = 0; s < TF_MAX_SEMANTICS; s++)
            {
                geomData->pCpuData->pAttributes[s] = NULL;
            }

            fsWriteToStream(&fStream, geomData->pCpuData, cpuDataSize);
    ...
    if (gltfFiles)
    {
        for (uint32_t gltfFileIndex = 0; gltfFileIndex < gltfFileCount; ++gltfFileIndex)
        {
            bdestroy(&gltfFiles[gltfFileIndex]);
        }
        arrfree(gltfFiles);
    }
...
bool WriteZip(AssetPipelineParams* assetParams, WriteZipParams* zipParams)
{
    ...
    bool archiveIsCreated = bunyArLibCreate(assetParams->mRDOutput, zipParams->mZipFileName, &archiveCreateDesc, false);
...
bool ZipAllAssets(AssetPipelineParams* assetParams, WriteZipParams* zipParams)
{
    ...
    bool success = bunyArLibCreate(assetParams->mRDOutput, zipParams->mZipFileName, &archiveCreateDesc, false);
...
void OnDiscoverAnimation(TFResourceDirectory resourceDir, const char* filename, void* pUserData)
{
    ...
    TFFileStream file = {};
    if (!fsOpenStreamFromPath(resourceDir, filename, TF_FM_READ, &file))
    {
        LOGF(eERROR, "Failed to open gltf file %s", filename);
        return;
    }
...
int AssetPipelineRun(AssetPipelineParams* assetParams)
{
    ...
    if (assetParams->mProcessType == PROCESS_GLTF)
    {
        ...
        if (isHair)
        {
            ...
            vertexLayout.mAttribs[1].mSemantic = TF_SEMANTIC_TANGENT;
            vertexLayout.mAttribs[1].mBinding = 1;
            vertexLayout.mAttribs[2].mSemantic = TF_SEMANTIC_TEXCOORD0;
            vertexLayout.mAttribs[2].mBinding = 2;
            vertexLayout.mAttribs[3].mSemantic = TF_SEMANTIC_TEXCOORD2;
            vertexLayout.mAttribs[3].mBinding = 4;
            vertexLayout.mAttribs[4].mSemantic = TF_SEMANTIC_TEXCOORD3;
            vertexLayout.mAttribs[4].mBinding = 5;
            vertexLayout.mAttribs[5].mSemantic = TF_SEMANTIC_TEXCOORD6;
            vertexLayout.mAttribs[5].mBinding = 8;
            vertexLayout.mAttribs[6].mSemantic = TF_SEMANTIC_TEXCOORD7;
    ...
    if (assetParams->mProcessType == PROCESS_TEXTURES)
    {
        ...
        for (int i = 0; i < assetParams->mFlagsCount; i++)
        {
            ...
            else if (STRCMP(flag, "--cubemap"))
            {
                texturesParams.mProcessAsCubeMap = true;
            }
            ...
            else if (STRCMP(flag, "--target-size"))
            {
                texturesParams.mTargetHeight = texturesParams.mTargetWidth = atoi(assetParams->mFlags[++i]);
            }
            else if (STRCMP(flag, "--target-width"))
            {
                texturesParams.mTargetWidth = atoi(assetParams->mFlags[++i]);
            }
            else if (STRCMP(flag, "--target-height"))
            {
                texturesParams.mTargetHeight = atoi(assetParams->mFlags[++i]);
            }
    ...
    if (assetParams->mProcessType == PROCESS_IES)
    {
        TextureContainer outContainer = CONTAINER_DDS;
        for (int i = 0; i < assetParams->mFlagsCount; i++)
        {
            const char* flag = assetParams->mFlags[i];
            if (STRCMP(flag, "--out-ktx"))
            {
                outContainer = CONTAINER_KTX;
            }
            else if (STRCMP(flag, "--out-dds"))
            {
                outContainer = CONTAINER_DDS;
            }
            else if (STRCMP(flag, "--out-gnf-orbis"))
            {
#ifdef PROSPERO_GNF
                outContainer = CONTAINER_GNF_ORBIS;
#else
                LOGF(eERROR, "AssetPipeline wasn't compiled with Orbis GNF texture conversion support.");
#endif
            }
            else if (STRCMP(flag, "--out-gnf-prospero"))
            {
#ifdef PROSPERO_GNF
                outContainer = CONTAINER_GNF_PROSPERO;
#else
                LOGF(eERROR, "AssetPipeline wasn't compiled with Prospero GNF texture conversion support.");
#endif
            }
            ...
            {
#ifdef XBOX_SCARLETT_DDS
                outContainer = CONTAINER_SCARLETT_DDS;
#else
                LOGF(eERROR, "AssetPipeline wasn't compiled with Xbox DDS texture conversion support.");
#endif
            }
        ...
        BeginAssetPipelineSection("ProcessIES");
        bool error = ProcessIES(assetParams, outContainer);
        EndAssetPipelineSection("ProcessIES");
        return error;
    }

    if (assetParams->mProcessType == PROCESS_FONTS)
    {
        ProcessFontsParams params{};
        params.mGlyphResolution = 32;
        params.mPxRange = 1.0f;
        params.mOnlyASCII = false;

        // we always need second param
        for (int32_t i = 0; i < assetParams->mFlagsCount; ++i)
        {
            bool notLast = i < (assetParams->mFlagsCount - 1);
            if (notLast && strcmp(assetParams->mFlags[i], "--glyph_resolution") == 0)
                params.mGlyphResolution = (uint32_t)atoi(assetParams->mFlags[i + 1]);
            else if (notLast && strcmp(assetParams->mFlags[i], "--pixel_range") == 0)
                params.mPxRange = (float)atof(assetParams->mFlags[i + 1]);
            else if (strcmp(assetParams->mFlags[i], "--only_ascii") == 0)
                params.mOnlyASCII = true;
        }

        BeginAssetPipelineSection("ProcessFonts");
        bool result = ProcessFonts(assetParams, params);
        EndAssetPipelineSection("ProcessFonts");
        return result;
    }
    ...
    if (assetParams->mProcessType == PROCESS_PACKAGING)
    {
        ProcessPackageParams packageParams = {};

        packageParams.outputPlatform = PACKAGE_PLATFORM_UNDEFINED;

        for (int flagIndex = 0; flagIndex < assetParams->mFlagsCount; flagIndex++)
        {
            const char* flag = assetParams->mFlags[flagIndex];

            if (STRCMP(flag, "--platform-xbox"))
            {
                packageParams.outputPlatform = PACKAGE_PLATFORM_XBONE;
            }
            else if (STRCMP(flag, "--platform-scarlett"))
            {
                packageParams.outputPlatform = PACKAGE_PLATFORM_SCARLETT;
            }
            else if (STRCMP(flag, "--platform-orbis"))
            {
                packageParams.outputPlatform = PACKAGE_PLATFORM_ORBIS;
            }
            else if (STRCMP(flag, "--platform-prospero"))
            {
                packageParams.outputPlatform = PACKAGE_PLATFORM_PROSPERO;
            }
            else if (STRCMP(flag, "--platform-pc"))
            {
                packageParams.outputPlatform = PACKAGE_PLATFORM_PC;
            }
            else if (STRCMP(flag, "--platform-ios"))
            {
                packageParams.outputPlatform = PACKAGE_PLATFORM_IOS;
            }
            else if (STRCMP(flag, "--platform-macos"))
            {
                packageParams.outputPlatform = PACKAGE_PLATFORM_MACOS;
            }
            else if (STRCMP(flag, "--platform-quest"))
            {
                packageParams.outputPlatform = PACKAGE_PLATFORM_QUEST;
            }
            else if (STRCMP(flag, "--platform-switch"))
            {
                packageParams.outputPlatform = PACKAGE_PLATFORM_SWITCH;
            }
            else if (STRCMP(flag, "--platform-steamdeck"))
            {
                packageParams.outputPlatform = PACKAGE_PLATFORM_STEAM_DECK;
            }
            else if (STRCMP(flag, "--platform-android"))
            {
                packageParams.outputPlatform = PACKAGE_PLATFORM_ANDROID;
            }
            else if (STRCMP(flag, "--package-name"))
            {
                flagIndex++;
                packageParams.outputPackageName = assetParams->mFlags[flagIndex];
            }
            else if (STRCMP(flag, "--atlas"))
            {
                packageParams.generateAtlas = true;
            }
        }

        BeginAssetPipelineSection("ProcessPackaging");
        bool result = ProcessPackaging(assetParams, &packageParams);
        EndAssetPipelineSection("ProcessPackaging");
        return result ? 0 : 1;
    }
