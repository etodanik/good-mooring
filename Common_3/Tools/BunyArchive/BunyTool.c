...
static const enum TFResourceDirectory TF_RD = 0;
...
static int bunyArToolCreate(struct BunyArToolCtx* ctx)
{
    ...
    if (success)
    {
        ...
        success = bunyArLibCreate(TF_RD, ctx->archivePath, &info, false);
    }
...
static int bunyArToolInspect(struct BunyArToolCtx* ctx)
{
    ...
    struct TFArchiveOpenDesc adesc = { 0 };

    adesc.disableHashTable = true;
    adesc.validation = true;

    TFIFileSystem archiveFs;
    ...
    for (uint64_t i = 0; i < archiveInfo.nodeCount; ++i)
    {
        ...
        {
            TFFileStream fs;
            if (!fsIoOpenByUid(&archiveFs, i, TF_FM_READ, &fs))
            {
...
static bool tryDragNdrop(struct BunyArToolCtx* ctx, int* res, char** freeMe)
{
    *freeMe = NULL;
    ...
    {
        struct TFArchiveOpenDesc info = { 0 };
