...
if __name__ == '__main__':
    ...
    script, windowsSDK, dst, binary_dst, src, lang, compile, verbose, config, target_os, platform, tmp_path, cpu_h_dst, shader_h_dst, reload_server_port, cache_args = sys.argv
    tmp_path = tmp_path.strip()
    src = os.path.abspath(src)
    dst = os.path.abspath(dst)
    binary_dst = os.path.abspath(binary_dst)
    cpu_h_dst = os.path.abspath(cpu_h_dst)
    shader_h_dst = os.path.abspath(shader_h_dst)
    ...
    # Choose architecture target (for ISPC)
    if 'ANDROID' in platform.upper():
        arch = 'arm64'
    elif 'NX64' in platform.upper():
        arch = 'arm64'
    elif 'ORBIS' in platform.upper():
        arch = 'x86_64'
    elif 'PROSPERO' in platform.upper():
        arch = 'x86_64'
    elif 'SCARLETT' in platform.upper():
        arch = 'x86_64'
    elif 'XBOX' in platform.upper():
        arch = 'x86_64'
    elif 'X64' in platform.upper():
        arch = 'x86_64'
    else:
        arch = 'arm64'
    ...
    sys.argv = [
        ...
        '-l', lang,
        '--ispc_targetOs', target_os,
        '--ispc_architecture', arch,
        '--ispc_headerDestinationCPU', cpu_h_dst,
        '--ispc_headerDestinationShader', shader_h_dst,
        *reload_server_args,
        '--incremental',
    ]
