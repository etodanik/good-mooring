...
from utils import isArray, getArrayLen, getArrayLenFlat, getArrayBaseName, getMacroName, is_groupshared_decl, cleaneval
...
def hololens2(*args):
    return hlsl(Platforms.HOLOLENS2, *args)
...
def hlsl(platform, debug, binary: ShaderBinary, dst):
    ...
    if Platforms.HOLOLENS2 == platform:
        shader_src += ['#define TARGET_HOLOLENS2\n']    
    ...
    # math library
    header_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'includes', 'MathLibrary.h')
    header_lines = open(header_path).readlines()
    shader_src += header_lines + ['\n']
    ...
    for fi, line_index, line in iter_lines(shader.lines):
        ...
        if pssl:
            ...
            if stripped_line.startswith('BEGIN_SRT('):
                parsing_resources = True
                skip_line = True
                ...
                if stripped_line.startswith('BEGIN_SRT_SET('):
                    ...
                    if len(stripped_line) > 0 and stripped_line.startswith('#') is False:
                        ...
                        array_count = int(cleaneval(getArrayLenFlat(stripped_line))) if isArray(stripped_line) else 1
        ...
        if '_MAIN(' in line:
            ...
            if (not pssl and Features.MULTIVIEW in binary.features and shader.stage == Stages.VERT):
                if 'VS_MAIN' in line and '(' in line and 'SV_ViewID' not in line:
                    lparen = line.find('(')
                    rparen = line.rfind(')')
                    if lparen != -1 and rparen != -1:
                        params = line[lparen+1:rparen].strip()
                        injection = 'uint viewID : SV_ViewID'
                        if params:
                            new_params = params + ', ' + injection
                        else:
                            new_params = injection
                        line = line[:lparen+1] + new_params + line[rparen:]
