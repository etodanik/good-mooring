...
// ---------------------------------------------------------------------------------------------------------------------------------
static void dumpLine(TFFileStream* fileToWrite, const char* format, ...)
{
    va_list args;
...
#if MMGR_BACKTRACE
static void dumpBacktrace(TFFileStream* fh, const sAllocUnit* ptr)
{
    if (ptr->backtrace_nptrs)
    {
        ...
        char module_name[TF_FS_MAX_PATH];
        ...
        for (int i = ptr->backtrace_skip; i < ptr->backtrace_nptrs; i++)
        {
            ...
            else
            {
                ...
                GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT, (LPCTSTR)(address),
                                  &hModule);
                if (hModule != NULL)
                    GetModuleFileNameA(hModule, module_name, TF_FS_MAX_PATH);
...
static void dumpAllocations(TFFileStream* fh)
{
...
static void dumpLeakReport(void)
{
    ...
    if (mAppName)
    {
        ...
        if (!fsOpenStreamFromPath(TF_RD_LOG, outputFileName, TF_FM_WRITE, &fh))
        {
...
TFMemoryStatistics memGetStatistics(void)
{
    TFMemoryStatistics result = {
                    ...
                    size_t alignment, size_t reportedSize)
{
    ...
    MUTEX_LOCK(allocMutex);
    {
        ...
        MTRACY_ALLOC(au->reportedAddress, au->reportedSize, "Forge CPU");
        MUTEX_UNLOCK(allocMutex);
                      ...
                      size_t reportedSize, void* reportedAddress)
{
...

    {
        ...
        MTRACY_FREE(reportedAddress, "Forge CPU");
        MTRACY_ALLOC(au->reportedAddress, au->reportedSize, "Forge CPU");
        MUTEX_UNLOCK(allocMutex);
        return au->reportedAddress;
    }
                     ...
                     const void* reportedAddress)
{
    ...
    if (reportedAddress)
    {
        ...
        MTRACY_FREE(au->reportedAddress, "Forge CPU");
...
// ---------------------------------------------------------------------------------------------------------------------------------
static void fsPrintf(TFFileStream* fileStream, const char* format, ...)
{
    va_list args;
    char    buffer[BUFFER_SIZE] = { 0 };
    va_start(args, format);
...
void mmgrDumpMemoryReport(const char* filename, const bool overwrite)
{
    {
        TFFileStream fh = { 0 };
        bool       success = fsOpenStreamFromPath(TF_RD_LOG, filename, overwrite ? TF_FM_WRITE : TF_FM_APPEND, &fh);
