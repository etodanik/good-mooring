...
#define LOG_CALLBACK_MAX_ID TF_FS_MAX_PATH
...
{
    TFFileStream* fh = (TFFileStream*)user_data;
    ASSERT(fh);
...
void initLog(const char* appName, LogLevel level /* = eALL */)
{
    if (!gIsLoggerInitialized)
    {
        ...
        LOGF(eINFO, "'%s' application log initialized.", appName);
    }
}
...
void addLogFile(const char* filename, TFFileMode file_mode, LogLevel log_level)
{
    if (filename == NULL)
        return;

    TFFileStream fh;
    memset(&fh, 0, sizeof(TFFileStream));
    if (fsOpenStreamFromPath(TF_RD_LOG, filename, file_mode, &fh)) // If the File Exists
    {
        TFFileStream* user = (TFFileStream*)tf_malloc(sizeof(TFFileStream));
        *user = fh;
...
void writeLogVaList(uint32_t level, const char* filename, int line_number, const char* message, va_list args)
{
    ...
    mooringTracyLog(level, gLogBuffer + preable_end + LOG_LEVEL_SIZE, offset - preable_end - LOG_LEVEL_SIZE);
...
void writeRawLog(uint32_t level, bool error, const char* message, ...)
{
    ...
    va_end(args);
    mooringTracyLog(level, gLogBuffer, strlen(gLogBuffer));
...
static void addInitialLogFile(const char* appName)
{
    ...
    char exeFileName[TF_FS_MAX_PATH] = { 0 };
    ...
    addLogFile(exeFileName, TF_FM_WRITE_ALLOW_READ, eALL);
}
