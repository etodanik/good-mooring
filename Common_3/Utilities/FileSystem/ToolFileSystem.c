...
const char* fsGetResourceDirectory(TFResourceDirectory resourceDir)
{
    const TFResourceDirectoryInfo* dir = &gResourceDirectories[resourceDir];
...
RETURN_FALSE:
    ...
    if (strlen(path) > TF_FS_MAX_PATH - 1)
        return false;
...
FORGE_TOOL_API void fsAppendPathExtension(const char* basePath, const char* extension, char* output)
{
    ...
    if (!VERIFYMSG(maxPathLength < TF_FS_MAX_PATH, "Extension path length '%zu' greater than TF_FS_MAX_PATH", maxPathLength))
    {
        return;
    }
...
FORGE_TOOL_API void fsGetParentPath(const char* path, char* output)
{
    ...
    size_t reslen = outputLength > TF_FS_MAX_PATH - 1 ? TF_FS_MAX_PATH - 1 : outputLength;
...
FORGE_TOOL_API void fsGetPathExtension(const char* path, char* output)
{
    ...
    ASSERT(extensionLength < TF_FS_MAX_PATH);
    strncpy(output, dotLocation, TF_FS_MAX_PATH - 1);
    output[TF_FS_MAX_PATH - 1] = 0;
}
...
FORGE_TOOL_API void fsReplacePathExtension(const char* path, const char* newExtension, char* output)
{
    ...
    ASSERT(baseLength != 0);
    if (!VERIFYMSG(maxPathLength < TF_FS_MAX_PATH, "New extension path length '%zu' greater than TF_FS_MAX_PATH", maxPathLength))
    {
        return;
    }

    strncpy(output, path, TF_FS_MAX_PATH - 1);
    output[TF_FS_MAX_PATH - 1] = 0;
    ...
    char currentExtension[TF_FS_MAX_PATH] = { 0 };
...
FORGE_TOOL_API void fsGetPathFileName(const char* path, char* output)
{
    ...
    char parentPath[TF_FS_MAX_PATH] = { 0 };
    ...
    char extension[TF_FS_MAX_PATH] = { 0 };
...
bool fsGetFilesWithExtension(TFResourceDirectory resourceDir, const char* subDirectory, const char* extension, char*** out, int* count)
{
    *out = NULL;
    *count = 0;

    char directoryPath[TF_FS_MAX_PATH] = { 0 };
    fsAppendPathComponent(fsGetResourceDirectory(resourceDir), subDirectory, directoryPath);

    TFFsDirectoryIterator iterator = NULL;
    ...
    for (;;)
    {
        struct TFFsDirectoryIteratorEntry entry;
        ...
        char fileExt[TF_FS_MAX_PATH] = { 0 };
        fsGetPathExtension(entry.name, fileExt);
        size_t fileExtLen = strlen(fileExt);

        if ((!extension) || (extension[0] == 0 && fileExtLen == 0) || (fileExtLen > 0 && strcasecmp(fileExt, extension) == 0))
        {
            filesFound += 1;
        }
    }
    ...
    size_t stringsSize = filesFound * sizeof(char*) + filesFound * sizeof(char) * TF_FS_MAX_PATH;
    ...
    {
        stringList[i] = firstString + (sizeof(char) * TF_FS_MAX_PATH * i);
    }
...
bool fsGetSubDirectories(TFResourceDirectory resourceDir, const char* subDirectory, char*** out, int* count)
{
    *out = NULL;
    *count = 0;
...
bool fsDirectoryIteratorOpen(TFResourceDirectory rd, const char* dir, TFFsDirectoryIterator* out)
{
    ...
    *out = (TFFsDirectoryIterator)directory;
    return true;
}

void fsDirectoryIteratorClose(TFFsDirectoryIterator iterator)
{
...
bool fsDirectoryIteratorNext(TFFsDirectoryIterator iterator, struct TFFsDirectoryIteratorEntry* outEntry)
{
    *outEntry = (struct TFFsDirectoryIteratorEntry){ 0 };
