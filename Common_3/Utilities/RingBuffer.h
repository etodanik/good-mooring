...
typedef struct GpuCmdRingElement
{
    ...
    TFCmd**      pCmds;
    TFFence*     pFence;
    TFSemaphore* pSemaphore;
} GpuCmdRingElement;

// Lightweight wrapper that works as a ring for command pools, command buffers
typedef struct GpuCmdRing
{
    TFCmdPool*   pCmdPools[MAX_GPU_CMD_POOLS_PER_RING];
    TFCmd*       pCmds[MAX_GPU_CMD_POOLS_PER_RING][MAX_GPU_CMDS_PER_POOL];
    TFFence*     pFences[MAX_GPU_CMD_POOLS_PER_RING][MAX_GPU_CMDS_PER_POOL];
    TFSemaphore* pSemaphores[MAX_GPU_CMD_POOLS_PER_RING][MAX_GPU_CMDS_PER_POOL];
    uint32_t     mPoolIndex;
    uint32_t     mCmdIndex;
...
static inline void addGPURingBuffer(TFRenderer* pRenderer, const TFBufferDesc* pBufferDesc, GPURingBuffer* pRingBuffer)
{
    *pRingBuffer = {};
...
static inline void addUniformGPURingBuffer(TFRenderer* pRenderer, uint32_t requiredUniformBufferSize, GPURingBuffer* pRingBuffer,
                                           bool const            ownMemory = false,
                                           TFResourceMemoryUsage memoryUsage = TF_RESOURCE_MEMORY_USAGE_CPU_TO_GPU)
{
    *pRingBuffer = {};
    pRingBuffer->pRenderer = pRenderer;
    ...
    TFBufferDesc ubDesc = {};
    ubDesc.mDescriptors = TF_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    ubDesc.mMemoryUsage = memoryUsage;
    ubDesc.mFlags = (ubDesc.mMemoryUsage != TF_RESOURCE_MEMORY_USAGE_GPU_ONLY ? TF_BUFFER_CREATION_FLAG_PERSISTENT_MAP_BIT
                                                                              : TF_BUFFER_CREATION_FLAG_NONE) |
                    TF_BUFFER_CREATION_FLAG_NO_DESCRIPTOR_VIEW_CREATION;

    if (ownMemory)
        ubDesc.mFlags |= TF_BUFFER_CREATION_FLAG_OWN_MEMORY_BIT;
...
static inline void initGpuCmdRing(TFRenderer* pRenderer, const GpuCmdRingDesc* pDesc, GpuCmdRing* pOut)
{
...
static inline void exitGpuCmdRing(TFRenderer* pRenderer, GpuCmdRing* pRing)
{
