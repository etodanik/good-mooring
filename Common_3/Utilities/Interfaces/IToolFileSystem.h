...
extern "C"
{
    ...
    // MARK: - File Watcher
    /************************************************************************/
    typedef struct TFFileWatcher TFFileWatcher;

    typedef void (*TFFileWatcherCallback)(const char* fileName, uint32_t action, void* userData);

    typedef enum TFFileWatcherEventMask
    {
        TF_FWE_MODIFIED = 1 << 0,
        TF_FWE_ACCESSED = 1 << 1,
        TF_FWE_CREATED = 1 << 2,
        TF_FWE_DELETED = 1 << 3,
    } TFFileWatcherEventMask;

    /// Creates a new TFFileWatcher that watches for changes specified by `eventMask` at `path` and calls `callback` when changes occur.
    /// The return value must have `fsFreeFileWatcher` called to free it.
    FORGE_TOOL_API TFFileWatcher* fsCreateFileWatcher(const char* path, TFFileWatcherEventMask eventMask, TFFileWatcherCallback callback,
                                                      void* callbackUserData);

    /// Invalidates and frees `fileWatcher.
    FORGE_TOOL_API void fsFreeFileWatcher(TFFileWatcher* fileWatcher);
...
/************************************************************************/
    ...
    typedef void*       TFFsDirectoryIterator;
    FORGE_TOOL_API bool fsDirectoryIteratorOpen(TFResourceDirectory rd, const char* dir, TFFsDirectoryIterator* out);
    FORGE_TOOL_API void fsDirectoryIteratorClose(TFFsDirectoryIterator iterator);
    ...
    struct TFFsDirectoryIteratorEntry
    {
        bool        isDir;
        const char* name;
    };
    FORGE_TOOL_API bool fsDirectoryIteratorNext(TFFsDirectoryIterator iterator, struct TFFsDirectoryIteratorEntry* outEntry);

    /// Returns and allocates an array of all directories inside input resourceDir + directory. String also contains subDirectory
    FORGE_TOOL_API bool fsGetSubDirectories(TFResourceDirectory resourceDir, const char* subDirectory, char*** out, int* count);
    ...
    FORGE_TOOL_API bool fsGetFilesWithExtension(TFResourceDirectory resourceDir, const char* subDirectory, const char* extension,
                                                char*** out, int* count);
    /************************************************************************/
    /************************************************************************/
    ...
    FORGE_TOOL_API bool fsCheckPath(TFResourceDirectory resourceDir, const char* path, bool* exist, bool* isDir, bool* isFile);

    FORGE_TOOL_API bool fsRemoveFile(TFResourceDirectory resourceDir, const char* fileName);

    FORGE_TOOL_API bool fsRenameFile(TFResourceDirectory resourceDir, const char* fileName, const char* newFileName);

    FORGE_TOOL_API bool fsCopyFile(TFResourceDirectory sourceResourceDir, const char* sourceFileName,
                                   const TFResourceDirectory destResourceDir, const char* destFileName);

    FORGE_TOOL_API bool fsFileExist(TFResourceDirectory resourceDir, const char* fileName);

    /// Recursively create directories for given resourceDir and path
    /// Returns true if directories are created or already exist
    FORGE_TOOL_API bool fsCreateDirectory(TFResourceDirectory resourceDir, const char* path, bool recursive);
    ...
    /// requires dst buffer size to be >= TF_FS_MAX_PATH
    /// Default use-case:
    ///   char path[TF_FS_MAX_PATH];
    ...
    FORGE_TOOL_API time_t      fsGetLastModifiedTime(TFResourceDirectory resourceDir, const char* fileName);
    FORGE_TOOL_API const char* fsGetResourceDirectory(TFResourceDirectory resourceDi);
    /************************************************************************/
