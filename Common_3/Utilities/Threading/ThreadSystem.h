...
#include <stdbool.h>
    ...
    {
        // same as affinity mask from struct TFThreadDesc, but for all threads in pool
...
#define threadSystemAddTaskGroup(ts, func, count, userArray) threadSystemAddTasks(ts, func, count, sizeof *userArray, userArray)
    ...
    void threadSystemWaitIdle(ThreadSystem ts);
...
public:
    ...
    void addTask(TaskFunc func, void* data) const { threadSystemAddTasks(threadSystem, func, 1, 0, data); }
