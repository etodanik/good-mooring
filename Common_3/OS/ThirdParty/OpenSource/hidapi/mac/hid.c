...
struct hid_device_ {
	...
	ThreadHandle thread;
	TFMutex mutex; /* Protects input_reports */
...
static int cond_wait(const hid_device *dev, TFConditionVariable *cond, TFMutex *mutex)
{
...
static int cond_timedwait(const hid_device *dev, TFConditionVariable *cond, TFMutex *mutex, uint32_t abstime)
{
	while (!dev->input_reports) {
