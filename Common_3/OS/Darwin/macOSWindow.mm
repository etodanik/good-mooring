...
TFMonitorDesc* gMonitors = nullptr;
...
// STATIC STRUCTS
//------------------------------------------------------------------------
static TFRectDesc convertNSRectToRectWithDpiScale(NSRect nsRect, const float dpiScale[2])
{
...
static TFRectDesc convertNSRectToRect(const NSRect nsRect, const NSWindow* window)
{
...
static NSRect convertRectToNSRectWithDpi(TFRectDesc rect, const float dpiScale[2])
{
...
static NSRect convertRectToNSRect(const TFRectDesc rect, const NSWindow* window)
{
    const float scale = [window backingScaleFactor];
    const float dpiScale[2] = { scale, scale };
...
- (void)updateClientRect:(TFWindowDesc*)winDesc
{
...
static NSRect getCenteredWindowRect(TFWindowDesc* winDesc);

- (void)updateWindowedRect:(TFWindowDesc*)winDesc
{
...
CGDisplayModeRef FindClosestResolution(CGDirectDisplayID display, const TFResolution* pMode)
{
...
NSWindowStyleMask PrepareStyleMask(TFWindowDesc* winDesc)
{
...
void displayReconfigureCallback(CGDirectDisplayID displayID, CGDisplayChangeSummaryFlags flags, void* userInfo)
{
    ...
    TFMonitorDesc* pMonitor = &gMonitors[monitorIndex];

    const TFResolution* resolutions = pMonitor->resolutions;
...
void collectMonitorInfo()
{
    ...
    gMonitors = (TFMonitorDesc*)tf_calloc(displayCount, sizeof(TFMonitorDesc));
    ...
    for (uint32_t displayIndex = 0; displayIndex < displayCount; ++displayIndex)
    {
        ...
        TFMonitorDesc& display = gMonitors[displayIndex];
        ...
        TFRectDesc frameRect = convertNSRectToRectWithDpiScale(frameRectNS, gDPIScales[displayIndex]);
        ...
        TFResolution* displayResolutions = NULL;
        ...
        for (CFIndex i = 0; i < CFArrayGetCount(displayModes); ++i)
        {
            ...
            TFResolution displayResolution;
        ...
        qsort(displayResolutions, arrlen(displayResolutions), sizeof(TFResolution),
              [](const void* lhs, const void* rhs)
              {
                  TFResolution* pLhs = (TFResolution*)lhs;
                  TFResolution* pRhs = (TFResolution*)rhs;
        ...
        TFResolution highest = display.resolutions[arrlen(display.resolutions) - 1];
...
void closeWindow(TFWindowDesc* winDesc)
{
    // No-op
}

void setWindowRect(TFWindowDesc* winDesc, const TFRectDesc* pRect)
{
...
void setWindowSize(TFWindowDesc* winDesc, unsigned width, unsigned height)
{
    TFRectDesc desc{ winDesc->windowedRect.left, winDesc->windowedRect.top, winDesc->windowedRect.left + (int)width,
                   winDesc->windowedRect.top + (int)height };
    setWindowRect(winDesc, &desc);
}

void setWindowClientRect(TFWindowDesc* winDesc, const TFRectDesc* rect)
{
    ForgeMTLView* view = (__bridge ForgeMTLView*)(winDesc->handle.window);
    ASSERT(view);
    ...
    TFRectDesc          windowRect = convertNSRectToRect(styleAdjustedRect, window);
    setWindowRect(winDesc, &windowRect);
}
void setWindowClientSize(TFWindowDesc* winDesc, unsigned width, unsigned height)
{
    TFRectDesc desc{ winDesc->clientRect.left, winDesc->clientRect.top, winDesc->clientRect.left + (int)width,
                   winDesc->clientRect.top + (int)height };
    setWindowClientRect(winDesc, &desc);
}

static void toggleBorderless(TFWindowDesc* winDesc)
{
    ForgeMTLView* view = (__bridge ForgeMTLView*)(winDesc->handle.window);
    if (view == nil)
    {
    ...
    TFRectDesc newWindowedRect = winDesc->windowedRect;
    {
        ...
        TFWindowDesc tempWindowDesc = *winDesc;
...
void setWindowed(TFWindowDesc* winDesc)
{
    ...
    winDesc->mWindowMode = TFWindowMode::TF_WM_WINDOWED;
}

void setBorderless(TFWindowDesc* winDesc)
{
    if (winDesc->fullScreen)
    {
        toggleFullscreen(winDesc);
    ...
    winDesc->mWindowMode = TFWindowMode::TF_WM_BORDERLESS;
}

void setFullscreen(TFWindowDesc* winDesc)
{
    if (!winDesc->fullScreen)
    {
        toggleFullscreen(winDesc);
        winDesc->mWindowMode = TFWindowMode::TF_WM_FULLSCREEN;
    }
}

void showWindow(TFWindowDesc* winDesc)
{
...
void hideWindow(TFWindowDesc* winDesc)
{
...
void maximizeWindow(TFWindowDesc* winDesc)
{
    gCurrentWindow.hide = false;
    ForgeMTLView* view = (__bridge ForgeMTLView*)(gCurrentWindow.handle.window);
    if (view == nil)
...
void minimizeWindow(TFWindowDesc* winDesc)
{
    gCurrentWindow.hide = true;
    ForgeMTLView* view = (__bridge ForgeMTLView*)(gCurrentWindow.handle.window);
    if (view == nil)
...
void centerWindow(TFWindowDesc* winDesc)
{
    ForgeMTLView* view = (__bridge ForgeMTLView*)(gCurrentWindow.handle.window);
    if (view == nil)
    {
...
void captureCursor(TFWindowDesc* winDesc, bool bEnable)
{
    ASSERT(winDesc);
...
void setMousePositionRelative(const TFWindowDesc* winDesc, int32_t x, int32_t y)
{
    ForgeMTLView* view = (__bridge ForgeMTLView*)(gCurrentWindow.handle.window);
    if (view == nil)
    {
...
TFMonitorDesc* getMonitor(uint32_t index)
{
...
void getRecommendedWindowRect(TFWindowDesc* winDesc, TFRectDesc* rect)
{
    TFRectDesc recommendedClient = {};
...
void setResolution(const TFMonitorDesc* pMonitor, const TFResolution* pMode)
{
    ASSERT(pMonitor != nil);
    ASSERT(pMode != nil);
...
bool getResolutionSupport(const TFMonitorDesc* pMonitor, const TFResolution* pRes) { return false; }
