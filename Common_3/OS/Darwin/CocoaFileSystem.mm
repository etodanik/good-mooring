...
bool initFileSystem(TFFileSystemInitDesc* pDesc)
{
    ...
    if (!pDesc->mIsTool)
    {
        char debugDir[TF_FS_MAX_PATH] = { 0 };
        char bundleDir[TF_FS_MAX_PATH] = { 0 };
        ...
        ASSERT(resourcePathLen + 2 < TF_FS_MAX_PATH);
        ...
        // for non bundled and generated resource dirs usually TF_RD_DEBUG, TF_RD_LOG
        ...
        if (!error)
        {
            ...
            ASSERT(documentPathLen + 2 < TF_FS_MAX_PATH);
        ...
        ASSERT(bundlePathLen + 2 < TF_FS_MAX_PATH);
        strncpy(debugDir, bundlePath, bundlePathLen);
        debugDir[bundlePathLen] = '/';
#endif

        char pathStatmentsPath[TF_FS_MAX_PATH] = { 0 };
        ...
        NSURL* dirURL = [NSURL fileURLWithPath:[NSString stringWithUTF8String:gResourceDirectories[TF_RD_SCREENSHOTS].mPath] isDirectory:TRUE];
