...
#pragma mark - TFFileWatcher

struct TFFileWatcher
{
...
{
    TFFileWatcher* fswData = (TFFileWatcher*)data;
    const char** paths = (const char**)eventPaths;

    for (size_t i = 0; i < numEvents; ++i)
    {
        if ((fswData->mEventMask & TF_FWE_MODIFIED) &&
            ((eventFlags[i] & kFSEventStreamEventFlagItemModified) || (eventFlags[i] & kFSEventStreamEventFlagItemInodeMetaMod)))
        {
            fswData->mCallback(paths[i], TF_FWE_MODIFIED, fswData->mCallbackUserData);
        }
        if ((fswData->mEventMask & TF_FWE_ACCESSED) && (eventFlags[i] & kFSEventStreamEventFlagItemInodeMetaMod))
        {
            fswData->mCallback(paths[i], TF_FWE_ACCESSED, fswData->mCallbackUserData);
        }
        if ((fswData->mEventMask & TF_FWE_CREATED) && (eventFlags[i] & kFSEventStreamEventFlagItemCreated))
        {
            fswData->mCallback(paths[i], TF_FWE_CREATED, fswData->mCallbackUserData);
        }
        if ((fswData->mEventMask & TF_FWE_DELETED) && (eventFlags[i] & kFSEventStreamEventFlagItemRemoved))
        {
            fswData->mCallback(paths[i], TF_FWE_DELETED, fswData->mCallbackUserData);
        }
    }
};

TFFileWatcher* fsCreateFileWatcher(const char* path, TFFileWatcherEventMask eventMask, TFFileWatcherCallback callback, void* callbackUserData)
{
