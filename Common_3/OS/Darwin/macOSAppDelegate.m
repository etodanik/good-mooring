...
- (void)applicationDidFinishLaunching:(NSNotification*)aNotification
{
    ...
    dispatch_source_set_timer(renderTimer, dispatch_time(DISPATCH_TIME_NOW, 0),
                              benchmark ? NSEC_PER_MSEC : NSEC_PER_SEC / 30, benchmark ? 0 : NSEC_PER_MSEC);
    dispatch_source_set_event_handler(renderTimer, ^{ [weakSelf drawFunc]; });
...
{
    if (renderTimer) dispatch_source_cancel(renderTimer);
