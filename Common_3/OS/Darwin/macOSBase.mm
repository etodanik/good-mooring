...
// Protocol abstracting the platform specific view in order to keep the TFRenderer class independent from platform
...
void errorMessagePopup(const char* title, const char* msg, TFWindowHandle* windowHandle, errorMessagePopupCallbackFn callback)
{
...
extern void openWindow(const char* app_name, TFWindowDesc* winDesc, id<MTLDevice> device,
                       id<RenderDestinationProvider> delegateRenderProvider, int32_t monitorIndex);

// TFTimer used in the update function.
TFHiresTimer      deltaTimer;
...
#define EXIT_IF_FAILED(cond) \
    ...
    if (self)
    {
        ...
        TFMonitorDesc* pMonitor = getMonitor(pSettings->mMonitorIndex);
        ASSERT(pMonitor);
...
- (void)shutdown
{
    ...
    {
        TFMonitorDesc& monitor = gMonitors[i];
