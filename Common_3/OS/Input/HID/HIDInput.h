...
int  hidInit(const struct TFWindowHandle* window);
...
static TFTimer  gForceRecheckTimer;
...
{
    extern TFInputPortIndex GamepadAddHIDController(const HIDDeviceInfo* pDeviceInfo);
    TFInputPortIndex        portIndex = GamepadAddHIDController(pDeviceInfo);
...
static void HIDRemoveController(uint8_t devID)
{
    ...
    HIDController* con = gControllerBuffer + index;
    extern void    GamepadRemoveHIDController(TFInputPortIndex portIndex);
...
static void HIDUpdateDevices()
{
    ...
    for (; conIt < conEnd; ++conIt)
    {
        ...
        if (gpad->mLightReset || gpad->mLightUpdate)
        {
            ...
            COMPILE_ASSERT(TF_ARRAY_COUNT(colors) == TF_MAX_GAMEPADS);
...
/************************************************************************/
static void InitSystemNotificationReceiver(const TFWindowHandle* pWindow)
{
...
/************************************************************************/
int hidInit(const TFWindowHandle* pWindow)
{
    ASSERT(!gInitialized);
