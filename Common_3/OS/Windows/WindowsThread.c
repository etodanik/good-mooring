...
void acquireMutex(TFMutex* mutex) { EnterCriticalSection((CRITICAL_SECTION*)&mutex->mHandle); }

bool tryAcquireMutex(TFMutex* mutex) { return TryEnterCriticalSection((CRITICAL_SECTION*)&mutex->mHandle); }

void releaseMutex(TFMutex* mutex) { LeaveCriticalSection((CRITICAL_SECTION*)&mutex->mHandle); }

bool initConditionVariable(TFConditionVariable* cv)
{
...
void exitConditionVariable(TFConditionVariable* cv) { tf_free(cv->pHandle); }

void waitConditionVariable(TFConditionVariable* cv, TFMutex* pMutex, uint32_t ms)
{
    SleepConditionVariableCS((PCONDITION_VARIABLE)cv->pHandle, (PCRITICAL_SECTION)&pMutex->mHandle, ms);
}

void wakeOneConditionVariable(TFConditionVariable* cv) { WakeConditionVariable((PCONDITION_VARIABLE)cv->pHandle); }

void wakeAllConditionVariable(TFConditionVariable* cv) { WakeAllConditionVariable((PCONDITION_VARIABLE)cv->pHandle); }

bool initThreadSemaphore(TFThreadSemaphore* sem, uint32_t initialCount, uint32_t maxCount)
{
    if (!sem)
        return false;
    memset(sem, 0, sizeof(TFThreadSemaphore));
    sem->pHandle = CreateSemaphore(NULL, initialCount, min(maxCount, LONG_MAX), NULL);
    return sem->pHandle != NULL;
}
void exitThreadSemaphore(TFThreadSemaphore* sem)
{
    if (!sem)
        return;
    if (sem->pHandle)
    {
        CloseHandle(sem->pHandle);
        sem->pHandle = NULL;
    }
}
void waitThreadSemaphore(TFThreadSemaphore* sem, uint32_t count)
{
    if (!sem)
        return;
    if (sem->pHandle)
    {
        for (uint32_t i = 0; i < count; i += 1)
            WaitForSingleObject(sem->pHandle, INFINITE);
    }
}

int64_t getUSec(bool);
bool    waitThreadSemaphoreTimeout(TFThreadSemaphore* sem, uint32_t count, uint32_t msTimeout)
{
    if (!sem)
        return false;
    int64_t t0 = getUSec(false);
    int64_t usecTimeout = msTimeout * 1000;
    if (sem->pHandle)
    {
        for (uint32_t i = 0; i < count; i += 1)
        {
            int64_t elapsed = getUSec(false) - t0;
            if (elapsed >= usecTimeout)
                return false;
            DWORD result = WaitForSingleObject(sem->pHandle, (DWORD)(msTimeout - elapsed / 1000));
...
void signalThreadSemaphore(TFThreadSemaphore* sem, uint32_t count)
{
    if (!sem)
        return;
    if (sem->pHandle)
    {
        ReleaseSemaphore(sem->pHandle, count, NULL);
    }
}
...
bool initThread(TFThreadDesc* pDesc, ThreadHandle* pHandle)
{
    ASSERT(pHandle);
    *pHandle = NULL;

    // Copy the contents of TFThreadDesc because if the variable is in the stack we might access corrupted data.
    TFThreadDesc* pDescCopy = (TFThreadDesc*)tf_malloc(sizeof(TFThreadDesc));
    *pDescCopy = *pDesc;
...
TFPerformanceStats getPerformanceStats()
{
