...
bool gl_compileShader(TFRenderer* pRenderer, TFShaderStage stage, const char* fileName, uint32_t codeSize, const char* code,
                      TFBinaryShaderStageDesc* pOut, const char* pEntryPoint);
...
static bool load_shader_stage_byte_code(TFRenderer* pRenderer, const char* name, TFBinaryShaderStageDesc* pOut,
                                        ShaderByteCodeBuffer* pShaderByteCodeBuffer, FSLMetadata* pOutMetadata);

static void materialLoadBinaryShader(TFRenderer* pRenderer, const MaterialDesc* pMaterialDesc, const MaterialDesc::ShaderSet* pShaderSet,
                                     TFBinaryShaderDesc* pBinaryShaderDesc, ShaderByteCodeBuffer* pShaderBytecodeBuffer)
{
    ...
    const TFShaderStage pStages[MATERIAL_MAX_SHADER_STAGES] = { TF_SHADER_STAGE_VERT, TF_SHADER_STAGE_FRAG,
#ifndef METAL
                                                                TF_SHADER_STAGE_HULL, TF_SHADER_STAGE_DOMN, TF_SHADER_STAGE_GEOM,
#endif
                                                                TF_SHADER_STAGE_COMP };

    TFBinaryShaderStageDesc* pStageDescriptors[MATERIAL_MAX_SHADER_STAGES] = {
    ...
    for (uint32_t i = 0; i < MATERIAL_MAX_SHADER_STAGES; ++i)
    {
        if (shaderIds[i] < MATERIAL_MAX_SHADER_STAGES)
        {
            ...
            TFBinaryShaderStageDesc* pBinaryShaderStage = pStageDescriptors[i];
...
#endif
            load_shader_stage_byte_code(pRenderer, pMaterialDesc->pShaderNames[shaderIds[i]], pBinaryShaderStage, pShaderBytecodeBuffer,
                                        &metadata);

#if defined(QUEST_VR) || defined(HOLOLENS2)
            pBinaryShaderDesc->mIsMultiviewVR |= (bool)metadata.mUseMultiView;
#endif
        }
    }
}
...
static TFUMASupportFlags gUma = TF_UMA_SUPPORT_NONE;

TFUMASupportFlags getUmaFlags() { return gUma; }
...
TFResourceLoaderDesc          gDefaultResourceLoaderDesc = { 8ull * TF_MB, 2, false };
/************************************************************************/
// Surface Utils
/************************************************************************/
static inline TFResourceState ResourceStartState(bool uav)
{
    if (uav)
        return TF_RESOURCE_STATE_UNORDERED_ACCESS;
    else
        return TF_RESOURCE_STATE_SHADER_RESOURCE;
}

static inline TFResourceState ResourceStartState(const TFBufferDesc* pDesc)
{
    // Host visible (Upload Heap)
    if (pDesc->mMemoryUsage == TF_RESOURCE_MEMORY_USAGE_CPU_ONLY || pDesc->mMemoryUsage == TF_RESOURCE_MEMORY_USAGE_CPU_TO_GPU)
    {
        return TF_RESOURCE_STATE_GENERIC_READ;
    }
    // Device Local (Default Heap)
    else if (pDesc->mMemoryUsage == TF_RESOURCE_MEMORY_USAGE_GPU_ONLY)
    {
        TFDescriptorType usage = (TFDescriptorType)pDesc->mDescriptors;
        TFResourceState  ret = TF_RESOURCE_STATE_UNDEFINED;

        // Try to limit number of states used overall to avoid sync complexities
        if (usage & TF_DESCRIPTOR_TYPE_RW_BUFFER)
        {
            ret = TF_RESOURCE_STATE_UNORDERED_ACCESS;
        }
        else
        {
            if (usage & (TF_DESCRIPTOR_TYPE_VERTEX_BUFFER | TF_DESCRIPTOR_TYPE_UNIFORM_BUFFER))
            {
                ret |= TF_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER;
            }
            if (usage & TF_DESCRIPTOR_TYPE_INDEX_BUFFER)
            {
                ret |= TF_RESOURCE_STATE_INDEX_BUFFER;
            }
            if (usage & TF_DESCRIPTOR_TYPE_BUFFER)
            {
                ret |= TF_RESOURCE_STATE_SHADER_RESOURCE;
            }
        }
    ...
    {
        return TF_RESOURCE_STATE_COPY_DEST;
    }
}
...
// Internal Structures
/************************************************************************/
typedef void (*PreMipStepFn)(TFFileStream* pStream, uint32_t mip);
...
{
    TFFence*     pFence = NULL;
    TFSemaphore* pSemaphore = NULL;
    TFCmd*       pCmd = NULL;
    TFCmdPool*   pCmdPool = NULL;
    TFBuffer*    mBuffer = NULL;
    ...
    /// stb_ds array of TFBuffer*
    TFBuffer**  mTempBuffers = NULL;
    /// Textures replaced while handling invalid texture requests.
    /// Will be cleaned up after the fence for this set is complete.
    /// stb_ds array of Texture*
    TFTexture** mTempTextures = NULL;

#if defined(STRICT_QUEUE_TYPE_BARRIERS)
    TFCmd*     pPostCopyBarrierCmd = NULL;
    TFCmdPool* pPostCopyBarrierCmdPool = NULL;
    TFFence*   pPostCopyBarrierFence = NULL;
...
typedef struct CopyEngine
{
    ...
    uint32_t activeSet;
    /// Node index in linked GPU mode, TFRenderer index in unlinked mode
    uint32_t nodeIndex;
...
struct UpdateRequest
{
    ...
    UpdateRequest(const TFGeometryLoadDesc& geom): mType(UPDATE_REQUEST_LOAD_GEOMETRY), geomLoadDesc(geom) {}
    UpdateRequest(const TFTextureBarrier& barrier): mType(UPDATE_REQUEST_TEXTURE_BARRIER), textureBarrier(barrier) {}
    UpdateRequest(const TFTextureCopyDesc& texture): mType(UPDATE_REQUEST_COPY_TEXTURE), texCopyDesc(texture) {}
    ...
    union
    {
        ...
        TFTextureBarrier        textureBarrier;
        TFTextureCopyDesc       texCopyDesc;
    };
};

struct ResourceLoader
{
    TFRenderer* ppRenderers[TF_MAX_MULTIPLE_GPUS];
    uint32_t    mGpuCount;

    TFResourceLoaderDesc mDesc;

    volatile int mRun;
    ThreadHandle mThread;

    TFMutex             mQueueMutex;
    TFConditionVariable mQueueCond;
    TFMutex             mTokenMutex;
    TFConditionVariable mTokenCond;
    // array of stb_ds arrays
    UpdateRequest*      mRequestQueue[TF_MAX_MULTIPLE_GPUS];

    tfrg_atomic64_t mTokenCompleted;
    tfrg_atomic64_t mTokenSubmitted;
    tfrg_atomic64_t mTokenCounter;

    TFMutex mSemaphoreMutex;

    TFSyncToken mCurrentTokenState[MAX_FRAMES];
    TFSyncToken mMaxToken;

    CopyEngine pCopyEngines[TF_MAX_MULTIPLE_GPUS];
    CopyEngine pUploadEngines[TF_MAX_MULTIPLE_GPUS];
    TFMutex    mUploadEngineMutex;
};

static ResourceLoader* pResourceLoader = NULL;

static uint32_t util_get_texture_row_alignment(TFRenderer* pRenderer) { return max(1u, pRenderer->pGpu->mUploadBufferTextureRowAlignment); }

static uint32_t util_get_texture_subresource_alignment(TFRenderer* pRenderer, TinyImageFormat fmt = TinyImageFormat_UNDEFINED)
{
...
static void freeShaderByteCode(ShaderByteCodeBuffer* pShaderByteCodeBuffer, TFBinaryShaderDesc* pBinaryShaderDesc)
{
...
#else
static void freeShaderByteCode(ShaderByteCodeBuffer*, TFBinaryShaderDesc*) {}
#endif
...
static TFMappedMemoryRange allocateUploadMemory(TFRenderer* pRenderer, uint64_t memoryRequirement, uint32_t alignment)
{
    TFBuffer* buffer;
    ...
    bufferDesc.mMemoryUsage = TF_RESOURCE_MEMORY_USAGE_CPU_ONLY;
    bufferDesc.mFlags = TF_BUFFER_CREATION_FLAG_PERSISTENT_MAP_BIT;
...
static void setupCopyEngine(TFRenderer* pRenderer, CopyEngineDesc* pDesc, CopyEngine* pCopyEngine)
{
    TFQueueDesc desc = { pDesc->mQueueType, TF_QUEUE_FLAG_NONE, TF_QUEUE_PRIORITY_NORMAL, pDesc->mNodeIndex, pDesc->pQueueName };
    ...
    for (uint32_t i = 0; i < pDesc->mBufferCount; ++i)
    {
        ...
        static char buffer[TF_MAX_DEBUG_NAME_LENGTH];
...
static void cleanupCopyEngine(TFRenderer* pRenderer, CopyEngine* pCopyEngine)
{
    for (uint32_t i = 0; i < pCopyEngine->bufferCount; ++i)
    {
        ...
        for (ptrdiff_t j = 0; j < arrlen(resourceSet.mTempTextures); ++j)
        {
            removeTexture(pRenderer, resourceSet.mTempTextures[j]);
        }
        arrfree(resourceSet.mTempTextures);
...
static void waitCopyEngineSet(TFRenderer* pRenderer, CopyEngine* pCopyEngine)
{
    ...
    if (TF_FENCE_STATUS_INCOMPLETE == status)
    {
...
static void resetCopyEngineSet(TFRenderer* pRenderer, CopyEngine* pCopyEngine)
{
    ASSERT(!pCopyEngine->isRecording);
    pCopyEngine->resourceSets[pCopyEngine->activeSet].mAllocatedSpace = 0;
    pCopyEngine->isRecording = false;

    TFBuffer** tempBuffers = pCopyEngine->resourceSets[pCopyEngine->activeSet].mTempBuffers;
    ...
    TFTexture** tempTextures = pCopyEngine->resourceSets[pCopyEngine->activeSet].mTempTextures;
    ptrdiff_t   tempTextureCount = arrlen(tempTextures);
    for (ptrdiff_t i = 0; i < tempTextureCount; ++i)
    {
        removeTexture(pRenderer, tempTextures[i]);
    }
    arrsetlen(tempTextures, 0);
}

static TFCmd* acquireCmd(CopyEngine* pCopyEngine)
{
    CopyResourceSet& resourceSet = pCopyEngine->resourceSets[pCopyEngine->activeSet];
    if (!pCopyEngine->isRecording)
    {
        if (!pCopyEngine->isSetReady)
        {
        ...
        cmdBeginDebugMarker(resourceSet.pCmd, 1.0f, 0.5f, 0.1f,
                            TF_QUEUE_TYPE_TRANSFER == pCopyEngine->pQueue->mType ? "Copy Cmd" : "Upload Cmd");
#endif
...
static TFCmd* acquirePostCopyBarrierCmd(CopyEngine* pCopyEngine)
{
    if (!StrictQueueTypeBarriers() || pCopyEngine->pQueue->mType != TF_QUEUE_TYPE_TRANSFER)
    {
...
static TFMappedMemoryRange allocateStagingMemory(CopyEngine* pCopyEngine, uint64_t memoryRequirement, uint32_t alignment,
                                                 uint32_t nodeIndex)
{
    ...
    TFRenderer*      pRenderer = pResourceLoader->ppRenderers[nodeIndex];
    ...
    {
        TFMappedMemoryRange range = allocateUploadMemory(pRenderer, memoryRequirement, alignment);
        LOADER_LOGF(
            LogLevel::eINFO,
    ...
    {
        TFBuffer* buffer = pResourceSet->mBuffer;
...
static UploadFunctionResult updateBuffer(TFRenderer* pRenderer, CopyEngine* pCopyEngine, const TFBufferUpdateDesc& bufUpdateDesc)
{
    TFBuffer* pBuffer = bufUpdateDesc.pBuffer;
    ASSERT(pCopyEngine->pQueue->mNodeIndex == pBuffer->mNodeIndex);
    ASSERT(TF_RESOURCE_MEMORY_USAGE_GPU_ONLY == pBuffer->mMemoryUsage);

    TFCmd* pCmd = acquireCmd(pCopyEngine);

    if (IssueBufferCopyBarriers() && bufUpdateDesc.mCurrentState != TF_RESOURCE_STATE_COPY_DEST)
    {
        TFBufferBarrier barrier = { bufUpdateDesc.pBuffer, bufUpdateDesc.mCurrentState, TF_RESOURCE_STATE_COPY_DEST };
        cmdResourceBarrier(pCmd, 1, &barrier, 0, NULL, 0, NULL);
    }

    TFMappedMemoryRange range = bufUpdateDesc.mInternal.mMappedRange;
    ...
    if (IssueBufferCopyBarriers() && bufUpdateDesc.mCurrentState != TF_RESOURCE_STATE_COPY_DEST)
    {
        TFBufferBarrier barrier = { bufUpdateDesc.pBuffer, TF_RESOURCE_STATE_COPY_DEST, bufUpdateDesc.mCurrentState };
        cmdResourceBarrier(pCmd, 1, &barrier, 0, NULL, 0, NULL);
    }
...
static UploadFunctionResult loadBuffer(TFRenderer* pRenderer, CopyEngine* pCopyEngine, const UpdateRequest& updateRequest)
{
    const BufferLoadDescInternal& loadDesc = updateRequest.bufLoadDesc;
    TFBufferUpdateDesc            updateDesc = { loadDesc.pBuffer };
    updateDesc.mCurrentState = TF_RESOURCE_STATE_COPY_DEST;
    TFMappedMemoryRange range = {};
    bool                mapped = false;
    ...
    {
        if (IssueBufferCopyBarriers() && loadDesc.mStartState != TF_RESOURCE_STATE_COPY_DEST)
        {
            TFBufferBarrier barrier = { loadDesc.pBuffer, TF_RESOURCE_STATE_COPY_DEST, loadDesc.mStartState };
            TFCmd*          cmd = acquirePostCopyBarrierCmd(pCopyEngine);
...
static UploadFunctionResult updateTexture(TFRenderer* pRenderer, CopyEngine* pCopyEngine, const TextureUpdateDescInternal& texUpdateDesc)
{
    TFResourceState copyDestState =
        (pCopyEngine->pQueue->mType == TF_QUEUE_TYPE_TRANSFER) ? TF_RESOURCE_STATE_COMMON : TF_RESOURCE_STATE_COPY_DEST;
#ifdef VULKAN
    // Vulkan requires copy dest
    copyDestState = TF_RESOURCE_STATE_COPY_DEST;
#endif
    ...
    TFTexture*            texture = texUpdateDesc.pTexture;
    const TinyImageFormat fmt = (TinyImageFormat)texture->mFormat;
    TFFileStream          stream = texUpdateDesc.mStream;
    ...
    TFMappedMemoryRange upload =
        dataAlreadyFilled ? texUpdateDesc.mRange : allocateStagingMemory(pCopyEngine, requiredSize, sliceAlignment, texture->mNodeIndex);
    uint64_t offset = 0;

    TFCmd* cmd = texUpdateDesc.pCmd ? texUpdateDesc.pCmd : acquireCmd(pCopyEngine);
    if (IssueTextureCopyBarriers() && texUpdateDesc.mCurrentState != copyDestState)
    {
        TFTextureBarrier barrier = { texture, texUpdateDesc.mCurrentState, copyDestState };
    ...
    if (!dataAlreadyFilled)
    {
        ...
        fsCloseStream(&stream);
        fsOpenStreamFromMemory(nxTempBuffer, remainingBytes, TF_FM_READ, true, &stream);
    }
#endif
    ...
    TFAtlasPlacement* pAtlasPlacement = texUpdateDesc.pAtlasPlacement;

    uint32_t pixelsPerBlock = TinyImageFormat_WidthOfBlock(fmt);
    for (uint32_t j = firstStart; j < firstEnd; ++j)
    {
        ...
        for (uint32_t i = secondStart; i < secondEnd; ++i)
        {
            ...
            // Atlas placement is slice-based (no sub-rect packing), so source and destination mip extents are identical.
            uint32_t srcRowBytes = 0;
            uint32_t srcNumRows = 0;
            bool     ret = util_get_surface_info(w, h, fmt, NULL, &srcRowBytes, &srcNumRows);
            if (!ret)
            {
                if (stream.pIO)
                {
            ...
            uint32_t dstSubRowPitch = round_up(srcRowBytes, rowAlignment);
            uint32_t dstSubSlicePitch = round_up(dstSubRowPitch * srcNumRows, sliceAlignment);
            ...
            if (!dataAlreadyFilled)
            {
                ...
                {
                    uint8_t* dstData = data + dstSubSlicePitch * z + startOffset;
                    for (uint32_t r = 0; r < srcNumRows; ++r)
                    {
                        ssize_t bytesRead = fsReadFromStream(&stream, dstData + r * dstSubRowPitch, srcRowBytes);
                        if (bytesRead != srcRowBytes)
                        {
                            if (stream.pIO)
                            {
                                fsCloseStream(&stream);
            ...
            subresourceDesc.mX = 0;
            subresourceDesc.mY = 0;
            subresourceDesc.mWidth = w;
            subresourceDesc.mHeight = h;

            if (pAtlasPlacement)
            {
                if (subresourceDesc.mWidth < pixelsPerBlock || subresourceDesc.mHeight < pixelsPerBlock)
                {
                    continue;
                }
            }
...
#if defined(METAL) || defined(VULKAN)
            subresourceDesc.mSrcOffset += startOffset;
            subresourceDesc.mRowPitch = dstSubRowPitch;
            subresourceDesc.mSlicePitch = dstSubSlicePitch;
#endif
            cmdUpdateSubresource(cmd, texture, upload.pBuffer, &subresourceDesc);
            offset += subDepth * dstSubSlicePitch;
        }
    }

    if (IssueTextureCopyBarriers() && texUpdateDesc.mCurrentState != copyDestState)
    {
        TFTextureBarrier barrier = { texture, copyDestState, texUpdateDesc.mCurrentState };
        cmdResourceBarrier(cmd, 0, NULL, 1, &barrier, 0, NULL);
    }
...
static TFResourceState getTextureCopyDestState(const CopyEngine* pCopyEngine)
{
    TFResourceState copyDestState =
        (pCopyEngine->pQueue->mType == TF_QUEUE_TYPE_TRANSFER) ? TF_RESOURCE_STATE_COMMON : TF_RESOURCE_STATE_COPY_DEST;
#ifdef VULKAN
    ...
    copyDestState = TF_RESOURCE_STATE_COPY_DEST;
#endif
    return copyDestState;
}

static bool uploadErrorTexturePattern(TFRenderer* pRenderer, CopyEngine* pCopyEngine, TFTexture* pTexture)
{
    if (!pTexture)
    {
        return false;
    ...
    ASSERT(fmt == TinyImageFormat_R8G8B8A8_SRGB || fmt == TinyImageFormat_R8G8B8A8_UNORM);
    const uint32_t sliceAlignment = util_get_texture_subresource_alignment(pRenderer, fmt);
    const uint32_t rowAlignment = util_get_texture_row_alignment(pRenderer);
    const uint32_t mipLevels = pTexture->mMipLevels;
    const uint32_t layerCount = pTexture->mArraySizeMinusOne + 1u;
    const uint64_t requiredSize = util_get_surface_size(fmt, pTexture->mWidth, pTexture->mHeight, pTexture->mDepth, rowAlignment,
                                                        sliceAlignment, 0, mipLevels, 0, layerCount);

    TFMappedMemoryRange range = allocateStagingMemory(pCopyEngine, requiredSize, sliceAlignment, pTexture->mNodeIndex);
    if (!range.pData)
    {
        return false;
    }

    memset(range.pData, 0, (size_t)range.mSize);
    ...
    for (uint32_t layer = 0; layer < layerCount; ++layer)
    {
        ...
        {
            const uint32_t w = MIP_REDUCE(pTexture->mWidth, mip);
            const uint32_t h = MIP_REDUCE(pTexture->mHeight, mip);
            const uint32_t d = MIP_REDUCE(pTexture->mDepth, mip);

            uint32_t srcRowBytes = 0;
            uint32_t srcNumRows = 0;
            if (!util_get_surface_info(w, h, fmt, NULL, &srcRowBytes, &srcNumRows))
            {
                return false;
            }

            const uint32_t dstSubRowPitch = round_up(srcRowBytes, rowAlignment);
            const uint32_t dstSubSlicePitch = round_up(dstSubRowPitch * srcNumRows, sliceAlignment);

            for (uint32_t z = 0; z < d; ++z)
            {
                uint8_t* sliceData = range.pData + offset + z * dstSubSlicePitch;
                for (uint32_t row = 0; row < srcNumRows; ++row)
                {
                    uint8_t* rowData = sliceData + row * dstSubRowPitch;
                    for (uint32_t x = 0; x < w; ++x)
                    {
                        uint8_t* pixel = rowData + x * 4;
                        pixel[0] = 255;
                        pixel[1] = 0;
                        pixel[2] = 255;
                        pixel[3] = 255;
            ...
            offset += (uint64_t)d * (uint64_t)dstSubSlicePitch;
        }
    }
    ...
    updateDesc.mMipLevels = mipLevels;
    updateDesc.mBaseArrayLayer = 0;
    updateDesc.mLayerCount = layerCount;
    updateDesc.mCurrentState = getTextureCopyDestState(pCopyEngine);
    return updateTexture(pRenderer, pCopyEngine, updateDesc) == UPLOAD_FUNCTION_RESULT_COMPLETED;
}
static void handleInvalidTextureRequest(TFRenderer* pRenderer, CopyEngine* pCopyEngine, const TextureLoadDescInternal& textureLoadDesc)
{
    const char* textureName;
    if (textureLoadDesc.mForceReset)
    {
        textureName = "<forced texture reset>";
    }
    else
    {
        textureName = textureLoadDesc.mLoadFromPackage
                          ? "<texture package entry>"
                          : (textureLoadDesc.pFileName ? textureLoadDesc.pFileName : "<unnamed texture request>");
    }
    LOGF(eWARNING, "Invalid texture request while loading '%s'. Falling back to an error texture pattern.", textureName);

    if (!textureLoadDesc.mForceReset && textureLoadDesc.pAtlasTexture)
    {
        LOGF(eINFO, "Could not apply error texture pattern for '%s' is atlas texture and force reset is not set", textureName);
        return;
    }
    if (!textureLoadDesc.ppTexture || !*textureLoadDesc.ppTexture)
    {
        LOGF(eINFO, "Could not apply error texture pattern for '%s' because no texture object was created.", textureName);
        return;
    }

    TFTexture*            pTexture = *textureLoadDesc.ppTexture;
    const TinyImageFormat format = (TinyImageFormat)pTexture->mFormat;
    TinyImageFormat       fallbackFormat = TinyImageFormat_IsSRGB(format) ? TinyImageFormat_R8G8B8A8_SRGB : TinyImageFormat_R8G8B8A8_UNORM;

    TFTextureDesc fallbackDesc = {};
    fallbackDesc.pName = "Invalid Texture Fallback";
    fallbackDesc.mFlags = textureLoadDesc.mForceReset ? (TFTextureCreationFlags)0 : textureLoadDesc.mFlags;
    fallbackDesc.mWidth = 4;
    fallbackDesc.mHeight = 4;
    fallbackDesc.mDepth = max(1u, pTexture->mDepth);
    fallbackDesc.mArraySize = pTexture->mArraySizeMinusOne + 1u;
    fallbackDesc.mMipLevels = 1;
    fallbackDesc.mFormat = fallbackFormat;
    fallbackDesc.mDescriptors = TF_DESCRIPTOR_TYPE_TEXTURE;
    if (fallbackDesc.mDepth == 1 && fallbackDesc.mArraySize >= 6 && fallbackDesc.mArraySize % 6 == 0)
    {
        fallbackDesc.mDescriptors |= TF_DESCRIPTOR_TYPE_TEXTURE_CUBE;
    }
    fallbackDesc.mSampleCount = (TFSampleCount)pTexture->mSampleCount;
    fallbackDesc.mStartState = getTextureCopyDestState(pCopyEngine);
    fallbackDesc.mNodeIndex = pTexture->mNodeIndex;
    fallbackDesc.pPlacement = textureLoadDesc.pPlacement;
#if defined(VULKAN)
    if (textureLoadDesc.pYcbcrSampler && fallbackFormat == format)
    {
        fallbackDesc.pSamplerYcbcrConversionInfo = &textureLoadDesc.pYcbcrSampler->mVk.mSamplerYcbcrConversionInfo;
    }
#endif

    TFTexture* pFallbackTexture = NULL;
    addTexture(pRenderer, &fallbackDesc, &pFallbackTexture);
    if (pFallbackTexture)
    {
        if (IssueExplicitInitialStateBarrier())
        {
            TFTextureBarrier barrier = { pFallbackTexture, TF_RESOURCE_STATE_UNDEFINED, getTextureCopyDestState(pCopyEngine) };
            TFCmd*           cmd = acquireCmd(pCopyEngine);
            cmdResourceBarrier(cmd, 0, NULL, 1, &barrier, 0, NULL);
        }

        if (uploadErrorTexturePattern(pRenderer, pCopyEngine, pFallbackTexture))
        {
            if (IssueTextureCopyBarriers())
            {
                TFTextureBarrier barrier = { pFallbackTexture, getTextureCopyDestState(pCopyEngine), TF_RESOURCE_STATE_SHADER_RESOURCE };
                TFCmd*           cmd = acquirePostCopyBarrierCmd(pCopyEngine);
                cmdResourceBarrier(cmd, 0, NULL, 1, &barrier, 0, NULL);
            }

            arrpush(pCopyEngine->resourceSets[pCopyEngine->activeSet].mTempTextures, pTexture);
            *textureLoadDesc.ppTexture = pFallbackTexture;

            LOGF(eINFO, "Replaced invalid texture '%s' with %ux%u fallback texture (format: %s).", textureName, fallbackDesc.mWidth,
                 fallbackDesc.mHeight, TinyImageFormat_Name(fallbackFormat));
        }
        else
        {
            LOGF(eINFO, "Failed to upload error texture pattern for '%s'.", textureName);
        }
    }
    else
    {
        LOGF(eINFO, "Could not replace invalid texture '%s' with fallback texture.", textureName);
    }
}

static UploadFunctionResult loadTexture(TFRenderer* pRenderer, CopyEngine* pCopyEngine, const UpdateRequest& pTextureUpdate)
{
    const TextureLoadDescInternal* pTextureDesc = &pTextureUpdate.texLoadDesc;
    TFResourceState                copyDestState = getTextureCopyDestState(pCopyEngine);
    if (pTextureDesc->mForceReset)
    {
        TFTexture* texture = *pTextureDesc->ppTexture;

        if (IssueExplicitInitialStateBarrier())
        {
            TFCmd*           cmd = acquireCmd(pCopyEngine);
            TFTextureBarrier barrier = { texture, TF_RESOURCE_STATE_UNDEFINED, copyDestState };
            cmdResourceBarrier(cmd, 0, NULL, 1, &barrier, 0, NULL);
        }
        ...
        TFMappedMemoryRange range = allocateStagingMemory(pCopyEngine, requiredSize, sliceAlignment, texture->mNodeIndex);
        if (!range.pData)
        {
            return UPLOAD_FUNCTION_RESULT_STAGING_BUFFER_FULL;
        }
        ...
        updateDesc.mRange = range;
        updateDesc.mCurrentState = copyDestState;
        updateTexture(pRenderer, pCopyEngine, updateDesc);

        if (IssueTextureCopyBarriers() && pTextureDesc->mStartState != copyDestState)
        {
            TFTextureBarrier barrier = { texture, copyDestState, pTextureDesc->mStartState };
            TFCmd*           cmd = acquirePostCopyBarrierCmd(pCopyEngine);
            cmdResourceBarrier(cmd, 0, NULL, 1, &barrier, 0, NULL);
        }
    ...
    ASSERT((((pTextureDesc->mFlags & TF_TEXTURE_CREATION_FLAG_SRGB) == 0) || (pTextureDesc->pFileName != NULL)) &&
           "Only textures loaded from file can have TF_TEXTURE_CREATION_FLAG_SRGB. "
           "Please change format of the provided texture if you need srgb format.");

    if (pTextureDesc->pFileName || pTextureDesc->mLoadFromPackage)
    {
        TFFileStream stream = {};
        bool         success = false;

        TextureUpdateDescInternal updateDesc = {};
        TFTextureContainerType    container = pTextureDesc->mContainer;

        if (TF_TEXTURE_CONTAINER_DEFAULT == container)
        {
#if defined(TARGET_IOS) || defined(__ANDROID__) || defined(NX64)
            container = TF_TEXTURE_CONTAINER_KTX;
#elif defined(_WINDOWS) || defined(XBOX) || defined(__APPLE__) || defined(__linux__)
            container = TF_TEXTURE_CONTAINER_DDS;
#elif defined(ORBIS) || defined(PROSPERO)
            container = TF_TEXTURE_CONTAINER_GNF;
#endif
        }

        TFTextureDesc textureDesc = {};
        textureDesc.pName = pTextureDesc->pFileName;
        if (pTextureDesc->mLoadFromPackage)
        {
            static char textureName[TF_MAX_DEBUG_NAME_LENGTH];
            snprintf(textureName, sizeof(textureName), "Package Texture %u", pTextureDesc->mPackagePtr.mDataID);
            textureDesc.pName = textureName;
        }
        textureDesc.mFlags |= pTextureDesc->mFlags;

        // Validate that we have found the file format now
        ASSERT(container != TF_TEXTURE_CONTAINER_DEFAULT); //-V547
        if (TF_TEXTURE_CONTAINER_DEFAULT == container)     //-V547
        {
            return UPLOAD_FUNCTION_RESULT_INVALID_REQUEST;
        }
        ...
        if (pTextureDesc->mLoadFromPackage)
        {
            // Open the data with a package and ID instead of filename
            success = fsIoOpenByUid(pTextureDesc->mPackagePtr.pPackage, pTextureDesc->mPackagePtr.mDataID, TF_FM_READ, &stream);
        }
        else
        {
            success = fsOpenStreamFromPath(TF_RD_TEXTURES, pTextureDesc->pFileName, TF_FM_READ, &stream);
        }

        switch (container)
        {
        case TF_TEXTURE_CONTAINER_DDS:
        {
#if defined(XBOX)
            ...
            if (success)
            {
                extern uint32_t loadXDDSTexture(TFRenderer * pRenderer, TFFileStream * stream, const char* name,
                                                TFTextureCreationFlags flags, TFTexture** ppTexture);
            ...
            {
                LOGF(eERROR, "Failed to open texture file %s.Function %s failed with error: %s", pTextureDesc->pFileName, //-V576
                     FS_ERR_CTX.func,                                                                                     //-V576
                     getFSErrCodeString(FS_ERR_CTX.code));                                                                //-V576
            }
#endif
            break;
        }
        case TF_TEXTURE_CONTAINER_KTX:
        {
            if (success)
            {
                ...
                updateDesc.pPreMipFunc = [](TFFileStream* pStream, uint32_t)
                {
                    uint32_t mipSize = 0;
        ...
        case TF_TEXTURE_CONTAINER_GNF:
        {
#if defined(ORBIS) || defined(PROSPERO)
            uint32_t res = 1;
            if (success)
            {
                extern uint32_t loadGnfTexture(TFRenderer * pRenderer, TFFileStream * stream, const char* name,
                                               TFTextureCreationFlags flags, TFTexture** ppTexture);
        ...
        TFResourcePlacement placement = {};
        if (success)
        {
            textureDesc.mStartState = copyDestState;
            textureDesc.mNodeIndex = pTextureDesc->mNodeIndex;
            textureDesc.pPlacement = pTextureDesc->pPlacement;

            if (pTextureDesc->mOnTextureLoaded)
            {
                TFOnTextureLoadedData textureLoadedData = {};
                textureLoadedData.pTextureDesc = &textureDesc;
                textureLoadedData.pUserData = pTextureDesc->pUserData;

                TFResourceSizeAlign sizeAlign;
                getTextureSizeAlign(pRenderer, &textureDesc, &sizeAlign);
                textureLoadedData.mSize = sizeAlign.mSize;

                pTextureDesc->mOnTextureLoaded(&textureLoadedData, &placement);

                if (placement.pHeap)
                {
                    textureDesc.pPlacement = &placement;
                }
            }
...
#endif
            bool postBarrierToShaderResource = true;

            if (pTextureDesc->pAtlasTexture)
            {
                updateDesc.pTexture = pTextureDesc->pAtlasTexture;
                updateDesc.mMipLevels = pTextureDesc->pAtlasTexture->mMipLevels;
                updateDesc.pAtlasPlacement = pTextureDesc->pAtlasPlacement;
                updateDesc.mBaseArrayLayer = pTextureDesc->pAtlasPlacement->mAtlasSlice;
                updateDesc.mLayerCount = 1;

                postBarrierToShaderResource = pTextureDesc->pAtlasPlacement->mFlags == (uint32_t)TF_ATLAS_PLACEMENT_LAST_PLACEMENT;
            }
            else
            {
            ...
            if (IssueExplicitInitialStateBarrier())
            {
                TFTextureBarrier barrier = { updateDesc.pTexture, TF_RESOURCE_STATE_UNDEFINED, copyDestState };
                TFCmd*           cmd = acquireCmd(pCopyEngine);
                cmdResourceBarrier(cmd, 0, NULL, 1, &barrier, 0, NULL);
            }
            ...
            if (IssueTextureCopyBarriers() && UPLOAD_FUNCTION_RESULT_COMPLETED == res && postBarrierToShaderResource)
            {
                TFTextureBarrier barrier = { updateDesc.pTexture, copyDestState, TF_RESOURCE_STATE_SHADER_RESOURCE };
                TFCmd*           cmd = acquirePostCopyBarrierCmd(pCopyEngine);
                cmdResourceBarrier(cmd, 0, NULL, 1, &barrier, 0, NULL);
            }
...
static void fillGeometryUpdateDesc(TFRenderer* pRenderer, TFGeometryLoadDesc* pDesc, TFGeometry* geom, uint32_t* indexStride,
                                   TFBufferUpdateDesc vertexUpdateDesc[TF_MAX_VERTEX_BINDINGS], TFBufferUpdateDesc indexUpdateDesc[1])
{
    bool     structuredBuffers = (pDesc->mFlags & TF_GEOMETRY_LOAD_FLAG_STRUCTURED_BUFFERS) > 0;
    ...
    if (pDesc->pGeometryBuffer)
    {
        ...
        {
            *indexStride = pDesc->pGeometryBufferLayoutDesc->mIndexType == TF_INDEX_TYPE_UINT16 ? sizeof(uint16_t) : sizeof(uint32_t);
    ...
    {
        TFBufferDesc loadDesc = {};
        loadDesc.mDescriptors =
            TF_DESCRIPTOR_TYPE_INDEX_BUFFER | (structuredBuffers ? (TF_DESCRIPTOR_TYPE_BUFFER | TF_DESCRIPTOR_TYPE_RW_BUFFER)
                                                                 : (TF_DESCRIPTOR_TYPE_BUFFER_RAW | TF_DESCRIPTOR_TYPE_RW_BUFFER_RAW));
        loadDesc.mFlags |=
            (pDesc->mFlags & TF_GEOMETRY_LOAD_FLAG_RAYTRACING_INPUT)
                ? (TF_BUFFER_CREATION_FLAG_SHADER_DEVICE_ADDRESS | TF_BUFFER_CREATION_FLAG_ACCELERATION_STRUCTURE_BUILD_INPUT)
                : TF_BUFFER_CREATION_FLAG_NONE;
        ...
        loadDesc.mMemoryUsage = TF_RESOURCE_MEMORY_USAGE_GPU_ONLY;
        // Note: buffers are rw, so check only for rw support here
        loadDesc.mStartState = gUma == TF_UMA_SUPPORT_READ_WRITE ? gIndexBufferState : TF_RESOURCE_STATE_COPY_DEST;
    ...
    if (gUma == TF_UMA_SUPPORT_READ_WRITE && indexUpdateDesc->pBuffer->pCpuMappedAddress)
    {
    ...
    for (uint32_t i = 0; i < TF_MAX_VERTEX_BINDINGS; ++i)
    {
        ...
        {
            TFBufferDesc vertexBufferDesc = {};
            vertexBufferDesc.mDescriptors =
                TF_DESCRIPTOR_TYPE_VERTEX_BUFFER | (structuredBuffers ? (TF_DESCRIPTOR_TYPE_BUFFER | TF_DESCRIPTOR_TYPE_RW_BUFFER)
                                                                      : (TF_DESCRIPTOR_TYPE_BUFFER_RAW | TF_DESCRIPTOR_TYPE_RW_BUFFER_RAW));
            ...
            vertexBufferDesc.mMemoryUsage = TF_RESOURCE_MEMORY_USAGE_GPU_ONLY;
            // Note: buffers are rw, so check only for rw support here
            vertexBufferDesc.mStartState = gUma == TF_UMA_SUPPORT_READ_WRITE ? gVertexBufferState : TF_RESOURCE_STATE_COPY_DEST;
        ...
        // We need to check for pCpuMappedAddress because when we allocate a custom ResourceHeap with GPU_ONLY memory we don't get any CPU
        // mapped address and we need staging memory
        if (gUma == TF_UMA_SUPPORT_READ_WRITE && vertexUpdateDesc[i].pBuffer->pCpuMappedAddress)
        {
...
static UploadFunctionResult loadGeometryCustomMeshFormat(TFRenderer* pRenderer, TFGeometryLoadDesc* pDesc,
                                                         TFBufferUpdateDesc vertexUpdateDesc[TF_MAX_VERTEX_BINDINGS],
                                                         TFBufferUpdateDesc indexUpdateDesc[1])
{
    TFFileStream file = {};
    if (!pDesc->mLoadFromPackage)
    {
        if (!fsOpenStreamFromPath(TF_RD_MESHES, pDesc->pFileName, TF_FM_READ, &file))
        {
    ...
    {
        // Open the data with a package and ID instead of filename
        if (!fsIoOpenByUid(pDesc->mPackagePtr.pPackage, pDesc->mPackagePtr.mDataID, TF_FM_READ, &file))
        {
            LOGF(eERROR, "Failed to load data from Package.");
            ASSERT(false);
            return UPLOAD_FUNCTION_RESULT_INVALID_REQUEST;
        }
    }
    ...
    TFGeometry* geom = (TFGeometry*)tf_calloc(1, geomSize);
    ...
    TFGeometryData* geomData = (TFGeometryData*)tf_calloc(1, geomDataSize);
    ...
    uint32_t cpuDataSize = 0;
    fsReadFromStream(&file, &cpuDataSize, sizeof(uint32_t));
    ASSERT(cpuDataSize > 0);
    if (cpuDataSize < sizeof(*geomData->pCpuData))
    {
        LOGF(eERROR, "File '%s': Geometry object has cpu side copy with size less than %x, got %x", pDesc->pFileName,
             (int)sizeof(*geomData->pCpuData), (int)cpuDataSize);
        return UPLOAD_FUNCTION_RESULT_INVALID_REQUEST;
    }

    geomData->pCpuData = (TFGeometryData::CpuData*)tf_malloc(cpuDataSize);
    if (!geomData->pCpuData)
    {
        return UPLOAD_FUNCTION_RESULT_INVALID_REQUEST;
    }

    if (!VERIFYMSG(fsReadFromStream(&file, geomData->pCpuData, cpuDataSize) == cpuDataSize,
                   "File '%s': Failed to read Geometry object's cpu side copy data.", pDesc->pFileName))
    {
        return UPLOAD_FUNCTION_RESULT_INVALID_REQUEST;
    }
    ...
    if (geom->meshlets.mMeshletCount)
    {
        ...
        geom->meshlets.mMeshlets = (TFMeshlet*)mem;
        geom->meshlets.mMeshletsData = (TFMeshletData*)(geom->meshlets.mMeshlets + geom->meshlets.mMeshletCount);
    ...
    geom->pDrawArgs = (TFIndirectDrawIndexArguments*)(geom + 1); //-V1027
    ...
    geomData->pCpuData->pIndices = geomData->pCpuData + 1;

    geomData->pCpuData->pAttributes[TF_SEMANTIC_POSITION] = (uint8_t*)geomData->pCpuData->pIndices + (geom->mIndexCount * indexStride);

    for (uint32_t s = TF_SEMANTIC_POSITION + 1; s < TF_MAX_SEMANTICS; ++s)
        geomData->pCpuData->pAttributes[s] = (uint8_t*)geomData->pCpuData->pAttributes[s - 1] +
                                             geomData->pCpuData->mVertexStrides[s - 1] * geomData->pCpuData->mAttributeCount[s - 1];

    for (uint32_t i = 0; i < TF_ARRAY_COUNT(geomData->pCpuData->mVertexStrides); ++i)
    {
        if (geomData->pCpuData->mVertexStrides[i] == 0)
            geomData->pCpuData->pAttributes[i] = nullptr;
    }

    uint32_t vertexAttribCount[TF_MAX_VERTEX_BINDINGS] = {};
    uint32_t vertexOffsets[TF_MAX_SEMANTICS] = {}; // offset in the GPU layout
    uint32_t vertexBindings[TF_MAX_SEMANTICS] = {};
    ...
    {
        const TFVertexAttrib* attr = &pDesc->pVertexLayout->mAttribs[i];

        const uint32_t srcFormatSize = (uint32_t)geomData->pCpuData->mVertexStrides[attr->mSemantic]; //-V522

        uint32_t binding =
            pDesc->pGeometryBufferLayoutDesc ? pDesc->pGeometryBufferLayoutDesc->mSemanticBindings[attr->mSemantic] : attr->mBinding;
        ASSERT(binding < TF_MAX_VERTEX_BINDINGS);
    ...
    if (indexStride == dstIndexStride)
        memcpy(indexUpdateDesc->pMappedData, geomData->pCpuData->pIndices, indexStride * geom->mIndexCount);
    else
    {
        if (sizeof(uint16_t) == indexStride)
        {
            uint32_t*       dst = (uint32_t*)indexUpdateDesc->pMappedData;
            const uint16_t* src = (uint16_t*)geomData->pCpuData->pIndices;
    ...
    void* pVertexPositions = NULL;

    for (uint32_t i = 0; i < TF_MAX_SEMANTICS; ++i)
    {
        if (!geomData->pCpuData->pAttributes[i])
            continue;
        ...
        const uint8_t* src = (uint8_t*)geomData->pCpuData->pAttributes[i];
        uint8_t*       dst = (uint8_t*)vertexUpdateDesc[binding].pMappedData;
        ASSERT(src && dst);

        // we need the vertex position to prepare the bounding boxes
        if (TF_SEMANTIC_POSITION == i)
        {
            pVertexPositions = (void*)src;

            // prepare the bounding boxes using vertex positions + index buffer + draw args
            if (pDesc->pOnGeometryLoaded)
            {
                TFOnGeometryDataLoadedData streamingDesc = {};
                streamingDesc.geom = geom;
                streamingDesc.pIndexData = indexUpdateDesc->pMappedData;
                streamingDesc.pVertexPositions = pVertexPositions;
                streamingDesc.mIndexStride = indexStride;
                streamingDesc.pUserData = pDesc->pOnGeometryLoadedUserData;
                pDesc->pOnGeometryLoaded(&streamingDesc);
            }
        }
        ...
        {
            memcpy(dst, src, geomData->pCpuData->mVertexStrides[i] * geomData->pCpuData->mAttributeCount[i]);
        }
        else
        {
            ...
            for (uint32_t e = 0; e < geomData->pCpuData->mAttributeCount[i]; ++e)
                memcpy(dst + e * stride + offset, src + e * geomData->pCpuData->mVertexStrides[i], geomData->pCpuData->mVertexStrides[i]);
        }
    }

    // If the user doesn't want the CPU copy of the data we don't need it any more
    if ((pDesc->mFlags & TF_GEOMETRY_LOAD_FLAG_KEEP_CPU_COPY) != TF_GEOMETRY_LOAD_FLAG_KEEP_CPU_COPY)
    {
        tf_free(geomData->pCpuData);
        geomData->pCpuData = nullptr;
    }
...
static UploadFunctionResult loadGeometry(TFRenderer* pRenderer, CopyEngine* pCopyEngine, UpdateRequest& pGeometryLoad)
{
    TFGeometryLoadDesc* pDesc = &pGeometryLoad.geomLoadDesc;

    TFBufferUpdateDesc indexUpdateDesc = {};
    TFBufferUpdateDesc vertexUpdateDesc[TF_MAX_VERTEX_BINDINGS] = {};

    UploadFunctionResult res = loadGeometryCustomMeshFormat(pRenderer, pDesc, vertexUpdateDesc, &indexUpdateDesc);
    if (res != UPLOAD_FUNCTION_RESULT_COMPLETED)
        return res;

    // Upload mesh
    UploadFunctionResult uploadResult = UPLOAD_FUNCTION_RESULT_COMPLETED;
    TFBufferBarrier      barriers[TF_MAX_VERTEX_BINDINGS + 1] = {};
    uint32_t             barrierCount = 0;

    const bool indexUma = gUma != TF_UMA_SUPPORT_NONE &&
                          (gUma == TF_UMA_SUPPORT_READ_WRITE || (indexUpdateDesc.pBuffer->mDescriptors & TF_DESCRIPTOR_TYPE_RW_MASK) == 0);

    if (!indexUma || (indexUpdateDesc.pMappedData && !indexUpdateDesc.pBuffer->pCpuMappedAddress))
    {
        indexUpdateDesc.mCurrentState = indexUma ? indexUpdateDesc.mCurrentState : TF_RESOURCE_STATE_COPY_DEST;
    ...
    barriers[barrierCount++] = { indexUpdateDesc.pBuffer, TF_RESOURCE_STATE_COPY_DEST, gIndexBufferState };
    ...
    for (uint32_t i = 0; i < TF_MAX_VERTEX_BINDINGS; ++i)
    {
        ...
        {
            const bool vertexUma =
                gUma != TF_UMA_SUPPORT_NONE &&
                (gUma == TF_UMA_SUPPORT_READ_WRITE || (vertexUpdateDesc[i].pBuffer->mDescriptors & TF_DESCRIPTOR_TYPE_RW_MASK) == 0);
            ...
            {
                vertexUpdateDesc[i].mCurrentState = vertexUma ? vertexUpdateDesc[i].mCurrentState : TF_RESOURCE_STATE_COPY_DEST;
            ...
            barriers[barrierCount++] = { vertexUpdateDesc[i].pBuffer, TF_RESOURCE_STATE_COPY_DEST, gVertexBufferState };
        }
    }
...
static UploadFunctionResult copyTexture(TFRenderer* pRenderer, CopyEngine* pCopyEngine, TFTextureCopyDesc& pTextureCopy)
{
    UNREF_PARAM(pRenderer);
    TFTexture*            texture = pTextureCopy.pTexture;
    const TinyImageFormat fmt = (TinyImageFormat)texture->mFormat;
    ...
    TFTextureBarrier barrier = { texture, pTextureCopy.mTextureState, TF_RESOURCE_STATE_COPY_SOURCE };
    barrier.mAcquire = 1;
    ...
    barrier = { texture, TF_RESOURCE_STATE_COPY_SOURCE, pTextureCopy.mTextureState };
    barrier.mRelease = 1;
    barrier.mQueueType = pTextureCopy.mQueueType;
    cmdResourceBarrier(cmd, 0, NULL, 1, &barrier, 0, NULL);
...
{
    for (size_t i = 0; i < TF_MAX_MULTIPLE_GPUS; ++i)
    {
    ...
    while (pLoader->mRun)
    {
        ...
        for (uint32_t nodeIndex = 0; nodeIndex < pLoader->mGpuCount; ++nodeIndex)
        {
            ...
            *pRequestQueue = NULL;
            ...
            TFRenderer* pRenderer = pLoader->ppRenderers[nodeIndex];
            TFSyncToken maxNodeToken = {};
            ...
            for (ptrdiff_t j = 0; j < arrlen(activeQueue); ++j)
            {
                ...
                case UPDATE_REQUEST_LOAD_TEXTURE:
                    ...
                    if (result == UPLOAD_FUNCTION_RESULT_INVALID_REQUEST)
                    {
                        handleInvalidTextureRequest(pRenderer, pCopyEngine, updateState.texLoadDesc);
                    }
                    break;
        ...
        TFSyncToken nextToken = max(pLoader->mMaxToken, getLastTokenCompleted());
...
static void CopyEngineFlush(CopyEngine* pCopyEngine)
{
    ...
    TFSyncToken nextToken = max(pResourceLoader->mMaxToken, getLastTokenCompleted());
...
static void initResourceLoader(TFRenderer** ppRenderers, uint32_t rendererCount, TFResourceLoaderDesc* pDesc, ResourceLoader** ppLoader)
{
    ASSERT(rendererCount > 0);
    ASSERT(rendererCount <= TF_MAX_MULTIPLE_GPUS);
    ...
    if (ppRenderers[0]->mGpuMode != TF_GPU_MODE_UNLINKED)
    {
    ...
    {
        ASSERT(rendererCount == 1 || ppRenderers[i]->mGpuMode == TF_GPU_MODE_UNLINKED);
        ...
        gUma = (TFUMASupportFlags)ppRenderers[i]->pGpu->mUnifiedMemorySupport;
    }
    ...
    for (uint32_t i = 0; i < gpuCount; ++i)
    {
        ...
        desc.mNodeIndex = i;
        desc.mQueueType = TF_QUEUE_TYPE_GRAPHICS;
        ...
        desc.mBufferCount = pLoader->mDesc.mBufferCount;
        desc.mNodeIndex = i;
        desc.mQueueType = TF_QUEUE_TYPE_TRANSFER;
        desc.mSize = pLoader->mDesc.mBufferSize;
        ...
        // streamerThreadFunc rotates to the next set before use.
        // Seed the index so the first iteration uses staging set 0.
        copyEngine->activeSet = (copyEngine->bufferCount - 1);
...
static void exitResourceLoader(ResourceLoader* pLoader)
{
    ...
    for (uint32_t nodeIndex = 0; nodeIndex < pLoader->mGpuCount; ++nodeIndex)
    {
        ...
        TFRenderer* renderer = pLoader->ppRenderers[nodeIndex];
...
static void queueBufferLoad(ResourceLoader* pLoader, BufferLoadDescInternal* pBufferLoad, TFSyncToken* token)
{
    uint32_t nodeIndex = pBufferLoad->pBuffer->mNodeIndex;
    acquireMutex(&pLoader->mQueueMutex);

    TFSyncToken t = tfrg_atomic64_add_relaxed(&pLoader->mTokenCounter, 1) + 1;
...
static void queueTextureLoad(ResourceLoader* pLoader, TextureLoadDescInternal* pTextureLoad, TFSyncToken* token)
{
...
static void queueGeometryLoad(ResourceLoader* pLoader, TFGeometryLoadDesc* pGeometryLoad, TFSyncToken* token)
{
...
static void queueTextureBarrier(ResourceLoader* pLoader, TFTexture* pTexture, TFResourceState state, TFSyncToken* token)
{
    ...
    arrpush(pLoader->mRequestQueue[nodeIndex], UpdateRequest(TFTextureBarrier{ pTexture, TF_RESOURCE_STATE_UNDEFINED, state }));
    UpdateRequest* pLastRequest = arrback(pLoader->mRequestQueue[nodeIndex]);
    if (pLastRequest)
        pLastRequest->mWaitIndex = t;
...
static void queueTextureCopy(ResourceLoader* pLoader, TFTextureCopyDesc* pTextureCopy, TFSyncToken* token)
{
...
static void waitForTokenSubmitted(ResourceLoader* pLoader, const TFSyncToken* token)
{
    if (pLoader->mDesc.mSingleThreaded)
    {
        return;
...
/************************************************************************/
void initResourceLoaderInterface(TFRenderer** ppRenderers, uint32_t rendererCount, TFResourceLoaderDesc* pDesc)
{
    initResourceLoader(ppRenderers, rendererCount, pDesc, &pResourceLoader);

#ifdef ENABLE_FORGE_MATERIALS
    if (pDesc && pDesc->mUseMaterials && rendererCount == 1)
    {
...

        {
            ...
            TFFileStream stream = {};
            if (fsOpenStreamFromPath(TF_RD_COMPILED_MATERIALS, "material_lib.ini", TF_FM_READ, &stream))
            {
                char buffer[256] = {};
        ...
        totalSize += (sizeof(TFShader**) + sizeof(uint16_t)) * maxShaders + alignof(TFShader**); // Shaders
        totalSize += (sizeof(TFTexture**) + sizeof(uint16_t)) * maxTextures;                     // Textures
        ...
        pLib->pRenderer = ppRenderers[0];
        pLib->ppMaterialShaders = (TFShader**)alignMemory(pLib + 1, alignof(TFShader**));
        pLib->ppMaterialTextures = (TFTexture**)(pLib->ppMaterialShaders + maxShaders); //-V1027 // Memory already aligned to pointer
...
void exitResourceLoaderInterface(TFRenderer** ppRenderers, uint32_t rendererCount)
{
    UNREF_PARAM(ppRenderers);
...
void initResourceLoaderInterface(TFRenderer* pRenderer, TFResourceLoaderDesc* pDesc) { initResourceLoaderInterface(&pRenderer, 1, pDesc); }

void exitResourceLoaderInterface(TFRenderer* pRenderer) { exitResourceLoaderInterface(&pRenderer, 1); }

/// Multiple TFRenderer (unlinked GPU) variants. The Resource Loader must be shared between Renderers.
void initResourceLoaderInterface(TFRenderer** ppRenderers, uint32_t rendererCount)
{
    initResourceLoaderInterface(ppRenderers, rendererCount, NULL);
}

#ifdef ENABLE_FORGE_MATERIALS
static void parseMaterial(const char* pFileBuffer, uint64_t fileSize, TFMaterial** pOut)
{
    ASSERT(pOut);
    ...
    extraSize += sizeof(TFMaterial::LoadedMaterial) * numMaterialSets + alignof(TFMaterial::LoadedMaterial); // TFMaterial::pLoaded
    extraSize += sizeof(uint32_t) * numMaterialSets * maxTextureSetTextures; // TFMaterial::LoadedMaterial::pTextureIndexes;
    ...
    const uint64_t totalSize = sizeof(TFMaterial) + sizeof(MaterialDesc) + alignof(MaterialDesc) + extraSize;
    TFMaterial*    pMaterial = (TFMaterial*)tf_calloc(1, totalSize);
    pMaterial->pLoaded = (TFMaterial::LoadedMaterial*)alignMemory(pMaterial + 1, alignof(TFMaterial::LoadedMaterial));
...
uint32_t addMaterial(const char* pMaterialFileName, TFMaterial** pOutMaterial, TFSyncToken* pSyncToken)
{
    MaterialLibrary* pLib = pMaterialLibrary;
    ASSERT(pLib);
    ...
    if (fsOpenStreamFromPath(TF_RD_COMPILED_MATERIALS, pMaterialFileName, TF_FM_READ, &stream))
    {
    ...
    else
    {
        LOGF(eERROR, "Failed to open material file %s. Function %s failed with error: %s", pMaterialFileName, FS_ERR_CTX.func,
             getFSErrCodeString(FS_ERR_CTX.code));
        return TF_REGISTER_MATERIAL_BADFILE;
    }

    TFMaterial* pMaterial = nullptr;
    ...
    {
        TFMaterial::LoadedMaterial*      pLoadedMaterial = pMaterial->pLoaded + i;
        ...
        if (pLib->pMaterialShaderRefCount[shaderIndex] == 0)
        {
            ...
            TFShader** ppOutShader = &pLib->ppMaterialShaders[shaderIndex];

            TFBinaryShaderDesc shaderDesc = {};
        ...
        for (uint32_t j = 0; j < textureCount; ++j)
        {
            ...
            if (pLib->pMaterialTextureRefCount[textureIndex] == 0)
            {
                ...
                if (pMaterialDesc->pTextureFlags[j] & MaterialDesc::TextureFlags::SRGB)
                    desc.mCreationFlag |= TF_TEXTURE_CREATION_FLAG_SRGB;
    ...
    *pOutMaterial = pMaterial;
    return TF_REGISTER_MATERIAL_SUCCESS;
}

void removeMaterial(TFMaterial* pMaterial)
{
    MaterialLibrary* pLib = pMaterialLibrary;
    ASSERT(pLib);
    ASSERT(pMaterial && pMaterial->pDesc);
...
uint32_t getMaterialSetIndex(TFMaterial* pMaterial, const char* name)
{
    ASSERT(pMaterial && pMaterial->pDesc);
...
void getMaterialShader(TFMaterial* pMaterial, uint32_t materialSetIndex, TFShader** ppOutShader)
{
    MaterialLibrary* pLib = pMaterialLibrary;
    ASSERT(pLib);
    ASSERT(pMaterial && pMaterial->pDesc);
...
void getMaterialTextures(TFMaterial* pMaterial, uint32_t materialSetIndex, const char** ppOutTextureBindingNames, TFTexture** ppOutTextures,
                         uint32_t outTexturesSize)
{
    ...
    const TFMaterial::LoadedMaterial* pLoadedMaterial = pMaterial->pLoaded + materialSetIndex;
...
void getResourceSizeAlign_Buffer(const TFBufferLoadDesc* pDesc, TFResourceSizeAlign* pOut)
{
    getBufferSizeAlign(pResourceLoader->ppRenderers[pDesc->mDesc.mNodeIndex], &pDesc->mDesc, pOut);
}

void getResourceSizeAlign_Texture(const TFTextureLoadDesc* pDesc, TFResourceSizeAlign* pOut)
{
    getTextureSizeAlign(pResourceLoader->ppRenderers[pDesc->mNodeIndex], pDesc->pDesc, pOut);
}

void getResourceSizeAlign(const TFBufferLoadDesc* pDesc, TFResourceSizeAlign* pOut) //-V524
{
    getBufferSizeAlign(pResourceLoader->ppRenderers[pDesc->mDesc.mNodeIndex], &pDesc->mDesc, pOut);
}

void getResourceSizeAlign(const TFTextureLoadDesc* pDesc, TFResourceSizeAlign* pOut) //-V524
{
    getTextureSizeAlign(pResourceLoader->ppRenderers[pDesc->mNodeIndex], pDesc->pDesc, pOut);
}

void addResource_Buffer(TFBufferLoadDesc* pBufferDesc, TFSyncToken* token)
{
    if (token)
    {
        *token = max(0, *token);
    ...
    if (pBufferDesc->pData && !token)
    {
        LOADER_LOGF(eWARNING,
                    "addResource : TFBufferLoadDesc(%s)::pData is non NULL but token is NULL. It is undefined behavior if pData is freed "
    ...
    TFResourceState startState = pBufferDesc->mDesc.mStartState;
    if (TF_RESOURCE_MEMORY_USAGE_GPU_ONLY == pBufferDesc->mDesc.mMemoryUsage && !pBufferDesc->mDesc.mStartState)
    {
    ...
    TFRenderer* pRenderer = pResourceLoader->ppRenderers[pBufferDesc->mDesc.mNodeIndex];
    const bool  update = pBufferDesc->pData || pBufferDesc->mForceReset;
    const bool  isUma = gUma != TF_UMA_SUPPORT_NONE &&
                       (gUma == TF_UMA_SUPPORT_READ_WRITE || (pBufferDesc->mDesc.mDescriptors & TF_DESCRIPTOR_TYPE_RW_MASK) == 0);
    const bool gpuUpdate = pBufferDesc->mDesc.mMemoryUsage == TF_RESOURCE_MEMORY_USAGE_GPU_ONLY && update && !isUma;

    if (gpuUpdate)
    {
        pBufferDesc->mDesc.mStartState = TF_RESOURCE_STATE_COPY_DEST;
    }
    else
    {
    ...
    if (update)
    {
        ...
        if (gpuUpdate && TF_RESOURCE_MEMORY_USAGE_GPU_ONLY == loadDesc.pBuffer->mMemoryUsage)
        {
...
void addResource_Texture(TFTextureLoadDesc* pTextureDesc, TFSyncToken* token)
{
    ...
    if ((!pTextureDesc->pFileName && !pTextureDesc->mLoadFromPackage) && pTextureDesc->pDesc)
    {
        ASSERT(pTextureDesc->pDesc->mStartState);

        TFTextureDesc textureDesc = *pTextureDesc->pDesc;
        ...
        if (pTextureDesc->mForceReset)
        {
            ...
            textureDesc.mStartState = TF_RESOURCE_STATE_COPY_DEST;
        }
#endif
        ...
        if (pTextureDesc->mForceReset)
        {
            ...
            TFTexture* texture = *pTextureDesc->ppTexture;
#if defined(ORBIS)
            void*    ptr = texture->mDesc.mStruct.mSrvDescriptor.getBaseAddress();
            uint64_t size = texture->mDesc.mStruct.mSrvDescriptor.getSizeAlign().m_size;
#elif defined(PROSPERO)
            void*    ptr = texture->mDesc.mStruct.mSrv.getDataAddress();
            uint64_t size = PROSPERO_RENDERER_NAMESPACE::getSize(&texture->mDesc.mStruct.mSrv).m_size;
#elif defined(XBOX)
            void*               ptr = (void*)texture->mDx.pResource->GetGPUVirtualAddress();
            D3D12_RESOURCE_DESC desc = texture->mDx.pResource->GetDesc();
            uint64_t            size = 0;
            uint32_t            numSubResources =
                desc.Dimension == D3D12_RESOURCE_DIMENSION_TEXTURE3D ? desc.MipLevels : desc.MipLevels * desc.DepthOrArraySize;
            pResourceLoader->ppRenderers[pTextureDesc->mNodeIndex]->mDx.pDevice->GetCopyableFootprints(&desc, 0, numSubResources, 0, NULL,
                                                                                                       NULL, NULL, &size);
#else
#error : Not implemented
            ...
            loadDesc.mOnTextureLoaded = pTextureDesc->mOnTextureLoaded;
            loadDesc.pUserData = pTextureDesc->pUserData;
        ...
        if (IssueExplicitInitialStateBarrier())
        {
            TFResourceState startState = pTextureDesc->pDesc->mStartState;
            // Check whether this is required (user specified a state other than undefined / common)
            if (startState == TF_RESOURCE_STATE_UNDEFINED || startState == TF_RESOURCE_STATE_COMMON) //-V560
            {
                startState = ResourceStartState(pTextureDesc->pDesc->mDescriptors & TF_DESCRIPTOR_TYPE_RW_TEXTURE);
            }
    ...
    else
    {
        ...
        loadDesc.mLoadFromPackage = pTextureDesc->mLoadFromPackage;

        loadDesc.pAtlasTexture = pTextureDesc->pAtlasTexture;
        loadDesc.pAtlasPlacement = pTextureDesc->pAtlasPlacement;
        if (pTextureDesc->mLoadFromPackage)
        {
            loadDesc.mPackagePtr.pPackage = pTextureDesc->mPackagePtr.pPackage;
            loadDesc.mPackagePtr.mDataID = pTextureDesc->mPackagePtr.mDataID;
        }
        else
        {
...
void addResource_Geometry(TFGeometryLoadDesc* pGeomDesc, TFSyncToken* token)
{
    ASSERT(pGeomDesc->pVertexLayout);
    ASSERT(pGeomDesc->ppGeometry);

    TFGeometryLoadDesc updateDesc = *pGeomDesc;
    updateDesc.pFileName = pGeomDesc->pFileName;

    uint32_t extraSize = sizeof(TFVertexLayout);

    TFVertexLayout* pCopyVertexLayout = (TFVertexLayout*)tf_malloc(extraSize);
    memcpy(pCopyVertexLayout, pGeomDesc->pVertexLayout, sizeof(TFVertexLayout));
...
void removeResource_Texture(TFTexture* pTexture)
{
    if (!pTexture)
        return;
...
void removeResource_Geometry(TFGeometry* pGeom)
{
    if (!pGeom)
        return;
    ...
    {
        removeResource_Buffer(pGeom->pIndexBuffer);

        for (uint32_t i = 0; i < pGeom->mVertexBufferCount; ++i)
        {
            removeResource_Buffer(pGeom->pVertexBuffers[i]);
        }
    }
...
void removeResource_GeometryData(TFGeometryData* pGeom)
{
    if (!pGeom)
        return;

    removeGeometryCpuData(pGeom);
    tf_free(pGeom);
}

void removeResource(TFBuffer* pBuffer) { removeResource_Buffer(pBuffer); }

void removeResource(TFTexture* pTexture) { removeResource_Texture(pTexture); }

void removeResource(TFGeometry* pGeom) { removeResource_Geometry(pGeom); }

void removeResource(TFGeometryData* pGeom) { removeResource_GeometryData(pGeom); }

void removeGeometryCpuData(TFGeometryData* pGeom)
{
    if (!pGeom)
        return;
    ...
    if (pGeom->pCpuData)
    {
        tf_free(pGeom->pCpuData);
        pGeom->pCpuData = nullptr;
    }
}
...
static void addBufferChunkAllocator(BufferChunkAllocatorDesc* pDesc, TFBufferChunkAllocator* pOut)
{
    ASSERT(pDesc);
    ASSERT(pOut);
    ...
    TFBufferChunk firstUnusedChunk{ 0, (uint32_t)pDesc->pBuffer->mSize };
    arrpush(pOut->mUnusedChunks, firstUnusedChunk);
}

static void removeBufferChunkAllocator(TFBufferChunkAllocator* pBuffer)
{
    ASSERT(pBuffer);
    ...
    if (pBuffer->pBuffer)
    {
        ...
        // We are checking that the unnused chunk offset is 0 because we currently assume that a TFBufferChunkAllocator covers the entire
        ...
        // Splits: [------------------A-------------------|--------B------]
        //
        // One TFBufferChunkAllocator would cover memory range A and would be use by the App to store big chunks of data while other
        // TFBufferChunkAllocator would cover memory range B and be used to fit smaller ammounts of data.
        //
        // Note: If we want this behavior we would need extend BufferChunkAllocatorDesc to provide mOffset and mSize that the
        // TFBufferChunkAllocator would cover,
        ...
        //       allocate the TFBufferChunkAllocator or we want to include this splitting logic in addGeometryBuffer.
...
void addGeometryBuffer(TFGeometryBufferLoadDesc* pDesc)
{
    TFDescriptorType flags = TF_DESCRIPTOR_TYPE_BUFFER_RAW | TF_DESCRIPTOR_TYPE_RW_BUFFER_RAW;

    TFGeometryBuffer* pBuffer = (TFGeometryBuffer*)tf_calloc(1, sizeof *pBuffer);

    if (!VERIFYMSG(pBuffer, "Couldn't allocate TFGeometryBuffer"))
    {
        return;
    }
    ...
    *pDesc->pOutGeometryBuffer = pBuffer;
    ...
    TFBufferLoadDesc loadDesc = {};

    TFBuffer* pIndexBuffer = NULL;
    TFBuffer* pVertexBuffer = NULL;

    const bool isUma =
        gUma != TF_UMA_SUPPORT_NONE && (gUma == TF_UMA_SUPPORT_READ_WRITE || (flags & TF_DESCRIPTOR_TYPE_RW_MASK) == 0); //-V560

    loadDesc.mDesc.mMemoryUsage = TF_RESOURCE_MEMORY_USAGE_GPU_ONLY;
    loadDesc.mDesc.mSize = pDesc->mIndicesSize;
    loadDesc.ppBuffer = &pIndexBuffer;
    loadDesc.mDesc.mDescriptors = TF_DESCRIPTOR_TYPE_INDEX_BUFFER | flags;
    ...
    loadDesc.mDesc.pName = pDesc->pNameIndexBuffer ? pDesc->pNameIndexBuffer : "TFGeometryBuffer Indices (unnamed)";
    ...
    for (size_t i = 0; i < TF_ARRAY_COUNT(pDesc->mVerticesSizes); ++i)
    {
        ...
        loadDesc.mDesc.mDescriptors = TF_DESCRIPTOR_TYPE_VERTEX_BUFFER | flags;
        loadDesc.mDesc.mStructStride = sizeof(uint32_t);
        loadDesc.mDesc.mElementCount = (uint32_t)(loadDesc.mDesc.mSize / loadDesc.mDesc.mStructStride);
        loadDesc.mDesc.mStartState = isUma ? gVertexBufferState : pDesc->mStartState;
        loadDesc.mDesc.pName = pDesc->pNamesVertexBuffers[i] ? pDesc->pNamesVertexBuffers[i] : "TFGeometryBuffer Vertices (unnamed)";
...
void removeGeometryBuffer(TFGeometryBuffer* pGeomBuffer)
{
    if (!pGeomBuffer)
        return;
...
void addGeometryBufferPart(TFBufferChunkAllocator* pBuffer, uint32_t size, uint32_t alignment, TFBufferChunk* pOut,
                           TFBufferChunk* pRequestedChunk)
{
    if (size == 0)
        return;
    ...
    if (pRequestedChunk)
    {
        ...
        {
            TFBufferChunk* chunk = &pBuffer->mUnusedChunks[i];
            ...
            if (chunk->mOffset <= pRequestedChunk->mOffset && chunkEnd >= requestedEnd)
            {
                ...
                *pOut = *pRequestedChunk;
                ...
                {
                    TFBufferChunk prevChunk = { chunk->mOffset, pRequestedChunk->mOffset - chunk->mOffset };
    ...
    for (uint32_t i = 0; i < arrlenu(pBuffer->mUnusedChunks); ++i)
    {
        ...
        if (alignment > 0)
        {
            ...
            if (padding > chunk->mSize || chunk->mSize - padding < size)
                continue;

            if (padding > 0)
            {
                TFBufferChunk paddingChunk = {
                    chunk->mOffset,
                    padding,
                };
...
void addGeometryBufferPart(TFBufferChunkAllocator* buffer, uint32_t size, uint32_t alignment, TFBufferChunk* pOut)
{
    addGeometryBufferPart(buffer, size, alignment, pOut, NULL);
}

void removeGeometryBufferPart(TFBufferChunkAllocator* pBuffer, TFBufferChunk* pChunk)
{
...
void addHeapAllocator(const TFHeapAllocatorDesc* pDesc, TFHeapAllocator** ppAllocator)
{
    uint64_t allocationSize = sizeof(TFHeapAllocator);
    allocationSize += pDesc->mMaxChunksCount * sizeof(TFHeapChunk);
    TFHeapAllocator* allocator = (TFHeapAllocator*)tf_malloc(allocationSize);
    allocator->mTotalSize = pDesc->mSize;
    allocator->mFreeChunksCount = 1;
    allocator->mMaxChunksCount = pDesc->mMaxChunksCount;
    allocator->pFreeChunks = (TFHeapChunk*)(allocator + 1);
    allocator->mAlignement = pDesc->mAlignement;
    allocator->mUsedMemory = 0;
    allocator->mAllowAllocFail = pDesc->mAllowAllocFail;

    // Start with one large free chunk
    allocator->pFreeChunks[0].mOffset = 0;
    allocator->pFreeChunks[0].mSize = pDesc->mSize;
    *ppAllocator = allocator;
}

void removeHeapAllocator(TFHeapAllocator* pAllocator) { tf_free(pAllocator); }

bool heapAllocatorAlloc(TFHeapAllocator* allocator, uint64_t size, TFHeapChunk* pOut)
{
    ASSERTMSG(size > 0, "invalid call to heapAllocatorAlloc: size is zero");

    uint64_t alignement = allocator->mAlignement;
    for (size_t i = 0; i < allocator->mFreeChunksCount; i++)
    {
        TFHeapChunk* block = &allocator->pFreeChunks[i];

        uint64_t alignedOffset = (block->mOffset + (alignement - 1)) & ~(alignement - 1);
        uint64_t padding = alignedOffset - block->mOffset;

        if (block->mSize >= size + padding)
        {
            uint64_t allocatedOffset = alignedOffset;
            uint64_t nextOffset = ((allocatedOffset + size) + (alignement - 1)) & ~(alignement - 1);

            // Update free list
            if (nextOffset >= block->mOffset + block->mSize)
            {
                // Remove block completely
                memmove(&allocator->pFreeChunks[i], &allocator->pFreeChunks[i + 1],
                        (allocator->mFreeChunksCount - i - 1) * sizeof(TFHeapChunk));
                allocator->mFreeChunksCount--;
            }
            else
            {
                // Shrink block
                uint64_t previousOffset = block->mOffset;
                block->mOffset = nextOffset;
                block->mSize -= block->mOffset - previousOffset;
            }
            pOut->mOffset = allocatedOffset;
            pOut->mSize = size;
            allocator->mUsedMemory += size;
            break;
        }
    }
    // assert on failure
    ASSERTMSG(allocator->mAllowAllocFail || pOut->mSize > 0, "failed to allocate from resource heap, consider increasing its size ");
    return pOut->mSize > 0;
}

void heapAllocatorFree(TFHeapAllocator* allocator, uint64_t offset, uint64_t size)
{
    if (size == 0)
    {
        return;
    ...
    allocator->pFreeChunks[allocator->mFreeChunksCount].mOffset = offset;
    allocator->pFreeChunks[allocator->mFreeChunksCount].mSize = size;
    allocator->mFreeChunksCount++;

    // Sort and merge free blocks
    for (size_t i = 0; i < allocator->mFreeChunksCount - 1; i++)
    {
        for (size_t j = i + 1; j < allocator->mFreeChunksCount; j++)
        {
            if (allocator->pFreeChunks[i].mOffset > allocator->pFreeChunks[j].mOffset)
            {
                TFHeapChunk temp = allocator->pFreeChunks[i];
                allocator->pFreeChunks[i] = allocator->pFreeChunks[j];
                allocator->pFreeChunks[j] = temp;
            }
        }
    }
    ...
    for (size_t i = 1; i < allocator->mFreeChunksCount; i++)
    {
        TFHeapChunk* prev = &allocator->pFreeChunks[mergedCount - 1];
        TFHeapChunk* curr = &allocator->pFreeChunks[i];

        uint64_t nextOffset = prev->mOffset + prev->mSize;
        uint64_t alignment = allocator->mAlignement;
        uint64_t alignedNextOffset = (nextOffset + (alignment - 1)) & ~(alignment - 1);
        if (curr->mOffset == nextOffset)
        {
            prev->mSize += curr->mSize;
        }
        else if (curr->mOffset == alignedNextOffset)
        {
            uint64_t padding = alignedNextOffset - nextOffset;
            prev->mSize += padding + curr->mSize;
        }
        else
        {
            allocator->pFreeChunks[mergedCount++] = *curr;
        }
    }
    allocator->mFreeChunksCount = mergedCount;
    allocator->mUsedMemory -= size;
}
...
void beginUpdateResource(TFBufferUpdateDesc* pBufferUpdate)
{
    ...
    const bool isUma =
        gUma != TF_UMA_SUPPORT_NONE && (gUma == TF_UMA_SUPPORT_READ_WRITE || (pBuffer->mDescriptors & TF_DESCRIPTOR_TYPE_RW_MASK) == 0);

    TFResourceMemoryUsage memoryUsage = (TFResourceMemoryUsage)pBufferUpdate->pBuffer->mMemoryUsage;
    if (isUma || memoryUsage != TF_RESOURCE_MEMORY_USAGE_GPU_ONLY)
    {
    ...
    else
    {
        ...
        TFMappedMemoryRange range = allocateStagingMemory(pCopyEngine, size, 1, nodeIndex);
        if (!range.pData)
        {
...
TFTextureSubresourceUpdate TFTextureUpdateDesc::getSubresourceUpdateDesc(uint32_t mip, uint32_t layer)
{
    TFTextureSubresourceUpdate ret = {};
    TFTexture*                 texture = pTexture;
    const TinyImageFormat      fmt = (TinyImageFormat)texture->mFormat;
...
void beginUpdateResource(TFTextureUpdateDesc* pTextureUpdate)
{
    ...
    for (uint32_t mip = pTextureUpdate->mBaseMipLevel; mip < pTextureUpdate->mBaseMipLevel + pTextureUpdate->mMipLevels; ++mip)
    {
        uint32_t srcSliceStride = 0;
        uint32_t srcRowStride = 0;
        uint32_t rowCount = 0;
...
void beginUpdateResource_Buffer(TFBufferUpdateDesc* pBufferDesc) { beginUpdateResource(pBufferDesc); }
void beginUpdateResource_Texture(TFTextureUpdateDesc* pTextureDesc) { beginUpdateResource(pTextureDesc); }
...
TFSyncToken getLastTokenCompleted() { return tfrg_atomic64_load_acquire(&pResourceLoader->mTokenCompleted); }
...
bool isTokenSubmitted(const TFSyncToken* token) { return *token <= tfrg_atomic64_load_acquire(&pResourceLoader->mTokenSubmitted); }

void waitForTokenSubmitted(const TFSyncToken* token) { waitForTokenSubmitted(pResourceLoader, token); }
...
TFSemaphore* getLastSemaphoreSubmitted(uint32_t nodeIndex)
{
    acquireMutex(&pResourceLoader->mSemaphoreMutex);
    TFSemaphore* sem = pResourceLoader->pCopyEngines[nodeIndex].pLastSubmittedSemaphore;
    releaseMutex(&pResourceLoader->mSemaphoreMutex);
    return sem;
}
...
{
    UNREF_PARAM(pRenderer);
    char binaryShaderPath[TF_FS_MAX_PATH];
    ...
    TFFileStream binaryFileStream = {};

    // NOTE: On some platforms, we might not be allowed to write in the `TF_RD_SHADER_BINARIES` directory.
    ...
    const bool result = platformGetReloadBinary(binaryShaderPath, &pCachedByteCode, &cachedByteCodeSize)
                            ? fsOpenStreamFromMemory(pCachedByteCode, cachedByteCodeSize, TF_FM_READ, false, &binaryFileStream)
                            : fsOpenStreamFromPath(TF_RD_SHADER_BINARIES, binaryShaderPath, TF_FM_READ, &binaryFileStream);
#else
    const bool result = fsOpenStreamFromPath(TF_RD_SHADER_BINARIES, binaryShaderPath, TF_FM_READ, &binaryFileStream);
#endif
    ...
    if (strncmp("@FSL", header.mMagic, 4) != 0)
    {
        ...
        fsSeekStream(&binaryFileStream, TF_SBO_START_OF_FILE, 0);

#if defined(PROSPERO)
        extern void prospero_loadByteCode(TFRenderer*, TFFileStream*, ssize_t, TFBinaryShaderStageDesc*);
    ...
    else
    {
        ...
        fsSeekStream(&binaryFileStream, TF_SBO_START_OF_FILE, pDerivatives[0].mOffset);
        ...
        for (uint32_t i = 0; i < header.mDerivativeCount; ++i)
        {
            ...
            {
                if (!fsSeekStream(&binaryFileStream, TF_SBO_START_OF_FILE, pDerivatives[i].mOffset))
                {
...
#define SHADER_STAGE_INDEX_WORKGRAPH 6
    const TFShaderStageLoadDesc* stages[] = { &pDesc->mVert, &pDesc->mHull, &pDesc->mDomain, &pDesc->mGeom, &pDesc->mFrag, &pDesc->mComp,
    ...
    for (uint32_t i = 0; i < TF_ARRAY_COUNT(stages); ++i)
    {
        ...
        FSLMetadata metadata = {};
        if (!load_shader_stage_byte_code(pRenderer, fileName, pBinaryStageDesc, &shaderByteCodeBuffer, &metadata))
        {
            freeShaderByteCode(&shaderByteCodeBuffer, &binaryDesc);
            return;
        }

        if (TF_SHADER_STAGE_COMP == stage)
        {
...
#if defined(QUEST_VR) || defined(HOLOLENS2)
        binaryDesc.mIsMultiviewVR |= (bool)metadata.mUseMultiView;
#endif
        ...
        else if (TF_SHADER_STAGE_FRAG == stage)
        {
    ...
    TFShader* pShader = *ppShader;

#if defined(METAL)
    pShader->mICB = bIsICBCompatible;
#endif

    if (TF_SHADER_STAGE_COMP == binaryDesc.mStages)
    {
...
/************************************************************************/
void loadPipelineCache(TFRenderer* pRenderer, const TFPipelineCacheLoadDesc* pDesc, TFPipelineCache** ppPipelineCache)
{
#if defined(DIRECT3D12) || defined(VULKAN)
    ...
    ASSERT(strlen(rendererApi) + strlen(pDesc->pFileName) < sizeof(fileName));
    strcat(fileName, rendererApi);
    strcat(fileName, pDesc->pFileName);

    TFFileStream stream = {};
    bool         success = fsOpenStreamFromPath(TF_RD_PIPELINE_CACHE, fileName, TF_FM_READ, &stream);
    ssize_t      dataSize = 0;
    void*        data = NULL;
    if (success)
    {
    ...
    {
        if (FS_ERR_CTX.code == TF_FS_NOT_FOUND_ERR)
        {
...
void savePipelineCache(TFRenderer* pRenderer, TFPipelineCache* pPipelineCache, TFPipelineCacheSaveDesc* pDesc)
{
#if defined(DIRECT3D12) || defined(VULKAN)
    ...
    if (dataSize > 0)
    {
        ...
        if (data)
        {
            ...
            TFFileStream stream = {};
            if (fsOpenStreamFromPath(TF_RD_PIPELINE_CACHE, fileName, TF_FM_WRITE, &stream))
            {
                const size_t bytesWritten = fsWriteToStream(&stream, data, dataSize);
                const bool   flushSucceeded = (bytesWritten == dataSize) && fsFlushStream(&stream);
                const bool   closeSucceeded = fsCloseStream(&stream);
                ASSERT(flushSucceeded && closeSucceeded);
...
/************************************************************************/
#if defined(DIRECT3D12)
extern "C" void initRootSignatureImpl(TFRenderer*, const void*, uint32_t, ID3D12RootSignature**);
extern "C" void exitRootSignatureImpl(TFRenderer*, ID3D12RootSignature*);

static void loadRootSignature(TFRenderer* pRenderer, const char* pFileName, ID3D12RootSignature** ppOutRootSignature)
{
    ShaderByteCodeBuffer shaderByteCodeBuffer = {};
    char                 bytecodeStack[ShaderByteCodeBuffer::kStackSize] = {};
    shaderByteCodeBuffer.pStackMemory = bytecodeStack;
    TFBinaryShaderDesc desc = {};
    FSLMetadata        metadata = {};
    if (!load_shader_stage_byte_code(pRenderer, pFileName, &desc.mComp, &shaderByteCodeBuffer, &metadata))
    {
...
void initRootSignature(TFRenderer* pRenderer, const TFRootSignatureDesc* pDesc)
{
#if defined(DIRECT3D12)
    ASSERT(pRenderer);
    ASSERT(pDesc);
...
void exitRootSignature(TFRenderer* pRenderer)
{
#if defined(DIRECT3D12)
    ASSERT(pRenderer);
...
/*
 * Will load the package and populate the desc data with the metadata from the package.
 * If loadGeoData is set to true, then will also load the geo data from the package.
 * If loadTexData is set to true, then will also load the tex data from the Package.
 * Fill pNullTextureResource with a blank texture for when material attributes don't use a texture.
 */
bool addResourcesFromPackage(TFPackageLoadDesc* pDesc, TFTexture* pNullTextureResource)
{
    ASSERT(pDesc->packageName);
    ASSERT(pDesc->ppOutPackage);
    ...
    TFPackage* pOutPackage = (TFPackage*)tf_malloc(sizeof(TFPackage));
    memset(pOutPackage, 0, sizeof(TFPackage));

    pOutPackage->pPackageHandle = (TFIFileSystem*)tf_malloc(sizeof(TFIFileSystem));

    if (!fsArchiveOpen(TF_RD_OTHER_FILES, pDesc->packageName, &openDesc, pOutPackage->pPackageHandle))
    {
        LOGF(LogLevel::eERROR, "Failed to open the Package File %s", pDesc->packageName);
        return false;
    }

    TFFileStream packageMetadataStream = { 0 };
    pOutPackage->pPackageMetadata = (TFPackageDesc*)tf_malloc(sizeof(TFPackageDesc));
    memset(pOutPackage->pPackageMetadata, 0, sizeof(TFPackageDesc));
    pOutPackage->pTextureMetadata = NULL;

    TFTextureBundleID* pFullTextureBundleIDBuffer = NULL;
    TFMaterialProps*   pFullMaterialPropsBuffer = NULL;

    const uint32_t geoMetadataNodeID = 0;
    size_t         totalTextureCount = 0;
    size_t         totalMeshCount = 0;
    if (!fsIoOpenByUid(pOutPackage->pPackageHandle, geoMetadataNodeID, TF_FM_READ, &packageMetadataStream))
    {
        LOGF(LogLevel::eERROR, "Failed to read package metadata from archive");
        loadSuccess = false;
    }
    else
    {
        TFPackageDesc* pPackageMetadata = pOutPackage->pPackageMetadata;
        size_t         totalSize = fsGetStreamFileSize(&packageMetadataStream);
        size_t         readSize = 0;
        readSize += fsReadFromStream(&packageMetadataStream, &pPackageMetadata->mHeaderStr, TF_PAK_HEADER_SIZE);
        if (strncmp(pPackageMetadata->mHeaderStr, TF_PAK_HEADER_STR, TF_PAK_HEADER_SIZE) != 0)
        {
            LOGF(LogLevel::eERROR, "Package header mismatch. Expected version: %s. Package version: %s", TF_PAK_HEADER_STR,
                 pPackageMetadata->mHeaderStr);
            LOGF(LogLevel::eERROR, "Ensure The current package was processed with the current version of Assetpipeline.");
            loadSuccess = false;
        }
        else
        {
            readSize += fsReadFromStream(&packageMetadataStream, &pPackageMetadata->mGeoCount, sizeof(size_t));

            pPackageMetadata->pGeoNodeIds = (size_t*)tf_malloc(pPackageMetadata->mGeoCount * sizeof(size_t));
            readSize +=
                fsReadFromStream(&packageMetadataStream, pPackageMetadata->pGeoNodeIds, pPackageMetadata->mGeoCount * sizeof(size_t));

            pPackageMetadata->pTexMetadataNodeIds = (size_t*)tf_malloc(pPackageMetadata->mGeoCount * sizeof(size_t));
            readSize += fsReadFromStream(&packageMetadataStream, pPackageMetadata->pTexMetadataNodeIds,
                                         pPackageMetadata->mGeoCount * sizeof(size_t));

            pPackageMetadata->pAnimationNodeIDRanges =
                (TFAnimationNodeIDRange*)tf_malloc(pPackageMetadata->mGeoCount * sizeof(TFAnimationNodeIDRange));
            readSize += fsReadFromStream(&packageMetadataStream, pPackageMetadata->pAnimationNodeIDRanges,
                                         pPackageMetadata->mGeoCount * sizeof(TFAnimationNodeIDRange));

            pPackageMetadata->pTextureStartNodeIds = (size_t*)tf_malloc(pPackageMetadata->mGeoCount * sizeof(size_t));
            readSize += fsReadFromStream(&packageMetadataStream, pPackageMetadata->pTextureStartNodeIds,
                                         pPackageMetadata->mGeoCount * sizeof(size_t));

            readSize += fsReadFromStream(&packageMetadataStream, &pPackageMetadata->mTextureAtlasNodeID, sizeof(size_t));
            readSize += fsReadFromStream(&packageMetadataStream, &pPackageMetadata->mNameBufferSize, sizeof(size_t));
#ifdef PACKAGE_STRINGS_ENABLED
            pPackageMetadata->pGeoNames = (char*)tf_malloc(sizeof(char) * pPackageMetadata->mNameBufferSize);
            readSize += fsReadFromStream(&packageMetadataStream, pPackageMetadata->pGeoNames, pPackageMetadata->mNameBufferSize);
#else
            // Not reading any string information from the package, so avoid considering the string size in the total size
            totalSize -= packageGeoData.mNameBufferSize;
#endif // PACKAGE_STRINGS_ENABLED
        }

        if (readSize != totalSize)
        {
            LOGF(LogLevel::eERROR, "Error in reading packaged data for GeoData.");
            loadSuccess = false;
        }
        fsCloseStream(&packageMetadataStream);

        if (loadSuccess)
        {
            pOutPackage->pTextureMetadata = (TFTexturePackagedDesc*)tf_calloc(pPackageMetadata->mGeoCount, sizeof(TFTexturePackagedDesc));
            if (pOutPackage->pTextureMetadata == NULL)
            {
                LOGF(LogLevel::eERROR, "Failed to allcoate memory for texture metadata");
                loadSuccess = false;
            }
        }
        ...
        if (loadSuccess)
        {
            for (uint32_t texMetadataIndex = 0; texMetadataIndex < pPackageMetadata->mGeoCount; texMetadataIndex++)
            {
                TFTexturePackagedDesc* pCurrTextureMetadata =
                    &pOutPackage->pTextureMetadata[texMetadataIndex]; //-V522 handled by loadSuccess
                TFFileStream texMetadataStream = {};
                if (!fsIoOpenByUid(pOutPackage->pPackageHandle, pOutPackage->pPackageMetadata->pTexMetadataNodeIds[texMetadataIndex],
                                   TF_FM_READ, &texMetadataStream))
                {
                    LOGF(LogLevel::eERROR, "Failed to read tex metadata from archive");
                    loadSuccess = false;
                    break;
                }

                size_t currNameBufferSize = 0;

                totalSize = fsGetStreamFileSize(&texMetadataStream);
                readSize = 0;
                readSize += fsReadFromStream(&texMetadataStream, &pCurrTextureMetadata->mTextureCount, //-V522 handled by loadSuccess
                                             sizeof(size_t));
                totalTextureCount += (uint32_t)pCurrTextureMetadata->mTextureCount;

                readSize += fsReadFromStream(&texMetadataStream, &currNameBufferSize, sizeof(size_t));
#ifdef PACKAGE_STRINGS_ENABLED
                pCurrTextureMetadata->pTextureNames = (char*)tf_malloc(sizeof(char) * currNameBufferSize);
                readSize += fsReadFromStream(&texMetadataStream, pCurrTextureMetadata->pTextureNames, sizeof(char) * currNameBufferSize);
#else
                // Not reading any string information from the package, so avoid considering the string size in the total size
                totalSize -= currNameBufferSize;
#endif // PACKAGE_STRINGS_ENABLED
                readSize += fsReadFromStream(&texMetadataStream, &pCurrTextureMetadata->mMeshCount, sizeof(size_t));

                const size_t bundleIDOffset = totalMeshCount;
                const size_t currentMeshCount = pCurrTextureMetadata->mMeshCount;
                totalMeshCount += currentMeshCount;
                const size_t readBundleIDsSize = sizeof(TFTextureBundleID) * currentMeshCount;
                const size_t readMaterialPropsSize = sizeof(TFMaterialProps) * currentMeshCount;
                const size_t readStreamZoneSize = sizeof(TFStreamZone) * currentMeshCount;

                pFullTextureBundleIDBuffer =
                    (TFTextureBundleID*)tf_realloc(pFullTextureBundleIDBuffer, sizeof(TFTextureBundleID) * totalMeshCount);
                if (pFullTextureBundleIDBuffer == NULL)
                {
                    LOGF(LogLevel::eERROR, "Failed to allcoate memory for pFullTextureBundleIDBuffer");
                    loadSuccess = false;
                    break;
                }
                readSize += fsReadFromStream(&texMetadataStream, pFullTextureBundleIDBuffer + bundleIDOffset, readBundleIDsSize);

                pFullMaterialPropsBuffer = (TFMaterialProps*)tf_realloc(pFullMaterialPropsBuffer, sizeof(TFMaterialProps) * totalMeshCount);
                if (pFullMaterialPropsBuffer == NULL)
                {
                    LOGF(LogLevel::eERROR, "Failed to allcoate memory for pFullMaterialPropsBuffer");
                    loadSuccess = false;
                    break;
                }
                readSize += fsReadFromStream(&texMetadataStream, pFullMaterialPropsBuffer + bundleIDOffset, readMaterialPropsSize);

                pCurrTextureMetadata->pStreamZones = (TFStreamZone*)tf_malloc(readStreamZoneSize);
                readSize += fsReadFromStream(&texMetadataStream, pCurrTextureMetadata->pStreamZones, readStreamZoneSize);
                if (readSize != totalSize)
                {
                    LOGF(LogLevel::eERROR, "Error in reading packaged data for Texture data.");
                    loadSuccess = false;
                    break;
                }

                fsCloseStream(&texMetadataStream);
            }
        }
    }
    ...
    if (!loadSuccess)
    {
        removeResourcesFromPackage(pOutPackage);
    }
    else
    {
        // Assign the TFTextureBundleID pointers correctly
        size_t bundleIDOffset = 0;
        for (uint32_t texMetadataIndex = 0; texMetadataIndex < pOutPackage->pPackageMetadata->mGeoCount; texMetadataIndex++)
        {
            TFTexturePackagedDesc* pCurrTextureMetadata = pOutPackage->pTextureMetadata + texMetadataIndex;
            ASSERT(pCurrTextureMetadata != NULL);
            pCurrTextureMetadata->pTextureBundleIDs = pFullTextureBundleIDBuffer + bundleIDOffset; //-V769 handled by loadSuccess
            pCurrTextureMetadata->pMaterialProps = pFullMaterialPropsBuffer + bundleIDOffset;      //-V769 handled by loadSuccess
            bundleIDOffset += pCurrTextureMetadata->mMeshCount;                                    //-V522
        }

        static TFVertexLayout defaultVertexLayout = {};
        memset(&defaultVertexLayout, 0, sizeof(TFVertexLayout));
        if (pDesc->pGeoVertexLayout)
        {
            memcpy(&defaultVertexLayout, pDesc->pGeoVertexLayout, sizeof(TFVertexLayout));
        }
        else
        {
            defaultVertexLayout.mAttribCount = 3;
            defaultVertexLayout.mBindingCount = 3;
            defaultVertexLayout.mAttribs[0].mSemantic = TF_SEMANTIC_POSITION;
            ...
            defaultVertexLayout.mAttribs[1].mSemantic = TF_SEMANTIC_TEXCOORD0;
            defaultVertexLayout.mAttribs[1].mFormat = TinyImageFormat_R16G16_SFLOAT;
            defaultVertexLayout.mAttribs[1].mBinding = 1;
            defaultVertexLayout.mAttribs[1].mLocation = 1;
            defaultVertexLayout.mAttribs[2].mSemantic = TF_SEMANTIC_NORMAL;
            defaultVertexLayout.mAttribs[2].mFormat = TinyImageFormat_R16G16_SFLOAT;
            defaultVertexLayout.mAttribs[2].mBinding = 2;
            defaultVertexLayout.mAttribs[2].mLocation = 2;
        }

        pOutPackage->pGeoData = (TFGeometryLoadDesc*)tf_calloc(pOutPackage->pPackageMetadata->mGeoCount, sizeof(TFGeometryLoadDesc));
        TFGeometry**     allGeo = NULL;
        TFGeometryData** allGeoData = NULL;
        pOutPackage->loadGeoData = pDesc->loadGeoData;
        if (pDesc->loadGeoData)
        {
            allGeo = (TFGeometry**)tf_calloc(pOutPackage->pPackageMetadata->mGeoCount, sizeof(TFGeometry*));
            allGeoData = (TFGeometryData**)tf_calloc(pOutPackage->pPackageMetadata->mGeoCount, sizeof(TFGeometryData*));
        }
        for (uint32_t geoIndex = 0; geoIndex < pOutPackage->pPackageMetadata->mGeoCount; geoIndex++)
        {
            TFGeometryLoadDesc* pLoadDesc = pOutPackage->pGeoData + geoIndex;
            pLoadDesc->pVertexLayout = &defaultVertexLayout;
            pLoadDesc->mFlags = pDesc->geometryLoadFlags;

            if (pOutPackage->pPackageMetadata->pAnimationNodeIDRanges[geoIndex].mAnimationCount > 0)
            {
                static TFVertexLayout animatedVertexLayout = {};
                memcpy(&animatedVertexLayout, &defaultVertexLayout, sizeof(TFVertexLayout));
                animatedVertexLayout.mAttribs[animatedVertexLayout.mAttribCount].mSemantic = TF_SEMANTIC_WEIGHTS;
                animatedVertexLayout.mAttribs[animatedVertexLayout.mAttribCount].mFormat = TinyImageFormat_R32G32B32A32_SFLOAT;
                animatedVertexLayout.mAttribs[animatedVertexLayout.mAttribCount].mBinding = animatedVertexLayout.mAttribCount;
                animatedVertexLayout.mAttribs[animatedVertexLayout.mAttribCount].mLocation = animatedVertexLayout.mAttribCount;
                animatedVertexLayout.mAttribs[animatedVertexLayout.mAttribCount + 1].mSemantic = TF_SEMANTIC_JOINTS;
                animatedVertexLayout.mAttribs[animatedVertexLayout.mAttribCount + 1].mFormat = TinyImageFormat_R16G16B16A16_UINT;
                animatedVertexLayout.mAttribs[animatedVertexLayout.mAttribCount + 1].mBinding = animatedVertexLayout.mAttribCount + 1;
                animatedVertexLayout.mAttribs[animatedVertexLayout.mAttribCount + 1].mLocation = animatedVertexLayout.mAttribCount + 1;
                animatedVertexLayout.mAttribCount += 2;
                animatedVertexLayout.mBindingCount += 2;
                pLoadDesc->pVertexLayout = &animatedVertexLayout;
            }

            pLoadDesc->mPackagePtr.pPackage = pOutPackage->pPackageHandle;
            pLoadDesc->mPackagePtr.mDataID = (uint32_t)pOutPackage->pPackageMetadata->pGeoNodeIds[geoIndex];
            pLoadDesc->mLoadFromPackage = true;
            if (pDesc->pGeometryBuffer != NULL)
            {
                pLoadDesc->pGeometryBuffer = pDesc->pGeometryBuffer;
                pLoadDesc->pGeometryBufferLayoutDesc = pDesc->pGeometryBufferLayout;
            }
            pLoadDesc->pOnGeometryLoaded = pDesc->pOnGeometryLoaded;
            pLoadDesc->pOnGeometryLoadedUserData = pDesc->pOnGeometryLoadedUserData;
            if (pDesc->loadGeoData)
            {
                ASSERT(allGeo != NULL && allGeoData != NULL);
                pLoadDesc->ppGeometry = allGeo + geoIndex;
                pLoadDesc->ppGeometryData = allGeoData + geoIndex;
                TFSyncToken token = {};
                addResource(pLoadDesc, &token);
                waitForToken(&token);
            }
        }

        if (pDesc->createAtlas)
        {
            const uint32_t atlasTexturesNodeID = (uint32_t)pOutPackage->pPackageMetadata->mTextureAtlasNodeID;
            TFFileStream   atlasTexturesStream = { 0 };
            if (!fsIoOpenByUid(pOutPackage->pPackageHandle, atlasTexturesNodeID, TF_FM_READ, &atlasTexturesStream))
            {
                LOGF(LogLevel::eERROR, "Failed to read atlas textures from archive");
                loadSuccess = false;
            }
            else
            {
                ...
                size_t      readSize = 0;
                readSize += fsReadFromStream(&atlasTexturesStream, &pOutPackage->mAtlasCount, sizeof(uint32_t));
                size_t atlasDescsSize = sizeof(TFAtlasTextureDesc) * pOutPackage->mAtlasCount;
                pOutPackage->pAtlasDescriptors = (TFAtlasTextureDesc*)tf_malloc(atlasDescsSize);
                readSize += fsReadFromStream(&atlasTexturesStream, pOutPackage->pAtlasDescriptors, atlasDescsSize);

                readSize += fsReadFromStream(&atlasTexturesStream, &pOutPackage->mAtlasPlacementsCount, sizeof(uint32_t));
                size_t atlasPlacementsSize = sizeof(TFAtlasPlacement) * pOutPackage->mAtlasPlacementsCount;
                size_t expectedReadSize = sizeof(uint32_t) + sizeof(uint32_t) + atlasPlacementsSize + atlasDescsSize;
                pOutPackage->pAtlasPlacements = (TFAtlasPlacement*)tf_malloc(atlasPlacementsSize);
                readSize += fsReadFromStream(&atlasTexturesStream, pOutPackage->pAtlasPlacements, atlasPlacementsSize);
                if (readSize != expectedReadSize)
                {
                    LOGF(LogLevel::eERROR, "Error in reading Atlas Textures.");
                    loadSuccess = false;
                }

                pOutPackage->ppAtlasTextures = (TFTexture**)tf_calloc(pOutPackage->mAtlasCount, sizeof(TFTexture*));

                // create the set of atlases , one per resolution
                for (uint32_t i = 0; i < pOutPackage->mAtlasCount; i++)
                {
                    TFAtlasTextureDesc* currAtlas = pOutPackage->pAtlasDescriptors + i;
                    TFTextureDesc       textureDesc = {};
                    textureDesc.mArraySize = currAtlas->mSliceCount;
                    textureDesc.mMipLevels = currAtlas->mMipLevels;
                    textureDesc.mWidth = currAtlas->mWidth;
                    textureDesc.mHeight = currAtlas->mHeight;
                    textureDesc.mDepth = 1;
                    textureDesc.mFormat = currAtlas->mFormat;
                    textureDesc.mDescriptors = TF_DESCRIPTOR_TYPE_TEXTURE;
                    textureDesc.mSampleCount = TF_SAMPLE_COUNT_1;
                    {
                        static char atlasName[TF_MAX_DEBUG_NAME_LENGTH];
                        snprintf(atlasName, sizeof(atlasName), "Atlas Texture %u (%ux%u) format: %u", i, currAtlas->mWidth,
                                 currAtlas->mHeight, (uint32_t)currAtlas->mFormat);
                        textureDesc.pName = atlasName;
                    }
                    textureDesc.mFlags = TF_TEXTURE_CREATION_FLAG_FORCE_ARRAY;
                    textureDesc.mStartState = TF_RESOURCE_STATE_COPY_DEST;
                    TFTextureLoadDesc atlasTextureDesc = {};
                    atlasTextureDesc.pDesc = &textureDesc;
                    atlasTextureDesc.ppTexture = &pOutPackage->ppAtlasTextures[i];
                    addResource(&atlasTextureDesc, NULL);
                }
                fsCloseStream(&atlasTexturesStream);

                // Create the buffer that contains the atlas placements
                uint32_t         shaderPlacementsCount = (uint32_t)(pOutPackage->mAtlasPlacementsCount - totalTextureCount);
                TFBufferLoadDesc atlasPlacementsDesc = {};
                atlasPlacementsDesc.mDesc.mDescriptors = TF_DESCRIPTOR_TYPE_BUFFER;
                atlasPlacementsDesc.mDesc.mMemoryUsage = TF_RESOURCE_MEMORY_USAGE_GPU_ONLY;
                atlasPlacementsDesc.mDesc.mElementCount = shaderPlacementsCount;
                atlasPlacementsDesc.mDesc.mStructStride = sizeof(TFAtlasPlacement);
                atlasPlacementsDesc.mDesc.mSize = atlasPlacementsDesc.mDesc.mElementCount * atlasPlacementsDesc.mDesc.mStructStride;
                atlasPlacementsDesc.pData = pOutPackage->pAtlasPlacements + totalTextureCount;
                atlasPlacementsDesc.ppBuffer = &pOutPackage->pAtlasPlacementsBuffer;
                atlasPlacementsDesc.mDesc.pName = "Atlas placements Desc";

                addResource(&atlasPlacementsDesc, &waitToken);
            }
        }

        pOutPackage->pTextureData = (TFTextureLoadDesc*)tf_calloc(totalTextureCount, sizeof(TFTextureLoadDesc));
        TFTexture**    allTextures = NULL;
        TFSyncToken*   loadTokens = NULL;
        TFTexture**    orderedTextures = NULL;
        const uint32_t texturesPerMesh = 3;
        if (pDesc->loadTexData)
        {
            allTextures = (TFTexture**)tf_calloc(totalTextureCount, sizeof(TFTexture*));
            loadTokens = (TFSyncToken*)tf_calloc(totalTextureCount, sizeof(TFSyncToken));
            orderedTextures = (TFTexture**)tf_calloc(totalMeshCount * texturesPerMesh, sizeof(TFTexture*));
            if (allTextures != NULL && loadTokens != NULL && orderedTextures != NULL)
            {
                pOutPackage->loadTexData = true;
            }
            else
            {
                pOutPackage->loadTexData = false;
                SAFE_FREE(orderedTextures);
                SAFE_FREE(loadTokens);
                SAFE_FREE(allTextures);
            }
        }

        if (pOutPackage->ppAtlasTextures)
        {
            TFSyncToken token = {};
            for (uint32_t texIndex = 0; texIndex < totalTextureCount; texIndex++)
            {
                TFAtlasPlacement*  pPlacement = pOutPackage->pAtlasPlacements + texIndex;
                TFTextureLoadDesc* pLoadDesc = pOutPackage->pTextureData + pPlacement->mTextureIndex;
                pLoadDesc->mPackagePtr.pPackage = pOutPackage->pPackageHandle;
                pLoadDesc->mPackagePtr.mDataID = (uint32_t)pOutPackage->pPackageMetadata->pTextureStartNodeIds[0] + texIndex;
                pLoadDesc->mLoadFromPackage = true;
                if (pOutPackage->loadTexData)
                {
                    pLoadDesc->pAtlasPlacement = pPlacement;
                    uint32_t atlasIndex = pLoadDesc->pAtlasPlacement->mAtlasIndex;
                    pLoadDesc->pAtlasTexture = pOutPackage->ppAtlasTextures[atlasIndex];
                    pLoadDesc->ppTexture = &pLoadDesc->pAtlasTexture;
                    addResource(pLoadDesc, &token);
                }
            }
            pOutPackage->loadTexData = false;
        ...
        else
        {
            ...
            for (uint32_t geoIndex = 0; geoIndex < pOutPackage->pPackageMetadata->mGeoCount; geoIndex++)
            {
                for (uint32_t texIndex = 0; texIndex < pOutPackage->pTextureMetadata[geoIndex].mTextureCount; texIndex++)
                {
                    TFTextureLoadDesc* pLoadDesc = pOutPackage->pTextureData + texOffset;
                    pLoadDesc->mPackagePtr.pPackage = pOutPackage->pPackageHandle;
                    pLoadDesc->mPackagePtr.mDataID = (uint32_t)pOutPackage->pPackageMetadata->pTextureStartNodeIds[geoIndex] + texIndex;
                    pLoadDesc->mLoadFromPackage = true;
                    if (pOutPackage->loadTexData)
                    {
                        ASSERT(allTextures != NULL);
                        ASSERT(loadTokens != NULL);
                        pLoadDesc->ppTexture = allTextures + texOffset;
                        addResource(pLoadDesc, loadTokens + texOffset);
                    }
                    texOffset++;
                }
            }
            ...
            if (pOutPackage->loadTexData)
            {
                ASSERT(orderedTextures != NULL);
                ASSERT(loadTokens != NULL);
                // Assign the texture pointers to the right order specified in the TFTextureBundleID
                uint32_t orderedTexOffset = 0;
                for (uint32_t geoIndex = 0; geoIndex < pOutPackage->pPackageMetadata->mGeoCount; geoIndex++)
                {
                    for (uint32_t meshIndex = 0; meshIndex < pOutPackage->pTextureMetadata[geoIndex].mMeshCount; meshIndex++)
                    {
                        TFTextureBundleID texBundleID = pOutPackage->pTextureMetadata[geoIndex].pTextureBundleIDs[meshIndex];
                        TFTextureID       colorTexID = GET_COLOR_TEXID(texBundleID);
                        if (colorTexID != TEX_IDX_INVALID)
                        {
                            if (isTokenCompleted(loadTokens + colorTexID))
                            {
                                orderedTextures[orderedTexOffset++] = pOutPackage->pTextureData->ppTexture[colorTexID];
                            }
                            else
                            {
                                waitForToken(loadTokens + colorTexID);
                                orderedTextures[orderedTexOffset++] = pOutPackage->pTextureData->ppTexture[colorTexID];
                            }
                        }
                        else
                        {
                            orderedTextures[orderedTexOffset++] = pNullTextureResource;
                        }

                        TFTextureID normTexID = GET_NORMAL_TEXID(texBundleID);
                        if (normTexID != TEX_IDX_INVALID)
                        {
                            if (isTokenCompleted(loadTokens + normTexID))
                            {
                                orderedTextures[orderedTexOffset++] = pOutPackage->pTextureData->ppTexture[normTexID];
                            }
                            else
                            {
                                waitForToken(loadTokens + normTexID);
                                orderedTextures[orderedTexOffset++] = pOutPackage->pTextureData->ppTexture[normTexID];
                            }
                        }
                        else
                        ...
                        if (specTexID != TEX_IDX_INVALID)
                        {
                            if (isTokenCompleted(loadTokens + specTexID))
                            {
                                orderedTextures[orderedTexOffset++] = pOutPackage->pTextureData->ppTexture[specTexID];
                            }
                            else
                            {
                                waitForToken(loadTokens + specTexID);
                                orderedTextures[orderedTexOffset++] = pOutPackage->pTextureData->ppTexture[specTexID];
                            }
                        }
                        else
                ...
                pOutPackage->ppAllTextures = orderedTextures;
    ...
    if (loadSuccess)
    {
        *pDesc->ppOutPackage = pOutPackage;
    }
    else
    {
        *pDesc->ppOutPackage = NULL;
    }
    return loadSuccess;
}

void removeResourcesFromPackage(TFPackage* pPackage)
{
    if (!pPackage)
        return;

    size_t         totalTextureCount = 0;
    TFPackageDesc* pPackageGeoMetadata = pPackage->pPackageMetadata;
    if (pPackageGeoMetadata)
    {
        for (uint32_t geoDataIndex = 0; geoDataIndex < pPackageGeoMetadata->mGeoCount; geoDataIndex++)
        {
            TFGeometryLoadDesc* pPackagedGeoData = pPackage->pGeoData + geoDataIndex;
            if (pPackage->loadGeoData && pPackagedGeoData)
            {
                if (pPackagedGeoData->ppGeometry)
                {
                    removeResource(*pPackagedGeoData->ppGeometry);
                }
                if (pPackagedGeoData->ppGeometryData)
                {
                    removeResource(*pPackagedGeoData->ppGeometryData);
                }
            }

            if (pPackage->pTextureMetadata)
            {
                TFTexturePackagedDesc* pPackageTextureMetadata = &pPackage->pTextureMetadata[geoDataIndex];
                totalTextureCount += pPackageTextureMetadata->mTextureCount;
                SAFE_FREE(pPackageTextureMetadata->pTextureNames);
                SAFE_FREE(pPackageTextureMetadata->pStreamZones);
            }
        }
        if (pPackage->loadGeoData && pPackage->pGeoData) // If false, then the package doesn't own this data
        {
            SAFE_FREE(pPackage->pGeoData->ppGeometry);
            SAFE_FREE(pPackage->pGeoData->ppGeometryData);
        }
    }

    if (pPackage->pTextureMetadata)
    {
        // The TextureBundleIDs and TFMaterialProps are single buffers so just one free is required.
        SAFE_FREE(pPackage->pTextureMetadata->pTextureBundleIDs);
        SAFE_FREE(pPackage->pTextureMetadata->pMaterialProps);
    }
    SAFE_FREE(pPackage->ppAllTextures);
    SAFE_FREE(pPackage->pTextureMetadata);

    SAFE_FREE(pPackage->pAtlasPlacements);
    SAFE_FREE(pPackage->pAtlasDescriptors);
    if (pPackageGeoMetadata)
    {
        SAFE_FREE(pPackageGeoMetadata->pGeoNodeIds);
        SAFE_FREE(pPackageGeoMetadata->pTexMetadataNodeIds);
        SAFE_FREE(pPackageGeoMetadata->pAnimationNodeIDRanges);
        SAFE_FREE(pPackageGeoMetadata->pTextureStartNodeIds);
        SAFE_FREE(pPackageGeoMetadata->pGeoNames);
    }
    SAFE_FREE(pPackage->pGeoData);
    SAFE_FREE(pPackageGeoMetadata);

    TFTextureLoadDesc* pPackagedTextureData = pPackage->pTextureData;
    if (pPackagedTextureData)
    {
        if (pPackage->ppAtlasTextures)
        {
            for (uint32_t i = 0; i < pPackage->mAtlasCount; i++)
            {
                removeResource(pPackage->ppAtlasTextures[i]);
            }
            tf_free(pPackage->ppAtlasTextures);
            removeResource(pPackage->pAtlasPlacementsBuffer);
        }
        else
        {
            for (uint32_t texIndex = 0; texIndex < totalTextureCount; texIndex++)
            {
                TFTextureLoadDesc* pTextureData = pPackagedTextureData + texIndex;
                if (pPackage->loadTexData && pTextureData->ppTexture)
                {
                    removeResource(*pTextureData->ppTexture);
                }
            }
        }
        if (pPackage->loadTexData) // If false, then the package doesn't own this data
        {
            SAFE_FREE(pPackagedTextureData->ppTexture);
        }
    }
    SAFE_FREE(pPackagedTextureData);

    fsArchiveClose(pPackage->pPackageHandle);
    SAFE_FREE(pPackage->pPackageHandle);
    SAFE_FREE(pPackage);
}
