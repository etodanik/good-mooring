...
public:
    ...
    inline Matrix4 GetJointWorldMatNoScale(uint32_t index) const
    {
        ...
        vec4 col0 = vec4(normalize(withScale.v[0].getXYZ()), withScale.v[0].w);
        vec4 col1 = vec4(normalize(withScale.v[1].getXYZ()), withScale.v[1].w);
        vec4 col2 = vec4(normalize(withScale.v[2].getXYZ()), withScale.v[2].w);

        mat4 withoutScale = mat4(col0, col1, col2, withScale[3]);
        return withoutScale;
    }
