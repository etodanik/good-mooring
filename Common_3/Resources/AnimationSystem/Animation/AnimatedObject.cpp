...
void AnimatedObject::Initialize(Rig* rig, Animation* animation)
{
    ...
    {
        mLocalTrans[i] = soaTransformIdentity();
    }
}
...
void AnimatedObject::ComputeJointScales(const Matrix4& rootTransform)
{
    ...
    for (uint32_t childIndex = 0; childIndex < mRig->mNumJoints; childIndex++)
    {
        ...
        vec3  boneDir = childMat.v[3].getXYZ() - parentMat.v[3].getXYZ();
...
bool AnimatedObject::AimIK(AimIKDesc* params, const Point3& target)
{
         ...
         ++i, previous_joint = joint, joint = params->mJointChain[i])
    {
        ...
        {
            const Vector3 corrected_forward_ms((models[previous_joint] * quatRotateVector(correction, ikJob.forward)).getXYZ());
            const Point3 corrected_offset_ms((models[previous_joint] * Vector4(quatRotateVector(correction, ikJob.offset), 1.0f)).getXYZ());

            Matrix4 inv_joint = inverse(models[joint]);
            ikJob.forward = (inv_joint * Vector4(corrected_forward_ms, 0.0f)).getXYZ();
            ikJob.offset = (inv_joint * Vector4(corrected_offset_ms, 1.0f)).getXYZ();
        }
