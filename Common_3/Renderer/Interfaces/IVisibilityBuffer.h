...
{
    TFBuffer*  pVBConstantBuffer;
    TFBuffer** ppFilteredIndexBuffer;
    TFBuffer** ppIndirectDataBuffer;
    TFBuffer** ppFilterDispatchGroupDataBuffer; // Per batch indirection data, uint32_t per each batch. Stores material id, geometry set and
                                                // the maximum number of triangles in a batch.
    TFBuffer** ppIndirectDrawArgBuffer;
    VBConstants*    pVBConstants;
    TFResourceHeap* pFilteredIndexBufferHeap;
...
FORGE_RENDERER_API bool initVisibilityBuffer(TFRenderer* pRenderer, const VisibilityBufferDesc* pDesc,
                                             VisibilityBuffer** ppVisibilityBuffer);
...
{ //-V802
    TFPipeline* pPipelineClearBuffers;
    TFPipeline* pPipelineTriangleFiltering;

    TFDescriptorSet* pDescriptorSetTriangleFilteringPerBatch;
...
FORGE_RENDERER_API void cmdVBTriangleFilteringPass(VisibilityBuffer* pVisibilityBuffer, TFCmd* pCmd, TriangleFilteringPassDesc* pDesc);
...
typedef struct PreSkinACAliasedBuffer
{
    ...
    TFResourcePlacement mVBPlacement; // Placement that allocates all memory used by the main Vertex Buffer
    ...
    TFBuffer* pPreSkinBuffers[VISIBILITY_BUFFER_MAX_NUM_BUFFERS];
...
typedef struct PreSkinACVertexBuffers
{
    ...
    TFBuffer* pShaderOffsets[VISIBILITY_BUFFER_MAX_NUM_BUFFERS];
} PreSkinACVertexBuffers;

FORGE_RENDERER_API void initVBAsyncComputePreSkinVertexBuffers(TFRenderer* pRenderer, PreSkinACVertexBuffersDesc* pDesc,
                                                               PreSkinACVertexBuffers** pOut);
FORGE_RENDERER_API void exitVBAsyncComputePreSkinVertexBuffers(TFRenderer* pRenderer, PreSkinACVertexBuffers* pBuffers);
...
typedef struct PreSkinVertexesPassDesc
{
    ...
    TFPipeline*      pPipelinePreSkinVertexes;
    TFDescriptorSet* pDescriptorSetPreSkinVertexes;
    TFDescriptorSet* pDescriptorSetPreSkinVertexesPerFrame;
    TFDescriptorSet* pDescriptorSetPreSkinVertexesPerDraw;

    TFBuffer**              ppPreSkinOutputVertexBuffers;
...
FORGE_RENDERER_API PreSkinVertexesStats cmdVisibilityBufferPreSkinVertexesPass(VisibilityBuffer* pVisibilityBuffer, TFCmd* pCmd,
