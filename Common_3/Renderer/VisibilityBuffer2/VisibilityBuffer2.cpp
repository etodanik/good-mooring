...
#include "Shaders/FSL/VisibilityBufferStructs.h.fsl"
...
static void DispatchFilterTriangles(TFCmd* pCmd, FilterBatchChunk* pBatchChunk, GPURingBufferOffset* ringBufferOffset,
...
FilteringStats cmdVisibilityBufferTriangleFilteringPass(VisibilityBuffer* pVisibilityBuffer, TFCmd* pCmd, TriangleFilteringPassDesc* pDesc)
{
    ...
    bufferIndex = 0;
    barrier[bufferIndex++] = { pVisibilityBuffer->ppBinBuffer[pDesc->mBuffersIndex], TF_RESOURCE_STATE_UNORDERED_ACCESS,
...
static TFBufferLoadDesc MainVertexBufferLoadDesc(PreSkinACVertexBuffersDesc* pDesc, uint32_t structStride)
{
    TFBufferLoadDesc vertexBufferDesc = {};
    vertexBufferDesc.mDesc.mDescriptors =
    ...
    vertexBufferDesc.mDesc.mMemoryUsage = TF_RESOURCE_MEMORY_USAGE_GPU_ONLY;
...
void initVBAsyncComputePreSkinVertexBuffers(TFRenderer* pRenderer, PreSkinACVertexBuffersDesc* pDesc, PreSkinACVertexBuffers** pOut)
{
    ...
    TFResourceSizeAlign sizeAligns[3] = {};
    ...
    // This memory includes extra memory for pre-skinned vertex output.
    {
        ...
        const TFBufferLoadDesc VBDescs[] = {
...
bool initVisibilityBuffer(TFRenderer* pRenderer, const VisibilityBufferDesc* pDesc, VisibilityBuffer** ppVisibilityBuffer)
{
    ...
    pVisibilityBuffer->ppBinBuffer = (TFBuffer**)tf_malloc(sizeof(TFBuffer*) * pDesc->mNumBuffers);

    TFBufferLoadDesc filterIbDesc = {};
    filterIbDesc.mDesc.mDescriptors = TF_DESCRIPTOR_TYPE_BUFFER | TF_DESCRIPTOR_TYPE_RW_BUFFER;
    filterIbDesc.mDesc.mMemoryUsage = TF_RESOURCE_MEMORY_USAGE_GPU_ONLY;
    filterIbDesc.mDesc.mElementCount = BinBufferViewOffset(pDesc->mNumViews * MAX_BINNING_DISPATCHES);
