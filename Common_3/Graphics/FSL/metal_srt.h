...
#define END_SRT(name)                                                         \
    ...
    ;                                                                         \
    static const TFMetalDescriptorSet* getSrt##name##Ptr()                      \
    {                                                                         \
        static SRT_##name   srt = {};                                         \
        TFMetalDescriptorSet* destSet = (TFMetalDescriptorSet*)((uint64_t*)&srt); \
        return &destSet[0];                                                   \
    }

#define SRT_SET_COUNT(name) (sizeof(name) / sizeof(TFMetalDescriptorSet))
...
#define END_SRT_SET(freq)                                                              \
    ...
    *p##freq;                                                                          \
    ...
    {                                                                                  \
        ...
        static freq layout = {};                                                       \
        TFDescriptor* desc = (TFDescriptor*)((uint64_t*)&layout);                          \
        return &desc[0];                                                               \
    }                                                                                  \
    const uint32_t    mResourceCount##freq = sizeof(struct freq) / sizeof(TFDescriptor); \
    const TFDescriptor* pDescriptors##freq = freq##Ptr();

#define SRT_RES_IDX(srt, freq, name) (offsetof(SRT_##srt::freq, name) / sizeof(TFDescriptor))
#define SRT_SET_DESC(srt, freq, maxSets, nodeIndex)                                                                                 \
    {                                                                                                                               \
        (offsetof(SRT_##srt, p##freq) / ((uint32_t)(sizeof(TFMetalDescriptorSet)))), (maxSets), (nodeIndex),                          \
            (sizeof(SRT_##srt::freq) / sizeof(TFDescriptor)), SRT_##srt::freq##Ptr(), SET_##freq, 0, getSrt##srt##Ptr(), SRT_SET_COUNT(SRT_##srt) \
    }

#define SRT_SET_DESC_LARGE_RW(srt, freq, maxSets, nodeIndex)                                                                        \
    {                                                                                                                               \
        (offsetof(SRT_##srt, p##freq) / ((uint32_t)(sizeof(TFMetalDescriptorSet)))), (maxSets), (nodeIndex),                          \
            (sizeof(SRT_##srt::freq) / sizeof(TFDescriptor)), SRT_##srt::freq##Ptr(), SET_##freq, 1, getSrt##srt##Ptr(), SRT_SET_COUNT(SRT_##srt) \
    }

#define IS_AB_RES(is_ab) (is_ab & (isParamsOnly() ? 0u : 1u))

#define DECL_SAMPLER(freq, type, name) \
    const TFDescriptor name = { IF_VALIDATE_DESCRIPTOR(#name, SET_##freq, ) TF_DESCRIPTOR_TYPE_SAMPLER, 1, 0, IS_AB_RES(0) };
#define DECL_CBUFFER(freq, type, name) \
    const TFDescriptor name = { IF_VALIDATE_DESCRIPTOR(#name, SET_##freq, ) TF_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1, 0, IS_AB_RES(1) };
#define DECL_TEXTURE(freq, type, name) \
    const TFDescriptor name = { IF_VALIDATE_DESCRIPTOR(#name, SET_##freq, ) TF_DESCRIPTOR_TYPE_TEXTURE, 1, 0, IS_AB_RES(1) };

#define DECL_ARRAY_TEXTURES(freq, type, name, count) \
    const TFDescriptor name = { IF_VALIDATE_DESCRIPTOR(#name, SET_##freq, ) TF_DESCRIPTOR_TYPE_TEXTURE, count, 0, IS_AB_RES(1) };

#define DECL_BUFFER(freq, type, name) \
    const TFDescriptor name = { IF_VALIDATE_DESCRIPTOR(#name, SET_##freq, ) TF_DESCRIPTOR_TYPE_BUFFER, 1, 0, IS_AB_RES(1) };
#define DECL_RAYTRACING(freq, type, name) \
    const TFDescriptor name = { IF_VALIDATE_DESCRIPTOR(#name, SET_##freq, ) TF_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE, 1, 0, IS_AB_RES(1) };
#define DECL_RWTEXTURE(freq, type, name) \
    const TFDescriptor name = { IF_VALIDATE_DESCRIPTOR(#name, SET_##freq, ) TF_DESCRIPTOR_TYPE_RW_TEXTURE, 1, 0, IS_AB_RES(0) };
#define DECL_WTEXTURE(freq, type, name) \
    const TFDescriptor name = { IF_VALIDATE_DESCRIPTOR(#name, SET_##freq, ) TF_DESCRIPTOR_TYPE_RW_TEXTURE, 1, 0, IS_AB_RES(0) };
#define DECL_RWBUFFER(freq, type, name) \
    const TFDescriptor name = { IF_VALIDATE_DESCRIPTOR(#name, SET_##freq, ) TF_DESCRIPTOR_TYPE_RW_BUFFER, 1, 0, IS_AB_RES(0) };

#define DECL_ARRAY_RWTEXTURES(freq, type, name, count) \
    const TFDescriptor name = { IF_VALIDATE_DESCRIPTOR(#name, SET_##freq, ) TF_DESCRIPTOR_TYPE_RW_TEXTURE, count, 0, IS_AB_RES(0) };

#define DECL_ARRAY_RWBUFFERS(freq, type, name, count) \
    const TFDescriptor name = { IF_VALIDATE_DESCRIPTOR(#name, SET_##freq, ) TF_DESCRIPTOR_TYPE_RW_BUFFER, count, 0, IS_AB_RES(0) };
