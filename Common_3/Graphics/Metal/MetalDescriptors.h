...
static void metalInitTextureDescriptor(const TFTextureDescriptorDesc* desc, TFTextureDescriptor* view)
{
    ...
    if (desc->mIsStencil)
    {
        metalFormat = texture->pTexture.pixelFormat == MTLPixelFormatDepth32Float_Stencil8
            ? MTLPixelFormatX32_Stencil8 : MTLPixelFormatStencil8;
    }
    ...
    view->pSrvUav = [texture->pTexture newTextureViewWithPixelFormat:metalFormat textureType:type
        levels:NSMakeRange(desc->mBaseMipLevel, mipCount) slices:NSMakeRange(desc->mBaseArrayLayer, layerCount)];
    view->mFormat = format;
    ASSERT(view->pSrvUav);
}
