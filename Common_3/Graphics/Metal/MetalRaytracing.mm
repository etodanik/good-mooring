...
void add_texture(TFRenderer* pRenderer, const TFTextureDesc* pDesc, TFTexture** pTexture, const bool isRT);
...
struct IOS17_API TFAccelerationStructure
{
...
bool initRaytracing(TFRenderer* pRenderer, TFRaytracing** ppRaytracing)
{
    ...
    TFRaytracing* pRaytracing = (TFRaytracing*)tf_calloc(1, sizeof(TFRaytracing));
    ASSERT(pRaytracing);
...
IOS17_API
static inline FORGE_CONSTEXPR MTLAccelerationStructureInstanceOptions ToMTLASOptions(TFAccelerationStructureInstanceFlags flags)
{
                              ...
                              TFAccelerationStructure** ppAccelerationStructure)
{
    ...
    if (TF_ACCELERATION_STRUCTURE_TYPE_TOP == pDesc->mType)
    {
    ...
    if (TF_ACCELERATION_STRUCTURE_TYPE_BOTTOM == pDesc->mType)
    {
        ...
        for (uint32_t j = 0; j < pAS->mDescCount; ++j)
        {
            ...
            if (pGeom->mIndexCount)
            {
                ...
                geomDesc.indexType = (pGeom->mIndexType == TF_INDEX_TYPE_UINT16 ? MTLIndexTypeUInt16 : MTLIndexTypeUInt32);
    ...
    else
    {
        ...
        instanceDesc.mMemoryUsage = TF_RESOURCE_MEMORY_USAGE_CPU_TO_GPU;
        instanceDesc.mFlags = TF_BUFFER_CREATION_FLAG_PERSISTENT_MAP_BIT;
    ...
    scratchBufferDesc.mDescriptors = TF_DESCRIPTOR_TYPE_RW_BUFFER;
    scratchBufferDesc.mMemoryUsage = TF_RESOURCE_MEMORY_USAGE_GPU_ONLY;
    scratchBufferDesc.mStartState = TF_RESOURCE_STATE_COMMON;
    scratchBufferDesc.mFlags = TF_BUFFER_CREATION_FLAG_NO_DESCRIPTOR_VIEW_CREATION;
...
void removeAccelerationStructure(TFRaytracing* pRaytracing, TFAccelerationStructure* pAccelerationStructure)
{
    ...
    if (TF_ACCELERATION_STRUCTURE_TYPE_TOP == pAccelerationStructure->mType)
    {
...
void cmdBuildAccelerationStructure(TFCmd* pCmd, TFRaytracing* pRaytracing, TFRaytracingBuildASDesc* pDesc)
{
    ...
    if (TF_ACCELERATION_STRUCTURE_TYPE_BOTTOM == as->mType)
    {
...
IOS17_API
id<MTLAccelerationStructure> getMTLAccelerationStructure(TFAccelerationStructure* pAccelerationStructure)
{
    return pAccelerationStructure->mAS;
}

IOS17_API
void getMTLAccelerationStructureBottomReferences(TFAccelerationStructure* pAccelerationStructure, uint32_t* pOutReferenceCount,
                                                 NOREFS id** pOutReferences)
{
    ASSERT(TF_ACCELERATION_STRUCTURE_TYPE_TOP == pAccelerationStructure->mType);
...
{
    TFSSVGFDenoiser* denoiser = (TFSSVGFDenoiser*)tf_calloc(1, sizeof(TFSSVGFDenoiser));
                     ...
                     TFTexture* pDepthNormalTexture, TFTexture* pPreviousDepthNormalTexture, TFTexture** ppOut)
{
    ...
    TFTextureDesc resultTextureDesc = {};
    resultTextureDesc.mFlags = TF_TEXTURE_CREATION_FLAG_NONE;
    ...
    resultTextureDesc.mSampleCount = TF_SAMPLE_COUNT_1;
    ...
    resultTextureDesc.mDescriptors = TF_DESCRIPTOR_TYPE_TEXTURE;
    resultTextureDesc.mStartState = TF_RESOURCE_STATE_UNORDERED_ACCESS;
    resultTextureDesc.pNativeHandle = &handle;

    resultTextureDesc.mDescriptors = TF_DESCRIPTOR_TYPE_TEXTURE | TF_DESCRIPTOR_TYPE_RW_TEXTURE;
