...
inline void mtlCapsBuilder(id<MTLDevice> pDevice, TFGpuDesc* pGpuDesc)
{
...
#ifndef TARGET_IOS
            pGpuDesc->mFormatCaps[i] = TinyImageFormat_MTLPixelFormatOnMac(mtlFmt) ? TF_FORMAT_CAP_READ : TF_FORMAT_CAP_NONE;
#else
            pGpuDesc->mFormatCaps[i] = TinyImageFormat_MTLPixelFormatOnIOS(mtlFmt) ? TF_FORMAT_CAP_READ : TF_FORMAT_CAP_NONE;
#endif
        }
        else
        {
            pGpuDesc->mFormatCaps[i] = TF_FORMAT_CAP_NONE;
        }
    }

#define CAN_SHADER_SAMPLE_LINEAR(x) pGpuDesc->mFormatCaps[TinyImageFormat_##x] |= TF_FORMAT_CAP_LINEAR_FILTER;
#define CAN_SHADER_WRITE(x)         pGpuDesc->mFormatCaps[TinyImageFormat_##x] |= TF_FORMAT_CAP_WRITE;
#define CAN_RENDER_TARGET_WRITE(x)  pGpuDesc->mFormatCaps[TinyImageFormat_##x] |= TF_FORMAT_CAP_RENDER_TARGET;
#define CAN_SHADER_READ_WRITE(x)    pGpuDesc->mFormatCaps[TinyImageFormat_##x] |= TF_FORMAT_CAP_READ_WRITE;
