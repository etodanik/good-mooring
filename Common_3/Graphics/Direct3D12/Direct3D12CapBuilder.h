...
inline void d3d12CapsBuilder(ID3D12Device* pDevice, TFGpuDesc* pGpuDesc)
{
    ...
    for (uint32_t i = 0; i < TinyImageFormat_Count; ++i)
    {
        ...
        {
            pGpuDesc->mFormatCaps[i] |= TF_FORMAT_CAP_LINEAR_FILTER | TF_FORMAT_CAP_READ;
        }
        if (formatSupport.Support2 & D3D12_FORMAT_SUPPORT2_UAV_TYPED_STORE)
        {
            pGpuDesc->mFormatCaps[i] |= TF_FORMAT_CAP_WRITE;
        }
        if (formatSupport.Support2 & loadStore)
        {
            pGpuDesc->mFormatCaps[i] |= TF_FORMAT_CAP_READ_WRITE;
        }
        if (formatSupport.Support1 & D3D12_FORMAT_SUPPORT1_RENDER_TARGET)
        {
            pGpuDesc->mFormatCaps[i] |= TF_FORMAT_CAP_RENDER_TARGET;
        }
        if (formatSupport.Support1 & D3D12_FORMAT_SUPPORT1_DEPTH_STENCIL)
        {
            pGpuDesc->mFormatCaps[i] |= TF_FORMAT_CAP_DEPTH_STENCIL;
        }
        ...
        {
            pGpuDesc->mFormatCaps[i] |= TF_FORMAT_CAP_READ;
        }

        // Depth formats are created as typeless resources and sampled through a view-compatible SRV format
        // Reflect the shader-visible path in mFormatCaps so app-side format selection matches D3D12 behavior
        if (TinyImageFormat_HasDepth((TinyImageFormat)i))
        {
            DXGI_FORMAT srvFormat = util_to_dx12_srv_format(fmt, false);
            if (srvFormat != fmt)
            {
                D3D12_FEATURE_DATA_FORMAT_SUPPORT srvFormatSupport = { srvFormat };
                COM_CALL(CheckFeatureSupport, pDevice, D3D12_FEATURE_FORMAT_SUPPORT, &srvFormatSupport, sizeof(srvFormatSupport));

                if (srvFormatSupport.Support1 & D3D12_FORMAT_SUPPORT1_SHADER_SAMPLE)
                {
                    pGpuDesc->mFormatCaps[i] |= TF_FORMAT_CAP_LINEAR_FILTER | TF_FORMAT_CAP_READ;
                }
                if (srvFormatSupport.Support1 &
                    (D3D12_FORMAT_SUPPORT1_TEXTURE1D | D3D12_FORMAT_SUPPORT1_TEXTURE2D | D3D12_FORMAT_SUPPORT1_TEXTURECUBE))
                {
                    pGpuDesc->mFormatCaps[i] |= TF_FORMAT_CAP_READ;
                }
