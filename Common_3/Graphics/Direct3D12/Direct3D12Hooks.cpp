...
void hook_post_init_renderer(TFRenderer*) {}

void hook_post_remove_renderer(TFRenderer*) {}
...
HRESULT hook_create_copy_cmd(ID3D12Device* pDevice, uint32_t nodeMask, ID3D12CommandAllocator* pAlloc, TFCmd* pCmd)
{
    return pDevice->CreateCommandList(nodeMask, D3D12_COMMAND_LIST_TYPE_COPY, pAlloc, NULL, IID_PPV_ARGS(&pCmd->mDx.pCmdList));
}

void hook_remove_copy_cmd(TFCmd* pCmd)
{
...
HRESULT hook_add_special_resource(TFRenderer* pRenderer, const D3D12_RESOURCE_DESC* pDesc, const D3D12_CLEAR_VALUE*,
...
HRESULT hook_add_special_resource(TFRenderer* pRenderer, const D3D12_RESOURCE_DESC* pDesc, const D3D12_CLEAR_VALUE* pClearValue,
...
uint32_t hook_get_swapchain_image_index(TFSwapChain* pSwapChain) { return pSwapChain->mDx.pSwapChain->GetCurrentBackBufferIndex(); }

HRESULT hook_acquire_next_image(ID3D12Device*, TFSwapChain*) { return S_OK; }

HRESULT hook_queue_present(TFQueue*, TFSwapChain* pSwapChain, uint32_t)
{
...
HRESULT hook_signal(TFQueue* pQueue, ID3D12Fence* pFence, uint64_t fenceValue) { return pQueue->mDx.pQueue->Signal(pFence, fenceValue); }
...
extern void hook_fill_gpu_desc(ID3D12Device* pDevice, D3D_FEATURE_LEVEL featureLevel, DXGPUInfo* pInOutDesc)
{
    ...
    wcstombs_s(&numConverted, gpuDesc.mName, desc.Description, TF_MAX_GPU_VENDOR_STRING_LENGTH);
}
...
void hook_modify_heap_flags(TFDescriptorType, D3D12_HEAP_FLAGS*) {}

void hook_modify_buffer_resource_desc(const TFBufferDesc*, D3D12_RESOURCE_DESC*) {}

void hook_modify_texture_resource_flags(TFTextureCreationFlags, D3D12_RESOURCE_FLAGS*) {}
...
void hook_fill_occlusion_query(TFCmd*, uint32_t) {}
#endif