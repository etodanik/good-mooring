...
/************************************************************************/
D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS util_to_dx_acceleration_structure_build_flags(TFAccelerationStructureBuildFlags flags);
D3D12_RAYTRACING_GEOMETRY_FLAGS                     util_to_dx_geometry_flags(TFAccelerationStructureGeometryFlags flags);
D3D12_RAYTRACING_INSTANCE_FLAGS                     util_to_dx_instance_flags(TFAccelerationStructureInstanceFlags flags);
/************************************************************************/
// Forge Raytracing Implementation using DXR
/************************************************************************/
typedef struct TFRaytracing
{
...
typedef struct TFAccelerationStructure
{
    TFBuffer* pASBuffer;
    TFBuffer* pScratchBuffer;
    union
    {
        struct
        {
...
bool initRaytracing(TFRenderer* pRenderer, TFRaytracing** ppRaytracing)
{
    ...
    TFRaytracing* pRaytracing = (TFRaytracing*)tf_calloc(1, sizeof(*pRaytracing));
    ASSERT(pRaytracing);

    pRaytracing->pRenderer = pRenderer;
    COM_CALL(QueryInterface, pRenderer->mDx.pDevice, &IID_ID3D12Device5_Copy, &pRaytracing->prDevice);
...
static inline FORGE_CONSTEXPR D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE ToDXRASType(TFAccelerationStructureType type)
{
    return TF_ACCELERATION_STRUCTURE_TYPE_BOTTOM == type ? D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL
                                                         : D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL;
}

extern void AddSrv(TFRenderer*, struct DescriptorHeap*, ID3D12Resource*, const D3D12_SHADER_RESOURCE_VIEW_DESC*, DxDescriptorID*);
                              ...
                              TFAccelerationStructure** ppAccelerationStructure)
{
    ...
    size_t memSize = sizeof(TFAccelerationStructure);
    if (TF_ACCELERATION_STRUCTURE_TYPE_BOTTOM == pDesc->mType)
    {
        memSize += pDesc->mBottom.mDescCount * sizeof(D3D12_RAYTRACING_GEOMETRY_DESC);
    }

    TFAccelerationStructure* pAS = (TFAccelerationStructure*)tf_calloc(1, memSize);
    ASSERT(pAS);
    ...
    if (TF_ACCELERATION_STRUCTURE_TYPE_BOTTOM == pDesc->mType)
    {
        ...
        {
            TFAccelerationStructureGeometryDesc* pGeom = &pDesc->mBottom.pGeometryDescs[j];
            ...
            if (pGeom->mIndexCount)
            {
                ...
                pGeomD3D12->Triangles.IndexFormat =
                    (pGeom->mIndexType == TF_INDEX_TYPE_UINT16 ? DXGI_FORMAT_R16_UINT : DXGI_FORMAT_R32_UINT);
            }
        ...
        TFBufferDesc bufferDesc = {
            ...
            .mFlags = TF_BUFFER_CREATION_FLAG_OWN_MEMORY_BIT | TF_BUFFER_CREATION_FLAG_NO_DESCRIPTOR_VIEW_CREATION,
            .mStructStride = 0,
            .mFirstElement = 0,
            .mElementCount = (uint32_t)(info.ResultDataMaxSizeInBytes / sizeof(UINT32)),
            .mSize = info.ResultDataMaxSizeInBytes,
            .mStartState = TF_RESOURCE_STATE_ACCELERATION_STRUCTURE_WRITE,
        };
    ...
    else
    {
        ...
        {
            TFAccelerationStructureInstanceDesc* pInst = &pDesc->mTop.pInstanceDescs[i];
            ASSERT(pInst->pBottomAS);

            const TFBuffer* pASBuffer = pInst->pBottomAS->pASBuffer;
        ...
        TFBufferDesc instanceDesc = {
            .mMemoryUsage = TF_RESOURCE_MEMORY_USAGE_CPU_TO_GPU,
            .mFlags = TF_BUFFER_CREATION_FLAG_PERSISTENT_MAP_BIT,
        ...
        /************************************************************************/
        TFBufferDesc bufferDesc = {
            .mDescriptors = TF_DESCRIPTOR_TYPE_RW_BUFFER_RAW | TF_DESCRIPTOR_TYPE_BUFFER_RAW,
            .mMemoryUsage = TF_RESOURCE_MEMORY_USAGE_GPU_ONLY,
    ...
    TFBufferDesc scratchBufferDesc = {
        .mDescriptors = TF_DESCRIPTOR_TYPE_RW_BUFFER,
        .mMemoryUsage = TF_RESOURCE_MEMORY_USAGE_GPU_ONLY,
...
void cmdBuildAccelerationStructure(TFCmd* pCmd, TFRaytracing* pRaytracing, TFRaytracingBuildASDesc* pDesc)
{
    ...
    COM_CALL(QueryInterface, pCmd->mDx.pCmdList, &IID_ID3D12GraphicsCommandList4_Copy, &dxrCmd);
    ASSERT(dxrCmd);

    TFAccelerationStructure*                           as = pDesc->pAccelerationStructure;
    ...
    {
        TFBufferBarrier barrier = { as->pASBuffer, TF_RESOURCE_STATE_ACCELERATION_STRUCTURE_WRITE,
                                    TF_RESOURCE_STATE_ACCELERATION_STRUCTURE_READ };
...
D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS util_to_dx_acceleration_structure_build_flags(TFAccelerationStructureBuildFlags flags)
{
    ...
    if (flags & TF_ACCELERATION_STRUCTURE_BUILD_FLAG_ALLOW_COMPACTION)
        ret |= D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_ALLOW_COMPACTION;
    if (flags & TF_ACCELERATION_STRUCTURE_BUILD_FLAG_ALLOW_UPDATE)
        ret |= D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_ALLOW_UPDATE;
    if (flags & TF_ACCELERATION_STRUCTURE_BUILD_FLAG_MINIMIZE_MEMORY)
        ret |= D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_MINIMIZE_MEMORY;
    if (flags & TF_ACCELERATION_STRUCTURE_BUILD_FLAG_PERFORM_UPDATE)
        ret |= D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_PERFORM_UPDATE;
    if (flags & TF_ACCELERATION_STRUCTURE_BUILD_FLAG_PREFER_FAST_BUILD)
        ret |= D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_PREFER_FAST_BUILD;
    if (flags & TF_ACCELERATION_STRUCTURE_BUILD_FLAG_PREFER_FAST_TRACE)
...
D3D12_RAYTRACING_GEOMETRY_FLAGS util_to_dx_geometry_flags(TFAccelerationStructureGeometryFlags flags)
{
    ...
    if (flags & TF_ACCELERATION_STRUCTURE_GEOMETRY_FLAG_OPAQUE)
        ret |= D3D12_RAYTRACING_GEOMETRY_FLAG_OPAQUE;
    if (flags & TF_ACCELERATION_STRUCTURE_GEOMETRY_FLAG_NO_DUPLICATE_ANYHIT_INVOCATION)
...
D3D12_RAYTRACING_INSTANCE_FLAGS util_to_dx_instance_flags(TFAccelerationStructureInstanceFlags flags)
{
    ...
    if (flags & TF_ACCELERATION_STRUCTURE_INSTANCE_FLAG_FORCE_OPAQUE)
        ret |= D3D12_RAYTRACING_INSTANCE_FLAG_FORCE_OPAQUE;
    if (flags & TF_ACCELERATION_STRUCTURE_INSTANCE_FLAG_TRIANGLE_CULL_DISABLE)
        ret |= D3D12_RAYTRACING_INSTANCE_FLAG_TRIANGLE_CULL_DISABLE;
    if (flags & TF_ACCELERATION_STRUCTURE_INSTANCE_FLAG_TRIANGLE_FRONT_COUNTERCLOCKWISE)
        ret |= D3D12_RAYTRACING_INSTANCE_FLAG_TRIANGLE_FRONT_COUNTERCLOCKWISE;

    return ret;
}

void fillRaytracingDescriptorHandle(TFAccelerationStructure* pAccelerationStructure, DxDescriptorID* pOutId)
{
