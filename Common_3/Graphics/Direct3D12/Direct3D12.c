...
// add windows macro before including directx header (NTDDI_VERSION, WINVER, _WIN32_WINNT)
#include "../../Application/Config.h"

// Generate the missing IID symbols required for COM interfaces (IID_ID3D12Device, IID_ID3D12Resource, ...)
// This is the official approach demonstrated in the DirectX Samples; previously we relied on #pragma comment(lib, "dxguid.lib").
// Now we ensure they are aligned with the DirectX Agility SDK that we ship with our binaries.
#if !defined(XBOX)
...
#else
#include "../../../Common_3/Graphics/ThirdParty/OpenSource/Direct3d12AgilityPreview/include/d3d12.h"
#include "../../../Common_3/Graphics/ThirdParty/OpenSource/Direct3d12AgilityPreview/include/dxgi1_6.h"
#endif
...
#if defined(TF_ENABLE_GRAPHICS_VALIDATION) && !defined(XBOX)
#include <dxgidebug.h>
#endif
...
#define MAX_OPENXR_LAYERS 16 // should probably be pulled dynamic when device created
#define MAX_OPENXR_VIEWS  2  // same? or just 2 always
...
typedef struct OpenXRPresentDesc
{
    ...
    TFVR2DLayerDesc     m2DLayerSettings;
} OpenXRPresentDesc;

// shared OpenXR functions
extern void                    initXRSession(TFRenderer* pRenderer, TFQueue* pGraphicsQueue);
extern XrSession               getXRSession();
extern XrViewConfigurationView getXRViewConfigView(uint32_t viewIndex);
...
extern XrFrameState*           getXRCurrentFrameState();
...
extern void fillRaytracingDescriptorHandle(TFAccelerationStructure* pAccelerationStructure, DxDescriptorID* pOutId);
#endif
...
// Enabling DRED
#if defined(_WIN32) && defined(_DEBUG) && defined(DRED) && !defined(HOLOLENS2)
#define USE_DRED 1
#endif
...
void cmdUpdateSubresource(TFCmd* pCmd, TFTexture* pTexture, TFBuffer* pSrcBuffer, const SubresourceDataDesc* pSubresourceDesc);
...
static void SetObjectName(ID3D12Object* pObject, const char* pName)
{
    ...
    wchar_t wName[TF_MAX_DEBUG_NAME_LENGTH] = { 0 };
    size_t  numConverted = 0;
    mbstowcs_s(&numConverted, wName, TF_MAX_DEBUG_NAME_LENGTH, pName, TF_MAX_DEBUG_NAME_LENGTH);
    COM_CALL(SetName, pObject, wName);
#endif
}

// clang-format off
D3D12_BLEND_OP gDx12BlendOpTranslator[TF_MAX_BLEND_MODES] =
{
	D3D12_BLEND_OP_ADD,
...
D3D12_BLEND gDx12BlendConstantTranslator[TF_MAX_BLEND_CONSTANTS] =
{
	D3D12_BLEND_ZERO,
	D3D12_BLEND_ONE,
...
D3D12_COMPARISON_FUNC gDx12ComparisonFuncTranslator[TF_MAX_COMPARE_MODES] =
{
...
D3D12_STENCIL_OP gDx12StencilOpTranslator[TF_MAX_STENCIL_OPS] =
{
...
D3D12_CULL_MODE gDx12CullModeTranslator[TF_MAX_CULL_MODES] =
{
...
D3D12_FILL_MODE gDx12FillModeTranslator[TF_MAX_FILL_MODES] =
{
	D3D12_FILL_MODE_SOLID,
	D3D12_FILL_MODE_WIREFRAME,
};

const D3D12_COMMAND_LIST_TYPE gDx12CmdTypeTranslator[TF_MAX_QUEUE_TYPE] =
{
...
const D3D12_COMMAND_QUEUE_PRIORITY gDx12QueuePriorityTranslator[TF_MAX_QUEUE_PRIORITY] =
{
...
#if !defined(XBOX) && !defined(TF_D3D12_DYNAMIC_LOADING)
...
D3D12_SHADER_VISIBILITY       util_to_dx12_shader_visibility(TFShaderStage stages);
D3D12_DESCRIPTOR_RANGE_TYPE   util_to_dx12_descriptor_range(TFDescriptorType type);
D3D12_RESOURCE_STATES         util_to_dx12_resource_state(TFResourceState state);
D3D12_FILTER                  util_to_dx12_filter(TFFilterType minFilter, TFFilterType magFilter, TFMipMapMode mipMapMode, bool aniso,
                                                  bool comparisonFilterEnabled);
D3D12_TEXTURE_ADDRESS_MODE    util_to_dx12_texture_address_mode(TFAddressMode addressMode);
D3D12_PRIMITIVE_TOPOLOGY_TYPE util_to_dx12_primitive_topology_type(TFPrimitiveTopology topology);
...
{
    ID3D12CommandSignature* pDrawCommandSignature[TF_MAX_LINKED_GPUS];
    ID3D12CommandSignature* pDrawIndexCommandSignature[TF_MAX_LINKED_GPUS];
    ID3D12CommandSignature* pDispatchCommandSignature[TF_MAX_LINKED_GPUS];
#if defined(XBOX)
    ID3D12CommandSignature* pAsyncDispatchCommandSignature[TF_MAX_LINKED_GPUS];
#endif

    // Default NULL Descriptors for binding at empty descriptor slots to make sure all descriptors are bound at submit
    DxDescriptorID mNullTextureSRV[TF_TEXTURE_DIM_COUNT];
    DxDescriptorID mNullTextureUAV[TF_TEXTURE_DIM_COUNT];
...
/************************************************************************/
uint32_t util_calculate_shared_node_mask(TFRenderer* pRenderer)
{
    if (pRenderer->mGpuMode == TF_GPU_MODE_LINKED)
        return (1 << pRenderer->mLinkedNodeCount) - 1;
    else
        return 0;
}

uint32_t util_calculate_node_mask(TFRenderer* pRenderer, uint32_t i)
{
    if (pRenderer->mGpuMode == TF_GPU_MODE_LINKED)
        return (1 << i);
    else
...
/************************************************************************/
/************************************************************************/
FORGE_CONSTEXPR D3D12_DEPTH_STENCIL_DESC util_to_depth_desc(const TFDepthStateDesc* pDesc)
{
    ASSERT(pDesc->mDepthFunc < TF_MAX_COMPARE_MODES);
    ASSERT(pDesc->mStencilFrontFunc < TF_MAX_COMPARE_MODES);
    ASSERT(pDesc->mStencilFrontFail < TF_MAX_STENCIL_OPS);
    ASSERT(pDesc->mDepthFrontFail < TF_MAX_STENCIL_OPS);
    ASSERT(pDesc->mStencilFrontPass < TF_MAX_STENCIL_OPS);
    ASSERT(pDesc->mStencilBackFunc < TF_MAX_COMPARE_MODES);
    ASSERT(pDesc->mStencilBackFail < TF_MAX_STENCIL_OPS);
    ASSERT(pDesc->mDepthBackFail < TF_MAX_STENCIL_OPS);
    ASSERT(pDesc->mStencilBackPass < TF_MAX_STENCIL_OPS);
...
static inline FORGE_CONSTEXPR uint8_t ToColorWriteMask(TFColorMask mask)
{
    uint8_t ret = 0;
    if (mask & TF_COLOR_MASK_RED)
    {
        ret |= D3D12_COLOR_WRITE_ENABLE_RED;
    }
    if (mask & TF_COLOR_MASK_GREEN)
    {
        ret |= D3D12_COLOR_WRITE_ENABLE_GREEN;
    }
    if (mask & TF_COLOR_MASK_BLUE)
    {
        ret |= D3D12_COLOR_WRITE_ENABLE_BLUE;
    }
    if (mask & TF_COLOR_MASK_ALPHA)
    {
...
FORGE_CONSTEXPR D3D12_BLEND_DESC util_to_blend_desc(const TFBlendStateDesc* pDesc)
{
    int blendDescIndex = 0;
#if defined(TF_ENABLE_GRAPHICS_RUNTIME_CHECK)

    for (int i = 0; i < TF_MAX_RENDER_TARGET_ATTACHMENTS; ++i)
    {
        if (pDesc->mRenderTargetMask & (1 << i))
        {
            ASSERT(pDesc->mSrcFactors[blendDescIndex] < TF_MAX_BLEND_CONSTANTS);
            ASSERT(pDesc->mDstFactors[blendDescIndex] < TF_MAX_BLEND_CONSTANTS);
            ASSERT(pDesc->mSrcAlphaFactors[blendDescIndex] < TF_MAX_BLEND_CONSTANTS);
            ASSERT(pDesc->mDstAlphaFactors[blendDescIndex] < TF_MAX_BLEND_CONSTANTS);
            ASSERT(pDesc->mBlendModes[blendDescIndex] < TF_MAX_BLEND_MODES);
            ASSERT(pDesc->mBlendAlphaModes[blendDescIndex] < TF_MAX_BLEND_MODES);
        }
...
FORGE_CONSTEXPR D3D12_RASTERIZER_DESC util_to_rasterizer_desc(const TFRasterizerStateDesc* pDesc)
{
    ASSERT(pDesc->mFillMode < TF_MAX_FILL_MODES);
    ASSERT(pDesc->mCullMode < TF_MAX_CULL_MODES);
    ASSERT(pDesc->mFrontFace == TF_FRONT_FACE_CCW || pDesc->mFrontFace == TF_FRONT_FACE_CW);

    return (D3D12_RASTERIZER_DESC){
        .FillMode = gDx12FillModeTranslator[pDesc->mFillMode],
        .CullMode = gDx12CullModeTranslator[pDesc->mCullMode],
        .FrontCounterClockwise = pDesc->mFrontFace == TF_FRONT_FACE_CCW,
...
// Logging functions
/************************************************************************/
void                  AddSrv(TFRenderer* pRenderer, DescriptorHeap* pOptionalHeap, ID3D12Resource* pResource,
                             const D3D12_SHADER_RESOURCE_VIEW_DESC* pSrvDesc, DxDescriptorID* pInOutId)
{
...
static void AddBufferSrv(TFRenderer* pRenderer, DescriptorHeap* pOptionalHeap, ID3D12Resource* pBuffer, bool raw, uint32_t firstElement,
...
static void AddTypedBufferSrv(TFRenderer* pRenderer, DescriptorHeap* pOptionalHeap, ID3D12Resource* pBuffer, uint32_t firstElement,
...
static void AddUav(TFRenderer* pRenderer, DescriptorHeap* pOptionalHeap, ID3D12Resource* pResource, ID3D12Resource* pCounterResource,
...
static void AddBufferUav(TFRenderer* pRenderer, DescriptorHeap* pOptionalHeap, ID3D12Resource* pBuffer, ID3D12Resource* pCounterBuffer,
...
static void AddTypedBufferUav(TFRenderer* pRenderer, DescriptorHeap* pOptionalHeap, ID3D12Resource* pBuffer, uint32_t firstElement,
...
// Builds and registers a texture SRV for the given mip/layer range.
// Pass isCube=true for cube/cube-array views; isStencil=true to select the stencil plane.
static void AddTextureSrv(TFRenderer* pRenderer, ID3D12Resource* pResource, const D3D12_RESOURCE_DESC* pResourceDesc, DXGI_FORMAT dxFormat,
                          bool isCube, bool isStencil, UINT baseMip, UINT mipCount, UINT baseLayer, UINT layerCount,
                          DxDescriptorID* pOutSrv)
{
    ASSERT(!isCube || !isStencil); // cube stencil SRV not supported
    D3D12_SHADER_RESOURCE_VIEW_DESC srvDesc = { 0 };
    srvDesc.Format = util_to_dx12_srv_format(dxFormat, isStencil);
    if (isStencil)
    ...
    srvDesc.Shader4ComponentMapping =
        isStencil ? D3D12_ENCODE_SHADER_4_COMPONENT_MAPPING(1, 1, 1, 1) : D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;

    switch (pResourceDesc->Dimension)
    {
    ...
    case D3D12_RESOURCE_DIMENSION_TEXTURE2D:
        ...
        else if (pResourceDesc->SampleDesc.Count > 1)
        {
            if (layerCount > 1)
            {
        ...
        else if (layerCount > 1)
        {
            ...
            srvDesc.Texture2DArray.PlaneSlice = isStencil ? 1 : 0;
        }
        else
        {
            ...
            srvDesc.Texture2D.PlaneSlice = isStencil ? 1 : 0;
        }
        break;
    case D3D12_RESOURCE_DIMENSION_TEXTURE3D:
        ...
        break;
    default:
        ASSERT(false && "Unsupported resource dimension for SRV texture view");
        break;
    }

    AddSrv(pRenderer, NULL, pResource, &srvDesc, pOutSrv);
}

// Builds and registers a texture UAV for the given mip/layer range.
static void AddTextureUav(TFRenderer* pRenderer, ID3D12Resource* pResource, const D3D12_RESOURCE_DESC* pResourceDesc, DXGI_FORMAT dxFormat,
                          UINT baseMip, UINT baseLayer, UINT layerCount, DxDescriptorID* pOutUav)
{
    ...
    case D3D12_RESOURCE_DIMENSION_TEXTURE1D:
        if (layerCount > 1)
        {
            ...
            uavDesc.Texture1DArray.MipSlice = baseMip;
    ...
    case D3D12_RESOURCE_DIMENSION_TEXTURE3D:
    {
        // D3D12 requires WSize <= depth at the target mip level
        const UINT mipDepth = (UINT)pResourceDesc->DepthOrArraySize >> baseMip;
        const UINT depthAtMip = mipDepth ? mipDepth : 1u;
        uavDesc.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE3D;
        uavDesc.Texture3D.MipSlice = baseMip;
        uavDesc.Texture3D.FirstWSlice = baseLayer;
        uavDesc.Texture3D.WSize = layerCount ? (layerCount + baseLayer < depthAtMip ? layerCount : depthAtMip - baseLayer) : depthAtMip;
        break;
    }
    default:
        ASSERT(false && "Unsupported resource dimension for UAV texture view");
        break;
    }

    AddUav(pRenderer, NULL, pResource, NULL, &uavDesc, pOutUav);
}

static void AddCbv(TFRenderer* pRenderer, DescriptorHeap* pOptionalHeap, const D3D12_CONSTANT_BUFFER_VIEW_DESC* pCbvDesc,
...
static void AddRtv(TFRenderer* pRenderer, DescriptorHeap* pOptionalHeap, ID3D12Resource* pResource, DXGI_FORMAT format, uint32_t mipSlice,
...
static void AddDsv(TFRenderer* pRenderer, DescriptorHeap* pOptionalHeap, ID3D12Resource* pResource, DXGI_FORMAT format, uint32_t mipSlice,
                   uint32_t arraySlice, DxDescriptorID* pInOutId)
{
...
static void AddSampler(TFRenderer* pRenderer, DescriptorHeap* pOptionalHeap, const D3D12_SAMPLER_DESC* pSamplerDesc,
                       DxDescriptorID* pInOutId)
{
...
static void add_default_resources(TFRenderer* pRenderer)
{
    ...
    srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE1D;
    uavDesc.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE1D;
    AddSrv(pRenderer, NULL, NULL, &srvDesc, &pRenderer->pNullDescriptors->mNullTextureSRV[TF_TEXTURE_DIM_1D]);
    AddUav(pRenderer, NULL, NULL, NULL, &uavDesc, &pRenderer->pNullDescriptors->mNullTextureUAV[TF_TEXTURE_DIM_1D]);
    srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
    uavDesc.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE2D;
    AddSrv(pRenderer, NULL, NULL, &srvDesc, &pRenderer->pNullDescriptors->mNullTextureSRV[TF_TEXTURE_DIM_2D]);
    AddUav(pRenderer, NULL, NULL, NULL, &uavDesc, &pRenderer->pNullDescriptors->mNullTextureUAV[TF_TEXTURE_DIM_2D]);
    srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2DMS;
    AddSrv(pRenderer, NULL, NULL, &srvDesc, &pRenderer->pNullDescriptors->mNullTextureSRV[TF_TEXTURE_DIM_2DMS]);
    srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE3D;
    uavDesc.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE3D;
    AddSrv(pRenderer, NULL, NULL, &srvDesc, &pRenderer->pNullDescriptors->mNullTextureSRV[TF_TEXTURE_DIM_3D]);
    AddUav(pRenderer, NULL, NULL, NULL, &uavDesc, &pRenderer->pNullDescriptors->mNullTextureUAV[TF_TEXTURE_DIM_3D]);
    srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE1DARRAY;
    uavDesc.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE1DARRAY;
    AddSrv(pRenderer, NULL, NULL, &srvDesc, &pRenderer->pNullDescriptors->mNullTextureSRV[TF_TEXTURE_DIM_1D_ARRAY]);
    AddUav(pRenderer, NULL, NULL, NULL, &uavDesc, &pRenderer->pNullDescriptors->mNullTextureUAV[TF_TEXTURE_DIM_1D_ARRAY]);
    srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2DARRAY;
    uavDesc.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE2DARRAY;
    AddSrv(pRenderer, NULL, NULL, &srvDesc, &pRenderer->pNullDescriptors->mNullTextureSRV[TF_TEXTURE_DIM_2D_ARRAY]);
    AddUav(pRenderer, NULL, NULL, NULL, &uavDesc, &pRenderer->pNullDescriptors->mNullTextureUAV[TF_TEXTURE_DIM_2D_ARRAY]);
    srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2DMSARRAY;
    AddSrv(pRenderer, NULL, NULL, &srvDesc, &pRenderer->pNullDescriptors->mNullTextureSRV[TF_TEXTURE_DIM_2DMS_ARRAY]);
    srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURECUBE;
    AddSrv(pRenderer, NULL, NULL, &srvDesc, &pRenderer->pNullDescriptors->mNullTextureSRV[TF_TEXTURE_DIM_CUBE]);
    srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURECUBEARRAY;
    AddSrv(pRenderer, NULL, NULL, &srvDesc, &pRenderer->pNullDescriptors->mNullTextureSRV[TF_TEXTURE_DIM_CUBE_ARRAY]);
    ...
    TFBlendStateDesc blendStateDesc = {
        .mDstAlphaFactors = { TF_BC_ZERO },
        .mDstFactors = { TF_BC_ZERO },
        .mSrcAlphaFactors = { TF_BC_ONE },
        .mSrcFactors = { TF_BC_ONE },
        .mColorWriteMasks = { TF_COLOR_MASK_ALL },
        .mRenderTargetMask = TF_BLEND_STATE_TARGET_ALL,
        .mIndependentBlend = false,
    };
    gDefaultBlendDesc = util_to_blend_desc(&blendStateDesc);

    TFDepthStateDesc depthStateDesc = {
        .mDepthFunc = TF_CMP_LEQUAL,
        .mDepthTest = false,
        .mDepthWrite = false,
        .mStencilBackFunc = TF_CMP_ALWAYS,
        .mStencilFrontFunc = TF_CMP_ALWAYS,
    ...
    for (uint32_t l = 0; l < pRenderer->mLinkedNodeCount; ++l)
    {
        ...
        desc.ByteStride = sizeof(TFIndirectDrawArguments);
        CHECK_HRESULT(COM_CALL(CreateCommandSignature, pRenderer->mDx.pDevice, &desc, NULL, &IID_ID3D12CommandSignature_Copy,
                               &pRenderer->pNullDescriptors->pDrawCommandSignature[l]));

        arg.Type = D3D12_INDIRECT_ARGUMENT_TYPE_DRAW_INDEXED;
        desc.ByteStride = sizeof(TFIndirectDrawIndexArguments);
        CHECK_HRESULT(COM_CALL(CreateCommandSignature, pRenderer->mDx.pDevice, &desc, NULL, &IID_ID3D12CommandSignature_Copy,
                               &pRenderer->pNullDescriptors->pDrawIndexCommandSignature[l]));

        arg.Type = D3D12_INDIRECT_ARGUMENT_TYPE_DISPATCH;
        desc.ByteStride = sizeof(TFIndirectDispatchArguments);
        CHECK_HRESULT(COM_CALL(CreateCommandSignature, pRenderer->mDx.pDevice, &desc, NULL, &IID_ID3D12CommandSignature_Copy,
                               &pRenderer->pNullDescriptors->pDispatchCommandSignature[l]));

#if defined(XBOX)
        hook_fill_dispatch_indirect_argument_desc(&arg, true);
        desc.ByteStride = sizeof(TFIndirectDispatchArguments);
        CHECK_HRESULT(COM_CALL(CreateCommandSignature, pRenderer->mDx.pDevice, &desc, NULL, &IID_ID3D12CommandSignature_Copy,
                               &pRenderer->pNullDescriptors->pAsyncDispatchCommandSignature[l]));
#endif
    }
}

static void remove_default_resources(TFRenderer* pRenderer)
{
...
typedef HRESULT(WINAPI* PFN_D3D12_CREATE_DEVICE_FIXED)(_In_opt_ void* pAdapter, D3D_FEATURE_LEVEL MinimumFeatureLevel, _In_ REFIID riid,
                                                       _COM_Outptr_opt_ void** ppDevice);
#if defined(TF_D3D12_DYNAMIC_LOADING)
...
/************************************************************************/
D3D12_FILTER
util_to_dx12_filter(TFFilterType minFilter, TFFilterType magFilter, TFMipMapMode mipMapMode, bool aniso, bool comparisonFilterEnabled)
{
    if (aniso)
...
D3D12_HEAP_TYPE util_to_heap_type(TFResourceMemoryUsage memoryUsage)
{
    switch (memoryUsage)
    {
    case TF_RESOURCE_MEMORY_USAGE_GPU_ONLY:
        return D3D12_HEAP_TYPE_DEFAULT;
    case TF_RESOURCE_MEMORY_USAGE_CPU_ONLY:
    case TF_RESOURCE_MEMORY_USAGE_CPU_TO_GPU:
        return D3D12_HEAP_TYPE_UPLOAD;
    case TF_RESOURCE_MEMORY_USAGE_GPU_TO_CPU:
...
D3D12_HEAP_FLAGS util_to_heap_flags(TFResourceHeapCreationFlags flags)
{
    D3D12_HEAP_FLAGS outFlags = D3D12_HEAP_FLAG_NONE;

    if (flags & TF_RESOURCE_HEAP_FLAG_SHARED)
        outFlags |= D3D12_HEAP_FLAG_SHARED;
    if (flags & TF_RESOURCE_HEAP_FLAG_DENY_BUFFERS)
        outFlags |= D3D12_HEAP_FLAG_DENY_BUFFERS;
    if (flags & TF_RESOURCE_HEAP_FLAG_ALLOW_DISPLAY)
        outFlags |= D3D12_HEAP_FLAG_ALLOW_DISPLAY;
    if (flags & TF_RESOURCE_HEAP_FLAG_SHARED_CROSS_ADAPTER)
        outFlags |= D3D12_HEAP_FLAG_SHARED_CROSS_ADAPTER;
    if (flags & TF_RESOURCE_HEAP_FLAG_DENY_RT_DS_TEXTURES)
        outFlags |= D3D12_HEAP_FLAG_DENY_RT_DS_TEXTURES;
    if (flags & TF_RESOURCE_HEAP_FLAG_DENY_NON_RT_DS_TEXTURES)
        outFlags |= D3D12_HEAP_FLAG_DENY_NON_RT_DS_TEXTURES;
    if (flags & TF_RESOURCE_HEAP_FLAG_HARDWARE_PROTECTED)
        outFlags |= D3D12_HEAP_FLAG_HARDWARE_PROTECTED;
    if (flags & TF_RESOURCE_HEAP_FLAG_ALLOW_WRITE_WATCH)
        outFlags |= D3D12_HEAP_FLAG_ALLOW_WRITE_WATCH;

#if !defined(XBOX)
    if (flags & TF_RESOURCE_HEAP_FLAG_ALLOW_SHADER_ATOMICS)
        outFlags |= D3D12_HEAP_FLAG_ALLOW_SHADER_ATOMICS;
#endif

    if (flags == TF_RESOURCE_HEAP_FLAG_ALLOW_ALL_BUFFERS_AND_TEXTURES)
...
D3D12_TEXTURE_ADDRESS_MODE util_to_dx12_texture_address_mode(TFAddressMode addressMode)
{
    switch (addressMode)
    {
    case TF_ADDRESS_MODE_MIRROR:
        return D3D12_TEXTURE_ADDRESS_MODE_MIRROR;
    case TF_ADDRESS_MODE_REPEAT:
        return D3D12_TEXTURE_ADDRESS_MODE_WRAP;
    case TF_ADDRESS_MODE_CLAMP_TO_EDGE:
        return D3D12_TEXTURE_ADDRESS_MODE_CLAMP;
    case TF_ADDRESS_MODE_CLAMP_TO_BORDER:
...
D3D12_PRIMITIVE_TOPOLOGY_TYPE util_to_dx12_primitive_topology_type(TFPrimitiveTopology topology)
{
    ...
    case TF_PRIMITIVE_TOPO_PATCH_LIST:
...
// DXGI flip model only supports swapchain backbuffer in these formats:
// DXGI_FORMAT_R16G16B16A16_FLOAT, DXGI_FORMAT_B8G8R8A8_UNORM, DXGI_FORMAT_R8G8B8A8_UNORM
// The intended gamma format suffix is accounted for in the swapchain's RenderTarget View
DXGI_FORMAT util_to_dx12_swapchain_format(TinyImageFormat const format)
{
    ...
    case TinyImageFormat_B8G8R8A8_SRGB:
        result = DXGI_FORMAT_B8G8R8A8_UNORM;
        LOGF(eWARNING, "Falling back to format: (%u) for swapchain buffer", (uint32_t)TinyImageFormat_B8G8R8A8_UNORM);
        break;
    ...
    case TinyImageFormat_R8G8B8A8_SRGB:
        result = DXGI_FORMAT_R8G8B8A8_UNORM;
        LOGF(eWARNING, "Falling back to format: (%u) for swapchain buffer", (uint32_t)TinyImageFormat_R8G8B8A8_UNORM);
        break;
...
DXGI_COLOR_SPACE_TYPE util_to_dx12_colorspace(TFColorSpace colorspace)
{
    switch (colorspace)
    {
    case TF_COLOR_SPACE_SDR_LINEAR:
    case TF_COLOR_SPACE_SDR_SRGB:
        return DXGI_COLOR_SPACE_RGB_FULL_G22_NONE_P709;
    case TF_COLOR_SPACE_P2020:
        return DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020;
    case TF_COLOR_SPACE_EXTENDED_SRGB:
...
D3D12_SHADER_VISIBILITY util_to_dx12_shader_visibility(TFShaderStage stages)
{
    ...
    else if (stages == TF_SHADER_STAGE_WORKGRAPH)
    {
        return D3D12_SHADER_VISIBILITY_ALL;
    }
#endif
    ...
    if (stages & TF_SHADER_STAGE_GEOM)
    {
        res = D3D12_SHADER_VISIBILITY_GEOMETRY;
        ++stageCount;
    }
    if (stages & TF_SHADER_STAGE_HULL)
    {
        res = D3D12_SHADER_VISIBILITY_HULL;
        ++stageCount;
    }
    if (stages & TF_SHADER_STAGE_DOMN)
    {
...
D3D12_DESCRIPTOR_RANGE_TYPE util_to_dx12_descriptor_range(TFDescriptorType type)
{
    ...
    case TF_DESCRIPTOR_TYPE_ROOT_CONSTANT:
...
D3D12_RESOURCE_STATES util_to_dx12_resource_state(TFResourceState state)
{
    ...
    if (state == TF_RESOURCE_STATE_GENERIC_READ)
    {
    ...
    if (state == TF_RESOURCE_STATE_COMMON)
        return D3D12_RESOURCE_STATE_COMMON;
    if (state == TF_RESOURCE_STATE_PRESENT)
#ifdef SCARLETT
        return (D3D12_RESOURCE_STATES)0x100000;
#else
        return D3D12_RESOURCE_STATE_PRESENT;
#endif

    if (state & TF_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER)
        ret |= D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER;
    if (state & TF_RESOURCE_STATE_INDEX_BUFFER)
        ret |= D3D12_RESOURCE_STATE_INDEX_BUFFER;
    if (state & TF_RESOURCE_STATE_RENDER_TARGET)
        ret |= D3D12_RESOURCE_STATE_RENDER_TARGET;
    if (state & TF_RESOURCE_STATE_UNORDERED_ACCESS)
        ret |= D3D12_RESOURCE_STATE_UNORDERED_ACCESS;
    if (state & TF_RESOURCE_STATE_DEPTH_WRITE)
        ret |= D3D12_RESOURCE_STATE_DEPTH_WRITE;
    else if (state & TF_RESOURCE_STATE_DEPTH_READ)
        ret |= D3D12_RESOURCE_STATE_DEPTH_READ;
    if (state & TF_RESOURCE_STATE_STREAM_OUT)
        ret |= D3D12_RESOURCE_STATE_STREAM_OUT;
    if (state & TF_RESOURCE_STATE_INDIRECT_ARGUMENT)
        ret |= D3D12_RESOURCE_STATE_INDIRECT_ARGUMENT;
    if (state & TF_RESOURCE_STATE_COPY_DEST)
        ret |= D3D12_RESOURCE_STATE_COPY_DEST;
    if (state & TF_RESOURCE_STATE_COPY_SOURCE)
        ret |= D3D12_RESOURCE_STATE_COPY_SOURCE;
    if (state & TF_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE)
        ret |= D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE;
    if (state & TF_RESOURCE_STATE_PIXEL_SHADER_RESOURCE)
        ret |= D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
#ifdef D3D12_RAYTRACING_AVAILABLE
    if (state & (TF_RESOURCE_STATE_ACCELERATION_STRUCTURE_READ | TF_RESOURCE_STATE_ACCELERATION_STRUCTURE_WRITE))
...
static inline FORGE_CONSTEXPR D3D12_QUERY_HEAP_TYPE ToDX12QueryHeapType(TFQueryType type)
{
    switch (type)
    {
    case TF_QUERY_TYPE_TIMESTAMP:
        return D3D12_QUERY_HEAP_TYPE_TIMESTAMP;
    case TF_QUERY_TYPE_PIPELINE_STATISTICS:
        return D3D12_QUERY_HEAP_TYPE_PIPELINE_STATISTICS;
    case TF_QUERY_TYPE_OCCLUSION:
...
static inline FORGE_CONSTEXPR D3D12_QUERY_TYPE ToDX12QueryType(TFQueryType type)
{
    switch (type)
    {
    case TF_QUERY_TYPE_TIMESTAMP:
...
static inline FORGE_CONSTEXPR uint32_t ToQueryWidth(TFQueryType type)
{
    switch (type)
    {
    case TF_QUERY_TYPE_PIPELINE_STATISTICS:
...
void util_enumerate_gpus(IDXGIFactory6* dxgiFactory, uint32_t* pGpuCount, DXGPUInfo* gpuDesc, bool* pFoundSoftwareAdapter)
{
    ...
    for (UINT i = 0; DXGI_ERROR_NOT_FOUND != COM_CALL(EnumAdapterByGpuPreference, dxgiFactory, i, DXGI_GPU_PREFERENCE_HIGH_PERFORMANCE,
                                                      IID_REF(IDXGIAdapter4), &adapter);
         ++i)
    {
        if (gpuCount >= TF_MAX_MULTIPLE_GPUS)
        {
            break;
        }
        ...
        if (!(desc.Flags & DXGI_ADAPTER_FLAG_SOFTWARE))
        {
            for (uint32_t level = 0; level < sizeof(feature_levels) / sizeof(feature_levels[0]); ++level)
            {
                ...
                if (SUCCEEDED(d3d12dll_CreateDevice(adapter, feature_levels[level], IID_REF(ID3D12Device), NULL)))
                {
                    ...
                    HRESULT    hres = COM_CALL(QueryInterface, adapter, IID_REF(IDXGIAdapter4), &pDXGPUInfo->pGpu);
                    if (SUCCEEDED(hres))
                    {
                        if (gpuDesc)
                        {
...
static void QueryRaytracingSupport(ID3D12Device* pDevice, TFGpuDesc* pGpuDesc)
{
    UNREF_PARAM(pDevice);
...
static void QueryCooperativeVectorSupport(ID3D12Device* pDevice, TFGpuDesc* pGpuDesc)
{
    UNREF_PARAM(pDevice);
    UNREF_PARAM(pGpuDesc);
#if defined(D3D12_COOP_VECTORS_AVAILABLE)
    ASSERT(pDevice);
    D3D12_FEATURE_DATA_D3D12_OPTIONS_EXPERIMENTAL optsExp = { 0 };
    HRESULT hresVectors = COM_CALL(CheckFeatureSupport, pDevice, D3D12_FEATURE_D3D12_OPTIONS_EXPERIMENTAL, &optsExp, sizeof(optsExp));
    D3D12_FEATURE_DATA_SHADER_MODEL shadModel = { D3D_SHADER_MODEL_6_9 };
    HRESULT hresShaders = COM_CALL(CheckFeatureSupport, pDevice, (D3D12_FEATURE)D3D12_FEATURE_SHADER_MODEL, &shadModel, sizeof(shadModel));
    // Currently the cooperative vectors feature requires both the experimental options and a shader model 6.9
    if (SUCCEEDED(hresVectors) && SUCCEEDED(hresShaders))
    {
        pGpuDesc->mCoopVectorsSupported = (optsExp.CooperativeVectorTier != D3D12_COOPERATIVE_VECTOR_TIER_NOT_SUPPORTED &&
                                           shadModel.HighestShaderModel >= D3D_SHADER_MODEL_6_9);
    }
#endif
}

static void QueryWorkGraphSupport(ID3D12Device* pDevice, TFGpuDesc* pGpuDesc)
{
    UNREF_PARAM(pDevice);
    UNREF_PARAM(pGpuDesc);
#if defined(TF_ENABLE_WORKGRAPH)
...
static void Query64BitAtomicsSupport(ID3D12Device* pDevice, TFGpuDesc* pGpuDesc)
{
    ASSERT(pDevice);
...
static void QueryRelaxedFormatCastingSupport(ID3D12Device* pDevice, TFGpuDesc* pGpuDesc)
{
    ASSERT(pDevice);
#ifdef SCARLETT
    pGpuDesc->mRelaxedFormatCastingSupported = true;
#else
    D3D12_FEATURE_DATA_D3D12_OPTIONS12 opts12 = { 0 };
    HRESULT hres = COM_CALL(CheckFeatureSupport, pDevice, D3D12_FEATURE_D3D12_OPTIONS12, &opts12, sizeof(opts12));
    if (SUCCEEDED(hres))
    {
        pGpuDesc->mRelaxedFormatCastingSupported = opts12.RelaxedFormatCastingSupported;
    }
#endif
}

void setDefaultGPUDirect3D12Properties(TFGpuDesc* pGpuDesc);

void QueryGpuDesc(ID3D12Device* pDevice, const DXGPUInfo* pDXGPUInfo, TFGpuDesc* pGpuDesc)
{
    setDefaultGPUProperties(pGpuDesc);
    setDefaultGPUDirect3D12Properties(pGpuDesc);
    ...
    // get name from api
    strncpy(pGpuDesc->mGpuVendorPreset.mGpuName, pDXGPUInfo->mName, TF_MAX_GPU_VENDOR_STRING_LENGTH);
    // get preset
    ...
    pGpuDesc->mWaveOpsSupportedStageFlags |= TF_SHADER_STAGE_ALL_GRAPHICS | TF_SHADER_STAGE_COMP;
...
#if defined(XBOX)
    pGpuDesc->mUnifiedMemorySupport = TF_UMA_SUPPORT_READ_WRITE;
#endif
    ...
    if (gpuVendorEquals(adapterDesc.VendorId, "nvidia"))
    {
        ...
        {
            snprintf(pGpuDesc->mGpuVendorPreset.mGpuDriverVersion, TF_MAX_GPU_VENDOR_STRING_LENGTH, "%lu.%lu",
    ...
    else if (gpuVendorEquals(adapterDesc.VendorId, "amd"))
    {
        ...
        {
            snprintf(pGpuDesc->mGpuVendorPreset.mGpuDriverVersion, TF_MAX_GPU_VENDOR_STRING_LENGTH, "%s", gAgsGpuInfo.driverVersion);
        }
#endif
    }
    ...
    else
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            snprintf(pGpuDesc->mGpuVendorPreset.mGpuDriverVersion, TF_MAX_GPU_VENDOR_STRING_LENGTH, "%d.%d", subVersion, build);
        }
    }
    pGpuDesc->mMaxRootSignatureDWORDS = 13;
    pGpuDesc->mMaxBoundTextures = UINT32_MAX;

    // Checks for Intel ARC A and B series GPUs (0x5600 and 0xE200)
    pGpuDesc->mIsIntelIntegrated =
        (pGpuDesc->mGpuVendorPreset.mVendorId == 0x8086 || pGpuDesc->mGpuVendorPreset.mVendorId == 0x8087 ||
         pGpuDesc->mGpuVendorPreset.mVendorId == 0x163C) &&
        !((pGpuDesc->mGpuVendorPreset.mModelId & 0x5600) == 0x5600 || (pGpuDesc->mGpuVendorPreset.mModelId & 0xE200) == 0xE200);

    QueryRaytracingSupport(pDevice, pGpuDesc);
    QueryWorkGraphSupport(pDevice, pGpuDesc);
    Query64BitAtomicsSupport(pDevice, pGpuDesc);
    QueryCooperativeVectorSupport(pDevice, pGpuDesc);
    QueryRelaxedFormatCastingSupport(pDevice, pGpuDesc);
}

static void InitializeBufferDesc(TFRenderer* pRenderer, const TFBufferDesc* pDesc, D3D12_RESOURCE_DESC* desc)
{
    ASSERT(pRenderer);
    ASSERT(pDesc);
    ASSERT(desc);
    ...
    if (pDesc->mDescriptors & TF_DESCRIPTOR_TYPE_RW_BUFFER)
    {
    ...
    if (TF_RESOURCE_MEMORY_USAGE_GPU_TO_CPU == pDesc->mMemoryUsage)
    {
        desc->Flags |= D3D12_RESOURCE_FLAG_DENY_SHADER_RESOURCE;
    }
}

static void InitializeTextureDesc(TFRenderer* pRenderer, const TFTextureDesc* pDesc, D3D12_RESOURCE_DESC* desc,
                                  TFResourceState* pStartResourceState)
{
    ASSERT(pRenderer);
    ASSERT(pDesc);
    ASSERT(desc);
    ...
    TFResourceState actualStartState = pDesc->mStartState;

    // Decide UAV flags
    if (pDesc->mDescriptors & TF_DESCRIPTOR_TYPE_RW_TEXTURE)
    {
        desc->Flags |= D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
    }

    // Decide render target flags
    if (pDesc->mStartState & TF_RESOURCE_STATE_RENDER_TARGET)
    {
        desc->Flags |= D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET;
        actualStartState = (pDesc->mStartState > TF_RESOURCE_STATE_RENDER_TARGET)
                               ? (pDesc->mStartState & (TFResourceState)~TF_RESOURCE_STATE_RENDER_TARGET)
                               : TF_RESOURCE_STATE_RENDER_TARGET;
    }
    else if (pDesc->mStartState & TF_RESOURCE_STATE_DEPTH_WRITE)
    {
        desc->Flags |= D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL;
        actualStartState = (pDesc->mStartState > TF_RESOURCE_STATE_DEPTH_WRITE)
                               ? (pDesc->mStartState & (TFResourceState)~TF_RESOURCE_STATE_DEPTH_WRITE)
                               : TF_RESOURCE_STATE_DEPTH_WRITE;
    }

    // Decide sharing flags
    if (pDesc->mFlags & TF_TEXTURE_CREATION_FLAG_EXPORT_ADAPTER_BIT)
    {
    ...
    if (pDesc->mFlags & TF_TEXTURE_CREATION_FLAG_ALLOW_DISPLAY_TARGET)
    {
        actualStartState = TF_RESOURCE_STATE_PRESENT;
    }
...
#if defined(_WINDOWS) && defined(TF_ENABLE_GRAPHICS_VALIDATION) && !defined(HOLOLENS2)
                          ...
                          void* pContext)
{
    ...
    // Only leave corruptions, errors and warnings for now
    const D3D12_MESSAGE_SEVERITY maximumSeverity = D3D12_MESSAGE_SEVERITY_WARNING;
...
#if defined(D3D12_COOP_VECTORS_AVAILABLE)
static HRESULT EnableExperimentalCooperativeVectors()
{
    // Enables both the reqired shader model and the cooperative vectors feature
    const GUID features[2] = { D3D12ExperimentalShaderModels, D3D12CooperativeVectorExperiment };
    return d3d12dll_EnableExperimentalFeatures(2, &features[0], NULL, NULL);
}
#endif
...
static bool SelectBestGpu(TFRenderer* pRenderer, const TFRendererDesc* pDesc, D3D_FEATURE_LEVEL* pFeatureLevel, uint32_t* pOutGpuCount)
{
    UNREF_PARAM(pDesc);
    TFGpuDesc          gpuDesc[TF_MAX_MULTIPLE_GPUS] = { 0 };
    TFRendererContext* pContext = pRenderer->pContext;
    for (uint32_t i = 0; i < pContext->mGpuCount; ++i)
    {
        gpuDesc[i] = pContext->mGpus[i];
    }
    uint32_t gpuIndex = selectBestSupportedGpu(gpuDesc, pContext->mGpuCount);
    if (gpuIndex == UINT32_MAX)
    {
        LOGF(eERROR, "Failed to Init Renderer: no supported GPU found");
        return false;
    }
...
static bool AddDevice(const TFRendererDesc* pDesc, TFRenderer* pRenderer)
{
...
#if defined(_WINDOWS) && defined(TF_ENABLE_GRAPHICS_VALIDATION) && !defined(HOLOLENS2)
    HRESULT hr = COM_CALL(QueryInterface, pRenderer->mDx.pDevice, IID_REF(ID3D12InfoQueue1), &pRenderer->mDx.pDebugValidation);
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (pRenderer->mDx.mUseDebugCallback)
        {
            ...
            HRESULT res = COM_CALL(RegisterMessageCallback, pRenderer->mDx.pDebugValidation, DebugMessageCallback,
                                   D3D12_MESSAGE_CALLBACK_IGNORE_FILTERS, pRenderer, &pRenderer->mDx.mCallbackCookie);
            if (!SUCCEEDED(res) || pRenderer->mDx.mCallbackCookie == 0)
            {
                LOGF(eERROR, "AddDevice | RegisterMessageCallback failed - disabling DirectX12 ID3D12InfoQueue1 debug callbacks");
            }
        }
    }
#elif defined(_WINDOWS) && defined(TF_ENABLE_GRAPHICS_VALIDATION) && defined(HOLOLENS2)
    HRESULT hr = COM_CALL(QueryInterface, pRenderer->mDx.pDevice, IID_REF(ID3D12InfoQueue), (void**)&pRenderer->mDx.pDebugValidation);
    if (SUCCEEDED(hr))
    {
        // Harmless/falsely reported warnings on HL2
        D3D12_MESSAGE_ID hide[] = {
...
static void RemoveDevice(TFRenderer* pRenderer)
{
    ...
    if (pRenderer->mDx.mUseDebugCallback && pRenderer->mDx.mCallbackCookie != 0)
    {
        COM_CALL(UnregisterMessageCallback, pRenderer->mDx.pDebugValidation, pRenderer->mDx.mCallbackCookie);
        pRenderer->mDx.mCallbackCookie = 0;
    }
    SAFE_RELEASE(pRenderer->mDx.pDebugValidation);
#endif
...
void InitCommon(const TFRendererContextDesc* pDesc, TFRendererContext* pContext)
{
    UNREF_PARAM(pDesc);
    UNREF_PARAM(pContext);
    d3d12dll_init();
    ...
    CHECK_HRESULT(d3d12dll_CreateDXGIFactory2(flags, IID_REF(IDXGIFactory6), &pContext->mDx.pDXGIFactory));
#endif
...
void ExitCommon(TFRendererContext* pContext)
{
...
void initRendererContext(const char* appName, const TFRendererContextDesc* pDesc, TFRendererContext** ppContext)
{
    ...
    TFRendererContext* pContext = (TFRendererContext*)tf_calloc_memalign(1, sizeof(void*), sizeof(TFRendererContext));
    ASSERT(pContext);
    ...
    TFGpuDesc* gpu = &pContext->mGpus[0];
...
#if defined(D3D12_COOP_VECTORS_AVAILABLE)
    // Enable experimental features before the device creation, these need to be changed based on the SDK version
    EnableExperimentalCooperativeVectors();
#endif

    DXGPUInfo gpuDesc[TF_MAX_MULTIPLE_GPUS] = { 0 };
...
/************************************************************************/
void initRenderer(const char* appName, const TFRendererDesc* pDesc, TFRenderer** ppRenderer)
{
    ASSERT(appName);
    ASSERT(pDesc);
    ASSERT(ppRenderer);

    TFRenderer* pRenderer = (TFRenderer*)tf_calloc_memalign(1, TF_ALIGN_Renderer, sizeof(TFRenderer));
    ASSERT(pRenderer);

    pRenderer->mRendererApi = TF_RENDERER_API_D3D12;
    pRenderer->mGpuMode = pDesc->mGpuMode;
    pRenderer->mShaderTarget = pDesc->mShaderTarget;
    pRenderer->pName = appName;
#ifdef FORGE_DEBUG
    pRenderer->mRenderDocQueriedAndLoaded = isRenderDocAttached();
#endif
    ...
    // Initialize the D3D12 bits
    {
        if (!pDesc->pContext)
        {
            TFRendererContextDesc contextDesc = { 0 };
...
#ifdef HOLOLENS2
            // OpenXR requires the D3D12 device to be created on a specific adapter.
            // Query the required LUID now and find the matching GPU before AddDevice runs.
            PFN_xrGetD3D12GraphicsRequirementsKHR pfnGetReqs = NULL;
            xrGetInstanceProcAddr(getXRInstance(), "xrGetD3D12GraphicsRequirementsKHR", (PFN_xrVoidFunction*)&pfnGetReqs);

            if (pfnGetReqs)
            {
                XrGraphicsRequirementsD3D12KHR reqs = { XR_TYPE_GRAPHICS_REQUIREMENTS_D3D12_KHR };
                XrResult                       result = pfnGetReqs(getXRInstance(), getXRSystemId(), &reqs);
                if (XR_SUCCEEDED(result))
                {
                    for (uint32_t i = 0; i < pRenderer->pContext->mGpuCount; i++)
                    {
                        DXGI_ADAPTER_DESC1 desc = { 0 };
                        COM_CALL(GetDesc1, pRenderer->pContext->mGpus[i].mDx.pGpu, &desc);
                        LOGF(eINFO, "GPU[%u] LUID: %u|%d", i, desc.AdapterLuid.LowPart, desc.AdapterLuid.HighPart);
                        if (memcmp(&desc.AdapterLuid, &reqs.adapterLuid, sizeof(LUID)) == 0)
                        {
                            LOGF(eINFO, "OpenXR matched GPU[%u]: %ls", i, desc.Description);
                            ((TFRendererDesc*)pDesc)->mGpuIndex = i;
                            break;
                        }
                    }
                }
                ...
                {
                    LOGF(eWARNING, "xrGetD3D12GraphicsRequirements failed (%d), using default GPU", (int)result);
                }
            }
            else
            {
                LOGF(eWARNING, "xrGetD3D12GraphicsRequirementsKHR not found, using default GPU");
            }
#endif
        }
        else
        ...
        if (pRenderer->pGpu->mGpuVendorPreset.mPresetLevel < TF_GPU_PRESET_VERYLOW)
        {
            ...
            ASSERT(pRenderer->pGpu->mGpuVendorPreset.mPresetLevel >= TF_GPU_PRESET_VERYLOW); //-V547 //-V522
...
#endif // HOLOLENS2
        if (pRenderer->mShaderTarget >= TF_SHADER_TARGET_6_0)
        {
            ...
            // If the device doesn't support SM6 or Wave Intrinsics, try enabling the experimental feature for Shader Model 6 and
            // creating the device again.
            if (shaderModelSupport.HighestShaderModel != D3D_SHADER_MODEL_6_0 || waveIntrinsicsSupport.WaveOps == FALSE)
            {
                ...
                if (!rdoc_api || !rdoc_api->IsTargetControlConnected())
                {
                    ...
                    // up your application to use the highest supported shader model. For simplicity we just exit the application here.
                ...
                {
                    LOGF(eWARNING, "\nRenderDoc does not support SM 6.0 or higher. Application might work but you won't be able to "
                                   "debug the SM 6.0+ "
        ...
        pRenderer->mLinkedNodeCount = (pRenderer->mGpuMode == TF_GPU_MODE_LINKED) ? gpuCount : 1;
        if (pRenderer->mGpuMode == TF_GPU_MODE_LINKED && pRenderer->mLinkedNodeCount < 2)
            pRenderer->mGpuMode = TF_GPU_MODE_SINGLE;
        /************************************************************************/
        // Descriptor heaps
        /************************************************************************/
    ...
    ++gRendererCount;
    ASSERT(gRendererCount <= TF_MAX_UNLINKED_GPUS);
...
void initFence(TFRenderer* pRenderer, TFFence** ppFence)
{
    ...
    // create a TFFence and ASSERT that it is valid
...
void initQueue(TFRenderer* pRenderer, TFQueueDesc* pDesc, TFQueue** ppQueue)
{
    ...
    const uint32_t nodeIndex = pRenderer->mGpuMode == TF_GPU_MODE_UNLINKED ? 0 : pDesc->mNodeIndex;
    if (nodeIndex)
    {
        ASSERT(pRenderer->mGpuMode == TF_GPU_MODE_LINKED && "Node Masking can only be used with Linked Multi GPU");
    }
    ...
    if (pDesc->mFlag & TF_QUEUE_FLAG_DISABLE_GPU_TIMEOUT)
    ...
    if (firstQueue)
    {
        char queueTypeBuffer[TF_MAX_DEBUG_NAME_LENGTH] = { 0 };
        if (!pDesc->pName)
        {
            ...
            snprintf(queueTypeBuffer, TF_MAX_DEBUG_NAME_LENGTH, "%s %u", queueNames[queueDesc.Type], pDesc->mNodeIndex);
        }
    ...
    // override node index
    if (pRenderer->mGpuMode == TF_GPU_MODE_UNLINKED)
...
#ifdef HOLOLENS2
    if (pDesc->mType == TF_QUEUE_TYPE_GRAPHICS)
    {
        initXRSession(pRenderer, pQueue);
    }
#endif
...
void initCmdPool(TFRenderer* pRenderer, const TFCmdPoolDesc* pDesc, TFCmdPool** ppCmdPool)
{
    ...
    // create one new TFCmdPool and add to renderer
...
void initCmd(TFRenderer* pRenderer, const TFCmdDesc* pDesc, TFCmd** ppCmd)
{
    ...
    TFCmd* pCmd = (TFCmd*)tf_calloc_memalign(1, TF_ALIGN_Cmd, sizeof(TFCmd));
    ASSERT(pCmd);

    // set command pool of new command
    pCmd->mDx.mNodeIndex = pRenderer->mGpuMode == TF_GPU_MODE_LINKED ? pDesc->pPool->pQueue->mNodeIndex : 0;
...
#if defined(TF_ENABLE_GRAPHICS_VALIDATION) && defined(_WINDOWS) && !defined(HOLOLENS2)
...
bool getSwapchainFormatSupport(TFRenderer* pRenderer, TFQueue* pQueue, TinyImageFormat format, TFColorSpace colorspace);

#ifdef HOLOLENS2
void addSwapchainOpenXR(TFRenderer* pRenderer, const TFSwapChainDesc* pDesc, bool multiView, TFSwapChain** ppSwapChain)
{
    ASSERT(pRenderer);
    ASSERT(pDesc);
    ASSERT(ppSwapChain);
    ...
    // Create the swapchain first; the runtime owns the image count, which sizes ppRenderTargets below.
    XrSwapchainCreateInfo swapchainCI = { XR_TYPE_SWAPCHAIN_CREATE_INFO };
    swapchainCI.usageFlags = XR_SWAPCHAIN_USAGE_SAMPLED_BIT | XR_SWAPCHAIN_USAGE_COLOR_ATTACHMENT_BIT;
    swapchainCI.format = util_to_dx12_swapchain_format(pDesc->mColorFormat);
    swapchainCI.width = getXRViewConfigView(0).recommendedImageRectWidth;
    swapchainCI.height = getXRViewConfigView(0).recommendedImageRectHeight;
    swapchainCI.sampleCount = 1;
    swapchainCI.faceCount = 1;
    swapchainCI.arraySize = multiView ? 2 : 1; // stereo for projection, single for 2D quad layer. left eye = slice 0, right eye = slice 1
    swapchainCI.mipCount = 1;

    XrSwapchain xrSwapchain = XR_NULL_HANDLE;
    CHECK_OXRCMD(xrCreateSwapchain(getXRSession(), &swapchainCI, &xrSwapchain));

    uint32_t imageCount = 0;
    CHECK_OXRCMD(xrEnumerateSwapchainImages(xrSwapchain, 0, &imageCount, NULL));
    LOGF(eINFO, "XR D3D12 swapchain (%s) has %u images, pDesc requested %u", multiView ? "stereo" : "2DLayer", imageCount,
         pDesc->mImageCount);
    ASSERT(imageCount > 0);

    TFSwapChain* pSwapChain = (TFSwapChain*)tf_calloc(1, sizeof(TFSwapChain) + imageCount * sizeof(TFRenderTarget*));
    ASSERT(pSwapChain);
    pSwapChain->ppRenderTargets = (TFRenderTarget**)(pSwapChain + 1);
    ASSERT(pSwapChain->ppRenderTargets);

    XrSwapchainImageD3D12KHR* images = (XrSwapchainImageD3D12KHR*)tf_calloc(imageCount, sizeof(XrSwapchainImageD3D12KHR));
    ASSERT(images);
    for (uint32_t i = 0; i < imageCount; ++i)
    {
        images[i].type = XR_TYPE_SWAPCHAIN_IMAGE_D3D12_KHR;
        images[i].next = NULL; // hl2 no foveation
    }
    CHECK_OXRCMD(xrEnumerateSwapchainImages(xrSwapchain, imageCount, &imageCount, (XrSwapchainImageBaseHeader*)images));

    pSwapChain->mVR.pSwapchain = xrSwapchain;
    pSwapChain->mDx.pSwapChain = NULL;
    pSwapChain->mDx.mSyncInterval = 0; // XR runtime manages its own sync

    TFRenderTargetDesc descColor = { 0 };
    descColor.mWidth = swapchainCI.width;
    descColor.mHeight = swapchainCI.height;
    descColor.mDepth = 1;
    descColor.mArraySize = (uint32_t)swapchainCI.arraySize;
    ...
    descColor.mStartState = TF_RESOURCE_STATE_PRESENT;
    descColor.mDescriptors = TF_DESCRIPTOR_TYPE_UNDEFINED;
    if (multiView)
        descColor.mFlags |= TF_TEXTURE_CREATION_FLAG_VR_MULTIVIEW;
#ifdef AUTOMATED_TESTING
    descColor.mDescriptors |= TF_DESCRIPTOR_TYPE_TEXTURE;
#endif

    char nameBuffer[32] = { 0 };
    for (uint32_t i = 0; i < imageCount; ++i)
    {
        snprintf(nameBuffer, sizeof(nameBuffer), multiView ? "XR Swapchain RT[%u]" : "XR 2DLayer RT[%u]", i);
        descColor.pName = nameBuffer;
        descColor.pNativeHandle = (void*)images[i].texture;
...
void addSwapChain(TFRenderer* pRenderer, const TFSwapChainDesc* pDesc, TFSwapChain** ppSwapChain)
{
...
#if defined(HOLOLENS2)
    addSwapchainOpenXR(pRenderer, pDesc, true, ppSwapChain);

    // *should* always be true..
    bool shouldAdd2DLayer = pDesc->mFlags & TF_SWAP_CHAIN_CREATION_FLAG_ENABLE_2D_VR_LAYER;
    if (shouldAdd2DLayer)
    {
        TFSwapChainDesc swapChainDesc = { 0 };
        ...
        addSwapchainOpenXR(pRenderer, &swapChainDesc, false, &(*ppSwapChain)->mVR.m2DLayer.pSwapchain);

        TFSwapChain* p2DSwapChain = (*ppSwapChain)->mVR.m2DLayer.pSwapchain;
        p2DSwapChain->mVR.m2DLayer.mCurrentSwapChainIndex = 0;
        p2DSwapChain->mVR.m2DLayer.mDesc = pDesc->mVR.m2DLayer;
    }

    return;
    // Guard rest of function behind #if #else because we get unreachable code warning otherwise
#else
    ...
    if (pRenderer->mGpuMode == TF_GPU_MODE_LINKED && pDesc->mPresentQueueCount > 1)
    {
    ...
    TFRenderTargetDesc descColor = {
        ...
        .mSampleCount = TF_SAMPLE_COUNT_1,
        .mSampleQuality = 0,
        .pNativeHandle = NULL,
        .mFlags = TF_TEXTURE_CREATION_FLAG_ALLOW_DISPLAY_TARGET,
        .mStartState = TF_RESOURCE_STATE_PRESENT,
    };
...
#endif
#if defined(XBOX)
    descColor.mFlags |= TF_TEXTURE_CREATION_FLAG_OWN_MEMORY_BIT;
    pSwapChain->mDx.pPresentQueue = pDesc->mPresentQueueCount ? pDesc->ppPresentQueues[0] : NULL;
#endif

    for (uint32_t i = 0; i < pDesc->mImageCount; ++i)
    {
#if !defined(XBOX) && !defined(HOLOLENS2) // Basically, we are a windows pc
...
void removeSwapChain(TFRenderer* pRenderer, TFSwapChain* pSwapChain)
{
    ...
    {
        removeSwapChain(pRenderer, pSwapChain->mVR.m2DLayer.pSwapchain);
        pSwapChain->mVR.m2DLayer.pSwapchain = NULL;
    }
#endif

    for (uint32_t i = 0; i < pSwapChain->mImageCount; ++i)
    {
#if !defined(XBOX) && !defined(HOLOLENS2)
...
#if defined(HOLOLENS2)
    if (pSwapChain->mVR.pSwapchain != XR_NULL_HANDLE)
    {
        xrDestroySwapchain(pSwapChain->mVR.pSwapchain);
        pSwapChain->mVR.pSwapchain = XR_NULL_HANDLE;
    }
#endif
...
void getBufferSizeAlign(TFRenderer* pRenderer, const TFBufferDesc* pDesc, TFResourceSizeAlign* pOut)
{
    ...
    ASSERT(pDesc);
    ASSERT(pOut);
    ASSERT(pRenderer->mGpuMode != TF_GPU_MODE_UNLINKED || pDesc->mNodeIndex == pRenderer->mUnlinkedRendererIndex);
...
void addBuffer(TFRenderer* pRenderer, const TFBufferDesc* pDesc, TFBuffer** ppBuffer)
{
    ...
    TFBuffer* pBuffer = (TFBuffer*)tf_calloc_memalign(1, TF_ALIGN_Buffer, sizeof(TFBuffer));
    ...
    D3D12MA_ALLOCATION_DESC alloc_desc = {
        ...
        .mUseDedicatedAllocation = (pDesc->mFlags & TF_BUFFER_CREATION_FLAG_OWN_MEMORY_BIT) != 0,
    };
    ...
    TFResourceState start_state = pDesc->mStartState;
    if (pDesc->mMemoryUsage == TF_RESOURCE_MEMORY_USAGE_CPU_TO_GPU || pDesc->mMemoryUsage == TF_RESOURCE_MEMORY_USAGE_CPU_ONLY)
    {
        start_state = TF_RESOURCE_STATE_GENERIC_READ;
    }
    else if (pDesc->mMemoryUsage == TF_RESOURCE_MEMORY_USAGE_GPU_TO_CPU)
    {
        start_state = TF_RESOURCE_STATE_COPY_DEST;
    }
    ...
    if (pDesc->mMemoryUsage != TF_RESOURCE_MEMORY_USAGE_GPU_ONLY && pDesc->mFlags & TF_BUFFER_CREATION_FLAG_PERSISTENT_MAP_BIT)
    {
    ...
    if (!(pDesc->mFlags & TF_BUFFER_CREATION_FLAG_NO_DESCRIPTOR_VIEW_CREATION))
    {
        DescriptorHeap* pHeap = pRenderer->mDx.pCPUDescriptorHeaps[D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV];
        uint32_t        handleCount = ((pDesc->mDescriptors & TF_DESCRIPTOR_TYPE_UNIFORM_BUFFER) ? 1 : 0) +
                               ((pDesc->mDescriptors & TF_DESCRIPTOR_TYPE_BUFFER) ? 1 : 0) +
                               ((pDesc->mDescriptors & TF_DESCRIPTOR_TYPE_RW_BUFFER) ? 1 : 0);
        ...
        if (pDesc->mDescriptors & TF_DESCRIPTOR_TYPE_BUFFER)
        {
            ...
            {
                const bool raw = TF_DESCRIPTOR_TYPE_BUFFER_RAW == (pDesc->mDescriptors & TF_DESCRIPTOR_TYPE_BUFFER_RAW);
        ...
        if (pDesc->mDescriptors & TF_DESCRIPTOR_TYPE_RW_BUFFER)
        {
            ...
            {
                const bool      raw = TF_DESCRIPTOR_TYPE_RW_BUFFER_RAW == (pDesc->mDescriptors & TF_DESCRIPTOR_TYPE_RW_BUFFER_RAW);
...
void removeBuffer(TFRenderer* pRenderer, TFBuffer* pBuffer)
{
    ...
    {
        uint32_t handleCount = ((pBuffer->mDescriptors & TF_DESCRIPTOR_TYPE_UNIFORM_BUFFER) ? 1 : 0) +
                               ((pBuffer->mDescriptors & TF_DESCRIPTOR_TYPE_BUFFER) ? 1 : 0) +
                               ((pBuffer->mDescriptors & TF_DESCRIPTOR_TYPE_RW_BUFFER) ? 1 : 0);
...
{
    UNREF_PARAM(pRenderer);
    ASSERT(pBuffer->mMemoryUsage != TF_RESOURCE_MEMORY_USAGE_GPU_ONLY && "Trying to map non-cpu accessible resource");
...
{
    UNREF_PARAM(pRenderer);
    ASSERT(pBuffer->mMemoryUsage != TF_RESOURCE_MEMORY_USAGE_GPU_ONLY && "Trying to unmap non-cpu accessible resource");
...
void addTexture(TFRenderer* pRenderer, const TFTextureDesc* pDesc, TFTexture** ppTexture)
{
    ...
    TFTexture* pTexture = (TFTexture*)tf_calloc_memalign(1, TF_ALIGN_Texture, sizeof(TFTexture));
    pTexture->mDesc.mDx.mSrv = D3D12_DESCRIPTOR_ID_NONE;
    pTexture->mDesc.mDx.mUav = D3D12_DESCRIPTOR_ID_NONE;
    ASSERT(pTexture);
    ...
    TFDescriptorType descriptors = pDesc->mDescriptors;
    ...
    if (NULL == pTexture->mDx.pResource)
    {
        ...
        D3D12MA_ALLOCATION_DESC alloc_desc = {
            ...
            .mUseDedicatedAllocation = (pDesc->mFlags & TF_TEXTURE_CREATION_FLAG_OWN_MEMORY_BIT) != 0,
        ...
        else
        {
            ...
            {
#if defined(__ID3D12Device10_INTERFACE_DEFINED__) && !defined(XBOX)
                const bool useCastableFormats = (pDesc->mFlags & TF_TEXTURE_CREATION_FLAG_MUTABLE_FORMAT) != 0 &&
                                                pDesc->mAliasFormatCount > 0 && pRenderer->pGpu->mRelaxedFormatCastingSupported;
                if (useCastableFormats)
                {
                    DXGI_FORMAT* castableFormats = (DXGI_FORMAT*)alloca(pDesc->mAliasFormatCount * sizeof(DXGI_FORMAT));
                    for (uint32_t i = 0; i < pDesc->mAliasFormatCount; ++i)
                        castableFormats[i] = (DXGI_FORMAT)TinyImageFormat_ToDXGI_FORMAT(pDesc->pAliasFormats[i]);
                    ...
                    // Always use COMMON so the resource remains interoperable with legacy ResourceBarrier.
                    // Non-COMMON layouts put the resource into enhanced-barrier-only mode.
                    // TODO: Implement enhanced barriers?
                    D3D12MA_ALLOCATION_DESC3 alloc_desc3 = {
                        .InitialLayout = D3D12_BARRIER_LAYOUT_COMMON,
                        .HeapType = alloc_desc.HeapType,
                        .ExtraHeapFlags = alloc_desc.ExtraHeapFlags,
                        .CreationNodeMask = alloc_desc.CreationNodeMask,
                        .VisibleNodeMask = alloc_desc.VisibleNodeMask,
                        .pOptimizedClearValue = alloc_desc.pOptimizedClearValue,
                        .mUseDedicatedAllocation = alloc_desc.mUseDedicatedAllocation,
                        .NumCastableFormats = pDesc->mAliasFormatCount,
                        .pCastableFormats = castableFormats,
                    };
                    CHECK_HRESULT(D3D12MA_CreateResource3(pRenderer->mDx.pResourceAllocator, &alloc_desc3, &desc1,
                                                          &pTexture->mDx.pAllocation, &pTexture->mDx.pResource));
                }
                else
#else
                ASSERT(!(pDesc->mFlags & TF_TEXTURE_CREATION_FLAG_MUTABLE_FORMAT) ||
                       pDesc->mAliasFormatCount == 0 &&
                           "TF_TEXTURE_CREATION_FLAG_MUTABLE_FORMAT with alias formats requires ID3D12Device10");
#endif
                {
    ...
    {
        D3D12_GET_DESC(pTexture->mDx.pResource, &desc);
        if (pDesc->mFormat == TinyImageFormat_UNDEFINED)
        {
    ...
    if (descriptors & TF_DESCRIPTOR_TYPE_TEXTURE)
    {
        const bool isCube = TF_DESCRIPTOR_TYPE_TEXTURE_CUBE == (descriptors & TF_DESCRIPTOR_TYPE_TEXTURE_CUBE);
        AddTextureSrv(pRenderer, pTexture->mDx.pResource, &desc, dxFormat, isCube, false, 0, desc.MipLevels, 0, desc.DepthOrArraySize,
                      &pTexture->mDesc.mDx.mSrv);
    }

    if (descriptors & TF_DESCRIPTOR_TYPE_RW_TEXTURE)
    {
        // Single UAV for mip 0; use addTextureDescriptor for per-mip views.
        AddTextureUav(pRenderer, pTexture->mDx.pResource, &desc, dxFormat, 0, 0, desc.DepthOrArraySize, &pTexture->mDesc.mDx.mUav);
    }
...
void removeTexture(TFRenderer* pRenderer, TFTexture* pTexture)
{
    ...
    DescriptorHeap* pHeapSrvUav = pRenderer->mDx.pCPUDescriptorHeaps[D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV];
    if (pTexture->mDesc.mDx.mSrv != D3D12_DESCRIPTOR_ID_NONE)
        return_descriptor_handles(pHeapSrvUav, pTexture->mDesc.mDx.mSrv, 1);
    if (pTexture->mDesc.mDx.mUav != D3D12_DESCRIPTOR_ID_NONE)
        return_descriptor_handles(pHeapSrvUav, pTexture->mDesc.mDx.mUav, 1);
...
{
    ASSERT(pRenderer);
    ASSERT(pDesc && pDesc->pTexture);
    ASSERT(ppDescriptor);

    D3D12_RESOURCE_DESC resourceDesc = { 0 };
    D3D12_GET_DESC(pDesc->pTexture->mDx.pResource, &resourceDesc);

    // If no format is specified, fall back to the texture's own typed format.
    // Do NOT use resourceDesc.Format here — it is always TYPELESS (see InitializeTextureDesc),
    // and D3D12 rejects DXGI_FORMAT_UNKNOWN views on typeless resources.
    const TinyImageFormat viewFormat = pDesc->mFormat != TinyImageFormat_UNDEFINED ? pDesc->mFormat : pDesc->pTexture->mFormat;
    const DXGI_FORMAT     dxFormat = (DXGI_FORMAT)TinyImageFormat_ToDXGI_FORMAT(viewFormat);
    const bool            isUAV = (pDesc->mDescriptors & TF_DESCRIPTOR_TYPE_RW_TEXTURE) != 0;

    const UINT baseMip = pDesc->mBaseMipLevel;
    const UINT mipCount = pDesc->mMipLevelCount ? pDesc->mMipLevelCount : (resourceDesc.MipLevels - baseMip);
    const UINT baseLayer = pDesc->mBaseArrayLayer;
    const UINT layerCount = pDesc->mArrayLayerCount ? pDesc->mArrayLayerCount : (resourceDesc.DepthOrArraySize - baseLayer);

    TFTextureDescriptor* pDescriptor = (TFTextureDescriptor*)tf_calloc_memalign(1, TF_ALIGN_TextureDescriptor, sizeof(TFTextureDescriptor));
    ASSERT(pDescriptor);
    pDescriptor->mFormat = pDesc->mFormat;
    pDescriptor->mDx.mSrv = D3D12_DESCRIPTOR_ID_NONE;
    pDescriptor->mDx.mUav = D3D12_DESCRIPTOR_ID_NONE;

    if (isUAV)
        AddTextureUav(pRenderer, pDesc->pTexture->mDx.pResource, &resourceDesc, dxFormat, baseMip, baseLayer, layerCount,
                      &pDescriptor->mDx.mUav);
    else
    {
        const bool isStencil = pDesc->mIsStencil != 0;
        AddTextureSrv(pRenderer, pDesc->pTexture->mDx.pResource, &resourceDesc, dxFormat, false, isStencil, baseMip, mipCount, baseLayer,
                      layerCount, &pDescriptor->mDx.mSrv);
    }
...
void removeTextureDescriptor(TFRenderer* pRenderer, TFTextureDescriptor* pDescriptor)
{
    ...
    DescriptorHeap* pHeap = pRenderer->mDx.pCPUDescriptorHeaps[D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV];
    if (pDescriptor->mDx.mSrv != D3D12_DESCRIPTOR_ID_NONE)
        return_descriptor_handles(pHeap, pDescriptor->mDx.mSrv, 1);
    if (pDescriptor->mDx.mUav != D3D12_DESCRIPTOR_ID_NONE)
        return_descriptor_handles(pHeap, pDescriptor->mDx.mUav, 1);
    SAFE_FREE(pDescriptor);
}
...
void addRenderTarget(TFRenderer* pRenderer, const TFRenderTargetDesc* pDesc, TFRenderTarget** ppRenderTarget)
{
    ...
    ASSERT(!((isDepth) && ((pDesc->mDescriptors & TF_DESCRIPTOR_TYPE_RW_TEXTURE) == TF_DESCRIPTOR_TYPE_RW_TEXTURE)) &&
           "Cannot use depth stencil as UAV");

    ((TFRenderTargetDesc*)pDesc)->mMipLevels = (uint32_t)maxu(1U, pDesc->mMipLevels);

    uint32_t arraySize = pDesc->mArraySize;
#if defined(HOLOLENS2)
    if (pDesc->mFlags & TF_TEXTURE_CREATION_FLAG_VR_MULTIVIEW)
        arraySize = 2;
#endif

    TFRenderTarget* pRenderTarget = (TFRenderTarget*)tf_calloc_memalign(1, TF_ALIGN_RenderTarget, sizeof(TFRenderTarget));
    ...
    if (!isDepth)
        textureDesc.mStartState |= TF_RESOURCE_STATE_RENDER_TARGET;
    else
        textureDesc.mStartState |= TF_RESOURCE_STATE_DEPTH_WRITE;
    ...
    D3D12_RESOURCE_DESC desc = { 0 };
    D3D12_GET_DESC(pRenderTarget->pTexture->mDx.pResource, &desc);
...
#if defined(HOLOLENS2)
    pRenderTarget->mVRMultiview = (pDesc->mFlags & TF_TEXTURE_CREATION_FLAG_VR_MULTIVIEW) != 0;
    pRenderTarget->mVRFoveatedRendering = false;
#endif

    // Pre-bake the default full-RT view (mip 0, full array) directly into the embedded mDesc.
    // Load/store actions are specified inline at bind time; nothing to bake there.
    {
        ...
        pRenderTarget->mDesc.mDx.mDescriptorID = consume_descriptor_handles(pHeap, 1);

        if (isDepth)
            AddDsv(pRenderer, NULL, pRenderTarget->pTexture->mDx.pResource, dxFormat, 0, (uint32_t)-1,
                   &pRenderTarget->mDesc.mDx.mDescriptorID);
        else
            AddRtv(pRenderer, NULL, pRenderTarget->pTexture->mDx.pResource, dxFormat, 0, (uint32_t)-1,
                   &pRenderTarget->mDesc.mDx.mDescriptorID);

        pRenderTarget->mDesc.pRenderTarget = pRenderTarget;
    }
...
{
    const bool isDepth = TinyImageFormat_HasDepth(pRenderTarget->mFormat);

    // Release the embedded default descriptor's heap handle directly — do NOT free it
    // (it is embedded in TFRenderTarget, not heap-allocated).
    DescriptorHeap* pHeap = isDepth ? pRenderer->mDx.pCPUDescriptorHeaps[D3D12_DESCRIPTOR_HEAP_TYPE_DSV]
                                    : pRenderer->mDx.pCPUDescriptorHeaps[D3D12_DESCRIPTOR_HEAP_TYPE_RTV];
    return_descriptor_handles(pHeap, pRenderTarget->mDesc.mDx.mDescriptorID, 1);
...
{
    ASSERT(pRenderer);
    ASSERT(pDesc && pDesc->pRenderTarget);
    ASSERT(ppDescriptor);

    TFRenderTargetDescriptor* pRTDesc =
        (TFRenderTargetDescriptor*)tf_calloc_memalign(1, TF_ALIGN_RenderTargetDescriptor, sizeof(TFRenderTargetDescriptor));
    ASSERT(pRTDesc);

    const TFRenderTarget* pRT = pDesc->pRenderTarget;
    const bool            isDepth = TinyImageFormat_HasDepth(pRT->mFormat);
    const DXGI_FORMAT     dxFormat = (DXGI_FORMAT)TinyImageFormat_ToDXGI_FORMAT(pRT->mFormat);

    DescriptorHeap* pHeap = isDepth ? pRenderer->mDx.pCPUDescriptorHeaps[D3D12_DESCRIPTOR_HEAP_TYPE_DSV]
                                    : pRenderer->mDx.pCPUDescriptorHeaps[D3D12_DESCRIPTOR_HEAP_TYPE_RTV];
    pRTDesc->mDx.mDescriptorID = consume_descriptor_handles(pHeap, 1);

    const uint32_t mipSlice = pDesc->mUseMipSlice ? pDesc->mMipSlice : 0;
    const uint32_t arraySlice = pDesc->mUseArraySlice ? pDesc->mArraySlice : (uint32_t)-1;

    if (isDepth)
        AddDsv(pRenderer, NULL, pRT->pTexture->mDx.pResource, dxFormat, mipSlice, arraySlice, &pRTDesc->mDx.mDescriptorID);
    else
        AddRtv(pRenderer, NULL, pRT->pTexture->mDx.pResource, dxFormat, mipSlice, arraySlice, &pRTDesc->mDx.mDescriptorID);

    pRTDesc->pRenderTarget = pDesc->pRenderTarget;
    pRTDesc->mMipSlice = pDesc->mMipSlice;
    pRTDesc->mArraySlice = pDesc->mArraySlice;
    pRTDesc->mUseMipSlice = pDesc->mUseMipSlice;
    pRTDesc->mUseArraySlice = pDesc->mUseArraySlice;

    *ppDescriptor = pRTDesc;
}
...
void removeRenderTargetDescriptor(TFRenderer* pRenderer, TFRenderTargetDescriptor* pDescriptor)
{
    ...
    const bool      isDepth = TinyImageFormat_HasDepth(pDescriptor->pRenderTarget->mFormat);
    DescriptorHeap* pHeap = isDepth ? pRenderer->mDx.pCPUDescriptorHeaps[D3D12_DESCRIPTOR_HEAP_TYPE_DSV]
                                    : pRenderer->mDx.pCPUDescriptorHeaps[D3D12_DESCRIPTOR_HEAP_TYPE_RTV];
    return_descriptor_handles(pHeap, pDescriptor->mDx.mDescriptorID, 1);
    SAFE_FREE(pDescriptor);
}
...
void addSampler(TFRenderer* pRenderer, const TFSamplerDesc* pDesc, TFSampler** ppSampler)
{
    ...
    TFSampler* pSampler = (TFSampler*)tf_calloc_memalign(1, TF_ALIGN_Sampler, sizeof(TFSampler));
    ...
    float maxSamplerLod = pDesc->mMipMapMode == TF_MIPMAP_MODE_LINEAR ? D3D12_FLOAT32_MAX : 0;
    // user provided lods
    ...
    D3D12_SAMPLER_DESC desc = {
        .Filter = util_to_dx12_filter(pDesc->mMinFilter, pDesc->mMagFilter, pDesc->mMipMapMode, pDesc->mMaxAnisotropy > 0.0f,
                                      (pDesc->mCompareFunc != TF_CMP_NEVER ? true : false)),
        ...
        .MaxAnisotropy = (uint32_t)maxu((UINT)pDesc->mMaxAnisotropy, 1U),
...
#if defined(TF_ENABLE_WORKGRAPH)
            case TF_SHADER_STAGE_WORKGRAPH:
#endif
                pStage = &pDesc->mComp;
...
/************************************************************************/
void initRootSignatureImpl(TFRenderer* pRenderer, const void* pData, uint32_t dataSize, ID3D12RootSignature** ppOutRootSignature)
{
    ASSERT(pRenderer);
    ASSERT(pData);
    ASSERT(dataSize);
...
void exitRootSignatureImpl(TFRenderer* pRenderer, ID3D12RootSignature* pRootSignature)
{
    ASSERT(pRenderer);
...
void addDescriptorSet(TFRenderer* pRenderer, const TFDescriptorSetDesc* pDesc, TFDescriptorSet** ppDescriptorSet)
{
    ...
    TFDescriptorSet* pDescriptorSet = (TFDescriptorSet*)tf_calloc_memalign(1, TF_ALIGN_DescriptorSet, sizeof(TFDescriptorSet));
    ASSERT(pDescriptorSet);

    const TFDescriptor* highestViewDesc = NULL;
    const TFDescriptor* highestSamplerDesc = NULL;
    for (uint32_t descIndex = 0; descIndex < pDesc->mDescriptorCount; ++descIndex)
    {
        const TFDescriptor* desc = &pDesc->pDescriptors[descIndex];
        if (TF_DESCRIPTOR_TYPE_SAMPLER == desc->mType)
        {
    ...
    const uint32_t nodeIndex = pRenderer->mGpuMode == TF_GPU_MODE_LINKED ? pDesc->mNodeIndex : 0;
    ...
    if (highestViewDesc || highestSamplerDesc)
    {
        if (highestViewDesc)
        {
            ...
            {
                const TFDescriptor* pDescInfo = &pDesc->pDescriptors[i];
                {
                    TFDescriptorType type = (TFDescriptorType)pDescInfo->mType;
                    DxDescriptorID   srcHandle = D3D12_DESCRIPTOR_ID_NONE;
                    switch (type)
                    {
                    case TF_DESCRIPTOR_TYPE_TEXTURE:
                        // #TODO: desc->mDim?
                        srcHandle = pRenderer->pNullDescriptors->mNullTextureSRV[TF_TEXTURE_DIM_2D];
                        break;
                    case TF_DESCRIPTOR_TYPE_BUFFER:
                    ...
                    case TF_DESCRIPTOR_TYPE_RW_TEXTURE:
                        // #TODO: desc->mDim?
                        srcHandle = pRenderer->pNullDescriptors->mNullTextureUAV[TF_TEXTURE_DIM_2D];
                        break;
                    case TF_DESCRIPTOR_TYPE_RW_BUFFER:
...
void updateDescriptorSet(TFRenderer* pRenderer, uint32_t index, TFDescriptorSet* pDescriptorSet, uint32_t count,
                         const TFDescriptorData* pParams)
{
    ASSERT(pRenderer);
    ASSERT(pDescriptorSet);
    ...
    for (uint32_t i = 0; i < count; ++i)
    {
        ...
        const TFDescriptor*     pDesc = (pDescriptorSet->mDx.pDescriptors + pParam->mIndex);
        const TFDescriptorType  type = (TFDescriptorType)pDesc->mType; //-V522
        const uint32_t          arrayStart = pParam->mArrayOffset;
        const uint32_t          arrayCount = (uint32_t)maxu(1U, pParam->mCount);

        IF_VALIDATE_DESCRIPTOR(const uint32_t rootParamIndex = TF_DESCRIPTOR_TYPE_SAMPLER == type
        ...
        case TF_DESCRIPTOR_TYPE_TEXTURE:
        {
            if (pParam->mUseTextureDescriptors)
            {
                VALIDATE_DESCRIPTOR(pParam->ppTextureDescriptors, "NULL TextureDescriptor array (%s)", pDesc->pName);
                for (uint32_t arr = 0; arr < arrayCount; ++arr)
                {
                    VALIDATE_DESCRIPTOR(pParam->ppTextureDescriptors[arr], "NULL TextureDescriptor (%s [%u] )", pDesc->pName, arr);
                    copy_descriptor_handle(pRenderer->mDx.pCPUDescriptorHeaps[D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV],
                                           pParam->ppTextureDescriptors[arr]->mDx.mSrv, pRenderer->mDx.pCbvSrvUavHeaps[nodeIndex],
            ...
            else
            {
                ...
                for (uint32_t arr = 0; arr < arrayCount; ++arr)
                {
                    ...
                    copy_descriptor_handle(pRenderer->mDx.pCPUDescriptorHeaps[D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV],
                                           pParam->ppTextures[arr]->mDesc.mDx.mSrv, pRenderer->mDx.pCbvSrvUavHeaps[nodeIndex],
                                           pDescriptorSet->mDx.mCbvSrvUavHandle + index * pDescriptorSet->mDx.mCbvSrvUavStride +
                                               pDesc->mOffset + arrayStart + arr);
                }
            }
        ...
        case TF_DESCRIPTOR_TYPE_RW_TEXTURE:
        {
            VALIDATE_DESCRIPTOR(pParam->ppTextures || pParam->mUseTextureDescriptors, "NULL RW Texture (%s)", pDesc->pName);
            ...
            if (pParam->mUseTextureDescriptors)
            {
                ...
                for (uint32_t arr = 0; arr < arrayCount; ++arr)
                {
                    VALIDATE_DESCRIPTOR(pParam->ppTextureDescriptors[arr], "NULL TextureDescriptor (%s [%u] )", pDesc->pName, arr);
                    copy_descriptor_handle(pRenderer->mDx.pCPUDescriptorHeaps[D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV],
                                           pParam->ppTextureDescriptors[arr]->mDx.mUav, pRenderer->mDx.pCbvSrvUavHeaps[nodeIndex],
                                           pDescriptorSet->mDx.mCbvSrvUavHandle + index * pDescriptorSet->mDx.mCbvSrvUavStride +
                                               pDesc->mOffset + arrayStart + arr);
                }
            }
            ...
            {
                VALIDATE_DESCRIPTOR(pParam->ppTextures, "NULL RW Texture array (%s)", pDesc->pName);
                for (uint32_t arr = 0; arr < arrayCount; ++arr)
                {
                    VALIDATE_DESCRIPTOR(pParam->ppTextures[arr], "NULL RW Texture (%s [%u] )", pDesc->pName, arr);
                    // Single UAV (mip 0); use ppTextureDescriptors + addTextureDescriptor for per-mip access.
                    DxDescriptorID srcId = pParam->ppTextures[arr]->mDesc.mDx.mUav;
        ...
        case TF_DESCRIPTOR_TYPE_BUFFER_RAW:
        {
            ...
            if (pParam->pRanges)
            {
                const bool raw = TF_DESCRIPTOR_TYPE_BUFFER_RAW == type;
                for (uint32_t arr = 0; arr < arrayCount; ++arr)
                {
                    TFDescriptorDataRange range = pParam->pRanges[arr];
        ...
        case TF_DESCRIPTOR_TYPE_RW_BUFFER_RAW:
        {
            ...
            if (pParam->pRanges)
            {
                const bool raw = TF_DESCRIPTOR_TYPE_RW_BUFFER_RAW == type;
                for (uint32_t arr = 0; arr < arrayCount; ++arr)
                {
                    TFDescriptorDataRange range = pParam->pRanges[arr];
...
static void FlushDescriptorSets(TFCmd* pCmd)
{
    const uint32_t pipeline = pCmd->mDx.mPipelineType == TF_PIPELINE_TYPE_GRAPHICS ? 0 : 1;
    for (uint32_t tableIndex = 0; tableIndex < TF_MAX_DESCRIPTOR_TABLES; ++tableIndex)
    {
...
void cmdBindDescriptorSet(TFCmd* pCmd, uint32_t index, TFDescriptorSet* pDescriptorSet)
{
    ...
    if (pDescriptorSet->mDx.mCbvSrvUavHandle != D3D12_DESCRIPTOR_ID_NONE)
    {
        ...
        if (handle.ptr != pCmd->mDx.mBoundDescriptorSets[tableIndex].ptr)
        {
            ...
            if (pCmd->mDx.mPipelineType != TF_PIPELINE_TYPE_UNDEFINED)
            {
                if (TF_PIPELINE_TYPE_GRAPHICS == pCmd->mDx.mPipelineType)
                {
                    hook_SetGraphicsRootDescriptorTable(pCmd->mDx.pCmdList, tableIndex, handle);
                }
                else
...
/************************************************************************/
static const TFShader* getGraphicsShaderStage(const TFGraphicsPipelineDesc* pDesc, TFShaderStage stage)
{
    if (!pDesc)
        return NULL;

    if (pDesc->pShaderProgram)
        return (pDesc->pShaderProgram->mStages & stage) ? pDesc->pShaderProgram : NULL;

    const TFShader* pShader = NULL;
    switch (stage)
    {
    case TF_SHADER_STAGE_VERT:
        pShader = pDesc->pVertexShader;
        break;
    case TF_SHADER_STAGE_TESC:
        pShader = pDesc->pHullShader;
        break;
    case TF_SHADER_STAGE_TESE:
        pShader = pDesc->pDomainShader;
        break;
    case TF_SHADER_STAGE_GEOM:
        pShader = pDesc->pGeometryShader;
        break;
    case TF_SHADER_STAGE_FRAG:
        pShader = pDesc->pFragmentShader;
        break;
    default:
        return NULL;
    }

    return (pShader && (pShader->mStages & stage)) ? pShader : NULL;
}
static TFShaderStage getGraphicsShaderStages(const TFGraphicsPipelineDesc* pDesc)
{
    TFShaderStage stages = TF_SHADER_STAGE_NONE;

    if (getGraphicsShaderStage(pDesc, TF_SHADER_STAGE_VERT))
        stages = (TFShaderStage)(stages | TF_SHADER_STAGE_VERT);
    if (getGraphicsShaderStage(pDesc, TF_SHADER_STAGE_TESC))
        stages = (TFShaderStage)(stages | TF_SHADER_STAGE_TESC);
    if (getGraphicsShaderStage(pDesc, TF_SHADER_STAGE_TESE))
        stages = (TFShaderStage)(stages | TF_SHADER_STAGE_TESE);
    if (getGraphicsShaderStage(pDesc, TF_SHADER_STAGE_GEOM))
        stages = (TFShaderStage)(stages | TF_SHADER_STAGE_GEOM);
    if (getGraphicsShaderStage(pDesc, TF_SHADER_STAGE_FRAG))
        stages = (TFShaderStage)(stages | TF_SHADER_STAGE_FRAG);

    return stages;
}
static void addGraphicsPipeline(TFRenderer* pRenderer, const TFPipelineDesc* pMainDesc, TFPipeline** ppPipeline)
{
    ASSERT(pRenderer);
    ASSERT(ppPipeline);
    ASSERT(pMainDesc);

    const TFGraphicsPipelineDesc* pDesc = &pMainDesc->mGraphicsDesc;

    ASSERT(getGraphicsShaderStages(pDesc) != TF_SHADER_STAGE_NONE);
    ASSERT(pRenderer->mDx.pGraphicsRootSignature);

    TFPipeline* pPipeline = (TFPipeline*)tf_calloc_memalign(1, TF_ALIGN_Pipeline, sizeof(TFPipeline));
    ASSERT(pPipeline);

    const TFShader*       pVertexShader = getGraphicsShaderStage(pDesc, TF_SHADER_STAGE_VERT);
    const TFShader*       pFragmentShader = getGraphicsShaderStage(pDesc, TF_SHADER_STAGE_FRAG);
    const TFShader*       pHullShader = getGraphicsShaderStage(pDesc, TF_SHADER_STAGE_TESC);
    const TFShader*       pDomainShader = getGraphicsShaderStage(pDesc, TF_SHADER_STAGE_TESE);
    const TFShader*       pGeometryShader = getGraphicsShaderStage(pDesc, TF_SHADER_STAGE_GEOM);
    const TFVertexLayout* pVertexLayout = pDesc->pVertexLayout;
    ...
    pPipeline->mDx.mType = TF_PIPELINE_TYPE_GRAPHICS;
    ...
    if (pVertexShader)
    {
        VS.BytecodeLength = IDxcBlobEncoding_GetBufferSize(pVertexShader->mDx.pVSBlob);
        VS.pShaderBytecode = IDxcBlobEncoding_GetBufferPointer(pVertexShader->mDx.pVSBlob);
    }
    if (pFragmentShader)
    {
        PS.BytecodeLength = IDxcBlobEncoding_GetBufferSize(pFragmentShader->mDx.pPSBlob);
        PS.pShaderBytecode = IDxcBlobEncoding_GetBufferPointer(pFragmentShader->mDx.pPSBlob);
    }
    if (pHullShader)
    {
        HS.BytecodeLength = IDxcBlobEncoding_GetBufferSize(pHullShader->mDx.pHSBlob);
        HS.pShaderBytecode = IDxcBlobEncoding_GetBufferPointer(pHullShader->mDx.pHSBlob);
    }
    if (pDomainShader)
    {
        DS.BytecodeLength = IDxcBlobEncoding_GetBufferSize(pDomainShader->mDx.pDSBlob);
        DS.pShaderBytecode = IDxcBlobEncoding_GetBufferPointer(pDomainShader->mDx.pDSBlob);
    }
    if (pGeometryShader)
    {
        GS.BytecodeLength = IDxcBlobEncoding_GetBufferSize(pGeometryShader->mDx.pGSBlob);
        GS.pShaderBytecode = IDxcBlobEncoding_GetBufferPointer(pGeometryShader->mDx.pGSBlob);
    }
    ...
    D3D12_INPUT_ELEMENT_DESC input_elements[TF_MAX_VERTEX_ATTRIBS] = { 0 };

    char semantic_names[TF_MAX_VERTEX_ATTRIBS][TF_MAX_SEMANTIC_NAME_LENGTH] = { 0 };
    ...
    if (pVertexLayout != NULL)
    {
        ...
        {
            const TFVertexAttrib* attrib = &(pVertexLayout->mAttribs[attrib_index]);

            ASSERT(TF_SEMANTIC_UNDEFINED != attrib->mSemantic);
            ASSERT(attrib->mBinding < pVertexLayout->mBindingCount);

            if (attrib->mSemanticNameLength > 0)
            {
                uint32_t name_length = (uint32_t)minu((uint32_t)TF_MAX_SEMANTIC_NAME_LENGTH, attrib->mSemanticNameLength);
                strncpy_s(semantic_names[attrib_index], TF_MAX_SEMANTIC_NAME_LENGTH, attrib->mSemanticName, name_length);
            }
            else
            {
                switch (attrib->mSemantic)
                {
                case TF_SEMANTIC_POSITION:
                    strcpy_s(semantic_names[attrib_index], TF_MAX_SEMANTIC_NAME_LENGTH, "POSITION");
                    break;
                case TF_SEMANTIC_NORMAL:
                    strcpy_s(semantic_names[attrib_index], TF_MAX_SEMANTIC_NAME_LENGTH, "NORMAL");
                    break;
                case TF_SEMANTIC_COLOR:
                    strcpy_s(semantic_names[attrib_index], TF_MAX_SEMANTIC_NAME_LENGTH, "COLOR");
                    break;
                case TF_SEMANTIC_TANGENT:
                    strcpy_s(semantic_names[attrib_index], TF_MAX_SEMANTIC_NAME_LENGTH, "TANGENT");
                    break;
                case TF_SEMANTIC_BITANGENT:
                    strcpy_s(semantic_names[attrib_index], TF_MAX_SEMANTIC_NAME_LENGTH, "BITANGENT");
                    break;
                case TF_SEMANTIC_JOINTS:
                    strcpy_s(semantic_names[attrib_index], TF_MAX_SEMANTIC_NAME_LENGTH, "JOINTS");
                    break;
                case TF_SEMANTIC_WEIGHTS:
                    strcpy_s(semantic_names[attrib_index], TF_MAX_SEMANTIC_NAME_LENGTH, "WEIGHTS");
                    break;
                case TF_SEMANTIC_CUSTOM:
                    strcpy_s(semantic_names[attrib_index], TF_MAX_SEMANTIC_NAME_LENGTH, "CUSTOM");
                    break;
                case TF_SEMANTIC_TEXCOORD0:
                case TF_SEMANTIC_TEXCOORD1:
                case TF_SEMANTIC_TEXCOORD2:
                case TF_SEMANTIC_TEXCOORD3:
                case TF_SEMANTIC_TEXCOORD4:
                case TF_SEMANTIC_TEXCOORD5:
                case TF_SEMANTIC_TEXCOORD6:
                case TF_SEMANTIC_TEXCOORD7:
                case TF_SEMANTIC_TEXCOORD8:
                case TF_SEMANTIC_TEXCOORD9:
                    strcpy_s(semantic_names[attrib_index], TF_MAX_SEMANTIC_NAME_LENGTH, "TEXCOORD");
                    break;
                default:
                    ASSERT(false);
                    break;
            ...
            if (pVertexLayout->mBindings[attrib->mBinding].mRate == TF_VERTEX_BINDING_RATE_INSTANCE)
            {
            ...
            if (psoCache)
            {
                psoRenderHash = tf_mem_hash_uint8_t((uint8_t*)&attrib->mSemantic, sizeof(TFShaderSemantic), psoRenderHash);
                ...
                psoRenderHash = tf_mem_hash_uint8_t((uint8_t*)&pVertexLayout->mBindings[attrib->mBinding].mRate,
                                                    sizeof(TFVertexBindingRate), psoRenderHash);
            }
#endif
    ...
    uint32_t render_target_count = (uint32_t)minu(pDesc->mRenderTargetCount, (uint32_t)TF_MAX_RENDER_TARGET_ATTACHMENTS);
    render_target_count = (uint32_t)minu(render_target_count, (uint32_t)D3D12_SIMULTANEOUS_RENDER_TARGET_COUNT);
...
#ifndef DISABLE_PIPELINE_LIBRARY
    wchar_t pipelineName[TF_MAX_DEBUG_NAME_LENGTH + 32] = { 0 };
    if (psoCache)
    {
        ...
        swprintf(pipelineName, TF_MAX_DEBUG_NAME_LENGTH + 32, L"%S_S%zuR%zu", (pMainDesc->pName ? pMainDesc->pName : "GRAPHICSPSO"),
    ...
    {
#if defined(HOLOLENS2)
        // Same as the Vulkan backend (bIsMultiviewVR in Vulkan.c)
        bool vrMultiview = false;
        vrMultiview |= (pVertexShader && pVertexShader->mIsMultiviewVR);
        vrMultiview |= (pHullShader && pHullShader->mIsMultiviewVR);
        vrMultiview |= (pDomainShader && pDomainShader->mIsMultiviewVR);
        vrMultiview |= (pGeometryShader && pGeometryShader->mIsMultiviewVR);
        vrMultiview |= (pFragmentShader && pFragmentShader->mIsMultiviewVR);
        CHECK_HRESULT(hook_create_graphics_pipeline_state_vr(pRenderer->mDx.pDevice, &pipeline_state_desc, vrMultiview,
    ...
    case TF_PRIMITIVE_TOPO_PATCH_LIST:
    {
        uint32_t controlPoint = pHullShader ? pHullShader->mNumControlPoints : 0;
...
static void addComputePipeline(TFRenderer* pRenderer, const TFPipelineDesc* pMainDesc, TFPipeline** ppPipeline)
{
    ASSERT(pRenderer);
    ASSERT(ppPipeline);
    ASSERT(pMainDesc);

    const TFComputePipelineDesc* pDesc = &pMainDesc->mComputeDesc;
    ...
    pPipeline->mDx.mType = TF_PIPELINE_TYPE_COMPUTE;
    ...
    if (psoCache)
    {
        ...
        swprintf(pipelineName, TF_MAX_DEBUG_NAME_LENGTH + 32, L"%S_S%zu", (pMainDesc->pName ? pMainDesc->pName : "COMPUTEPSO"),
                 psoShaderHash);
...
#if defined(TF_ENABLE_WORKGRAPH)
static void addWorkgraphPipeline(TFRenderer* pRenderer, const TFPipelineDesc* pMainDesc, TFPipeline** ppPipeline);
#endif
...
#if defined(TF_ENABLE_WORKGRAPH)
    case TF_PIPELINE_TYPE_WORKGRAPH:
    {
...
void addPipelineCache(TFRenderer* pRenderer, const TFPipelineCacheDesc* pDesc, TFPipelineCache** ppPipelineCache)
{
    ...
    TFPipelineCache* pPipelineCache = (TFPipelineCache*)tf_calloc(1, sizeof(TFPipelineCache));
    ...
    if (SUCCEEDED(result))
    {
        ...
        if (feature.SupportFlags & D3D12_SHADER_CACHE_SUPPORT_LIBRARY)
        {
            ...
            result = COM_CALL(QueryInterface, pRenderer->mDx.pDevice, &IID_ID3D12Device1_Copy, &device1);
            if (SUCCEEDED(result))
            {
                result = hook_CreatePipelineLibrary(device1, pPipelineCache->mDx.pData, pDesc->mSize, &IID_ID3D12PipelineLibrary_Copy,
                                                    &pPipelineCache->mDx.pLibrary);
            }
...
void beginCmd(TFCmd* pCmd)
{
    ...
    if (pCmd->mDx.mType != TF_QUEUE_TYPE_TRANSFER)
    {
        ...
        if (pCmd->mDx.mType == TF_QUEUE_TYPE_GRAPHICS)
        {
    ...
    pCmd->mDx.mPipelineType = TF_PIPELINE_TYPE_UNDEFINED;
    pCmd->mDx.mDescriptorTableDirtyMask = 0;
...
/// Resolves the descriptor for a TFBindRenderTargetDesc.
/// When mUseRenderTargetDescriptor is set the caller supplied an explicit TFRenderTargetDescriptor*;
/// otherwise the default embedded descriptor (mip 0, full array) is read from pRenderTarget->mDesc.
static inline const TFRenderTargetDescriptor* GetRTDescriptor(const TFBindRenderTargetDesc* entry)
{
    return entry->mUseRenderTargetDescriptor ? entry->pDescriptor : &entry->pRenderTarget->mDesc;
}
...
void cmdBindRenderTargets(TFCmd* pCmd, const TFBindRenderTargetsDesc* pDesc)
{
    ...
    const bool hasDepthStencilEntry = pDesc->mDepthStencil.pRenderTarget || pDesc->mDepthStencil.mUseRenderTargetDescriptor;
    if (!pDesc->mRenderTargetCount && !hasDepthStencilEntry)
    {
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE rtvs[TF_MAX_RENDER_TARGET_ATTACHMENTS] = { 0 };
    const bool                  hasDepth = hasDepthStencilEntry;
    TFSampleCount               sampleCount = TF_SAMPLE_COUNT_1;

    for (uint32_t i = 0; i < pDesc->mRenderTargetCount; ++i)
    {
        const TFBindRenderTargetDesc*   entry = &pDesc->mRenderTargets[i];
        const TFRenderTargetDescriptor* desc = GetRTDescriptor(entry);
        sampleCount = desc->pRenderTarget->mSampleCount;
#if defined(XBOX)
        pCmd->mDx.mSampleCount = desc->pRenderTarget->mSampleCount;
#endif
        rtvs[i] =
            descriptor_id_to_cpu_handle(pCmd->pRenderer->mDx.pCPUDescriptorHeaps[D3D12_DESCRIPTOR_HEAP_TYPE_RTV], desc->mDx.mDescriptorID);

        if (entry->mLoadAction == TF_LOAD_ACTION_CLEAR)
        {
            const float* clearValue = entry->mOverrideClearValue ? &entry->mClearValue.r : &desc->pRenderTarget->mClearValue.r;
    ...
    const TFSampleLocationDesc* pSampleLocation = &pDesc->mSampleLocation;
    ...
    if (sampleLocationsCount > 1)
    {
        ...
        D3D12_SAMPLE_POSITION sampleLocations[TF_MAX_SAMPLE_LOCATIONS];
    ...
    if (hasDepth)
    {
        const TFBindRenderTargetDesc*   entry = &pDesc->mDepthStencil;
        const TFRenderTargetDescriptor* desc = GetRTDescriptor(entry);
#if defined(XBOX)
        pCmd->mDx.mSampleCount = desc->pRenderTarget->mSampleCount;
#endif
        dsv =
            descriptor_id_to_cpu_handle(pCmd->pRenderer->mDx.pCPUDescriptorHeaps[D3D12_DESCRIPTOR_HEAP_TYPE_DSV], desc->mDx.mDescriptorID);

        ASSERT(dsv.ptr != D3D12_GPU_VIRTUAL_ADDRESS_NULL);
        if (entry->mLoadAction == TF_LOAD_ACTION_CLEAR || entry->mLoadActionStencil == TF_LOAD_ACTION_CLEAR)
        {
            ...
            if (entry->mLoadActionStencil == TF_LOAD_ACTION_CLEAR)
            {
                flags |= D3D12_CLEAR_FLAG_STENCIL;
                ASSERT(TinyImageFormat_HasStencil(desc->pRenderTarget->mFormat));
            }
            ASSERT(flags > 0);
            const TFClearValue* cv = entry->mOverrideClearValue ? &entry->mClearValue : &desc->pRenderTarget->mClearValue;
            hook_ClearDepthStencilView(pCmd->mDx.pCmdList, dsv, flags, cv->depth, (UINT8)cv->stencil, 0, NULL);
        }
    }
...
void cmdBindPipeline(TFCmd* pCmd, TFPipeline* pPipeline)
{
    ...
    if (pPipeline->mDx.mType == TF_PIPELINE_TYPE_GRAPHICS)
    {
        ...
        pCmd->mDx.mPipelineType = TF_PIPELINE_TYPE_GRAPHICS;
    }
    else
    {
        ASSERT(pPipeline->mDx.pPipelineState);
        hook_SetPipelineState(pCmd->mDx.pCmdList, pPipeline->mDx.pPipelineState);
        pCmd->mDx.mPipelineType = TF_PIPELINE_TYPE_COMPUTE;
    }
...
void cmdBindIndexBuffer(TFCmd* pCmd, TFBuffer* pBuffer, uint32_t indexType, uint64_t offset)
{
    ...
    D3D12_INDEX_BUFFER_VIEW ibView = {
        ...
        .Format = (TF_INDEX_TYPE_UINT16 == indexType) ? DXGI_FORMAT_R16_UINT : DXGI_FORMAT_R32_UINT,
...
void cmdBindVertexBuffer(TFCmd* pCmd, uint32_t bufferCount, TFBuffer** ppBuffers, const uint32_t* pStrides, const uint64_t* pOffsets)
{
    ...
    D3D12_VERTEX_BUFFER_VIEW views[TF_MAX_VERTEX_ATTRIBS] = { 0 };
...
#if defined(_WINDOWS) && defined(D3D12_RAYTRACING_AVAILABLE) && defined(TF_ENABLE_GRAPHICS_VALIDATION)
    // Bug in validation when using acceleration structure in compute or graphics pipeline
    // D3D12 ERROR: ID3D12CommandList::Dispatch: Static Descriptor SRV resource dimensions (UNKNOWN (11)) differs from that expected by
    // shader (D3D12_SRV_DIMENSION_BUFFER) UNKNOWN (11) is D3D12_SRV_DIMENSION_RAYTRACING_ACCELERATION_STRUCTURE
    // // #TODO
                        ...
                        TFTextureBarrier* pTextureBarriers, uint32_t numRtBarriers, TFRenderTargetBarrier* pRtBarriers)
{
    ...
    {
        TFBufferBarrier*        pTransBarrier = &pBufferBarriers[i];
        D3D12_RESOURCE_BARRIER* pBarrier = &barriers[transitionCount];
        TFBuffer*               pBuffer = pTransBarrier->pBuffer;
        ...
        if (pBuffer->mMemoryUsage == TF_RESOURCE_MEMORY_USAGE_GPU_ONLY || pBuffer->mMemoryUsage == TF_RESOURCE_MEMORY_USAGE_GPU_TO_CPU ||
            (pBuffer->mMemoryUsage == TF_RESOURCE_MEMORY_USAGE_CPU_TO_GPU && (pBuffer->mDescriptors & TF_DESCRIPTOR_TYPE_RW_BUFFER)))
        {
            // if (!(pBuffer->mCurrentState & pTransBarrier->mNewState) && pBuffer->mCurrentState != pTransBarrier->mNewState)
            if (TF_RESOURCE_STATE_UNORDERED_ACCESS == pTransBarrier->mCurrentState &&
                TF_RESOURCE_STATE_UNORDERED_ACCESS == pTransBarrier->mNewState)
            {
...
#ifdef D3D12_RAYTRACING_AVAILABLE
            else if ((TF_RESOURCE_STATE_ACCELERATION_STRUCTURE_WRITE & pTransBarrier->mCurrentState) &&
                     (TF_RESOURCE_STATE_ACCELERATION_STRUCTURE_READ & pTransBarrier->mNewState))
            {
                pBarrier->Type = D3D12_RESOURCE_BARRIER_TYPE_UAV;
                pBarrier->Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
                pBarrier->UAV.pResource = pBuffer->mDx.pResource;
    ...
    {
        TFTextureBarrier*       pTrans = &pTextureBarriers[i];
        D3D12_RESOURCE_BARRIER* pBarrier = &barriers[transitionCount];
        TFTexture*              pTexture = pTrans->pTexture;

        if (TF_RESOURCE_STATE_UNORDERED_ACCESS == pTrans->mCurrentState && TF_RESOURCE_STATE_UNORDERED_ACCESS == pTrans->mNewState)
        {
            pBarrier->Type = D3D12_RESOURCE_BARRIER_TYPE_UAV;
            pBarrier->Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
    ...
    {
        TFRenderTargetBarrier*  pTrans = &pRtBarriers[i];
        D3D12_RESOURCE_BARRIER* pBarrier = &barriers[transitionCount];
        TFTexture*              pTexture = pTrans->pRenderTarget->pTexture;
...
void cmdUpdateSubresource(TFCmd* pCmd, TFTexture* pTexture, TFBuffer* pSrcBuffer, const SubresourceDataDesc* pDesc)
{
    uint32_t subresource =
        CALC_SUBRESOURCE_INDEX(pDesc->mMipLevel, pDesc->mArrayLayer, 0, pTexture->mMipLevels, pTexture->mArraySizeMinusOne + 1);
    D3D12_RESOURCE_DESC resourceDesc;
    D3D12_GET_DESC(pTexture->mDx.pResource, &resourceDesc);
    ...
    COM_CALL(GetCopyableFootprints, pCmd->pRenderer->mDx.pDevice, &resourceDesc, 0, 1, pDesc->mSrcOffset, &src.PlacedFootprint, NULL, NULL,
             NULL);
    ...
    hook_copy_texture_region(pCmd, &dst, pDesc->mX, pDesc->mY, 0, &src, NULL);
}

void cmdCopySubresource(TFCmd* pCmd, TFBuffer* pDstBuffer, TFTexture* pTexture, const SubresourceDataDesc* pDesc)
{
    uint32_t subresource =
        CALC_SUBRESOURCE_INDEX(pDesc->mMipLevel, pDesc->mArrayLayer, 0, pTexture->mMipLevels, pTexture->mArraySizeMinusOne + 1);
    D3D12_RESOURCE_DESC resourceDesc;
    ...
    COM_CALL(GetCopyableFootprints, pCmd->pRenderer->mDx.pDevice, &resourceDesc, 0, 1, pDesc->mSrcOffset, &dst.PlacedFootprint, NULL, NULL,
             NULL);
...
/************************************************************************/
#ifdef HOLOLENS2
void acquireNextImageOpenXR(const XrSwapchain swapchainHandle, uint32_t* outSwapchainImageIndex)
{
    XrSwapchainImageAcquireInfo acquireInfo = { .type = XR_TYPE_SWAPCHAIN_IMAGE_ACQUIRE_INFO };
    CHECK_OXRCMD(xrAcquireSwapchainImage(swapchainHandle, &acquireInfo, outSwapchainImageIndex));

    XrSwapchainImageWaitInfo waitInfo = { .type = XR_TYPE_SWAPCHAIN_IMAGE_WAIT_INFO };
    waitInfo.timeout = XR_INFINITE_DURATION;
    CHECK_OXRCMD(xrWaitSwapchainImage(swapchainHandle, &waitInfo));
}
#endif

void acquireNextImage(TFRenderer* pRenderer, TFSwapChain* pSwapChain, TFSemaphore* pSignalSemaphore, TFFence* pFence,
...
#ifdef HOLOLENS2
    acquireNextImageOpenXR(pSwapChain->mVR.pSwapchain, pSwapChainImageIndex);

    TFSwapChain* p2DLayerSwapchain = pSwapChain->mVR.m2DLayer.pSwapchain;
    if (p2DLayerSwapchain)
        acquireNextImageOpenXR(p2DLayerSwapchain->mVR.pSwapchain, &pSwapChain->mVR.m2DLayer.mCurrentSwapChainIndex);
...
#ifdef HOLOLENS2
void endFrameOpenXR(OpenXRPresentDesc* desc)
{
    XrSession xrSession = getXRSession();
    ASSERT(xrSession != XR_NULL_HANDLE);

    XrFrameState* pCurrentFrameState = getXRCurrentFrameState();
    ASSERT(pCurrentFrameState->type == XR_TYPE_FRAME_STATE);

    // shouldn't ever hit this, if we shouldn't render
    // the current frame we skip it at the application level
    if (!pCurrentFrameState->shouldRender)
    {
        // we shouldnt render, end frame with 0 layers
        XrFrameEndInfo endInfo = { .type = XR_TYPE_FRAME_END_INFO };
        endInfo.displayTime = pCurrentFrameState->predictedDisplayTime;
    ...
    XrCompositionLayerProjectionView
        projectionLayerViews[MAX_OPENXR_VIEWS] = { [LEFT_EYE_VIEW_INDEX] = { 0 }, [RIGHT_EYE_VIEW_INDEX] = { 0 } };
    ...
    if (desc->m2DLayer.mSwapchainHandle != XR_NULL_HANDLE)
    {
        ...
        pose.position.y =
            desc->m2DLayerSettings.m2DLayerPosition.y - 0.1f; // TODO this is a temporary fix to make UI top bar visible in display FOV
        ...
        // Extra scale to fit the 2D layer quad in the HoloLens 2 display FOV. The UI controller
        // raycast compensates for this (see CheckController2DLayerIntersection in UI.cpp).
        const float hololensUIScale = 0.55f;
        float       effectiveScale = desc->m2DLayerSettings.m2DLayerScale * hololensUIScale;
        if (effectiveScale <= 0.0f)
            effectiveScale = hololensUIScale;
    ...
    CHECK_OXRCMD(xrEndFrame(xrSession, &frameEndInfo));

    // Preformance warning when drawing ui: NotExactlyOneProjLayerSubmitted
    // Exactly one projection layer passed in xrEndFrame is neceesary to enable the fastest composition path
}

void queuePresentOpenXR(const TFQueuePresentDesc* pQueuePresentDesc)
{
    TFSwapChain* pSwapchain = pQueuePresentDesc->pSwapChain;
    TFSwapChain* p2DLayerSwapChain = pSwapchain->mVR.m2DLayer.pSwapchain;

    XrSwapchainImageReleaseInfo releaseInfo = { .type = XR_TYPE_SWAPCHAIN_IMAGE_RELEASE_INFO };
    CHECK_OXRCMD(xrReleaseSwapchainImage(pSwapchain->mVR.pSwapchain, &releaseInfo));

    OpenXRPresentDesc presentDesc;
    memset(&presentDesc, 0, sizeof(presentDesc));

    presentDesc.mMainView.mSwapchainHandle = pSwapchain->mVR.pSwapchain;
    presentDesc.mMainView.mWidth = pSwapchain->ppRenderTargets[0]->mWidth;
    presentDesc.mMainView.mHeight = pSwapchain->ppRenderTargets[0]->mHeight;

    presentDesc.m2DLayer.mSwapchainHandle = XR_NULL_HANDLE;

    if (p2DLayerSwapChain != NULL)
    {
        CHECK_OXRCMD(xrReleaseSwapchainImage(p2DLayerSwapChain->mVR.pSwapchain, &releaseInfo));

        presentDesc.m2DLayer.mSwapchainHandle = p2DLayerSwapChain->mVR.pSwapchain;
        presentDesc.m2DLayer.mWidth = p2DLayerSwapChain->ppRenderTargets[0]->mWidth;
        presentDesc.m2DLayer.mHeight = p2DLayerSwapChain->ppRenderTargets[0]->mHeight;
        presentDesc.m2DLayerSettings = p2DLayerSwapChain->mVR.m2DLayer.mDesc;
    }

    endFrameOpenXR(&presentDesc);
}
#endif // HOLOLENS2
...
void queuePresent(TFQueue* pQueue, const TFQueuePresentDesc* pDesc)
{
    ...
    queuePresentOpenXR(pDesc);
    return;
#else // unreachable code warning

    TFSwapChain* pSwapChain = pDesc->pSwapChain;
#if defined(_WINDOWS) && defined(TF_ENABLE_GRAPHICS_VALIDATION)
    TFRenderer*  pRenderer = pQueue->mDx.pRenderer;
    ...
    if (FAILED(hr))
    {
        ...
        {
            pRenderer->mDeviceLost = 1;
        }
        VERIFY(SUCCEEDED(removeHr));
...
static inline void GetFenceStatus(TFFence* pFence, TFFenceStatus* pFenceStatus)
{
    if (!pFence->mDx.mFenceValue)
    {
        *pFenceStatus = TF_FENCE_STATUS_NOTSUBMITTED;
    }
    else if (COM_CALL(GetCompletedValue, pFence->mDx.pFence) < pFence->mDx.mFenceValue)
    {
        *pFenceStatus = TF_FENCE_STATUS_INCOMPLETE;
    }
    else
    {
...
static void WaitForFences(uint32_t fenceCount, TFFence** ppFences)
{
...
uint32_t getRecommendedSwapchainImageCount(TFRenderer* pRenderer, const TFWindowHandle* hwnd)
{
    ...
    if (imageCount == 0)
    {
        // minimal swapchain desc just to query the image count
        XrSwapchainCreateInfo swapchainCI = { XR_TYPE_SWAPCHAIN_CREATE_INFO };
        swapchainCI.usageFlags = XR_SWAPCHAIN_USAGE_SAMPLED_BIT | XR_SWAPCHAIN_USAGE_COLOR_ATTACHMENT_BIT;
        // must match getSupportedSwapchainFormat so the queried count applies to the real swapchain
        swapchainCI.format = util_to_dx12_swapchain_format(TinyImageFormat_B8G8R8A8_UNORM);
        swapchainCI.width = getXRViewConfigView(0).recommendedImageRectWidth;
        swapchainCI.height = getXRViewConfigView(0).recommendedImageRectHeight;
        swapchainCI.sampleCount = 1;
        swapchainCI.faceCount = 1;
        ...
        CHECK_OXRCMD(xrCreateSwapchain(getXRSession(), &swapchainCI, &xrSwapchain));
        CHECK_OXRCMD(xrEnumerateSwapchainImages(xrSwapchain, 0, &imageCount, NULL));
        CHECK_OXRCMD(xrDestroySwapchain(xrSwapchain));

        ASSERT(imageCount > 0);
        LOGF(eINFO, "OpenXR recommended swapchain image count: %u", imageCount);
    }
...
/************************************************************************/
void cmdExecuteIndirect(TFCmd* pCmd, TFIndirectArgumentType type, uint32_t maxCommandCount, TFBuffer* pIndirectBuffer,
                        uint64_t bufferOffset, TFBuffer* pCounterBuffer, uint64_t counterBufferOffset)
{
    ...
    case TF_INDIRECT_DRAW_INDEX:
        ...
        break;
    case TF_INDIRECT_DISPATCH:
#if defined(XBOX)
        if (TF_QUEUE_TYPE_COMPUTE == pCmd->mDx.mType)
        {
...
static void addWorkgraphPipeline(TFRenderer* pRenderer, const TFPipelineDesc* pMainDesc, TFPipeline** ppPipeline)
{
    ...
    const TFWorkgraphPipelineDesc* desc = &pMainDesc->mWorkgraphDesc;

    size_t      extSize = sizeof(WCHAR) * (strlen(desc->pWorkgraphName) + 1);
    TFPipeline* pipeline = (TFPipeline*)tf_calloc_memalign(1, TF_ALIGN_Pipeline, sizeof(TFPipeline) + extSize);
    ASSERT(pipeline);

    pipeline->mDx.mType = TF_PIPELINE_TYPE_WORKGRAPH;
    ...
    HRESULT        hres = COM_CALL(QueryInterface, pRenderer->mDx.pDevice, IID_REF(ID3D12Device9), &device);
    if (!SUCCEEDED(hres))
    {
...
void addWorkgraph(TFRenderer* pRenderer, const TFWorkgraphDesc* pDesc, TFWorkgraph** ppWorkgraph)
{
    ...
    TFWorkgraph* workgraph = (TFWorkgraph*)tf_calloc_memalign(1, sizeof(void*), sizeof(TFWorkgraph));
    ASSERT(workgraph);
    ...
    {
        TFBufferDesc backingDesc = {
            .mMemoryUsage = TF_RESOURCE_MEMORY_USAGE_GPU_ONLY,
            .mFlags = TF_BUFFER_CREATION_FLAG_NO_DESCRIPTOR_VIEW_CREATION,
            .mDescriptors = TF_DESCRIPTOR_TYPE_RW_BUFFER,
            .mStartState = TF_RESOURCE_STATE_COMMON,
...
void cmdDispatchWorkgraph(TFCmd* pCmd, const TFDispatchGraphDesc* pDesc)
{
    ...
    const TFDispatchGraphInputType input = pDesc->mInputType;
    switch (input)
    {
    case TF_DISPATCH_GRAPH_INPUT_CPU:
    {
    ...
    case TF_DISPATCH_GRAPH_INPUT_GPU:
    {
    ...
    default:
        ASSERTFAIL("Invalid TFDispatchGraphInputType");
        break;
    }
...
{
    D3D12_LINEAR_ALGEBRA_MATRIX_CONVERSION_INFO* allConvDesc =
        (D3D12_LINEAR_ALGEBRA_MATRIX_CONVERSION_INFO*)tf_calloc(descCount, sizeof(D3D12_LINEAR_ALGEBRA_MATRIX_CONVERSION_INFO));
    ID3D12DevicePreview* device = NULL;
    HRESULT              hres = E_FAIL;

    for (uint32_t i = 0; i < descCount; i++)
    {
        bool inputLayoutOptimal =
            (pDesc[i].mInputMatrixDesc.mLayout == TF_MUL_OPTIMAL || pDesc[i].mInputMatrixDesc.mLayout == TF_OUTER_PRODUCT_OPTIMAL);
        bool outputLayoutOptimal =
            (pDesc[i].mOutputMatrixDesc.mLayout == TF_MUL_OPTIMAL || pDesc[i].mOutputMatrixDesc.mLayout == TF_OUTER_PRODUCT_OPTIMAL);
        D3D12_LINEAR_ALGEBRA_MATRIX_CONVERSION_INFO convDesc = {
            // DestInfo
            { outputLayoutOptimal ? 0 : pDesc[i].mWidth * pDesc[i].mHeight * pDesc[i].mOutputMatrixDesc.mElemSize,
              (D3D12_LINEAR_ALGEBRA_MATRIX_LAYOUT)pDesc[i].mOutputMatrixDesc.mLayout,
              outputLayoutOptimal ? 0 : pDesc[i].mWidth * pDesc[i].mOutputMatrixDesc.mElemSize, pDesc[i].mWidth, pDesc[i].mHeight,
              (D3D12_LINEAR_ALGEBRA_DATATYPE)pDesc[i].mOutputMatrixDesc.mDataType },

            // SrcInfo
            { inputLayoutOptimal ? 0 : pDesc[i].mWidth * pDesc[i].mHeight * pDesc[i].mInputMatrixDesc.mElemSize,
              (D3D12_LINEAR_ALGEBRA_DATATYPE)pDesc[i].mInputMatrixDesc.mDataType,
              (D3D12_LINEAR_ALGEBRA_MATRIX_LAYOUT)pDesc[i].mInputMatrixDesc.mLayout,
              inputLayoutOptimal ? 0 : pDesc[i].mWidth * pDesc[i].mInputMatrixDesc.mElemSize },

            // DataDesc
            { pDesc[i].mOutputMatrixDesc.mMatrixBuffer->mDx.mGpuAddress + pDesc[i].mOutputMatrixDesc.mOffset,
              pDesc[i].mInputMatrixDesc.mMatrixBuffer->mDx.mGpuAddress + pDesc[i].mInputMatrixDesc.mOffset }
        };
        allConvDesc[i] = convDesc;
        hres = COM_CALL(QueryInterface, pRenderer->mDx.pDevice, IID_REF(ID3D12DevicePreview), &device);
        if (!SUCCEEDED(hres))
        {
            ASSERTFAIL("ID3D12DevicePreview query failed");
            return;
        }
        COM_CALL(GetLinearAlgebraMatrixConversionDestinationInfo, device, &allConvDesc[i].DestInfo);
    }
    ID3D12GraphicsCommandListPreview* cmd = NULL;
    hres = COM_CALL(QueryInterface, pCmd->mDx.pCmdList, IID_REF(ID3D12GraphicsCommandListPreview), &cmd);
    if (!SUCCEEDED(hres))
    {
        ASSERTFAIL("ID3D12GraphicsCommandListPreview query failed");
        return;
    }
    COM_CALL(ConvertLinearAlgebraMatrix, cmd, allConvDesc, descCount);
...
void initQueryPool(TFRenderer* pRenderer, const TFQueryPoolDesc* pDesc, TFQueryPool** ppQueryPool)
{
    ...
    const uint32_t queryCount = pDesc->mQueryCount * (TF_QUERY_TYPE_TIMESTAMP == pDesc->mType ? 2 : 1);
    ...
    COM_CALL(CreateQueryHeap, pRenderer->mDx.pDevice, &desc, &IID_ID3D12QueryHeap_Copy, &pQueryPool->mDx.pQueryHeap);
    SetObjectName((ID3D12Object*)pQueryPool->mDx.pQueryHeap, pDesc->pName); // -V1027

    TFBufferDesc bufDesc = {
        .mMemoryUsage = TF_RESOURCE_MEMORY_USAGE_GPU_TO_CPU,
        .mFlags = TF_BUFFER_CREATION_FLAG_OWN_MEMORY_BIT,
        .mElementCount = queryCount,
        .mSize = queryCount * pQueryPool->mStride,
        .mStructStride = pQueryPool->mStride,
        .pName = pDesc->pName,
        .mNodeIndex = pDesc->mNodeIndex,
        .mStartState = TF_RESOURCE_STATE_COPY_DEST,
    };
...
/************************************************************************/
void logMemoryStats(TFRenderer* pRenderer) { D3D12MA_BuildStatsString(pRenderer->mDx.pResourceAllocator, TRUE); }
...
void cmdWriteMarker(TFCmd* pCmd, const TFMarkerDesc* pDesc)
{
    ...
    if (pDesc->mFlags & TF_MARKER_FLAG_WAIT_FOR_WRITE)
    {
