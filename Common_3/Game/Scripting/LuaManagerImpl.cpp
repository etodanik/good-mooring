...
{
    UNREF_PARAM(L);
    TFFileStream* pHandle = (TFFileStream*)ud;
...
bool RunScriptFile(const char* scriptFile, lua_State* L)
{
    ...
    if (!fsOpenStreamFromPath(TF_RD_SCRIPTS, scriptFile, TF_FM_READ, &fh))
    {
#ifdef AUTOMATED_TESTING
...
bool LuaManagerImpl::SetUpdatableScript(const char* scriptFile, const char* updateFunctionName, const char* exitFunctionName)
{
    ...
    if ((void*)m_UpdateFunctionName.data != (void*)updateFunctionName)
    {
        bassigncstr(&m_UpdateFunctionName, updateFunctionName);
        ASSERT(!bownsdata(&m_UpdateFunctionName));
    }

    if ((void*)m_UpdatableScriptExitName.data != (void*)exitFunctionName)
    {
    ...
    TFFileStream fHandle = {};
    if (!fsOpenStreamFromPath(TF_RD_SCRIPTS, scriptFile, TF_FM_READ, &fHandle))
    {
        LOGF(eERROR, "Failed to open script file %s. Function %s failed with error: %s", scriptFile, FS_ERR_CTX.func,
             getFSErrCodeString(FS_ERR_CTX.code));
        return false;
...
void LuaManagerImpl::AddAsyncScript(const char* scriptFile, IScriptCallbackWrap* callbackLambda)
{
    ...
    // TFThreadDesc* pItem = (TFThreadDesc*)tf_calloc(1, sizeof(*pItem));

    // pItem->pFunc = AsyncScriptExecute;
    // pItem->pData = info;

    {
        TFMutexLock lock(m_AddAsyncScriptMutex);
...
void LuaManagerImpl::SetFunction(ILuaFunctionWrap* wrap)
{
    ...
    for (int i = 0; i < MAX_LUA_WORKERS; ++i)
    {
        TFMutexLock lock(m_AsyncLuaStatesMutex[i]);
