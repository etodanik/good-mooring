...
#if (defined(QUEST_VR) || defined(HOLOLENS2)) && defined(ENABLE_FORGE_UI)
...
#define MONO_CAMERA_VIEW_INDEX LEFT_EYE_VIEW_INDEX
...
//   - FORGE_RENDERER_API: is used to expose TFRenderer functionality, wich usually only the Engine needs access to (the Game should
//   leverage the Engine and never call the TFRenderer).
//                         This define allows to hide TFRenderer functionality to the Game, so that if someone tries to use it it'll get a
//                         linker error.
...
#if defined(PROSPERO)
#define SUPPORT_LOW_ENERGY_MODE 1
#endif
