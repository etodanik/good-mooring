...
struct TFRenderer;
struct TFQueue;
struct TFFontDrawDesc;
struct TFUserInterface;
struct TFMSDFFontDrawDesc;

typedef struct TFProfilerDesc
{
    TFRenderer* pRenderer = NULL;
    TFQueue**   ppQueues = NULL;
...
FORGE_API void initProfiler(TFProfilerDesc* pDesc);
...
FORGE_API float2 cmdDrawGpuProfile(TFCmd* pCmd, float2 screenCoordsInPx, ProfileToken nProfileToken, TFFontDrawDesc* pDrawDesc);

// Call once per frame before AppUI.Draw, draw requested Cpu profile time
// Returns text dimensions so caller can align other UI elements
FORGE_API float2 cmdDrawCpuProfile(TFCmd* pCmd, float2 screenCoordsInPx, TFFontDrawDesc* pDrawDesc);
...
FORGE_API ProfileToken initGpuProfiler(TFRenderer* pRenderer, TFQueue* pQueue, const char* pProfilerName);
...
FORGE_API void cmdBeginGpuFrameProfile(TFCmd* pCmd, ProfileToken nProfileToken, bool bUseMarker = true);
...
FORGE_API void cmdEndGpuFrameProfile(TFCmd* pCmd, ProfileToken nProfileToken);

FORGE_API ProfileToken cmdBeginGpuTimestampQuery(TFCmd* pCmd, ProfileToken nProfileToken, const char* pName, bool bUseMarker = true);

FORGE_API void cmdEndGpuTimestampQuery(TFCmd* pCmd, ProfileToken nProfileToken);
...
struct TFCpuProfileScopeMarker
{
    ProfileToken nToken;
    uint64_t     nTick;
    TFCpuProfileScopeMarker(const char* pGroup, const char* pName, uint32_t nColor)
    {
        nToken = getCpuProfileToken(pGroup, pName, nColor);
        nTick = cpuProfileEnter(nToken);
    }
    ~TFCpuProfileScopeMarker() { cpuProfileLeave(nToken, nTick); }
};
...
#define PROFILER_SET_CPU_SCOPE(group, name, color) TFCpuProfileScopeMarker PROFILER_CONCAT(marker, __LINE__)(group, name, color)
