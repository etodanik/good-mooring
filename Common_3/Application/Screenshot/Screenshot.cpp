...
typedef struct ScreenshotCapturer
{
    ...
    TFShader*        pCopyShader;
    TFShader*        pCopyArrayShader;
    TFPipeline*      pCopyPipeline;
    TFPipeline*      pCopyArrayPipeline;
    TFDescriptorSet* pCopyDescriptorSet;
    TFCmdPool*       pCmdPool;
    TFCmd*           pCmd;
    TFQueue*         pQueue;
    TFFence*         pFence;
    TFBuffer*        pScreenshotScratchBuffer;
    TFBuffer*        pScreenShotUniformsBuffer;
    TFSampler*       pSampler;
    size_t           mAppNameLen;
    bool             mScreenshotCaptureRequested;
    char             mScreenshotName[TF_FS_MAX_PATH];
} ScreenshotCapturer;
...
static void saveScreenshotToDisk(const char* name, TFBuffer* pBuffer, bool saveAsHDR, bool discardAlpha, uint32_t width, uint32_t height)
{
    char        screenshotFileName[TF_FS_MAX_PATH] = {};
    ...
    TFFileStream fs = {};
    if (fsOpenStreamFromPath(TF_RD_SCREENSHOTS, screenshotFileName, TF_FM_WRITE, &fs))
    {
...
void initScreenshotCapturer(TFRenderer* pRenderer, TFQueue* pGraphicsQueue, const char* appName)
{
#ifdef ENABLE_SCREENSHOT
    ...
    *pScreenshotCapturer = {};
    ...
    if (VERIFYMSG(pScreenshotCapturer->mAppNameLen < TF_FS_MAX_PATH,
                  "App name exceeds TF_FS_MAX_PATH. It is ignored by screenshot capturer."))
    {
    ...
    TFSamplerDesc pointSamplerDesc = {};
    pointSamplerDesc.mMinFilter = TF_FILTER_NEAREST;
    pointSamplerDesc.mMagFilter = TF_FILTER_NEAREST;
    pointSamplerDesc.mMipMapMode = TF_MIPMAP_MODE_NEAREST;
    pointSamplerDesc.mAddressU = TF_ADDRESS_MODE_CLAMP_TO_EDGE;
    pointSamplerDesc.mAddressV = TF_ADDRESS_MODE_CLAMP_TO_EDGE;
    pointSamplerDesc.mAddressW = TF_ADDRESS_MODE_CLAMP_TO_EDGE;
    ...
    TFShaderLoadDesc copyShaderDesc = {};
    ...
    TFDescriptorSetDesc descriptorSetDesc = SRT_SET_DESC(SrtCopyCompData, PerDraw, 1, 0);
    ...
    TFComputePipelineDesc* copyPipelineDesc = &pipelineDesc.mComputeDesc;
...
#ifdef ENABLE_SCREENSHOT
    ASSERT(pScreenshotCapturer);
    TFRenderer* pRenderer = pScreenshotCapturer->pRenderer;
...
void requestScreenshotCapture(const char* name)
{
    ...
    if (VERIFYMSG(nameLen + appNameLen + 4 < TF_FS_MAX_PATH,
                  "App name + screenshot name's total size exceeds TF_FS_MAX_PATH. Only using app name."))
    {
...
void captureScreenshot(TFScreenshotDesc* pDesc)
{
    MTRACY_ZONE("captureScreenshot");
#ifdef ENABLE_SCREENSHOT
    ASSERT(pScreenshotCapturer);

    TFRenderer*     pRenderer = pScreenshotCapturer->pRenderer;
    TFRenderTarget* pRenderTarget = pDesc->pRenderTarget;

    bool saveAsHDR = pDesc->mColorSpace > TF_COLOR_SPACE_SDR_SRGB;
    bool convertToSrgb =
    ...
    TFCmd* cmd = pScreenshotCapturer->pCmd;
    ...
    if (!pScreenshotCapturer->pScreenshotScratchBuffer || rtSize > pScreenshotCapturer->pScreenshotScratchBuffer->mSize)
    {
        ...
        TFSyncToken      token = {};
        TFBufferLoadDesc scratchBufferLoadDesc = {};
        scratchBufferLoadDesc.mDesc.mDescriptors = TF_DESCRIPTOR_TYPE_RW_BUFFER_RAW;
        scratchBufferLoadDesc.mDesc.mSize = rtSize;
        scratchBufferLoadDesc.mDesc.mElementCount = (rtSize + 3) / sizeof(uint32_t);
        scratchBufferLoadDesc.mDesc.mMemoryUsage = TF_RESOURCE_MEMORY_USAGE_GPU_TO_CPU;
        // dx12 defaults to TF_RESOURCE_STATE_COMMON for RW buffer allocated in non-default heap
        scratchBufferLoadDesc.mDesc.mStartState = TF_RESOURCE_STATE_COMMON;
        scratchBufferLoadDesc.mDesc.mFlags = TF_BUFFER_CREATION_FLAG_PERSISTENT_MAP_BIT;
        ...
        TFBufferBarrier bufBarrier = {};
        bufBarrier.pBuffer = pScreenshotCapturer->pScreenshotScratchBuffer;
        bufBarrier.mCurrentState = TF_RESOURCE_STATE_COMMON;
        bufBarrier.mNewState = TF_RESOURCE_STATE_COPY_SOURCE;
    ...
    TFBufferUpdateDesc uniformsUpdateDesc = { pScreenshotCapturer->pScreenShotUniformsBuffer };
    ...
    TFPipeline*      pCopyPipeline = pScreenshotCapturer->pCopyPipeline;
    TFDescriptorData copyParam[3] = {};
    ...
    TFBufferBarrier bufBarrier = {};
    bufBarrier.pBuffer = pScreenshotCapturer->pScreenshotScratchBuffer;
    bufBarrier.mCurrentState = TF_RESOURCE_STATE_COPY_SOURCE;
    bufBarrier.mNewState = TF_RESOURCE_STATE_UNORDERED_ACCESS;
    TFRenderTargetBarrier rtBarrier = {};
    rtBarrier.pRenderTarget = pRenderTarget;
    rtBarrier.mCurrentState = TF_RESOURCE_STATE_PRESENT;
    rtBarrier.mNewState = TF_RESOURCE_STATE_SHADER_RESOURCE;
    cmdResourceBarrier(cmd, 1, &bufBarrier, 0, NULL, 1, &rtBarrier);

    uint3 threadGroupSize = *(uint3*)pScreenshotCapturer->pCopyShader->mNumThreadsPerGroup;
    ...
    bufBarrier.mCurrentState = TF_RESOURCE_STATE_UNORDERED_ACCESS;
    bufBarrier.mNewState = TF_RESOURCE_STATE_COPY_SOURCE;
    rtBarrier.mCurrentState = TF_RESOURCE_STATE_SHADER_RESOURCE;
    rtBarrier.mNewState = TF_RESOURCE_STATE_PRESENT;
    cmdResourceBarrier(cmd, 1, &bufBarrier, 0, NULL, 1, &rtBarrier);
