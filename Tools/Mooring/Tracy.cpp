...
{
    auto severity = level & eERROR     ? tracy::MessageSeverity::Error
                    : level & eWARNING ? tracy::MessageSeverity::Warning
                    : level & eDEBUG   ? tracy::MessageSeverity::Debug
...
void mooringTracyAppInfo(const char* app)
{
    ...
    TracyParameterRegister(
        [](void*, uint32_t index, int32_t value)
        {
            if (index == 0)
                mooringTracyMetalImages(unsigned(value > 0 ? value : 0));
        },
        nullptr);
...
{
    if (sceneSection && sceneConnection == connectionId())
        TracySectionLeave(sceneSection);
    sceneSection = 0;
...
void mooringTracyCpuEnter(const char* name, const char* group, unsigned color)
{
    ...
    {
        if (forgeDepth == 129)
            TracyMessageL("Forge CPU profiling exceeded 128 nested zones; deeper scopes are omitted.");
        return;
    }
    bool active = true;
#ifdef TRACY_ON_DEMAND
    ...
    const uint64_t source = active ? tracy::Profiler::AllocSourceLocation(__LINE__, __FILE__, sizeof(__FILE__) - 1, group, strlen(group),
...
{
    if (forgeDepth && --forgeDepth < 128)
        TracyCZoneEnd(forgeZones[forgeDepth]);
}
