...
#include <cstdlib>
#include <cstring>
#include "Common/Utilities/Interfaces/IThread.h"
...
static void worker(void* data)
{
    ...
    MTRACY_ZONE("Tracy test worker");
    MTRACY_VALUE(id);
    for (unsigned i = 0; i < iterations; ++i)
    {
        acquireMutex(&mutex);
        while (turn != id)
            waitConditionVariable(&condition, &mutex, 10);
        // Both recursive acquire APIs must preserve lock depth.
        acquireMutex(&mutex);
        CHECK(tryAcquireMutex(&mutex));
        releaseMutex(&mutex);
        releaseMutex(&mutex);
        turn = 1 - id;
        ...
        CHECK((reinterpret_cast<uintptr_t>(aligned) & 255) == 0);
        tf_free(aligned);
    }
}
...
int main(int argc, char** argv)
{
    ...
    tf_free(nullptr);
    exitConditionVariable(&condition);
    exitMutex(&mutex);
