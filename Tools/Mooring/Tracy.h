...
#define MTRACY_ZONE_COLOR(name, color)                                                                                                     \
    ...
    TracyCZoneCtx mtracyZone __attribute__((cleanup(mooringTracyEndZone))) = MTRACY_BEGIN(&TracyConcat(mtracySource, __LINE__))
#define MTRACY_ZONE(name) MTRACY_ZONE_COLOR(name, 0)
#define MTRACY_TEXT(text)                                 \
    do                                                    \
    {                                                     \
        if (mtracyZone.active)                            \
        {                                                 \
            const char* t = (text);                       \
            if (t)                                        \
                TracyCZoneText(mtracyZone, t, strlen(t)); \
        }                                                 \
    } while (0)
...
#define MTRACY_ALLOC(ptr, size, pool)                                  \
    do                                                                 \
    {                                                                  \
        if (ptr)                                                       \
            TracyCAllocNS(ptr, size, MOORING_TRACY_STACK_DEPTH, pool); \
    } while (0)
#define MTRACY_FREE(ptr, pool)                                  \
    do                                                          \
    {                                                           \
        if (ptr)                                                \
            TracyCFreeNS(ptr, MOORING_TRACY_STACK_DEPTH, pool); \
    } while (0)
#else
#define MTRACY_ALLOC(ptr, size, pool)      \
    do                                     \
    {                                      \
        if (ptr)                           \
            TracyCAllocN(ptr, size, pool); \
    } while (0)
#define MTRACY_FREE(ptr, pool)      \
    do                              \
    {                               \
        if (ptr)                    \
            TracyCFreeN(ptr, pool); \
    } while (0)
#endif
#ifdef __cplusplus
extern "C"
