...
void publish(Queue* q, const Encoder& e, uint64_t start, uint64_t end)
{
    ...
    const auto source = Profiler::AllocSourceLocation(__LINE__, __FILE__, sizeof(__FILE__)-1,
        "Metal encoder", 13, e.name, strlen(e.name), 0x5599CC);
    ...
    if (c->connection == connection() && recording() && c->pending.status == MTLCommandBufferStatusCompleted) {
        if (c->count) {
            ...
            for (unsigned i=0; i<c->count; ++i) {
                ...
                if (!e.work) continue; // Metal may optimize empty encoders away.
                if (e.samples != buffer || e.startIndex < first || e.endIndex > last) {
                    buffer = e.samples; first = e.startIndex; last = e.endIndex;
                ...
                if (start < c->queue->firstTimestamp || end < start || start <= c->lastTimestamp || end > gpuNow) {
                    if (dropped.fetch_add(1) < 4) {
                        ...
                        snprintf(message, sizeof(message), "Discarded Metal timestamps in %s: %llu..%llu, valid clock range %llu..%llu",
                            e.name, (unsigned long long)start, (unsigned long long)end,
                            (unsigned long long)c->queue->firstTimestamp, (unsigned long long)gpuNow);
        ...
        if (c->hasImage) {
            ...
            if (lag <= 255) FrameImage(c->image.contents, c->width, c->height, uint8_t(lag), false);
        }
    }
    c->pending = nil;
    c->hasImage = false;
...
Command* beginEncoder(TFCmd* cmd, const char* name)
{
    ...
    if (!c || !c->recording) return nullptr;
    if (c->count == MaxEncoders) { ++dropped; return nullptr; }
...
void mooringTracyMetalInitQueue(TFQueue* queue, TFRenderer* renderer)
{
    ...
    for (id<MTLCounterSet> set in q->device.counterSets)
        if ([set.name isEqualToString:MTLCommonCounterSetTimestamp]) q->counters = set;
    if (!q->counters) { tf_delete(q); return; }
...
void mooringTracyMetalExitCmd(TFCmd* cmd)
{
    ...
    if (c->pending) {
        ...
        for (auto* p = c->queue->head; p; previous = p, p = p->next) if (p == c) {
            if (previous) previous->next = c->next; else c->queue->head = c->next;
            if (c->queue->tail == c) c->queue->tail = previous;
            dropped += c->count;
            break;
        }
    }
...
void mooringTracyMetalBeginCmd(TFCmd* cmd)
{
    ...
    if (!c->samples) {
        ...
        if (!c->samples) { TracyMessageL("Cannot allocate Tracy Metal timestamp buffer."); c->recording = false; return; }
    }
...
{
    auto* c = static_cast<Command*>(cmd->pTracy);
    if (!c) { [cmd->pCommandBuffer commit]; return; }
    std::lock_guard lock(c->queue->publish);
    if (c->recording) {
        c->pending = cmd->pCommandBuffer;
        if (c->queue->tail) c->queue->tail->next = c; else c->queue->head = c;
        c->queue->tail = c;
    }
...
void mooringTracyMetalRender(TFCmd* cmd, MTLRenderPassDescriptor* desc)
{
    if (auto* c = beginEncoder(cmd, "Render")) {
        ...
        for (unsigned i=0; i<8; ++i) if (desc.colorAttachments[i].loadAction == MTLLoadActionClear) c->encoders[c->current].work = 1;
        if (desc.depthAttachment.loadAction == MTLLoadActionClear) c->encoders[c->current].work = 1;
    }
}
...
void mooringTracyMetalWork(TFCmd* cmd, const char* kind, uint64_t count)
{
    ...
    auto& e = c->encoders[c->current];
    e.work += count;
    if (e.pipeline == cmd->pBoundPipeline) return;
    NSString* name = nil;
    if (cmd->pBoundPipeline) name = cmd->pBoundPipeline->mType == TF_PIPELINE_TYPE_COMPUTE ?
        cmd->pBoundPipeline->pComputePipelineState.label : cmd->pBoundPipeline->pRenderPipelineState.label;
    if (name.length) {
        if (!e.pipeline && !e.namedPass) snprintf(e.name, sizeof(e.name), "%s / %s", kind, name.UTF8String);
        else {
...
void mooringTracyMetalFrameImage(TFCmd* cmd, TFRenderTarget* target)
{
    ...
    if (!interval || !c || !c->recording || frameIndex.load()%interval) return;
    ...
    if (!q->thumbnail) {
        ...
        if (!q->thumbnail) { TracyMessageL("Tracy thumbnail shader failed to compile."); imageInterval = 0; return; }
    }
