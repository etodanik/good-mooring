...
int main()
{
    ...
    for (unsigned sampleIndex = 0; sampleIndex < 64; ++sampleIndex)
        input[sampleIndex] = actual[sampleIndex] = { std::sin(sampleIndex * .37f) * .1f, std::cos(sampleIndex * .19f) * .1f };
    for (unsigned twiddleIndex = 0; twiddleIndex < 4; ++twiddleIndex)
        twiddles[twiddleIndex] = { std::cos(6.2831853f * twiddleIndex / 8), std::sin(6.2831853f * twiddleIndex / 8) };
    ...
    for (unsigned row = 0; row < 8; ++row)
        ...
        {
            Complex expected = {};
            for (unsigned ky = 0; ky < 8; ++ky)
                for (unsigned kx = 0; kx < 8; ++kx)
                {
                    double phase = 6.283185307179586 * (kx * column + ky * row) / 8;
                    auto   mode = input[ky * 8 + kx];
                    expected.real += mode.real * std::cos(phase) - mode.imaginary * std::sin(phase);
                    expected.imaginary += mode.real * std::sin(phase) + mode.imaginary * std::cos(phase);
                }
            error = std::max(error, std::fabs(expected.real - actual[row * 8 + column].real));
            error = std::max(error, std::fabs(expected.imaginary - actual[row * 8 + column].imaginary));
        }
    CHECK(error < .00002f);
    ...
    for (float depth : { 2.0f, 8.0f, 100.0f })
    {
        ...
        CHECK(std::fabs(dispersion(waveNumber, depth) / waveNumber - phaseSpeed) < .00001f);
    }
    CHECK(wakeWavelength(5, 2) == 0);
    float waveNumber = 6.28318530718f / wakeWavelength(3.8f * .81649658f, 100);
    float group = groupVelocity(waveNumber, 100);
    CHECK(std::fabs(group * .57735027f / (3.8f - group * .81649658f) - 1 / std::sqrt(8.0f)) < .00001f);
    SeaState sea;
    sea.swellHeight = 0;
    sea.choppiness = 0;
    sea.current = {};
    Ocean* ocean = createOcean(sea);
    Ocean* same = createOcean(sea);
    ...
    glass.windSpeed = 0;
    glass.choppiness = .2f;
    glass.windWaveHeight = 0;
    glass.swellHeight = .06f;
    auto* quiet = createOcean(glass);
    CHECK(oceanMetrics(quiet).whitecapFraction == 0);
    for (unsigned calibrationIndex = 0; calibrationIndex < 2; ++calibrationIndex)
        CHECK(oceanMetrics(quiet).breakingThreshold[calibrationIndex][0] == 0 &&
              oceanMetrics(quiet).breakingThreshold[calibrationIndex][2] == 0);
    destroyOcean(quiet);
    ...
    {
        SeaState wind = sea;
        wind.windSpeed = setting == 0 ? 0 : setting == 1 ? 1 : 7;
        ...
        for (const auto& mode : ripples)
        {
            ...
            if (power > 0)
                CHECK(mode.waveNumber >= 6.2831853f / .30f && mode.waveNumber <= 6.2831853f / .035f);
        }
        rippleSlope[setting] = std::sqrt(rippleSlope[setting]);
    }
    CHECK(rippleSlope[0] == 0);
    CHECK(rippleSlope[1] < .002f);
    ...
    for (unsigned sampleIndex = 0; sampleIndex < 120; ++sampleIndex)
    {
        updateOcean(ocean, sampleIndex * FixedStep);
        auto sample = sampleOcean(ocean, sampleIndex * .1f, -3);
    ...
    for (float worldX : { 27.999f, 28.0f, 28.001f })
    {
        auto  center = sampleWavePackets(ocean, worldX, 28);
        float dx = (sampleWavePackets(ocean, worldX + .001f, 28).height - sampleWavePackets(ocean, worldX - .001f, 28).height) / .002f;
        float dz = (sampleWavePackets(ocean, worldX, 28.001f).height - sampleWavePackets(ocean, worldX, 27.999f).height) / .002f;
        CHECK(std::fabs(center.dx - dx) < .00003f);
        CHECK(std::fabs(center.dz - dz) < .00003f);
    }
    clearWavePackets(ocean);
    ...
    advanceWavePackets(ocean, 13, { 0, 0, 0 });
    CHECK(activeWavePackets(ocean) == 0);
    CHECK(memGetStatistics().accumulatedAllocUnitCount == allocations);
    clearWavePackets(ocean);
    updateOcean(ocean, 2);
    ...
    for (float worldX : { -3.0f, 0.0f, 3.0f })
        for (float worldZ : { -5.0f, 0.0f, 4.0f })
        {
            auto  center = sampleWavePackets(ocean, worldX, worldZ);
            float dx =
                (sampleWavePackets(ocean, worldX + .001f, worldZ).height - sampleWavePackets(ocean, worldX - .001f, worldZ).height) / .002f;
            float dz =
                (sampleWavePackets(ocean, worldX, worldZ + .001f).height - sampleWavePackets(ocean, worldX, worldZ - .001f).height) / .002f;
            CHECK(std::fabs(center.dx - dx) < .0001f);
            CHECK(std::fabs(center.dz - dz) < .0001f);
        }
    hullWake.forward.z = -1;
    hullWake.velocity.z = -3;
    setHullWake(ocean, 0, hullWake);
    CHECK(std::fabs(sampleWavePackets(ocean, 2.5f, -4.8f).height - bow.height) < .000001f);
    hullWake.amplitude = 0;
    setHullWake(ocean, 0, hullWake);
    ...
    SeaState flat = sea;
    flat.windWaveHeight = flat.swellHeight = 0;
    auto* flatOcean = createOcean(flat);
    hullWake.amplitude = .25f;
    setHullWake(flatOcean, 0, hullWake);
    ...
    SeaState flowing = sea;
    flowing.current = { 1, 0, -1 };
    configureOcean(ocean, flowing);
    updateOcean(ocean, 2);
    ...
    CHECK(std::fabs(state.position.y) < 1.5f);
    ...
    for (unsigned stepIndex = 0; stepIndex < 6300; ++stepIndex)
        step(world);
    CHECK(statistics(world).steadyStepAllocations == 0);
    CHECK(droppedWavePackets(worldOcean(world)) == 0);
    CHECK(std::isfinite(snapshot(world, false).position.y));
    CHECK(statistics(world).physicsErrors == 0);
    for (unsigned resetIndex = 0; resetIndex < 12; ++resetIndex)
    {
        resetWorld(world);
        step(world);
    }
