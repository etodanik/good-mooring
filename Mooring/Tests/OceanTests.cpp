...
int main()
{
    ...
    for (unsigned i = 0; i < 64; ++i)
        input[i] = actual[i] = { std::sin(i * .37f) * .1f, std::cos(i * .19f) * .1f };
    for (unsigned i = 0; i < 4; ++i)
        twiddles[i] = { std::cos(6.2831853f * i / 8), std::sin(6.2831853f * i / 8) };
    ...
    for (unsigned p = 0; p < 2; ++p)
        CHECK(oceanMetrics(quiet).breakingThreshold[p][0] == 0 && oceanMetrics(quiet).breakingThreshold[p][2] == 0);
    destroyOcean(quiet);
    ...
    {
        updateOcean(ocean, i * FixedStep);
        auto s = sampleOcean(ocean, i * .1f, -3);
        CHECK(std::isfinite(s.height));
    }
