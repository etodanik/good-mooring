...
static void run(World* w, unsigned steps)
{
...
static float firstWashArrival(const Propeller& p, float distance)
{
    ...
    {
        float t = i * FixedStep;
        updatePropeller(p, state, 1, t, FixedStep);
        Vec3 point = p.position;
        point.z -= distance;
        if (rudderWash(p, state, point, t).z < -.001f)
            return t;
    }
    return 100;
}
int main(int argc, char** argv)
{
    ...
    {
        if (csv)
            std::fprintf(csv, "%s,%s,%.2f,%.4f,%.4f,%.4f,%.4f,%.4f\n", vessel, scenario, s.time, s.speedKnots, heading(s), s.position.x,
                         s.position.z, roll(s) * 57.2957795f);
    ...
    for (auto layout : { mono, cat })
    {
        ...
        run(w, 600);
        CHECK(std::fabs(snapshot(w, false).position.y) < .01f);
        InitialConditions displaced;
        displaced.position.y = .3f;
        displaced.rotation = { 0, 0, std::sin(angle * .5f), std::cos(angle * .5f) };
        resetWorld(w, &displaced);
        run(w, 1800);
        CHECK(std::fabs(snapshot(w, false).position.y) < .03f);
        CHECK(std::fabs(roll(snapshot(w, false))) < .01f);
        record(layout.name, "heave_roll_decay", snapshot(w, false));
        resetWorld(w);
        for (unsigned engine = 0; engine < layout.propellerCount; ++engine)
            enqueue(w, { CommandType::Throttle, 0, Station::Helm, uint8_t(engine), .7f });
        run(w, 3600);
        auto ahead = snapshot(w, false);
        ...
        for (unsigned engine = 0; engine < layout.propellerCount; ++engine)
            enqueue(w, { CommandType::Throttle, 0, Station::Helm, uint8_t(engine), 0 });
        run(w, 1200);
        auto coast = snapshot(w, false);
    ...
    for (unsigned i = 0; i < 240; ++i)
        updatePropeller(cat.propellers[0], state, 1, i * FixedStep, FixedStep);
    CHECK(rudderWash(cat.propellers[0], state, cat.rudders[0].position, 239 * FixedStep).z == 0);
    for (unsigned i = 240; i < 720; ++i)
        updatePropeller(cat.propellers[0], state, -1, i * FixedStep, FixedStep);
