...
static TFRenderTarget* sceneTarget(Scene* s, unsigned width, unsigned height, TinyImageFormat format, bool depth, const char* name,
...
static void triangle(Scene* s, Vec3 a, Vec3 b, Vec3 c, Vec3 color, float material = 0)
{
    ...
    s->mesh[s->count++] = { a, normal, color, material };
    s->mesh[s->count++] = { b, normal, color, material };
    s->mesh[s->count++] = { c, normal, color, material };
}
static void box(Scene* s, Vec3 p, Vec3 size, Vec3 color, const Snapshot* boat = nullptr, float material = 0)
{
    ...
    for (unsigned i = 0; i < 8; ++i)
    {
        ...
        if (boat)
            v[i] = deckToWorld(*boat, v[i]);
    }
    constexpr unsigned faces[6][4] = { { 0, 4, 6, 2 }, { 1, 3, 7, 5 }, { 0, 1, 5, 4 }, { 2, 6, 7, 3 }, { 0, 2, 3, 1 }, { 4, 5, 7, 6 } };
    for (auto& f : faces)
    {
        triangle(s, v[f[0]], v[f[1]], v[f[2]], color, material);
        triangle(s, v[f[0]], v[f[2]], v[f[3]], color, material);
    }
}
static void hull(Scene* s, const Snapshot& boat, const VesselLayout& layout, unsigned index)
{
    ...
    {
        Vec3 points[3];
        hullTriangle(layout, index, i, points);
...
Scene* createScene(TFRenderer* r)
{
    ...
    {
        load.mDesc.mSize = sizeof s->mesh;
        load.mDesc.mDescriptors = TF_DESCRIPTOR_TYPE_VERTEX_BUFFER;
        load.ppBuffer = &s->vertices[i];
        ...
        for (unsigned p = 0; p < 3; ++p)
        {
            ...
            load.ppBuffer = &s->cameras[i][p];
            addResource(&load, nullptr);
        }
        for (unsigned p = 0; p < TF_ARRAY_COUNT(s->postCameras[i]); ++p)
        {
            load.mDesc.mSize = sizeof(PostConstants);
            load.ppBuffer = &s->postCameras[i][p];
            addResource(&load, nullptr);
        }
    }
...
static bool loadSceneShaders(Scene* s, uint32_t format)
{
    ...
    {
        vertices.mAttribs[i].mFormat = TinyImageFormat_R32G32B32_SFLOAT;
        vertices.mAttribs[i].mLocation = i;
        vertices.mAttribs[i].mOffset = i * sizeof(Vec3);
    }
    ...
    TFDepthStateDesc ds = {};
    ds.mDepthTest = ds.mDepthWrite = true;
    ds.mDepthFunc = TF_CMP_LEQUAL;
    TFRasterizerStateDesc rs = {};
    rs.mCullMode = TF_CULL_MODE_NONE;
    TFPipelineDesc p = {};
    ...
    g.pShaderProgram = s->shader;
    g.pVertexLayout = &vertices;
    g.pDepthState = &ds;
...
void unloadScene(Scene* s)
{
    ...
    for (auto* target : { s->depth, s->opaque, s->composite, s->airRays, s->reflection, s->reflectionDepth, s->shadow, s->shadowDepth,
...
void destroyScene(Scene* s)
{
    ...
    for (unsigned i = 0; i < 2; ++i)
    {
        ...
        for (auto* b : s->cameras[i])
            removeResource(b);
        for (auto* b : s->postCameras[i])
            removeResource(b);
    }
    tf_delete(s);
}
void connectSceneWater(Scene* s, OceanRenderer* water)
{
    ...
    for (unsigned i = 0; i < 2; ++i)
        for (unsigned p = 0; p < 6; ++p)
        {
            ...
            d[0].mIndex = SRT_RES_IDX(MarinaPost, PerFrame, gPost);
            d[0].ppBuffers = &s->postCameras[i][p];
            d[1].mIndex = SRT_RES_IDX(MarinaPost, PerFrame, gSceneColor);
            d[1].ppTextures = p >= 4 ? &s->bloom[p - 4]->pTexture : p ? &s->composite->pTexture : &s->opaque->pTexture;
            d[2].mIndex = SRT_RES_IDX(MarinaPost, PerFrame, gSceneSampler);
            d[2].ppSamplers = &s->sampler;
            d[3].mIndex = SRT_RES_IDX(MarinaPost, PerFrame, gPostShadow);
            d[3].ppTextures = &s->shadow->pTexture;
            d[4].mIndex = SRT_RES_IDX(MarinaPost, PerFrame, gAirRays);
            d[4].ppTextures = p == 1 ? &s->airRays->pTexture : &s->opaque->pTexture;
            d[5].mIndex = SRT_RES_IDX(MarinaPost, PerFrame, gBloomNear);
            d[5].ppTextures = p == 1 ? &s->bloom[0]->pTexture : &s->opaque->pTexture;
            d[6].mIndex = SRT_RES_IDX(MarinaPost, PerFrame, gBloomMiddle);
            d[6].ppTextures = p == 1 ? &s->bloom[1]->pTexture : &s->opaque->pTexture;
            d[7].mIndex = SRT_RES_IDX(MarinaPost, PerFrame, gBloomFar);
            d[7].ppTextures = p == 1 ? &s->bloom[2]->pTexture : &s->opaque->pTexture;
            d[8].mIndex = SRT_RES_IDX(MarinaPost, PerFrame, gSkyRadiance);
            d[8].ppTextures = &sky;
    ...
    for (unsigned i = 0; i < 2; ++i)
        for (unsigned p = 0; p < 3; ++p)
        {
            ...
            d[0].mIndex = SRT_RES_IDX(MarinaScene, PerFrame, gCamera);
               ...
               const WaterProfile* profile)
{
    ...
    for (int n = -2; n <= 2; ++n)
    {
        box(s, { 8.95f, .83f, n * 5.0f }, { .2f, .15f, .65f }, { .45f, .48f, .47f });
    }
    bool cat = layout.hullSpacing > 0;
    if (cat)
    {
        hull(s, boat, layout, 0);
        hull(s, boat, layout, 1);
        box(s, { 0, .66f, -.5f }, { 6.0f, .14f, 8.8f }, { .78f, .8f, .74f }, &boat);
    }
    else
        hull(s, boat, layout, 0);
    ...
    {
        auto p = stationPosition(station);
        p.y = .77f;
    ...
    person.y = 1.41f;
    box(s, person, { .25f, .25f, .25f }, { .73f, .5f, .32f }, &boat);
    ...
    for (unsigned p = 0; p < 3; ++p)
    {
        cameras[p].modeLevel[0] = float(p);
        cameras[p].modeLevel[1] = level;
        cameras[p].modeLevel[2] = boat.time;
        float4 effects;
        look.parameters(cameras[p].weather, effects);
        cameras[p].sun = sun;
    ...
    post.forward = float4(forward, float(s->skyHeight));
    ...
    {
        if (std::max({ s->mesh[triangle].position.y, s->mesh[triangle + 1].position.y, s->mesh[triangle + 2].position.y }) < level - .04f)
            continue;
    ...
    transitionTarget(cmd, s->waterLight, false);
    cmdBindRenderTargets(cmd, nullptr);
