...
static void processCommands(World* w)
{
    ...
    while (w->commandRead != w->commandWrite)
    {
        ...
        if (c.actor >= MaxCrew || !std::isfinite(c.value))
        {
            ++w->stats.rejectedCommands;
            continue;
        }
        Actor& a = *actor(w, c.actor);
        Helm&  h = *helm(w);
        bool   accepted = false;
        if (c.type == CommandType::Attend)
        {
            const uint8_t target = static_cast<uint8_t>(c.station);
            if (target == 0 && a.helmSkill == 0)
                w->result = "Helm training required.";
            else if (target == 0 && h.occupant && h.occupant != w->crew[c.actor])
                w->result = "The helm is occupied.";
            else if (route(a, target))
            {
                if (a.travelling && h.occupant == w->crew[c.actor])
                    h.occupant = 0;
                if (!a.travelling && target == 0)
                    h.occupant = w->crew[c.actor];
                w->result = a.travelling ? "Travelling to the station." : "Already at the station.";
                accepted = true;
            }
            else
                w->result = "Finish travelling before choosing another station.";
        }
        else if (!canOperateHelm(w, c.actor))
            w->result = "Reach and occupy the helm before using its controls.";
        else
        {
            if (c.type == CommandType::Wheel)
            {
                h.rudder = std::clamp(c.value, -0.61f, 0.61f);
                h.rudderVelocity = 0;
                accepted = true;
            }
            if (c.type == CommandType::Throttle && c.engine < w->layout.propellerCount)
            {
                h.throttle[c.engine] = std::clamp(c.value, -1.0f, 1.0f);
                accepted = true;
            }
            if (c.type == CommandType::Brake)
            {
                h.brake = c.value != 0;
                accepted = true;
            }
...
static void travelSystem(ecs_iter_t* it)
{
    ...
    for (int i = 0; i < it->count; ++i)
    {
        ...
        {
            a.deckPosition = goal;
            a.node = a.path[a.pathCursor++];
            if (a.pathCursor == a.pathCount)
            {
                a.travelling = false;
                if (a.node == 0 && a.helmSkill && !helm(w)->occupant)
                    helm(w)->occupant = it->entities[i];
            }
        }
        else
            a.deckPosition = fromJolt(toJolt(a.deckPosition) + delta * (stride / distance));
    }
}
static void vesselSystem(ecs_iter_t* it)
{
    ...
    for (int v = 0; v < it->count; ++v)
    {
        ...
        attached.amplitude = std::min(.4f, layout.wakePressureCoefficient * surge * surge / (2 * 9.81f));
        attached.sourceBody = bodies[v].handle;
            ...
            if (speed > .35f && w->wakeEnergy > 0)
            {
                ...
                for (const auto& branch : branches)
                {
                    ...
                    for (unsigned hull = 0; hull < hulls; ++hull)
                    {
                        ...
                        emitWavePacket(w->ocean, fromJolt(point), fromJolt(direction), wavelength, w->wakeEnergy * branch[2] / hulls, false,
                                       bodies[v].handle);
                    }
                }
            }
...
static void capture(World* w)
{
    MTRACY_ZONE("capture");
    auto&        bi = w->physics.GetBodyInterface();
    const auto   id = bodyID(w);
    auto         q = bi.GetRotation(id);
    const Helm&  h = *helm(w);
    const Actor& a = *actor(w, 0);
    Snapshot&    s = w->current;
    s.position = fromJolt(bi.GetPosition(id));
    s.velocity = fromJolt(bi.GetLinearVelocity(id));
    s.angularVelocity = fromJolt(bi.GetAngularVelocity(id));
    s.rotation = { q.GetX(), q.GetY(), q.GetZ(), q.GetW() };
    s.skipperDeck = a.deckPosition;
    s.time = w->time;
    for (unsigned i = 0; i < 2; ++i)
    {
        s.throttle[i] = h.throttle[i];
        s.rpm[i] = w->propulsion[i].rpm;
    }
    s.rudder = h.rudder;
    s.brake = h.brake;
    s.travelling = a.travelling;
    s.destination = a.destination;
    s.atHelm = canOperateHelm(w, 0);
    s.speedKnots = std::sqrt(s.velocity.x * s.velocity.x + s.velocity.z * s.velocity.z) * 1.9438445f;
    s.travelRemaining = 0;
    Vec3 last = a.deckPosition;
    for (unsigned n = a.pathCursor; n < a.pathCount; ++n)
    {
        Vec3 next = Deck[a.path[n]];
        s.travelRemaining += (toJolt(next) - toJolt(last)).Length() / 1.25f;
        last = next;
    }
}
...
World* createWorld(const VesselLayout& definition)
{
    ...
    for (auto& crew : w->crew)
        crew = ecs_new(w->ecs);
    JPH::StaticCompoundShapeSettings compound;
    for (unsigned hull = 0; hull < (cat ? 2u : 1u); ++hull)
    {
        JPH::Vec3 points[HullVertices];
        for (unsigned i = 0; i < HullVertices; ++i)
            points[i] = toJolt(hullVertex(w->layout, hull, i));
    ...
    JPH::RefConst<JPH::Shape> shape =
        tf_new(JPH::OffsetCenterOfMassShape, hullShape, toJolt(w->layout.centerOfMass) - hullShape->GetCenterOfMass());
    ...
    travel.query.terms[0].id = w->actorType;
    ...
    vessel.query.terms[0].id = w->bodyType;
    vessel.query.terms[1].id = w->helmType;
...
void destroyWorld(World* w)
{
    ...
    bi.RemoveBody(id);
    bi.DestroyBody(id);
    bi.RemoveBody(w->dock);
    bi.DestroyBody(w->dock);
    bi.RemoveBody(w->seabed);
    bi.DestroyBody(w->seabed);
    destroyOcean(w->ocean);
    ecs_fini(w->ecs);
    tf_delete(w);
    if (!--gWorldCount)
    {
...
void resetWorld(World* w, const InitialConditions* conditions)
{
    ...
    w->commandRead = w->commandWrite = 0;
    w->accumulator = w->time = 0;
    w->stats = {};
    w->contacts.dock = w->dock;
    w->contacts.seabed = w->seabed;
    w->contacts.dockPreviously = w->contacts.groundPreviously = false;
    w->physics.SetContactListener(&w->contacts);
    updateOcean(w->ocean, 0);
    clearWavePackets(w->ocean);
    w->wakeEnergy = w->wakeTime = 0;
    memset(w->propulsion, 0, sizeof w->propulsion);
    for (unsigned i = 0; i < MaxCrew; ++i)
    {
        Actor a = {};
        a.node = a.destination = i == 0 ? 0 : static_cast<uint8_t>(i);
        a.deckPosition = Deck[a.node];
        a.helmSkill = i == 0 ? 3 : 0;
    ...
    bi.SetPositionAndRotation(w->seabed, JPH::RVec3(0, equilibrium - seaState(w->ocean).depth - .5f, 0), JPH::Quat::sIdentity(),
                              JPH::EActivation::DontActivate);
    InitialConditions initial = conditions ? *conditions : InitialConditions{};
    const auto&       q = initial.rotation;
    bi.SetPositionAndRotation(id, toJolt(initial.position) + JPH::RVec3(0, equilibrium, 0), JPH::Quat(q.x, q.y, q.z, q.w).Normalized(),
                              JPH::EActivation::Activate);
    bi.SetLinearAndAngularVelocity(id, toJolt(initial.velocity), toJolt(initial.angularVelocity));
    capture(w);
    w->previous = w->current;
    w->result = "At the helm.";
}
bool enqueue(World* w, Command c)
{
    MTRACY_ZONE("enqueue");
    if (w->commandWrite - w->commandRead == 64)
    {
        ++w->stats.commandOverflow;
        w->result = "Command queue is full.";
        return false;
    }
    w->commands[w->commandWrite++ % 64] = c;
    return true;
}
void step(World* w)
{
    ...
    processCommands(w);
    w->previous = w->current;
    w->contacts.dockNow = w->contacts.groundNow = false;
    w->contacts.speed = 0;
    ...
    capture(w);
#ifdef ENABLE_MEMORY_TRACKING
    auto m = memGetStatistics();
    w->stats.allocationCount = m.accumulatedAllocUnitCount;
    if (w->stats.steps > 120)
        w->stats.steadyStepAllocations += m.accumulatedAllocUnitCount - allocationsBefore;
...
Snapshot snapshot(const World* w, bool interpolate)
{
    ...
    if (interpolate)
    {
        ...
        const auto& a = w->previous.rotation;
        const auto& b = s.rotation;
...
void                setEnvironment(World* w, const SeaState& environment)
{
    ...
    configureOcean(w->ocean, environment);
    resetWorld(w);
}
const char* lastCommandResult(const World* w) { return w->result; }
void        setHelmSkill(World* w, uint8_t i, uint8_t level)
{
    if (i < MaxCrew)
        actor(w, i)->helmSkill = std::min<uint8_t>(level, 3);
}
float interactionTimeScale(Difficulty d, bool modal)
{
    return !modal ? 1 : d == Difficulty::Beginner ? 0 : d == Difficulty::Intermediate ? .25f : 1;
}
